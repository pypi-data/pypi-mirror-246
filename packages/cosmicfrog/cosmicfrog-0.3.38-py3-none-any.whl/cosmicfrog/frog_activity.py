...
class ModelActivity:
    ...
    async def create_activity(self) -> None:
        """
...
class AsyncFrogActivityHandler:
    ...
    async def __aenter__(self):
        try:
            activity_id = await self.activity.create_activity()
        ...
        try:
            ...
            if not self.activity.done:
                if exc_type is None and exc_val is None and exc_tb is None:
                    await self.activity.update_activity(ActivityStatus.COMPLETED)
                else:
                    await self.activity.update_activity(ActivityStatus.FAILED)
                    raise exc_val
    ...
    async def update_activity(
        ...
        """
        await self.activity.update_activity(activity_status, last_message, progress)
