...
class Config:
    ...
    def __init__(self, args=None):
        ...
        if self.cpu_count == -1:
            self.cpu_count = max(
                1, min(math.floor(multiprocessing.cpu_count() / 2), MAX_DEFAULT_JOBS)
            )
        ...
        except deidentify.ValidationError as exc:
    ...
    def get_import_parser():
        ...
        parser.add_argument(
            ...
            choices=range(-1, 9),
            help=(
                "The compression level to use for packfiles. -1 for default, 0 for "
                "store"
            ),
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            action="store_true",
            help=(
                "When --unique-uids argument is defined copy the duplicates to a "
                "specific folder"
            ),
        )
        parser.add_argument(
            "--duplicates-folder",
            help=(
                "Copy duplicates into this folder (default: level up from specified "
                "import folder)"
            ),
        )
        parser.add_argument(
    ...
    @staticmethod
    def deprecated_parsers():
        """Records deprecated parsers. Returns a dict."""
        deprecated = {
            "job": "The command 'job' has been deprecated and will be "
            "removed in a  future release.\n"
            "Users should refer to Jobs And Analyses in Flywheel SDK for more "
            "information, available at "
            "https://flywheel-io.gitlab.io/product/backend/sdk/branches/master/python/examples.html#jobs-and-analyses",
        }
        if os.environ.get("FLYWHEEL_CLI_LEGACY") == "true":
            deprecated["import"] = (
                "The import command has been replaced by the ingest command "
                "and will be removed in a future release.\nRefer to "
                "https://docs.flywheel.io/hc/en-us/articles/4416119193491 for "
                "details."
            )
        return deprecated
