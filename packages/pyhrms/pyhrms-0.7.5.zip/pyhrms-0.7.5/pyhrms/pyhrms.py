...
def peak_checking_area(ref_all, df1, name):
    ...
    area_all = [round(scipy.integrate.simps(
        df1.iloc[mz_locators[i][0]:mz_locators[i][1], rt_locators[i][0]:rt_locators[i][1]].values.sum(axis=0) - min(
            df1.iloc[mz_locators[i][0]:mz_locators[i][1], rt_locators[i][0]:rt_locators[i][1]].values.sum(axis=0))), 0) for i in range(len(mz_locators))]
...
def pubchem_search(file, sleep_time=1):
    ...
    import re
    df = pd.read_excel(file)
    # make sure these names in the columns
    ...
    for i in tqdm(range(len(df)), desc='Searching in pubchem'):
        ...
        ik = df.loc[i, 'ik']
        if np.isnan(name) | np.isnan(formula) | np.isnan(cas) | np.isnan(smile):
            qurey_num += 1
            try:
                ...
                pass
    print(
        f'Totoal query number = {qurey_num}; Total response number = {response_num}, Response rate = {round(response_num/qurey_num* 100, 2)} %')
    return df
