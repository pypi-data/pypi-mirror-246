...
function addGradientBarComponent(
  outerDiv,
  row,
  header,
  col,
  ...
  if (value !== "") {
    if (value <= orderedPivots[0]) {
      ...
      c[0] = Math.round(
        boundColors.color1[0] * (1.0 - ratio) + boundColors.color2[0] * ratio
      );
      c[1] = Math.round(
        boundColors.color1[1] * (1.0 - ratio) + boundColors.color2[1] * ratio
      );
      c[2] = Math.round(
        boundColors.color1[2] * (1.0 - ratio) + boundColors.color2[2] * ratio
      );
    }
  } else {
    c = [255, 255, 255];
  ...
  if (value !== "" && dtype != "string") {
    ...
    value =
      ...
      (Math.abs(parseFloat(minval)) + Math.abs(parseFloat(maxval)));
    var bar = paper.rect(value * barWidth, 0, 1, lineHeight, 1);
    bar.attr("fill", color);
    bar.attr("stroke", color);
  }
...
function getDetailWidgetDivs(
  ...
  div.setAttribute("widgetname", widgetName);
  var width = null;
  var height = null;
  var maxWidth = null;
  var maxHeight = null;
  ...
  if (
    typeof viewerWidgetSettings != "undefined" &&
    viewerWidgetSettings["info"] != undefined
  ) {
  ...
  btn.setAttribute("class", "detailwidget-camerabutton h-4 w-4 cursor-pointer");
  btn.setAttribute("title", "Capture to image file");
  ...
  path.setAttribute("stroke-linecap", "round");
  path.setAttribute("stroke-linejoin", "round");
  path.setAttribute("d", "M15 13a3 3 0 11-6 0 3 3 0 016 0z");
  addEl(btn, path);
  addEl(iconDiv, btn);

  // Pin button
  var btn = document.createElementNS("http://www.w3.org/2000/svg", "svg");
  btn.setAttribute("class", "detailwidget-pinbutton h-4 w-4 cursor-pointer");
  ...
  // Close button
  var btn = document.createElementNS("http://www.w3.org/2000/svg", "svg");
  btn.setAttribute("class", "detailwidget-closebutton h-4 w-4 cursor-pointer");
...
function addGradientStopBarComponent(
  outerDiv,
  row,
  header,
  col,
