...
const makeFilterElementControlDiv = function () {
  ...
  const removeDiv = $(getEl("img")).addClass("filter-element-control-icon");
...
const showHoverControl = function (evt) {
  if (visibleFilterHoverP != null) {
    ...
    visibleFilterHoverP
      .querySelectorAll(":scope > .hovercontrol")
      .forEach((el) => {
...
const makeFilterColDiv = (filter) => {
  ...
  colDiv.append(" ");
  const filterValsSpan = $(getEl("span")).addClass("filter-values-div");
...
const onFilterColumnSelectorChange = (evt) => {
  ...
  if (filter != null) {
    ...
    if (filter.type == "select") {
      $testSel.append(
        $(getEl("option")).val("select").append(filterTests.select.title)
      );
      var selIdx = null;
      ...
      for (var i = 0; i < filterTestNames.length; i++) {
        if (
          filterTests[testDiv.options[i].value].colTypes.indexOf(selColType) >=
          0
        ) {
...
const populateFilterColumnSelector = (colSel, groupTitle) => {
  ...
  for (let i = 0; i < allCols.length; i++) {
    ...
    const colOpt = $(getEl("option")).val(col.col).append(col.title);
...
const addFilterRuleHandler = (event) => {
  ...
  const elemsDiv = $(
    button
      .closest(".filter-group-div")
      .querySelector("div.filter-group-elements-div")
  );
  addFilterElement(elemsDiv, "rule", undefined);
};
...
const makeJoinOperatorDiv = function (operator) {
  ...
  joinOpDiv[0].addEventListener("drop", function (evt) {
    ...
    document
      ...
      .querySelectorAll(".filter-join-operator-div")
      .forEach((el) => {
    ...
    if (
      source.previousSibling == null ||
      source.previousSibling.classList.contains("filter-join-operator-div") ==
        false
    ) {
    ...
    if (
      ...
      source.nextSibling.classList.contains("filter-join-operator-div") == false
    ) {
      var newj = makeJoinOperatorDiv(p.getAttribute("join-operator"))[0];
      p.insertBefore(newj, source);
  ...
  for (let i = 0; i < colDivs.length; i++) {
    ...
    if (colFilter.test == "select" || colFilter.test == "multicategory") {
      ...
      if (valInputs.length === 0) {
        ...
        if (
          reportsubKeys.length > 0 &&
          (colFilter.test == "stringContains" ||
            colFilter.test == "stringStarts" ||
            colFilter.test == "stringEnds")
        ) {
          ...
          for (var j = 0; j < reportsubKeys.length; j++) {
            ...
            if (
              (colFilter.test == "stringStarts" && value.startsWith(val)) ||
...
const filterTests = {
  equals: {
    title: "equals",
    inputs: 1,
    colTypes: ["string", "float", "int", "select"],
  },
