...
def activate_network():
    """Active le réseau"""
    os.system("sudo ifconfig automatex up")
...
def run_container_host(enterprise):
    ...
    if container_name not in [container.name for container in containers]:
        ...
        """)
        os.system('ssh-keygen -f "~/.ssh/known_hosts" -R "[localhost]:2222" 2>/dev/null')
        os.system("sshpass -p 'kali' ssh -p 2222 -o StrictHostKeyChecking=no root@localhost /usr/local/bin/init-services.sh")
...
def run_container_macvlan(ip, enterprise):
    """Lance le conteneur automatexpertise"""
    containers = client.containers.list(all=True)
    container_name = f"automatexpertise-{enterprise}"
    ...
    if container_name not in [container.name for container in containers]:
        ...
        os.system(f'ssh-keygen -f "~/.ssh/known_hosts" -R "[{ip}]:2222" 2>/dev/null')
        os.system(f"sshpass -p 'kali' ssh -p 2222 -o StrictHostKeyChecking=no root@'{ip}' /usr/local/bin/init-services.sh")
...
def configure_network_host(container):
    # Obtenir la liste des objets de réseau
    interfaces = psutil.net_if_addrs()
...
def configure_network_macvlan(container):
    # Obtenir le nom d'hôte de la machine locale
    hostname = socket.gethostname()
...
def start_container(container_name):
    try:
        ...
        if network_mode == 'host':
            configure_network_host(container)

            os.system('ssh-keygen -f "~/.ssh/known_hosts" -R "[localhost]:2222" 2>/dev/null')
            os.system("sshpass -p 'kali' ssh -p 2222 -o StrictHostKeyChecking=no root@localhost /usr/local/bin/start-services.sh")
            print(f"Le conteneur {container_name} a été créé avec succès. Son IP: localhost")
        elif network_mode == 'automatex':
            # configure_network_macvlan(container)
            configure_network_macvlan(container)
            ...
            os.system(f'ssh-keygen -f "~/.ssh/known_hosts" -R "[{container_ip}]:2222" 2>/dev/null')
            os.system(f"sshpass -p 'kali' ssh -p 2222 -o StrictHostKeyChecking=no root@'{container_ip}' /usr/local/bin/start-services.sh")
        ...
        print(f"Le conteneur {container_name} a été démarré avec succès. Son IP est: {container_ip}")
        client.close()
...
def delete_container(container_name):
    try:
        ...
        if container.status == 'running':
            print(f"Le conteneur {container_name} est actuellement en cours d'exécution. Veuillez l'arrêter d'abord.")
        else:
