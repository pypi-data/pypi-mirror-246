...
class Trainer():
    ...
    def _load_cortex_yaml(self):
        ...
        config['modules'] = {}
        if config['module_path'] and config['module_path'] is not None:
            config['module_path'] = os.path.join(self._path, f'{config["module_path"][2:]}')
        walk_path = config['module_path'] if config['module_path'] and config['module_path'] is not None else '.'
    ...
    def _instantiate_model(self):
        ...
        # LOAD MODEL MODULE
        if self._cortex_config['module_path'] and self._cortex_config['module_path'] is not None:
