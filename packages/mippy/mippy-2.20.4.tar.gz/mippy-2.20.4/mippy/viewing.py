...
##            return False
    ...
    def update(self, xmove=0., ymove=0.):
        ...
        if not (xmove==0 and ymove==0):
            ...
            if 'center' in self.geometry.keys():
                self.geometry['center'] = (self.geometry['center'][0]+xmove,
                                            self.geometry['center'][1]+ymove)
            return
        else:
            arr_co = np.array(self.coords)
            self.bbox = (np.min(arr_co[:, 0]), np.min(arr_co[:, 1]), np.max(arr_co[:, 0]), np.max(arr_co[:, 1]))
            # Fixing something that might not be a problem - use new bbox to update center in geometry
            xc = (self.bbox[2]-self.bbox[0])/2 + self.bbox[0]
            yc = (self.bbox[3]-self.bbox[1])/2 + self.bbox[1]
            self.geometry['center'] = (xc,yc)
        return
...
class MIPPYCanvas(Canvas):
    ...
    def left_release(self, event):
        ...
        if self.drawing_roi:
            ...
            # print("Moved ROI",total_xmove,total_ymove)
            if len(self.roi_list) > 0:
                for roi in self.roi_list:
        ...
        if not self.auto_function is None:
            # print("Doing this bit")
            self.auto_function()
        # print("Not sure if doing this bit?")
        self.tempcoords = []
        self.tempx = None
        self.tempy = None
        self.delete('roi_drawing')
        ...
        # Debugging - individually draw pixels in an ROI
        # for y in range(self.get_active_image().rows):
        #     for x in range(self.get_active_image().columns):
        #         for roi in self.roi_list:
        #             if roi.contains(self.canvas_coords([(x,y)])[0]):
        #                 self.create_rectangle(x*self.zoom_factor,
        #                                       y*self.zoom_factor,
        #                                       (x+1)*self.zoom_factor,
        #                                       (y+1)*self.zoom_factor,
        #                                       fill='green')
