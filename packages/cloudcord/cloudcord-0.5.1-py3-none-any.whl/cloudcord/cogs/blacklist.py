...
from ..internal import SiConfig, t
...
async def _check_blacklist(interaction: discord.Interaction) -> bool:
    ...
    if interaction.user.id in bans and SiConfig.blacklist:
        if SiConfig.blacklist.raise_error:
            raise Blacklisted()
        else:
...
class Blacklist(Cog, hidden=True):
    ...
    async def cog_check(self, ctx):
        if SiConfig.blacklist.owner_only:
    ...
    if PYCORD:
        admin = discord.SlashCommandGroup(
            t("admin_group"),
            description="SiConfig admin commands",
            guild_ids=SiConfig.admin_guilds,
