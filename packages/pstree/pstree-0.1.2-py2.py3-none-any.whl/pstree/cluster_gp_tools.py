...
def add_pset_function(pset, max_arity, basic_primitive):
    ...
    if basic_primitive == "normalized":
        ...
        add_function = lambda func, arity: pset.addPrimitive(
            normalized_wrapper(func), arity
        )
    ...
    for i in range(k):
        ...
        chosen.append(
            random.choice(
                list(
                    filter(
                        lambda x: isclose(x.temp_rank, min_individual.temp_rank),
                        individuals,
                    )
                )
            )
...
def selRankSumTournament(individuals, k, tournsize=10):
    ...
    for i, ind in enumerate(individuals):
        fitness_list.append(ind.fitness.wvalues)
    fitness = (
        pd.DataFrame(fitness_list).rank(axis=0, ascending=False).values.sum(axis=1)
    )
...
def selBestSum(individuals, k, fit_attr="fitness"):
    return sorted(
        individuals,
        key=lambda x: float(np.sum(getattr(x, fit_attr).wvalues)),
        reverse=True,
    )[:k]
...
def selMOEAD(individuals, k):
    fun = lambda x: np.sum(
        softmax(np.random.normal(size=(len(x.fitness.wvalues))))
        * np.array(x.fitness.wvalues)
    )
    return sorted(individuals, key=lambda x: float(fun(x)), reverse=True)[:k]


def selIBEA(population, alpha=None, kappa=0.05):
    """IBEA Selector"""
