...
class FvTimeSeries(TimeSeries):
    ...
    def variables(self):
        ...
        return [
            x for x in self.ds[grp].data_vars.keys()
            if x not in ["X", "Y", "Z"]
        ]
    ...
    def get_mask_vector(self, site, time=None):
        ...
        if "stat" in ds.data_vars:
            ...
            return np.zeros((ds.dims["Time"]), dtype=bool)
    ...
    def get_data(self,
        ...
        # Data is 1D in z but has 2 dims (e.g. BED_MASS (time, SEDIMENT_FRACTION))
            ...
            print(
                "Warning: Data is a 1D variable with a secondary dimension: {0}"
                .format(dims[-1]))
            print("Summing data across the secondary dimension")
            print(
                "Consider using get_raw_data if this is not the desired action"
            )
    ...
    def get_timeseries(
        ...
        dst = ds.drop(["layerface_Z"] + dropvars).isel(NumLayers=0, missing_dims='ignore')
        dst.attrs[
            "Origin"] = "Timeseries extracted from TUFLOWFV cell-centered output using `tfv` python tools"
    ...
    def __prepare_file_handle__(self):
        ...
        self.ds = {}
        for grp in grps:
            ds = xr.open_mfdataset(self.file, group=grp)
            ds["Time"] = tvec
            ds.attrs["Label"] = grp
    ...
    def __prepare_time_vector__(self):
        ...
        self.time_vector = (
                pd.to_timedelta(self.nc["ResTime"][:], unit="h") + fv_epoch
            )
