...
def terminal_interface(interpreter, message):
    ...
    if not interpreter.auto_run and not interpreter.local:
        interpreter_intro_message = [
            "**RobotGPT Interpreter** will require approval before running code."
        ]
    ...
    while True:
        try:
            if interactive:
                ...
                if (
                    ...
                    ### This is the primary input for RobotGPT Interpreter.
                    message = input("> ").strip()
        ...
        if isinstance(message, str):
            ...
            if message.strip() == "interpreter --local":
                ...
                continue
            if message.strip() == "pip install --upgrade robotgpt-interpreter":
                print(
                    "Please exit this conversation, then run `pip install --upgrade robotgpt-interpreter`."
                )
                continue
