...
def setup_local_text_llm(interpreter):
    ...
    if repo_id != "TheBloke/Mistral-7B-Instruct-v0.1-GGUF":
        ...
        display_markdown_message(
            f"**RobotGPT Interpreter** will use `{repo_id}` for local execution."
        )
    ...
    def local_text_llm(messages):
        ...
        if "mistral" in repo_id.lower():
            ...
            messages[0][
                "content"
            ] = "You are RobotGPT Interpreter. You almost always run code to complete user requests. Outside code, use markdown."
            messages[0][
                "content"
        ...
        for token in ooba_llm.chat(messages):
            ...
            # instead of symbols in their code when used with RobotGPT Interpreter.
