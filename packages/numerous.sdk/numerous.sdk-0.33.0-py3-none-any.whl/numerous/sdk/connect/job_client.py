...
from numerous.sdk.connect.inputs import InputReader
...
class JobClient:
    ...
    def _setup_sigterm_handling(self):
        def _sigterm_handler(signalnum: int, frame: Optional[FrameType]):  # noqa: F841
            if self._command_handler.hibernating:
                sys.exit(0)
    ...
    def _create_command_handler(self) -> CommandHandler:
        ...
        return CommandHandler(subscription, lambda: self.state)
    ...
    def set_hibernate_callback(self, callback: Callable[[], Any]) -> None:
        ...
        self._command_handler._hibernation_callback = callback
    ...
    def read_inputs(
        ...
        Example:
            >>> result = 0
            >>> with JobClient.from_environment() as job_client:
