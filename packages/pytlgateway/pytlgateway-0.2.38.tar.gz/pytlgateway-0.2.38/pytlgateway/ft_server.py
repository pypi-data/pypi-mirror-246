...
class FtServer(object):
        ...
        try:
            ...
            if msg['status'] > 5:  # 错/废单
                ...
                if oid not in self.oid_to_req:
                    self.logger.warning(
                        f"[on_zidan_message] can'tfind_req (mid){mid}")
                    return
                ...
                # 计算订单成交量
                if mid not in self.sid_to_traded:
                    if target_type == 'buy' :
                        self.sid_to_traded[mid] = msg['trade_vol']
                    elif target_type == 'sell':
                        self.sid_to_traded[mid] = 0
                        self.sid_to_traded[mid] -= msg['trade_vol']
                ...
                # 更新持仓
                #self.update_position(msg, order, mid, oid, db_side, amt)

                if (abs(self.oid_to_traded[oid]) == entrust_vol):
                    self.update_order_filled(msg, mid, oid)
    ...
    def update_position(self, msg, order, mid , oid, side, trade_amt):
        try:
            target_account_name = order['target_account_name']
            acc = self.target_account_names_to_acc[target_account_name]
    ...
    def update_order_filled(self, data, mid, oid):
        try:
            order = self.oid_to_req[oid]
    ...
    def req_buy_order_insert(self, obj: dict):
        try:
            ...
            if data['code'] != 0:
                ...
                self.insert_ft_proposed_target(acc, mid, order_msg)
            ...
            if status >= 4:
                ...
                self.rtn_order_insert(mid, oid, exchange)
                #ft_order用于恢复
    ...
    def insert_ft_proposed_target(self, acc, mid, order_msg:dict):
        try:
            ft_proposed_target_collection = self.order_info_db[acc]['ft_proposed_target']
            insert_query = {'mid' : mid, 'target_account_name': order_msg['target_account_name']}
    ...
    def rtn_order_insert(self, mid, oid, exchange):
        try:
    ...
    def rtn_order_canceled(self, oid, mid):
        try:
            if oid not in self.oid_to_req:
                self.logger.warning(
        ...
        try:
            ...
            if oid not in self.oid_to_ref:
                ...
                if data['code'] == 0:
                    ...
                    self.rtn_order_canceled(oid, mid)
