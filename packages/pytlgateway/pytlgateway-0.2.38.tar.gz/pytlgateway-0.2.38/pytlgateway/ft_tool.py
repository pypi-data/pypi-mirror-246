...
class FtTool(object): 
                ...
                with open(path, encoding='gbk')as f:
                    ...
                    for row in reader:
                        ...
                        if traded_vol > 0:
                            ...
                            oid = 0
                            mid = "" # f"[PR_Ver_1_0_1_HX]{row.股票代码}T20221125"
                            entrust_vol = 0
                            side = 0
                            accountName = ""
                            order_type = 215
                    ...
                    for position in equity_positions:
                        ...
                        if mid.startswith('[ADD]'):
                            ...
                            if not sync_trades.count() == 0:
                                ...
                                for key,value in sid_to_traded_vol.items():
                                    ...
                                    if position_for_update is not None:
                                        ...
                                        self.add_position_for_add(acc, exchange = exchange, ticker = ticker, td_vol = 0, yd_vol = position_yd_vol, accountName = account_name, mid = position_sid)
                                query_for_delete_add = {'mid' : mid , 'accountName' : account_name}
                                equity_position_collection.delete_one(query_for_delete_add)
                                self.logger.info(f"delete_position (mid){mid}")
                            """
                            ...
                            if actual_yd_pos_long > 0:
                                query_positions = {'accountName': account_name, 'ticker': ticker, 'mid' :  {'$ne' : mid}}
                                ...
                                if ticker_positions.count() > 0:
                                    for ticker_position in ticker_positions:
                                        ...
                                        ticker_sid = ticker_position['mid']
    ...
    def add_position_for_add(self, acc, exchange, ticker, td_vol, yd_vol, accountName, mid):
        try:
            ...
            filter = {'mid' : record['mid'], 'accountName' : accountName}
    ...
    def update_position(self, target, acc, pos, ticker, td_vol, yd_vol,accountName):
        try:
            collection = self.order_info_db[acc]['EquityPosition']
            mid = target['mid']
            update_filter = {'mid': mid, 'accountName' : accountName}
            update_record = {
                'td_pos_long': td_vol,
                "yd_pos_long": yd_vol,
            ...
            self.logger.info(f"[update_position] (res){res} (mid){mid} (msg){update_record}")
    ...
    def add_position_for_new_order(self, acc, pos, ticker, td_vol, yd_vol, accountName):
        try:
            mid = '[ADD]' + self.generate_sid(ticker)
    ...
    def add_position_for_debug(self, acc, pos, ticker, td_vol, yd_vol, accountName):
        try:
            ...
            record['enter_date'] = datetime.now(timezone.utc)
            record['holding_days'] = 0 
            record['mid'] = '[ADD]' + self.generate_sid(ticker)
            record['exchange'] = exchange
            filter = {'mid' : record['mid'], 'accountName': accountName}
            res = collection.replace_one(filter, record, True) 
            self.logger.info(f"[add_position_for_debug] (res){res} (msg){record}")
    ...
    def update_position_with_record(self, trade_vol, order, mid , side, trade_amt, trade_price):
        try:
                    ...
                    if file.stem.startswith(path_head):
                        with open(file, encoding='utf-8-sig') as f:
                            ...
                            for row in reader:
                                ...
                                self.update_position_with_record(traded_vol, order, mid, side, trade_amt, traded_price)
                        os.remove(file)
    ...
    def restore_pos(self):
        try:
            for acc in self.accounts_run:
                ...
                query = {'accountName': target_account_name, 'mid': {'$not': {'$regex': 'ManualShort'} }}
    ...
    def generate_sid(self, ticker, exchange=None) -> str:
        mid = str(int(datetime.today().timestamp())) + '[T]' + ticker
