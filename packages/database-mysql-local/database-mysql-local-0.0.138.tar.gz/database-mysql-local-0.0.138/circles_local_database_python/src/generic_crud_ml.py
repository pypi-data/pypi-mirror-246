...
class GenericCRUDML:
    ...
    def select_one_by_where_with_none_option(
        ...
        select_query = f"SELECT {select_column_name} " f"FROM {self.schema_name}.{view_table_name} " + (
            f"WHERE {condition_column_name} = {condition_column_value} " if condition_column_name and condition_column_value else f"WHERE {condition_column_name} IS NULL ")
        try:
    ...
    # This method returns the values in a selected column by a condition, the condition can be none or a value
    def select_multi_by_where_with_none_option(self, condition_column_name, condition_column_value,
                                               view_table_name: str = None, select_column_name: str = None,
                                               limit: int = 100, order_by: str = "") -> list:
        view_table_name = view_table_name or self.default_view_table_name
        # Returns the row id if select_column_name is None
        select_column_name = select_column_name or self.default_column
