...
def update_dbt_relations(
    source: t.Dict, keys: t.List[str], default_catalog: t.Optional[str]
) -> None:
...
def migrate(state_sync, default_catalog: t.Optional[str], **kwargs):  # type: ignore
    ...
    for name, identifier, version, snapshot, kind_name in engine_adapter.fetchall(
        ...
        jinja_global_objs = node.get("jinja_macros", {}).get("global_objs")
        if jinja_global_objs:
            update_dbt_relations(jinja_global_objs, ["refs", "sources", "this"], default_catalog)
