...
class DataFilter(BaseModel):
    ...
    filter_cohort: Optional[str] = None
    """The cohort to perform the filter on if there are multiple cohorts for Federated Join. If unspecified will be the data cohort"""
...
class JoinMode(str, Enum):
    ...
    """
    @autoapi True Return values where the indentifiers are found in both the filter and data cohorts.
    """
    UNION = "union"
    """
...
class JoinableMetric(AggregatableMetric, ABC):
    ...
    """
    @autoapi True The mode to perform an optional Federated Join in. Defaults to intersection if join_field, data_cohorts, or filter_cohorts are defined
    """
    join_field: Optional[FilterVariableTypeOrColumnName] = None
    """
    @autoapi True A field to perform a join on if performing a Federated Join. This filter_variable will be performed on the filter_cohort(s)
    """
    data_cohorts: Optional[List[str]] = None
    """
    @autoapi False A list of cohorts to get data from. Used for Federated Join. Currently only supports 1 cohort for Intersection Mode. Supports any number of cohorts for Union mode. The order of cohorts determines the selection order in Intersection mode. Data from earlier cohorts has will be used over later cohorts.
    """
    filter_cohorts: Optional[List[str]] = None
    """
    @autoapi False A list of cohorts to perform the join filter logic against. Used for Federated Join Intersection. Currently only supports 1 cohort.
    """

    def __init__(self, **data):
        # Handle optional join_mode fields based on user input
        join_field = data.get("join_field")
        data_cohorts = data.get("data_cohorts")
        filter_cohorts = data.get("filter_cohorts")
        join_mode = data.get("join_mode")
        if any([x is not None for x in [join_field, data_cohorts, filter_cohorts, join_mode]]):
            # Cannot do the following check with the desired interface alias
            # if not data_cohorts:
            #     raise ValueError("data_cohorts cannot be empty for a Federated Join. Either remove all Join fields or provide a cohort UID")
            # if not filter_cohorts:
            #     raise ValueError("filter_cohorts cannot be empty for a Federated Join. Either remove all Join fields or provide a cohort UID")
            if not join_field:
                raise ValueError(
