#!/usr/bin/python3
#Copyright by Chen Chuan (chenc224@163.com)
...
def readdata(): #取出所有数据，这样换数据源就比较容易了
    with open("/root/work/config/server.json",'r',encoding="utf8") as f:
        data=json.load(f)
    ...
    def main(self):
        ...
        while True:
            ...
            if chr(c)>='a' and chr(c)<='z' and chr(c) in self.fwq:
                ...
                print("\x1b]2;@%s\x07" %(os.uname()[1]))
                self.initcurse()
    def readdata(self): #取出所有数据，这样换数据源就比较容易了
        if not os.path.isfile("/root/work/config/server.json"):return
        with open("/root/work/config/server.json",'r') as f:
            self.data=json.load(f)
        self.data.sort(key=lambda k:k["class"]+k["name"])
...
#       jg=self.c.find({"class":self.fz}).sort("info")
        ...
        for i in range(min(maxy-2,len(bq),len(jg))):
            stdscr.addstr(i+1,20,"%s  %s" %(bq[i],jg[i][u"info"]))
            self.fwq[bq[i]]=jg[i]
