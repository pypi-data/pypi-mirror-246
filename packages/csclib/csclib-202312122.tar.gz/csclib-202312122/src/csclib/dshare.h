...
//  int kq = blog(q-1)+1;
  ...
  //  ds2 = block_dshare_new(bs, g_inv, q);
    ds1 = block_dshare_new(1, g, qq); // 順列の方はブロックサイズは無関係
    ds2 = block_dshare_new(bs, g_inv, qq);
    perm_free(g_inv);
  } else {
  //  ds1 = block_dshare_new(bs, g, qq);
    ds1 = block_dshare_new(1, g, qq);
...
static share_array block_AppPerm_fwd_channel(int bs, share_array x, share_array sigma, int channel)
{
  if (_party >  2) return NULL;
    ...
    if (tbl = ds_tbl_list_search2(PRE_DS_tbl[channel], 0, n)) {printf("bs: %d\n", tbl->bs);
      int n2 = tbl->n;
