...
class DoplCommunicator:
    ...
    def __init__(self, url):
        ...
        self.__controller_data: ControllerData = None
    ...
    def on_controller_data(self, callback):
        def json_to_object_then_callback(controller_data_json):
            controller_data = ControllerData.fromJSON(controller_data_json)
            callback(controller_data)
    
        self.__sio.on("send_robot_controller_data", json_to_object_then_callback)
        ...
        while(self.__send_data):
            ...
            with self.__controller_data_lock:
                if(self.__controller_data_to_send):
                    data_to_send["robot_controller"] = self.__controller_data_to_send.toJSON()
