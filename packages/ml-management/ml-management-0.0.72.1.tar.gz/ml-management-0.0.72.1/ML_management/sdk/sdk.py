...
def _get_logs_url(job_name: str, stream: bool = True) -> str:
    server_url = get_server_websocket_url() if stream else get_server_url()
    url_base = posixpath.join(server_url, "logs-api")
    local_path = f"{'stream' if stream else 'filedump'}?job_id={job_name}"
...
def get_logs(job_name: str, stream: bool = True, file_name: Optional[str] = None) -> None:
    ...
    """
    url = _get_logs_url(job_name, stream=stream)
    if stream:
        ws = None
        file = sys.stdout
        try:
