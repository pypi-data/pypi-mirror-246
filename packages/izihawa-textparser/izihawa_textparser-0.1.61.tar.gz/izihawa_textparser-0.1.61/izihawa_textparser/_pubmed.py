...
def process_pubmed_central(text: str):
        ...
        document["issued_at"] = int(
            time.mktime(
                datetime.datetime(
                    ...
                    month=int(date.select_one('month').text),
                    day=int(date.select_one('day').text),
                ).utctimetuple()
            )
        )

    ready_authors = []
    for author in soup.select('contrib-group > contrib[contrib-type="author"]'):
        ready_author = {}
    ...
    for tag in soup.select('article-categories > subject'):
