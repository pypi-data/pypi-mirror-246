...
def list_shares(params) -> list:
    endpoint = params['endpoint']
    certificate = params['cert']
    key = params['key']
    container = re.match(r".+\/\/([^.]+)", endpoint).group(1)
    url = endpoint.replace('hdlfs://', 'https://') + f"/catalog/v2/shares"
...
def list_schemas(share: str, params) -> dict:
    endpoint = params['endpoint']
    certificate = params['cert']
    key = params['key']
    container = re.match(r".+\/\/([^.]+)", endpoint).group(1)
...
def add_share(share: str, params) -> None:
    endpoint = params['endpoint']
    certificate = params['cert']
    key = params['key']
    container = re.match(r".+\/\/([^.]+)", endpoint).group(1)
    ...
    if r.status_code not in [200, 201]:
        raise ValueError(f"Unsuccessful API-call - Status code: {r.status_code} - {r.text}")
    rprint(f"Share successfully created: [{tp.variable}]{share}[/{tp.variable}]")
...
def main():
    ...
    parser.add_argument("action", choices=['list', 'add', 'delete', 'get'], help=f"Command for \'target\'-argument")
    ...
    parser.add_argument("-d", "--disable_cdf", help="Enable CDF", action="store_true")
    ...
    match args.action:
        case 'list':
            ...
            match len(target):
                case 0:
                    tree = list_shares(hdlfs_params)
                    if args.recursive:
                        tree = dict.fromkeys(tree,list())
                        for share in tree.keys():
                            tree[share] = dict.fromkeys(list_schemas(share, hdlfs_params),list())
                ...
                case 1:
                    share = target[0]
                    tree = {share: list_schemas(share, hdlfs_params)}
                    if args.recursive:
        ...
        case 'add':
            match len(target):
                ...
                case 1: 
                    add_share(target[0], hdlfs_params)
                case 2: 
        ...
        case 'get':
            ...
            table_info = get_table(target[0],target[1], target[2], params=hdlfs_params)
