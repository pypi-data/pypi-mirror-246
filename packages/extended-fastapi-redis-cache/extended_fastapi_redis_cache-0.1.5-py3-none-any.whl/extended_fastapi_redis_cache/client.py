...
class FastApiRedisCache(metaclass=MetaSingleton):
    ...
    async def extend_cache(self, key: str, ttl: int) -> bool:
    ...
    async def expire_entire_router_cache(
        self, 
        router: str, 
        account_id: int, 
        ...
        if log_time_execution:
            start = time.time()
        keys = await self.redis.keys(f"{self.prefix}:api.routes.{router}.*{user_label}={account_id}*")
        if keys:
    ...
    async def expire_endpoint_cache(
        self, 
        router: str, 
        endpoint: str, 
        account_id: int, 
        ...
        if log_time_execution:
            start = time.time()
        keys = await self.redis.keys(f"{self.prefix}:api.routes.{router}.{endpoint}*{user_label}={account_id}*")
        if keys:
            await self.redis.delete(*keys)
            if log_time_execution:
                exp_msg = f" - Expire operation took {(time.time() - start) * 1000} ms."
