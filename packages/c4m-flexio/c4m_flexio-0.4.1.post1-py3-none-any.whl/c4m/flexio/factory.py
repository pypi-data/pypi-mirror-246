...
class _PadIOVdd(_cell._FactoryOnDemandCell):
    ...
    def _create_layout(self):
        ...
        if spec.invvdd_n_mosfet is None:
            ...
            # Add top metal connection on left and right for iovss if needed
            if not frame.has_pad:
                net = nets["iovss"]

                top_spec = specs[-1]
                bottom_spec = specs[-2]
                via = bottom_spec.top_via
                via_space = frame.tracksegment_viapitch - via.width

                for seg in frame._track_segments["iovss"]:
                    bottom_height = seg.height - bottom_spec.tracksegment_space
                    top_height = seg.height - top_spec.tracksegment_space
                    ...
                    x = -bb.left
                    l_via = layouter.place(_l_via, x=x, y=y)
                    topbb = l_via.bounds(mask=top_spec.prim.mask)
                    if (topbb.left - _geo.epsilon) > 0.0:
                        shape = _geo.Rect.from_rect(rect=topbb, left=0.0)
                        layouter.add_wire(net=net, wire=top_spec.prim, shape=shape)
                    x = frame.monocell_width - bb.right
                    l_via = layouter.place(_l_via, x=x, y=y)
                    topbb = l_via.bounds(mask=top_spec.prim.mask)
                    if (topbb.right + _geo.epsilon) < frame.monocell_width:
                        shape = _geo.Rect.from_rect(rect=topbb, right=frame.monocell_width)
                        layouter.add_wire(net=net, wire=top_spec.prim, shape=shape)
...
class IOFactory(_fab.CellFactory[_cell._FactoryCell]):
    ...
    def out(self, *,
        ...
        return self.getcreate_cell(
            name=name, cell_class=_PadOut, create_cb=create_cb,
            drive=drive,
        )
    ...
    def triout(self, *,
        ...
        return self.getcreate_cell(
            name=name, cell_class=_PadTriOut, create_cb=create_cb,
            drive=drive,
        )
    ...
    def inout(self, *,
        ...
        return self.getcreate_cell(
            name=name, cell_class=_PadInOut, create_cb=create_cb,
            drive=drive,
        )
