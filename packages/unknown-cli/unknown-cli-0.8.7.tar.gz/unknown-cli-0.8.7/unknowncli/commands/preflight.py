...
class CustomTool:
    ...
    def is_compatible(self) -> bool:
        return all([find_executable(exe) != None for exe in self.required_exe_list])
...
def get_shelve_preflight_build_id(shelve_changelist, type=PreflightType.FULL, find=False, print_output=True, abort_on_fail=True):
    cmd = [
        sys.executable,
        "s:/sn2-main/BuildScripts/trigger_shelve_preflight.py",
...
def get_teamcity_build_json(build_id):
    cmd = [
        sys.executable,
        "s:/sn2-main/BuildScripts/get_teamcity_build_json.py",
        "--build_id", build_id
    ]
    build_json = ""
    try:
...
def main(
    ...
    config_type: P4VConfigType = Option(None, help="The type of preflight to run.")
):
    """
    Preflight a changelist
    """
    try:
        install_p4v_tools(config_type)
    except Exception as e:
    ...
    if not build_id:
        ...
        build_id = get_shelve_preflight_build_id(shelve_changelist, type=type)
