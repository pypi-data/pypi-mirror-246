...
def manage_args(**merged_keys):
    def decorator(func):
        ...
        def wrapper(*a, **b):
            ...
            while True:
                try:
                    return func(*a, **b)  # 如果 func 错误并返回到这里，调试器看不到 a b 的值，或者有问题（缺失字典键值对）
...
check_arg_type = arg_type = is_type = used_type = \
    check_type
...
def bined_args(*args,
    ...
    for arg in args:
        if is_type(arg, [list]):
            ...
            t = any(enabled(arg) for arg in arg) or t
        else:
            t = enabled(arg) or t
...
def get_lock(name=None, interval=2, max_wait=30):
    """
    主机范围
    @param name: 锁名
    @param interval: 检查间隔
    ...
    while True:
        f = createfile(path=lockpath(f'{name}.txt'), silent=False, encoding='utf-8',
...
def look(path):
    ...
    if type(path) in [pcsv, Csv, txt, rtxt, rjson, cache, jsondata, json]:
        try_to_execute_script("""look(parentpath(path.path))""")
        return
...
class pcsv():
    ...
    @manage_args(simplify=['set'])
    def getback(self, simpilify=None, save=None):
        """
        从备份恢复添加并去重
        @return:
        """
...
@listed()
@manage_args(l=['path'])
def deletedirandfile(l=None, silent=True, rt=None,
                     b=None, **leak):
    """
    删除文件和文件夹
    @param l: 可以是txt路径
...
class cache(txt):
    ...
    @manage(is_json=['json'])
    def __init__(self, path=None, silent=None, is_json=True, b=None):
    ...
    def get(self, silent=None, interval=2):
        """
        会删除。
        ...
        ret = None
        while not ret:
            get_lock(name='Cache ' + self.path,interval=7,timeout='infinite')
            if self.is_json:
                Json.clear_inner(self)
                ret = Json.get(self)
            else:
...
inform = notify = message = message_box = desktop_info = \
    desktop_inform
...
def Elements(root=None, xpath=None, depth=3, silent=True,
    ...
    # endregion
    while True:
        ret = root.find_elements(method, xpath)
        if len(ret):
    ...
    def __init__(self, url=None, download_path=default_page_download_path,
        ...
        if check_type(family, [str]):
            for i in range(100):
                ...
                except (selenium.common.exceptions.SessionNotCreatedException,PermissionError) as e:
                    pass
            Exit('')
        self.silent = silent
...
if (element) {
    ...
    @manage(bottom=['cutbottom'], left=['cutleft'], right=['cutright'], top=['cuttop'])
    def fullscreen(self, path=None, scale=100, autodown=True, pause=1, clip=True, clipinterval=0.6,
                   top=0, bottom=0, left=0, right=0, maxheight=40000, _look=False, overwrite=None,
                   b=None, **leak):
        """
    ...
    # @useState
    @manage(bottom=['bottom'], left=['left'], right=['right'], top=['top'])
    def _savepage(self, path=None, video=False, minsize=(100, 100),
                  ...
                  top=0, bottom=0, left=0, right=0, clipinterval=2, maxheight=40000,
                  b=None, **leak,
                  ):
        """
        保存整个网页，自定义参数
...
def page_download(url=None, path=None, t=999, silent=True, depth=0,
    ...
    # region
    if not redownload:
        if exists(target_path) and not size(target_path) == 0:
            if not overwrite:
    ...
    try:
        ...
        if 'default_download_page' in globals() and not page == default_download_page:
            ...
            page.get(url)
        if tellstringsame(Str(page.title(depth=0)), '403 forbidden'):
    ...
    download_name = head(s=get_full_filename(download_root, download_name, recursive=True,
                                             loop=int(t / 2), silent=True), mark='.crdownload',
                         strict=False)
    if not download_name:
        warn(f'js执行下载后查找失败，没有检测到下载文件\
         {Str(download_root) + Str(download_name)}\n 下载地址： {url}')
    return _check_and_finish()
    # endregion
