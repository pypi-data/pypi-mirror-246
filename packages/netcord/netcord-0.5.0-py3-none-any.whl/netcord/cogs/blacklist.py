...
from ..internal import NeConfig, t
...
async def _check_blacklist(interaction: discord.Interaction) -> bool:
    ...
    if interaction.user.id in bans and neConfig.blacklist:
        if neConfig.blacklist.raise_error:
            raise Blacklisted()
        else:
...
class Blacklist(Cog, hidden=True):
    ...
    async def cog_check(self, ctx):
        if neConfig.blacklist.owner_only:
    ...
    if PYCORD:
        admin = discord.SlashCommandGroup(
            t("admin_group"),
            description="neCord admin commands",
            guild_ids=neConfig.admin_guilds,
