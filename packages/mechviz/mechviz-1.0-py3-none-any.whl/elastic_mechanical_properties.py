...
import numpy as np
#from read_parameters import readstruct
from material_analysis import MaterialAnalysis
from calc_elastic_elate import print_elastic_tensor


def elastic_mechanical_properties(elastic_constants_dict, pos, elastic_tensor, dimensional,latt_system,plotparameters):
    #pos = readstruct(pos)
    ...
    if dimensional == '3D':
        try:
            ...
            elastic_constants_dict['C[111]']   = lc_4
...
def calculate_hardness(*args,dim='3D'):
    ...
    if dim == "3D":    
        ...
        K1 = vol**(1./4)*G*(B/G)**(1./2)
        K2 = 8840**(-1./2.) * vol**(1./4.) *(k_v * Y ) **(3./2.)
        K3 = 2**(-1./2.) * vol**(1./4.) *(k_v * Y ) **(3./2.)
    elif dim == "1D":
