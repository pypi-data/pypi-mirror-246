...
async def annotate_genes(
    ...
    for gene in data:
        yield genes_annotations_list[gene]
...
async def annotate(
    ...
    # Parses data batch to CSRA if data consists of variant call doctypes
    # Otherwise, returns an unprocessed list of raw hgvs strings
    csra_batch = get_csra_batch_or_raw_batch(data, hgvs_batch, build)

    if csra_batch:
        annotations_list = await gosdk.async_call_with_retry(
            gosdk.sdk.annotations.post_annotations,
            batch=csra_batch,
            delete_if_exists=delete_if_exists,
            build=build,
        )
    ...
    _logger.debug(
        ("annotate: call_count=%s first_call=%s csra_batch=%s annotated_count=%s"
            " first_annotated=%s"),
        len(data),
        data[:1],
        csra_batch[:4],
...
@curry
def refresh(data, recalculate_hgvs=False, batch_size=50):
    ...
    while has_more:
        result = gosdk.call_with_retry(
            gosdk.sdk.annotations.refresh_annotations,
...
def determine_relative_orientation(alt):
    ...
    if re.match(same, alt):
        rel_orien = "|REL_ORIEN=same"
    elif re.match(reverse, alt):
        rel_orien = "|REL_ORIEN=reverse"
    return rel_orien
