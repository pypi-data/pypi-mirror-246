...
from typing import Callable, Iterable, List, Tuple, Union, overload
...
class Grid:
    ...
    def focal(
        ...
        return focal(func, buffer, circle, 1, self)[0]

    def focal_mean(
        self, buffer=1, circle: bool = False, ignore_nan: bool = False
    ) -> "Grid":
        return self.focal(np.nanmean if ignore_nan else np.mean, buffer, circle)

    def focal_std(
        self, buffer=1, circle: bool = False, ignore_nan: bool = False
    ) -> "Grid":
        return self.focal(np.nanstd if ignore_nan else np.std, buffer, circle)

    def focal_min(
        self, buffer=1, circle: bool = False, ignore_nan: bool = False
    ) -> "Grid":
        return self.focal(np.nanmin if ignore_nan else np.min, buffer, circle)

    def focal_max(
        self, buffer=1, circle: bool = False, ignore_nan: bool = False
    ) -> "Grid":
        return self.focal(np.nanmax if ignore_nan else np.max, buffer, circle)

    def focal_sum(
        self, buffer=1, circle: bool = False, ignore_nan: bool = False
    ) -> "Grid":
        return self.focal(np.nansum if ignore_nan else np.sum, buffer, circle)
...
def _blocks(grid: Grid, buffer: int, circle: bool):
    ...
    if circle:
        ...
        for y in range(size):
            for x in range(size):
                ...
                if d <= buffer:
                    filter.append((x, y))
        return np.array([[block[filter] for block in blocks] for blocks in strided])

    return strided


def focal(
    func: Callable[[np.ndarray], Union[Value, Tuple[Value, ...]]],
    buffer: int,
    circle: bool,
    max_workers: int,
    *grids: Grid,
    ...
    stacks = np.stack([_blocks(g, buffer, circle) for g in grids_adjusted], axis=2)

    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=RuntimeWarning)
        if max_workers > 1:
            with futures.ProcessPoolExecutor() as executor:
                rows = [list(executor.map(func, stack)) for stack in stacks]
        else:
            rows = [list(map(func, stack)) for stack in stacks]
