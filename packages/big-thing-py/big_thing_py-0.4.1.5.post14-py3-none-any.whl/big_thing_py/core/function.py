...
class MXFunction(MXService):
    ...
    def _wrapper(self, execute_request: Union[MXExecuteRequest, MXInnerExecuteRequest]) -> bool:
        ...
        try:
            ...
            if isinstance(execute_request, MXExecuteRequest):
                ...
                self._send_TM_RESULT_EXECUTE(scenario=execute_msg.scenario, error=error, is_inner=True)
    ...
    def start_execute_thread(self, execute_msg: MXExecuteMessage) -> MXThread:
        ...
        if execute_protocol == MXProtocolType.Base.MT_EXECUTE:
            ...
            execute_request = MXInnerExecuteRequest(
                trigger_msg=execute_msg,
                result_msg=MXExecuteResultMessage(function=self, scenario=execute_msg.scenario, action_type=MXActionType.INNER_EXECUTE),
            )
    ...
    def _send_TM_RESULT_EXECUTE(self, scenario: str, error: MXErrorCode, request_ID: str = None, is_inner: bool = False) -> None:
        if is_inner:
            action_type = MXActionType.INNER_EXECUTE
        else:
            action_type = MXActionType.EXECUTE
        execute_result_msg = self.generate_execute_result_message(scenario=scenario, error=error, request_ID=request_ID, action_type=action_type)
    ...
    def generate_execute_result_message(
        self, scenario: str, error: MXErrorCode, request_ID: str, action_type: MXActionType = MXActionType.EXECUTE
    ) -> MXExecuteResultMessage:
        execute_result_msg = MXExecuteResultMessage(function=self, scenario=scenario, request_ID=request_ID, error=error, action_type=action_type)
