...
class MXBigThing(MXThing):
    ...
    def _handle_MT_EXECUTE(self, msg: mqtt.MQTTMessage):
        ...
        if not self._compare_arg_list(target_function.get_arg_list(), list(execute_msg.tuple_arguments())):
            ...
            target_function._send_TM_RESULT_EXECUTE(scenario=execute_msg.scenario, error=MXErrorCode.FAIL, request_ID=execute_msg.request_ID)
            return False
        ...
        if self._is_parallel:
            ...
            if execute_msg.scenario in target_function.get_running_scenario_list():
                target_function._send_TM_RESULT_EXECUTE(scenario=execute_msg.scenario, error=MXErrorCode.DUPLICATE, request_ID=execute_msg.request_ID)
                return True
