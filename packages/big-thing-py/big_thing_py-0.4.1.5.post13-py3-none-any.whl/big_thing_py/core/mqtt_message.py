...
class MXExecuteMessage(MXMQTTReceiveMessage):
    def __init__(self, msg: mqtt.MQTTMessage) -> None:
        super().__init__(msg)
        self._protocol_type = MXProtocolType.get(msg.topic)
        if self._protocol_type == MXProtocolType.Base.MT_EXECUTE:
            ...
            if len(self.topic.split('/')) == 6:
            ...
            elif len(self.topic.split('/')) == 5:
                # MT/IN/EXECUTE/[FunctionName]/[ThingName] topic
                ...
                self.topic_error = True
        elif self._protocol_type == MXProtocolType.Base.MT_IN_EXECUTE:
...
class MXExecuteResultMessage(MXMQTTSendMessage):
    def __init__(
        ...
        if action_type == MXActionType.EXECUTE:
            ...
            self._protocol_type = MXProtocolType.Base.TM_IN_RESULT_EXECUTE
        ...
        if self.request_ID:
            ...
            if action_type == MXActionType.EXECUTE:
                self.topic = (self._protocol_type.value % (self.function.get_name(), self.function.get_thing_name(), '', '')).rstrip('/')
            elif action_type == MXActionType.INNER_EXECUTE:
                self.topic = self._protocol_type.value % (self.function.get_name(), self.function.get_thing_name())
