...
class FictiveForeignKey(PositiveIntegerField):
...
class TitledPositiveIntegerModelEnum(PositiveIntegerModelEnum, TitleFieldMixin):
    """
    Модель-перечисление с обязательными для заполнения целочисленным положительным полем value и текстовым полем title.
    """
...
class FictiveForeignKeyMixin:
    ...
    def fictive_foreign_key_field_names(self) -> List[str]:
        ...
        for field in self._meta.concrete_fields:
            if isinstance(field, FictiveForeignKey):
