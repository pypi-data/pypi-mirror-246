...
class Wrapper(object):
    ...
    def log_model(
        ...
        **kwargs):
        ...
        if model is None and (assets == None or len(assets) == 0):
        ...
        # Create tarball
        tarball_path = self._save_tarball(temp_folder, run_folder, assets)
    ...
    def _save_tarball(self, payload_dir, run_dir, assets=[]):
        ...
        elif type(assets) is str:
            try:
                asset_filenames.extend(self.copy_tree(assets, run_dir))
            except:
