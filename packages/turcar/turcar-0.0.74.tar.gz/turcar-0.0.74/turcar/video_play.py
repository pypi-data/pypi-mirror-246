...
class VideoPlay(tktextext.TextFrame):
    ...
    def populate_tree(self, parent, path):
        ...
        for item in os.listdir(path):
            ...
            if os.path.isdir(item_path):
                ...
                self.tree.insert(parent, "end", text=item, values=("file", item_path), iid=item)

    def on_tree_double_click(self, event):
        item = self.tree.selection()[0]
        # item_type = self.tree.set(item, "#1")
        self.media_path = item
    ...
    def show_browser(self, event):
        ...
        self.video_frame.place(relx=0, rely=0, relwidth=0.75, relheight=0.85)
        self.bottom_frame.place(relx=0, rely=0.85, relwidth=0.75, relheight=0.15)
