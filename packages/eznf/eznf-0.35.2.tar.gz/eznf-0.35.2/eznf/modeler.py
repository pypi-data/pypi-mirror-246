...
class Modeler:
    ...
    def constraint(self, constraint: str) -> None:
        ...
        #print(f"constraint clauses: ")
        for clause in clauses:
         #   print("received", clause)
            self.add_clause(clause)
          #  print("added: ")
           # self.print_clause(self._clauses[-1])
        #print('----')
    ...
    def debug(self, filename) -> None:
        ...
        if return_code == 10:
            ...
            if len(nxt) > 0:
                ...
                invalidate_assigment = input("Press enter to see what clauses are unsatisfied by an input assignment. ")
                relevant_lits = set()
                assignment = {}
                for clause in clauses:
                    for lit in clause:
...
def to_numerical(clause, modeler: Modeler) -> [int]:
    ...
    for slit in clause:
        if type(slit) == str:
            if slit[0] == '~' or slit[0] == '-':
                numerical_clause.append(-modeler.v(slit[1:]))
            else:
...
class CAtMostOne:
    ...
    def to_clauses(self) -> [[int]]:
        print("to _clauses of at most one over variables", self.variables)
