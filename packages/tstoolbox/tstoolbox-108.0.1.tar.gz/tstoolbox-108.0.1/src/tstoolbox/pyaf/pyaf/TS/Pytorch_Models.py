...
class cAbstract_RNN_Model(tsar.cAbstractAR):
    ...
    def fit_inputs_and_targets_scalers(self, iARInputs, iARTarget):
        ...
        lARTarget = self.mStandardScaler_Target.fit_transform(
            iARTarget.reshape(iARTarget.shape[0], 1)
        )
    ...
    def fit(self):
        ...
        (lARInputs, lARTarget) = self.fit_inputs_and_targets_scalers(
            lARInputs, lARTarget
        )
...
class cMLP_Model(cAbstract_RNN_Model):
    ...
    def create_model(self, iNbInputs, iHidden):
        ...
        model = nn.Sequential(
            nn.Linear(iNbInputs, iHidden), nn.Dropout(), nn.Linear(iHidden, 1)
        )
        return model.float()
    ...
    def build_RNN_Architecture(self, iARInputs, iARTarget):
        ...
        self.mModel = NeuralNetRegressor(
            self.create_model(lNbLags, self.mHiddenUnits),
        ...
        self.mOutName = (
            self.mCycleResidueName + "_" + lName + "(" + str(self.mNbLags) + ")"
        )
...
class cLSTM_Model(cAbstract_RNN_Model):
    ...
    def create_model(self, iNbInputs, iHidden):
        ...
        model = nn.Sequential(
            lLSTMWithOneOutput, nn.Dropout(p=0.1), nn.Linear(iHidden, 1)
        )
        return model.float()
