...
class cTemporalHierarchy(sighier.cSignalHierarchy):
    ...
    def aggregate_time_columns(self, level, signal, iAllLevelsDataset):
        ...
        for col in cols[1:]:
            # logical or
            new_col = iAllLevelsDataset[[signal, col]].apply(
                lambda x: x[1] if (x[0] is None) else x[0], axis=0
            )
    ...
    def add_temporal_data(self, df):
        ...
        for lPeriod in self.mPeriods:
            lName = lPrefix + "_" + lPeriod + "_start"
            df_resampled[lPeriod] = (
                df1[self.mSignal].resample(lPeriod).sum().reset_index()
            )
        ...
        for lPeriod in self.mPeriods:
            ...
            df_merge = df[[self.mDateColumn, self.mSignal]].merge(
                WData,
                left_on=self.mDateColumn,
                right_on=lName,
                how="left",
                suffixes=("_x", "_Period"),
                sort=True,
            ...
            logger.info(
                "FORECASTING_HIERARCHICAL_TEMPORAL_LEVEL "
                + str((lPeriod, lName, list(df.columns), WData.shape))
            )
    ...
    def compute_horizons(self, df):
        ...
        for lPeriod in self.mPeriods:
            ...
            logger.info(
                "FORECASTING_HIERARCHICAL_TEMPORAL_HORIZONS_FIRST_RESAMPLED_DATA "
                + str(lPeriod)
                + " "
                + str(df_resampled[lPeriod].head(5).to_dict())
            )
    ...
    def checkPhysicalTime(self):
        ...
        if not lIsPhysical:
            raise tsutil.PyAF_Error(
                "TIME_HIERARCHY_PHYSICAL_TIME_NEEDED "
                + str(self.mDateColumn)
                + " "
                + str(self.mTrainingDataset[self.mDateColumn].dtype)
            )
    ...
    def check_increasing_periods(self):
        ...
        lTimeFreqInSeconds = lHelper.computeTimeFrequency_in_seconds(
            self.mTrainingDataset[self.mDateColumn]
        )
        if lTimeFreqInSeconds > lPreviousPeriod:
            raise tsutil.PyAF_Error(
                "TIME_HIERARCHY_PHYSICAL_TIME_RESOLUTION_TOO_LOW_FOR_THIS_PERIOD "
                + str(self.mDateColumn)
                + " "
                + str(lTimeFreqInSeconds)
                + " "
                + self.mPeriods[0]
                + " "
                + str(lPreviousPeriod)
        ...
        for lPeriod in self.mPeriods[1:]:
            if lSeconds[lPeriod] < lPreviousPeriod:
                raise tsutil.PyAF_Error(
                    "TIME_HIERARCHY_NOT_MONOTONOUS " + str(self.mPeriods)
                )
