...
class cAbstract_Scikit_Model(tsar.cAbstractAR):
    ...
    def fit(self):
        ...
        try:
            ...
            print(
                "SCIKIT_MODEL_FEATURE_SELECTION_FAILURE",
                self.mOutName,
                lARInputs.shape,
                e,
            )
        ...
        try:
            ...
            print(
                "SCIKIT_MODEL_FIT_FAILURE",
                self.mOutName,
                lARInputsAfterSelection.shape,
                e,
            )
            if self.mOptions.mDebug:
        ...
        if self.mScikitModel is not None:
            ...
            lFullARInputsAfterSelection = (
                self.mFeatureSelector.transform(lFullARInputs)
                if self.mFeatureSelector
                else lFullARInputs
            )
...
class cAutoRegressiveModel(cAbstract_Scikit_Model):
    ...
    def dumpCoefficients(self, iMax=10):
        ...
        lDict = dict(
            zip(self.mInputNamesAfterSelection, self.mScikitModel.coef_.round(6))
        )
        lDict1 = dict(
            zip(self.mInputNamesAfterSelection, abs(self.mScikitModel.coef_.round(6)))
        )
        i = 1
...
class cLightGBM_Model(cAbstract_Scikit_Model):
    ...
    def get_default_lgbm_options(self):
        lLGBMOptions = dict(
            objective="regression", n_estimators=32, random_state=self.mOptions.mSeed
        )
        return lLGBMOptions
