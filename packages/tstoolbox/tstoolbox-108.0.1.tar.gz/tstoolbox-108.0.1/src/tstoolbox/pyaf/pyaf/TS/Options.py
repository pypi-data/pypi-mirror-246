...
class cSignalDecomposition_Options(cModelControl):
    def __init__(self):
        ...
        # to be deprecated when cross validation is OK.
        ...
        self.mSamplingThreshold = (
            8192  # Time series larger than this threshold will be sampled.
        )
        self.enable_fast_mode()
        self.mTimeDeltaComputationMethod = "AVG"
        # can be "AVG", "MODE", "USER"
        ...
        # "Target_Mean" or "Target_Median"
        self.mHierarchicalCombinationMethod = "BU"
        self.mForecastRectifier = (
            None  # can be "relu" to force positive forecast values
        )
        ...
        self.mDL_Backends = (
            "PyTorch",
        )  # Pytorch is the only supported backend for now
    ...
    def enable_slow_mode(self):
        self.mQuantiles = [5, 10, 20]
        # quintiles, deciles, and vingtiles;)
    ...
    def hasPytorchOrKerasInstalled(self, iModel):
        return self.has_module_installed("torch") or self.has_module_installed(
            "tensorflow"
        )
