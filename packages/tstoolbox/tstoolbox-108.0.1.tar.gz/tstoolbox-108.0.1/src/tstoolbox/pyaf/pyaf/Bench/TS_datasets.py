...
def load_airline_passengers():
    ...
    df_train = pd.read_csv(
        trainfile, names=cols, sep=r",", index_col="ID", engine="python", skiprows=1
    )
...
def load_cashflows():
    ...
    trainfile = (
        "https://raw.githubusercontent.com/antoinecarme/pyaf/master/data/CashFlows.txt"
    )
    tsspec.mFullDataset = pd.read_csv(trainfile, sep=r"\t", engine="python")
    tsspec.mFullDataset["Date"] = tsspec.mFullDataset["Date"].apply(
...
def load_ozone():
    ...
    df_train["Time"] = df_train["Month"].apply(
...
def load_ozone_exogenous():
    ...
    df_train["Time"] = df_train["Date"].apply(
        lambda x: datetime.datetime.strptime(x, "%Y-%m")
    )
...
def load_ozone_exogenous_categorical():
    ...
    lSig1 = df_train["Ozone"] * (ozone_shifted_2["Exog3"] == "AW") + df_train[
        "Ozone"
    ] * (ozone_shifted_1["Exog3"] == "AX")
...
def gen_cycle(N, cycle_length):
    ...
    if cycle_length > 0:
        ...
        lValues = (
            np.random.randint(0, cycle_length, size=(cycle_length, 1)) / cycle_length
        )
...
def generate_random_TS(
    ...
    tsspec = cTimeSeriesDatasetSpec()
    lName = generate_random_TS_name(
        N, FREQ, seed, trendtype, cycle_length, transform, sigma, exog_count, ar_order
    )
    tsspec.mName = lName
    ...
    df_train["Signal"] = (
        df_train["GeneratedTrend"]
        + df_train["GeneratedCycle"]
        + 1 * df_train["GeneratedAR"]
        + df_train["Noise"]
    )
...
def load_NN5():
    ...
    tsspec.mFullDataset["Day"] = tsspec.mFullDataset["Day"].apply(
...
def load_MWH_dataset(name):
    ...
    trainfile = (
        "https://raw.githubusercontent.com/antoinecarme/TimeSeriesData/master/FMA/"
        + name
        + ".csv"
    )
    # trainfile = "data/FMA/" + name + ".csv"
    df_train = pd.read_csv(
        trainfile, sep=r",", header=None, engine="python", skipinitialspace=True
    )
    ...
    if type1.kind == "O":
        ...
        df_train = pd.read_csv(
            trainfile, sep=r",", header=0, engine="python", skipinitialspace=True
        )
...
def load_M1_comp():
    ...
    tsspec.mHorizons["Series"] = tsspec.mHorizons["Series"].apply(
        lambda x: x.replace(" ", "")
    )
    tsspec.mFullDataset["Index"] = tsspec.mFullDataset["Series"]
    tsspec.mFullDataset["Index"] = tsspec.mFullDataset["Index"].apply(
        lambda x: x.replace(" ", "")
    )
    tsspec.mFullDataset.drop(
        ["Series", "N Obs", "Seasonality", "NF", "Type", "Starting date", "Category"],
        axis=1,
        inplace=True,
    )
...
def load_M2_comp():
    ...
    tsspec.mFullDataset.drop(
        ["Series", "N", "Seasonality", "Starting Date", "Ending Date"],
        axis=1,
        inplace=True,
    )
    tsspec.mFullDataset.set_index(["Index"], inplace=True)
...
def load_M3_Y_comp():
    ...
    trainfile = (
        "https://raw.githubusercontent.com/antoinecarme/pyaf/master/data/M3_Yearly.csv"
    )
    ...
    tsspec.mFullDataset.drop(
        ["Series", "N", "NF", "Starting Year", "Category", "Unnamed: 5"],
        axis=1,
        inplace=True,
    )
    tsspec.mFullDataset.set_index(["Index"], inplace=True)
...
def load_M3_Q_comp():
    ...
    tsspec.mFullDataset.drop(
        ["Series", "N", "NF", "Starting Year", "Category", "Starting Quarter"],
        axis=1,
        inplace=True,
    )
    tsspec.mFullDataset.set_index(["Index"], inplace=True)
...
def load_M3_M_comp():
    ...
    trainfile = (
        "https://raw.githubusercontent.com/antoinecarme/pyaf/master/data/M3_Monthly.csv"
    )
    ...
    tsspec.mFullDataset.drop(
        ["Series", "N", "NF", "Starting Year", "Category", "Starting Month"],
        axis=1,
        inplace=True,
    )
    tsspec.mFullDataset.set_index(["Index"], inplace=True)
...
def load_M3_Other_comp():
    ...
    trainfile = (
        "https://raw.githubusercontent.com/antoinecarme/pyaf/master/data/M3_Other.csv"
    )
    ...
    tsspec.mFullDataset.drop(
        ["Series", "N", "NF", "Category", "Unnamed: 4", "Unnamed: 5"],
        axis=1,
        inplace=True,
    )
    tsspec.mFullDataset.set_index(["Index"], inplace=True)
...
def load_M4_comp(iType=None):
    ...
    trainfile = (
        "https://github.com/antoinecarme/pyaf/blob/master/data/M4Comp/M4Comp_"
        + iType
        + ".csv.gz?raw=true"
    )
    ...
    df_full = pd.read_csv(
        trainfile, sep=",", header=0, engine="python", compression="gzip"
    )
    ...
    for i in range(0, df_full.shape[0]):
        ...
        tsspec.mFullDataset[series_name] = tsspec.mPastData.append(
            tsspec.mFutureData
        ).reindex()
        tsspec.mFullDataset["Date"] = range(0, tsspec.mFullDataset.shape[0])
        tsspec.mTimeVar = "Date"
...
def get_stock_web_link():
    YAHOO_LINKS_DATA = {}
    with open("pyaf/Bench/yahoo_data.json") as read_file:
        import json
...
def load_yahoo_stock_prices(symbol_list_key, stock=None):
    ...
    for stock in sorted(stocks):
        tsspec1 = None
        try:
            tsspec1 = load_yahoo_stock_price(
                symbol_list_key, stock, True, YAHOO_LINKS_DATA
            )
        except:
            # raise
            pass
...
def load_AU_hierarchical_dataset():
    ...
    df_train[lDateColumn] = df_train[lDateColumn].apply(
        lambda x: datetime.datetime.strptime(x, "%Y-%m-%d")
    )
...
def load_fpp2_dataset(name):
    ...
    trainfile = (
        "https://raw.githubusercontent.com/antoinecarme/TimeSeriesData/master/fpp2/"
        + name
        + ".csv"
    )
