...
class cTimeInfo:
        ...
        if self.mTimeMax is not None:
            ...
            if lType1.kind != lType2.kind:
                raise tsutil.PyAF_Error(
                    "Incompatible Time Column Type expected="
                    + str(lType1)
                    + " got: "
                    + str(lType2)
                    + "'"
    ...
    def transformDataset(self, df):
        ...
        df = pd.concat(
            [df, lLastRow], ignore_index=True, verify_integrity=True, sort=False
        )

        if self.mNormalizedTimeColumn not in df.columns:
            df[self.mRowNumberColumn] = np.arange(0, df.shape[0])
            df[self.mNormalizedTimeColumn] = self.compute_normalized_date_column(
                df[self.mTime]
            )
    ...
    def checkDateTypes(self):
        ...
        if type1.kind == "O":
            raise tsutil.PyAF_Error(
                "Invalid Time Column Type " + self.mTime + "[" + str(type1) + "]"
            )
    ...
    def adaptTimeDeltaToTimeResolution(self):
        if not self.isPhysicalTime():
            return
        lHelper = dtfunc.cDateTime_Helper()
        self.mTimeDelta = lHelper.adaptTimeDeltaToTimeResolution(
            self.mResolution, self.mTimeDelta
        )
    ...
    def estimate(self):
        ...
        self.mSignalFrame[
            self.mNormalizedTimeColumn
        ] = self.compute_normalized_date_column(self.mSignalFrame[self.mTime])
        self.dump()
