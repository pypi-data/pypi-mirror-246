...
class cGeneric_OneSignal_Tester:
    ...
    def reportTrainingDataInfo(self, iSignal, iHorizon):
        ...
        print(
            ...
            iHorizon,
            "HEAD=",
            df[lDate].head().values,
            "TAIL=",
            df[lDate].tail().values,
        )
        print(
            "SIGNAL : ",
            iSignal,
            ...
            iHorizon,
            "HEAD=",
            df[iSignal].head().values,
            "TAIL=",
            df[iSignal].tail().values,
        )
        # df.to_csv("bench.csv");
        print(df.head())
        print(df.info())
    ...
    def reportActualAndPredictedData(self, iSignal, iHorizon):
        print(
            "FORECAST_DETAIL_ACTUAL",
            self.mTSSpec.mName,
            iSignal,
            [self.mActual.values[h] for h in range(iHorizon)],
        )
        print(
            "FORECAST_DETAIL_PREDICTED",
            self.mTSSpec.mName,
            iSignal,
    ...
    def computePerfOnForecasts(self, iSignal, iHorizon):
        ...
        lForecastPerf = lAutoF1.computePerf(
            self.mActual,
            self.mPredicted,
            self.mBenchName + "_" + self.mTSSpec.mName + "_" + iSignal,
        )
    ...
    def getTestPerfs(self, iSignal, iHorizon):
        ...
        self.mPredicted = (
            self.mApplyOut[iSignal + "_Forecast"].tail(iHorizon).reset_index(drop=True)
        )
    ...
    def dumpForecastPerfs(self, iSignal, iHorizon):
        ...
        print(
            "BENCHMARK_PERF_DETAIL_SIGNAL_HORIZON",
            self.mTSSpec.mName,
            iSignal,
        ...
        print(
            "BENCHMARK_PERF_DETAIL_BEST_MODEL",
            self.mTSSpec.mName,
            iSignal,
        ...
        print(
            "BENCHMARK_PERF_DETAIL_PERF_MAPE_SMAPE_MASE",
            self.mTSSpec.mName,
            iSignal,
            lPerf.mMAPE,
            lPerf.mSMAPE,
        ...
        print(
            "BENCHMARK_PERF_DETAIL_PERF_L1_L2_R2",
            self.mTSSpec.mName,
            iSignal,
            lPerf.mL1,
            lPerf.mL2,
    ...
    def plot(self, iSignal, iHorizon):
        ...
        for k in ["Forecast", "Prediction_Intervals"]:  # lPlots.keys():
            ...
            # lOutput = lOutput + ["IMG"];
            lOutput = lOutput + [
                '<img src=\\"data:image/png;base64,' + str(lPlot_PNG_Base64) + '\\" />'
            ]
        return lOutput
