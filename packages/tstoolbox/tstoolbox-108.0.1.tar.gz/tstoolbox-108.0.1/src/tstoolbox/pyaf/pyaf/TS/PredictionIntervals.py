...
class cPredictionIntervalsEstimator:
    ...
    def computePerformances(self):
        ...
        (
            ...
            lOriginalForecast,
            lOriginalTest,
        ) = self.mModel.mTimeInfo.mSplit.cutFrame(df)
        df1 = df
        for h in range(0, self.mHorizon):
            df2 = None
            df2 = self.mModel.forecastOneStepAhead(
                df1, horizon_index=h + 1, perf_mode=True
            )
            df2 = df2.head(N)
            ...
            (
                ...
                lFrameForecast,
                lFrameTest,
            ) = self.mModel.mTimeInfo.mSplit.cutFrame(df2)
            self.mFitPerformances[lHorizonName] = tsperf.cPerf()
            self.mFitPerformances[lHorizonName].compute(
                lOriginalFit[lSignalColumn], lFrameFit[lForecastColumn], lHorizonName
            )
            self.mForecastPerformances[lHorizonName] = tsperf.cPerf()
            self.mForecastPerformances[lHorizonName].compute(
                lOriginalForecast[lSignalColumn],
                lFrameForecast[lForecastColumn],
                lHorizonName,
            )
            self.mTestPerformances[lHorizonName] = tsperf.cPerf()
            if lOriginalTest.shape[0] > 0:
                self.mTestPerformances[lHorizonName].compute(
                    lOriginalTest[lSignalColumn],
                    lFrameTest[lForecastColumn],
                    lHorizonName,
                )
            df1 = df2[
                [
    ...
    def dump_detailed(self):
        ...
        for h in range(0, self.mHorizon):
            ...
            hn = lHorizonName
            logger.info(
                "CONFIDENCE_INTERVAL_DUMP_FIT "
                + str(hn)
                + " "
                + str(self.mFitPerformances[hn].mL2)
                + " "
                + str(self.mFitPerformances[hn].mMAPE)
            )
            logger.info(
                "CONFIDENCE_INTERVAL_DUMP_FORECAST "
                + str(hn)
                + " "
                + str(self.mForecastPerformances[hn].mL2)
                + " "
                + str(self.mForecastPerformances[hn].mMAPE)
            )
            logger.info(
                "CONFIDENCE_INTERVAL_DUMP_TEST "
                + str(hn)
                + " "
                + str(self.mTestPerformances[hn].mL2)
                + " "
                + str(self.mTestPerformances[hn].mMAPE)
            )
