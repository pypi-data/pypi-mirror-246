...
class cDateTime_Helper:
    ...
    def guess_time_resolution(self, iEstimTime):
        ...
        lEstimDayOfMonth = self.apply_date_time_computer(
            eDatePart.DayOfMonth, iEstimTime
        )
    ...
    def adaptTimeDeltaToTimeResolution(self, iResolution, iTimeDelta):
        if eTimeResolution.SECOND == iResolution:
            lTimeDelta = pd.DateOffset(
                seconds=round(iTimeDelta / np.timedelta64(1, "s"))
            )
            return lTimeDelta
        if eTimeResolution.MINUTE == iResolution:
            lTimeDelta = pd.DateOffset(
                minutes=round(iTimeDelta / np.timedelta64(1, "m"))
            )
            return lTimeDelta
        ...
        if eTimeResolution.MONTH == iResolution:
            lTimeDelta = pd.DateOffset(
                months=round(iTimeDelta // np.timedelta64(30, "D"))
            )
            return lTimeDelta
        if eTimeResolution.YEAR == iResolution:
            lTimeDelta = pd.DateOffset(
                months=round(iTimeDelta // np.timedelta64(365, "D"))
            )
            return lTimeDelta
        pass
