...
def decomp_plot(
    ...
    fig = decomp_plot_internal(
        df, time, signal, estimator, residue, name, format, max_length, horizon
    )
    if name is not None:
        import matplotlib
        import matplotlib.pyplot as plt
...
def decomp_plot_as_png_base64(
    df, time, signal, estimator, residue, name=None, max_length=1000, horizon=1
):
    fig = decomp_plot_internal(
        df, time, signal, estimator, residue, name, format, max_length, horizon
    )
...
def prediction_interval_plot_internal(
    df,
    time,
    signal,
    estimator,
    ...
    axs.fill_between(
        df1[time].values, df1[lower], df1[upper], color=SHADED_COLOR, alpha=0.2
    )
...
def prediction_interval_plot(
    ...
    fig = prediction_interval_plot_internal(
        df,
        time,
        signal,
        estimator,
...
def prediction_interval_plot_as_png_base64(
    df,
    time,
    signal,
    estimator,
...
def quantiles_plot_internal(
    df, time, signal, estimator, iQuantiles, name=None, format="png", horizon=1
):
    assert df.shape[0] > 0
    assert df.shape[1] > 0
    assert time in df.columns
    ...
    for h in range(horizon):
        ...
        _, bins1, patches = axs[h].hist(
            q_values, bins=q_values, weights=[1] * len(lQuantileNames), density=True
        )
...
def quantiles_plot(
    ...
    fig = quantiles_plot_internal(
        df, time, signal, estimator, iQuantiles, name, format, horizon
    )
    import matplotlib
    import matplotlib.pyplot as plt
...
def quantiles_plot_as_png_base64(
    df, time, signal, estimator, iQuantiles, name=None, format="png", horizon=1
):
    fig = quantiles_plot_internal(
        df, time, signal, estimator, iQuantiles, name, format, horizon
...
def plot_hierarchy_internal(structure, iAnnotations, name):
    ...
    for level in lLevelsReversed:
        color = "#{:02x}{:02x}{:02x}".format(
            255,
            255,
            127 + int(128 * (1.0 - (level + 1.0) / len(lLevelsReversed))),
        )
        for col in structure[level].keys():
            ...
            node_col = pydot.Node(
                col, label=lLabel, style="filled", fillcolor=color, fontsize="12.0"
            )
            ...
            for col1 in structure[level][col]:
                ...
                color1 = "#{:02x}{:02x}{:02x}".format(
                    255,
                    255,
                    128 + int(128 * (1.0 - (level + 2.0) / len(lLevelsReversed))),
                )
                node_col1 = pydot.Node(
                    col1,
                    label=lLabel1,
                    style="filled",
                    fillcolor=color1,
                ...
                lEdge = pydot.Edge(
                    node_col, node_col1, color="red", label=lEdgeLabel, fontsize="12.0"
                )
