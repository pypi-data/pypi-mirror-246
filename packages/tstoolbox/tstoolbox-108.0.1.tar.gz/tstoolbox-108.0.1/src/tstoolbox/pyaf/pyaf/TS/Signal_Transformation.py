    ...
    def check_not_nan(self, sig, name):
        if np.isnan(sig).any():
            ...
            raise tsutil.Internal_PyAF_Error(
                "Invalid transformation for column '" + name + "'"
            )
        pass
...
class cSignalTransform_None(cAbstractSignalTransform):
    ...
    def dump_values(self):
        ...
        logger.info(
            "SIGNAL_TRANSFORMATION_MODEL_VALUES " + self.mFormula + " " + str(None)
        )
...
class cSignalTransform_Quantize(cAbstractSignalTransform):
    ...
    def dump_values(self):
        logger = tsutil.get_pyaf_logger()
        logger.info(
            "QUANTIZE_TRANSFORMATION_MIN_MAX_CURVE "
            + self.mFormula
            + " "
            + str((self.mMin, self.mMax))
            + " "
            + str(self.mCurve)
        )
...
class cSignalTransform_BoxCox(cAbstractSignalTransform):
    ...
    def dump_values(self):
        logger = tsutil.get_pyaf_logger()
        logger.info(
            "BOX_COX_TRANSFORMATION_LAMBDA " + self.mFormula + " " + str(self.mLambda)
        )
...
class cSignalTransform_Differencing(cAbstractSignalTransform):
    ...
    def dump_values(self):
        logger = tsutil.get_pyaf_logger()
        logger.info(
            "DIFFERENCING_TRANSFORMATION " + self.mFormula + " " + str(self.mFirstValue)
        )
...
class cSignalTransform_RelativeDifferencing(cAbstractSignalTransform):
    ...
    def specific_invert(self, sig):
        ...
        sig_orig = rate_cum.clip(-1.0e8, +1.0e8)
    ...
    def dump_values(self):
        logger = tsutil.get_pyaf_logger()
        logger.info(
            "REALTIVE_DIFFERENCING_TRANSFORMATION "
            + self.mFormula
            + " "
            + str(self.mFirstValue)
        )
...
class cSignalTransform_Anscombe(cAbstractSignalTransform):
    ...
    def dump_values(self):
        logger = tsutil.get_pyaf_logger()
        logger.info(
            "ANSCOMBE_TRANSFORMATION " + self.mFormula + " " + str(self.mConstant)
        )
