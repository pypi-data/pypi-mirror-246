...
class cAbstractCycle:
    ...
    def compute_cycle_residue(self, df):
        ...
        df[self.getCycleResidueName()] = df[self.getCycleResidueName()].astype(
            target.dtype
        )
    ...
    def compute_target_means_by_cycle_value(self, iCycleFrame, iCycleName):
        ...
        lGroupBy = lCycleFrameEstim.groupby(by=[iCycleName], sort=False)[
            self.mTrend_residue_name
        ]
    ...
    def computePerf(self):
        if self.mOptions.mDebug:
            self.check_not_nan(
                self.mCycleFrame[self.getCycleResidueName()], self.getCycleResidueName()
            )
        ...
        self.mCycleFitPerf.computeCriterion(
            lFrameFit[self.mTrend_residue_name],
            lFrameFit[self.getCycleName()],
            self.mOptions.mCycle_Criterion,
            self.getCycleName(),
        )
        self.mCycleForecastPerf.computeCriterion(
            lFrameForecast[self.mTrend_residue_name],
            lFrameForecast[self.getCycleName()],
            self.mOptions.mCycle_Criterion,
            self.getCycleName(),
        )
...
class cZeroCycle(cAbstractCycle):
    ...
    def dump_values(self):
        logger = tsutil.get_pyaf_logger()
        logger.info(
            "ZERO_CYCLE_MODEL_VALUES "
            + self.getCycleName()
            + " "
            + str(self.mConstantValue)
            + " {}"
        )
    ...
    def fit(self):
        ...
        self.mCycleFrame[self.mTrend_residue_name] = self.mTrendFrame[
            self.mTrend_residue_name
        ]
...
class cSeasonalPeriodic(cAbstractCycle):
    ...
    def dump_values(self):
        logger = tsutil.get_pyaf_logger()
        lDict = {k: round(v, 6) for (k, v) in self.mEncodedValueDict.items()}
        logger.info(
            "SEASONAL_MODEL_VALUES "
            + self.getCycleName()
            + " "
            + str(round(self.mDefaultValue, 6))
            + " "
            + str(lDict)
        )
    ...
    def fit(self):
        ...
        self.mEncodedValueDict = self.compute_target_means_by_cycle_value(
            self.mCycleFrame, self.getCycleName()
        )

        self.mCycleFrame[lName + "_enc"] = (
            self.mCycleFrame[lName]
            .map(self.mEncodedValueDict)
            .fillna(self.mDefaultValue)
        )
    ...
    def transformDataset(self, df):
        ...
        df[self.getCycleName()] = lDateParts.map(self.mEncodedValueDict).fillna(
            self.mDefaultValue
        )
        self.compute_cycle_residue(df)
        return df
...
class cBestCycleForTrend(cAbstractCycle):
    ...
    def dump_values(self):
        ...
        lDict = (
            {}
            if (self.mBestCycleLength is None)
            else self.mBestCycleValueDict[self.mBestCycleLength]
        )
        lDict = {k: round(v, 6) for (k, v) in lDict.items()}
        logger.info(
            "BEST_CYCLE_LENGTH_VALUES "
            + self.getCycleName()
            + " "
            + str(self.mBestCycleLength)
            + " "
            + str(round(self.mDefaultValue, 6))
            + " "
            + str(lDict)
    ...
    def generate_cycles(self):
        ...
        lCycleLengths = self.mOptions.mCycleLengths or range(
            2, lMaxRobustCycleLength + 1
        )
        lCycleFrame = pd.DataFrame(index=self.mTrendFrame.index)
        lCycleFrame[self.mTrend_residue_name] = self.mTrendFrame[
            self.mTrend_residue_name
        ]
        for lLength in lCycleLengths:
            if (lLength > 1) and (lLength <= lMaxRobustCycleLength):
                name_length = self.mTrend_residue_name + "_Cycle"
                lCycleFrame[name_length] = (
                    self.mCycleFrame[self.mTimeInfo.mRowNumberColumn] % lLength
                )
                lEncodedValueDict = self.compute_target_means_by_cycle_value(
                    lCycleFrame, name_length
                )
                lCycleFrame[name_length + "_enc"] = (
                    lCycleFrame[name_length]
                    .map(lEncodedValueDict)
                    .fillna(self.mDefaultValue)
                )
                ...
                lCritValue = lPerf.computeCriterion(
                    lValidFrame[self.mTrend_residue_name],
                    lValidFrame[name_length + "_enc"],
                    self.mCriterion,
                    "Validation",
                )
                if lPerf.is_acceptable_criterion_value(
                    self.mOptions.mCycle_Criterion, iRefValue=lCritValue
                ):
                    self.mCyclePerfByLength[lLength] = (
                        round(lCritValue, 3),
                        len(lEncodedValueDict),
                    )
                    if self.mOptions.mDebugCycles:
                        logger = tsutil.get_pyaf_logger()
                        logger.debug(
                            "CYCLE_INTERNAL_CRITERION "
                            + name_length
                            + " "
                            + str(lLength)
                            + " "
    ...
    def transformDataset(self, df):
        ...
        if self.mOptions.mDebug:
            self.check_not_nan(
                self.mCycleFrame[self.getCycleName()].values, self.getCycleName()
            )
...
class cCycleEstimator:
    ...
    def addSeasonal(self, trend, seas_type, resolution):
        if resolution >= self.mTimeInfo.mResolution:
            ...
            if self.mOptions.mActivePeriodics[lSeasonal.mFormula]:
                if lSeasonal.hasEnoughData(
                    self.mTimeInfo.mTimeMin, self.mTimeInfo.mTimeMax
                ):
                    ...
                    if self.mOptions.mDebugCycles:
                        ...
                        logger = tsutil.get_pyaf_logger()
                        logger.debug(
                            "NOT_ENOUGH_DATA_TO_ANAYLSE_SEASONAL_PATTERN "
                            + str(
                                (trend.__class__.__name__, seas_type, resolution, lDays)
                            )
                        )
    ...
    def defineCycles(self):
        for trend in self.mTrendList:
            ...
            if self.mTimeInfo.isPhysicalTime():
                ...
                self.addSeasonal(
                    trend, dtfunc.eDatePart.MonthOfYear, dtfunc.eTimeResolution.MONTH
                )
                self.addSeasonal(
                    trend, dtfunc.eDatePart.WeekOfYear, dtfunc.eTimeResolution.DAY
                )
                self.addSeasonal(
                    trend, dtfunc.eDatePart.DayOfMonth, dtfunc.eTimeResolution.DAY
                )
                self.addSeasonal(
                    trend, dtfunc.eDatePart.DayOfWeek, dtfunc.eTimeResolution.DAY
                )
                self.addSeasonal(
                    trend, dtfunc.eDatePart.DayOfYear, dtfunc.eTimeResolution.DAY
                )
                self.addSeasonal(
                    trend, dtfunc.eDatePart.Hour, dtfunc.eTimeResolution.HOUR
                )
                self.addSeasonal(
                    trend, dtfunc.eDatePart.Minute, dtfunc.eTimeResolution.MINUTE
                )
                self.addSeasonal(
                    trend, dtfunc.eDatePart.Second, dtfunc.eTimeResolution.SECOND
                )
                self.addSeasonal(
                    trend, dtfunc.eDatePart.HourOfWeek, dtfunc.eTimeResolution.HOUR
                )
                self.addSeasonal(
                    trend, dtfunc.eDatePart.TwoHourOfWeek, dtfunc.eTimeResolution.HOUR
                )
                self.addSeasonal(
                    trend, dtfunc.eDatePart.ThreeHourOfWeek, dtfunc.eTimeResolution.HOUR
                )
                self.addSeasonal(
                    trend, dtfunc.eDatePart.FourHourOfWeek, dtfunc.eTimeResolution.HOUR
                )
                self.addSeasonal(
                    trend, dtfunc.eDatePart.SixHourOfWeek, dtfunc.eTimeResolution.HOUR
                )
                self.addSeasonal(
                    trend, dtfunc.eDatePart.EightHourOfWeek, dtfunc.eTimeResolution.HOUR
                )
                self.addSeasonal(
                    trend,
                    dtfunc.eDatePart.TwelveHourOfWeek,
                    dtfunc.eTimeResolution.HOUR,
                )
                self.addSeasonal(
                    trend, dtfunc.eDatePart.WeekOfMonth, dtfunc.eTimeResolution.DAY
                )
                self.addSeasonal(
                    trend,
                    dtfunc.eDatePart.DayOfNthWeekOfMonth,
                    dtfunc.eTimeResolution.DAY,
                )
    ...
    def dumpCyclePerf(self, cycle):
        if self.mOptions.mDebugCycles:
            logger = tsutil.get_pyaf_logger()
            logger.debug(
                "CYCLE_PERF_DETAIL_COUNT_FIT_FORECAST "
                + cycle.mOutName
                + " %.3f" % (cycle.mCycleFitPerf.mCount)
                + " %.3f" % (cycle.mCycleForecastPerf.mCount)
            )
            logger.debug(
                "CYCLE_PERF_DETAIL_MAPE_FIT_FORECAST "
                + cycle.mOutName
                + " %.3f" % (cycle.mCycleFitPerf.mMAPE)
                + " %.3f" % (cycle.mCycleForecastPerf.mMAPE)
            )
            logger.debug(
                "CYCLE_PERF_DETAIL_L2_FIT_FORECAST "
                + cycle.mOutName
                + " %.3f" % (cycle.mCycleFitPerf.mL2)
                + " %.3f" % (cycle.mCycleForecastPerf.mL2)
            )
            logger.debug(
                "CYCLE_PERF_DETAIL_R2_FIT_FORECAST "
                + cycle.mOutName
                + " %.3f" % (cycle.mCycleFitPerf.mR2)
                + " %.3f" % (cycle.mCycleForecastPerf.mR2)
            )
            logger.debug(
                "CYCLE_PERF_DETAIL_PEARSONR_FIT_FORECAST "
                + cycle.mOutName
                + " %.3f" % (cycle.mCycleFitPerf.mPearsonR)
                + " %.3f" % (cycle.mCycleForecastPerf.mPearsonR)
            )
    ...
    def estimateCycles(self):
        ...
        for trend in self.mTrendList:
            ...
            self.mCycleFrame[lTrend_residue_name] = self.mTrendFrame[
                lTrend_residue_name
            ]
            for cycle in self.mCycleList[trend]:
                cycle.fit()
                ...
                self.mCycleFrame[cycle.getCycleName()] = cycle.mCycleFrame[
                    cycle.getCycleName()
                ]
                self.mCycleFrame[cycle.getCycleResidueName()] = cycle.mCycleFrame[
                    cycle.getCycleResidueName()
                ]
                if self.mOptions.mDebug:
                    cycle.check_not_nan(
                        self.mCycleFrame[cycle.getCycleResidueName()].values,
                        cycle.getCycleResidueName(),
                    )
        ...
        for trend in self.mTrendList:
            ...
            for cycle in self.mCycleList[trend]:
                if isinstance(cycle, cSeasonalPeriodic):
                    ...
                    if cycle.mCycleForecastPerf.is_acceptable_criterion_value(
                        self.mOptions.mCycle_Criterion
                    ):
                        lCritValue = cycle.mCycleForecastPerf.getCriterionValue(
                            self.mOptions.mCycle_Criterion
                        )
            ...
            lData_smallest = [
                x
                for x in lData
                if lBestPerf.is_close_criterion_value(
                    self.mOptions.mCycle_Criterion, x[1][0], iTolerance=0.05
                )
            ]
            lSortingMethod_By_Complexity = lambda x: (x[1][1], x[0])
            lData_smallest = sorted(lData_smallest, key=lSortingMethod_By_Complexity)
            ...
            if self.mOptions.mDebugCycles:
                logger.info(
                    "CYCLE_TRAINING_FILTER_SEASONALS_DATA "
                    + trend.mOutName
                    + " "
                    + str(lData_smallest)
                )
                logger.info(
                    "CYCLE_TRAINING_FILTER_SEASONALS_BEST "
                    + trend.mOutName
                    + " "
                    + lBestSeasonal.mOutName
                    + " "
                    + str(lBestCriterion)
                )
    ...
    def estimateAllCycles(self):
        ...
        if self.mOptions.mDebugProfile:
            lTimer = tsutil.cTimer(
                ("TRAINING_CYCLES", {"Signal": self.mTimeInfo.mSignal})
            )
        self.defineCycles()
        self.estimateCycles()
