...
class cTimeSeriesModel:
    ...
    def signal_info(self):
        ...
        lStr1 += (
            " Min=" + to_str(np.min(lSignal)) + " Max=" + to_str(np.max(lSignal)) + " "
        )
        lStr1 += (
            " Mean=" + to_str(np.mean(lSignal)) + " StdDev=" + to_str(np.std(lSignal))
        )
    ...
    def get_model_category(self):
        lModelCategory = (
            self.mTransformation.__class__.__name__,
            self.mTrend.__class__.__name__,
            self.mCycle.__class__.__name__,
            self.mAR.__class__.__name__,
        )
        lModelCategory = (
            self.mTransformation.mFormula
            + "_"
            + self.mTrend.mFormula
            + "_"
            + self.mCycle.mFormula
            + "_"
            + self.mAR.mFormula
        )
    ...
    def getComplexity(self):
        ...
        lComplexity = (
            self.mTransformation.mComplexity
            + self.mTrend.mComplexity
            + self.mCycle.mComplexity
            + self.mAR.mComplexity
        )
        lComplexity = lComplexity + lModelTypeComplexity.get(
            self.mDecompositionType, 0.0
        )
        return lComplexity
    ...
    def updatePerfs(self, compute_all_indicators=False):
        ...
        (lFrameFit, lFrameForecast, lFrameTest) = self.mTrend.mSplit.cutFrame(
            self.mModelFrame
        )

        if compute_all_indicators:
            lFitPerf.compute(
                lFrameFit[self.mOriginalSignal],
                lFrameFit[lForecastColumnName],
                self.mOutName + "_Fit",
            )
            lForecastPerf.compute(
                lFrameForecast[self.mOriginalSignal],
                lFrameForecast[lForecastColumnName],
                self.mOutName + "_Forecast",
            )
            if lFrameTest.shape[0] > 0:
                lTestPerf.compute(
                    lFrameTest[self.mOriginalSignal],
                    lFrameTest[lForecastColumnName],
                    self.mOutName + "_Test",
                )
            pass
        else:
            lFitPerf.computeCriterion(
                lFrameFit[self.mOriginalSignal],
                lFrameFit[lForecastColumnName],
                self.mTimeInfo.mOptions.mModelSelection_Criterion,
                self.mOutName + "_Fit",
            )
            lForecastPerf.computeCriterion(
                lFrameForecast[self.mOriginalSignal],
                lFrameForecast[lForecastColumnName],
                self.mTimeInfo.mOptions.mModelSelection_Criterion,
                self.mOutName + "_Forecast",
    ...
    def computePredictionIntervals(self):
        ...
        if self.mTimeInfo.mOptions.mAddPredictionIntervals:
            lTimer = tsutil.cTimer(
                ("COMPUTE_PREDICTION_INTERVALS", {"Signal": self.mOriginalSignal})
            )
    ...
    def getInfo(self):
        ...
        if self.mAR.mExogenousInfo:
            logger.info(
                "EXOGENOUS_DATA " + str(self.mAR.mExogenousInfo.mExogenousVariables)
            )
        logger.info("DECOMPOSITION_TYPE '" + self.mDecompositionType + "'")
        logger.info(
            "BEST_TRANSOFORMATION_TYPE '" + self.mTransformation.get_name("") + "'"
        )
        logger.info(
            "BEST_DECOMPOSITION  '" + self.mOutName + "' [" + self.getFormula() + "]"
        )
        logger.info(
            "TREND_DETAIL '" + self.mTrend.mOutName + "' [" + self.mTrend.mFormula + "]"
        )
        logger.info(
            "CYCLE_DETAIL '" + self.mCycle.mOutName + "' [" + self.mCycle.mFormula + "]"
        )
        logger.info(
            "AUTOREG_DETAIL '" + self.mAR.mOutName + "' [" + self.mAR.mFormula + "]"
        )
        logger.info(
            "MODEL_MAPE MAPE_Fit="
            + str(self.mFitPerf.mMAPE)
            + " MAPE_Forecast="
            + str(self.mForecastPerf.mMAPE)
            + " MAPE_Test="
            + str(self.mTestPerf.mMAPE)
        )
        logger.info(
            "MODEL_SMAPE SMAPE_Fit="
            + str(self.mFitPerf.mSMAPE)
            + " SMAPE_Forecast="
            + str(self.mForecastPerf.mSMAPE)
            + " SMAPE_Test="
            + str(self.mTestPerf.mSMAPE)
        )
        logger.info(
            "MODEL_MASE MASE_Fit="
            + str(self.mFitPerf.mMASE)
            + " MASE_Forecast="
            + str(self.mForecastPerf.mMASE)
            + " MASE_Test="
            + str(self.mTestPerf.mMASE)
        )
        logger.info(
            "MODEL_CRPS CRPS_Fit="
            + str(self.mFitPerf.mCRPS)
            + " CRPS_Forecast="
            + str(self.mForecastPerf.mCRPS)
            + " CRPS_Test="
            + str(self.mTestPerf.mCRPS)
        )
        logger.info(
            "MODEL_L1 L1_Fit="
            + str(self.mFitPerf.mL1)
            + " L1_Forecast="
            + str(self.mForecastPerf.mL1)
            + " L1_Test="
            + str(self.mTestPerf.mL1)
        )
        logger.info(
            "MODEL_L2 L2_Fit="
            + str(self.mFitPerf.mL2)
            + " L2_Forecast="
            + str(self.mForecastPerf.mL2)
            + " L2_Test="
            + str(self.mTestPerf.mL2)
        )
        logger.info(
            "MODEL_LnQ LnQ_Fit="
            + str(self.mFitPerf.mLnQ)
            + " LnQ_Forecast="
            + str(self.mForecastPerf.mLnQ)
            + " LnQ_Test="
            + str(self.mTestPerf.mLnQ)
        )
        logger.info(
            "MODEL_MEDIAN_AE MedAE_Fit="
            + str(self.mFitPerf.mMedAE)
            + " MedAE_Forecast="
            + str(self.mForecastPerf.mMedAE)
            + " MedAE_Test="
            + str(self.mTestPerf.mMedAE)
        )
        logger.info(
            "MODEL_KENDALL_TAU KENDALL_TAU_Fit="
            + str(self.mFitPerf.mKendallTau)
            + " KENDALL_TAU_Forecast="
            + str(self.mForecastPerf.mKendallTau)
            + " KENDALL_TAU_Test="
            + str(self.mTestPerf.mKendallTau)
        )
        logger.info(
            "MODEL_KOLOMOGOROV_SMIRNOV KS_Fit="
            + str(self.mFitPerf.mKS)
            + " KS_Forecast="
            + str(self.mForecastPerf.mKS)
            + " KS_Test="
            + str(self.mTestPerf.mKS)
        )
        logger.info(
            "MODEL_MANN_WHITNEY_U MWU_Fit="
            + str(self.mFitPerf.mMWU)
            + " MWU_Forecast="
            + str(self.mForecastPerf.mMWU)
            + " MWU_Test="
            + str(self.mTestPerf.mMWU)
        )
        logger.info(
            "MODEL_AUC AUC_Fit="
            + str(self.mFitPerf.mAUC)
            + " AUC_Forecast="
            + str(self.mForecastPerf.mAUC)
            + " AUC_Test="
            + str(self.mTestPerf.mAUC)
        )
    ...
    def forecastOneStepAhead(self, df, horizon_index=1, perf_mode=False):
        ...
        df2[lPrefix + "TransformedForecast"] = self.compute_model_forecast(
            lTrendColumn, lCycleColumn, lARColumn
        )
        df2[lPrefix2 + "Forecast"] = self.mTransformation.invert(
            df2[lPrefix + "TransformedForecast"]
        )

        if not perf_mode:
            df2[lPrefix + "TransformedResidue"] = (
                lSignal - df2[lPrefix + "TransformedForecast"]
            )
    ...
    def forecast(self, df, iHorizon):
        ...
        for h in range(0, iHorizon - 1):
            ...
            df1 = df1[
                [
                    ...
                    self.mTimeInfo.mRowNumberColumn,
        ...
        lFieldsToErase = [
            ...
            str(self.mOriginalSignal) + "_Residue",
        ]
    ...
    def applyForecastRectifier(self, df):
        ...
        if self.mTimeInfo.mOptions.mForecastRectifier == "relu":
            lForecastColumnName = str(self.mOriginalSignal) + "_Forecast"
            df1[lForecastColumnName] = df1[lForecastColumnName].apply(
                lambda x: max(x, 0)
            )
        return df1
    ...
    def addPredictionIntervals(self, iInputDS, iForecastFrame, iHorizon):
        ...
        lWidths = [
            lConfidence
            * self.mPredictionIntervalsEstimator.mForecastPerformances[
                lForecastColumn + "_" + str(h + 1)
            ].mL2
    ...
    def addForecastQuantiles(self, iInputDS, iForecastFrame, iHorizon):
        ...
        lQuantiles = self.mPredictionIntervalsEstimator.mForecastPerformances[
            lForecastColumn + "_1"
        ].mErrorQuantiles.keys()
        ...
        for q in lQuantiles:
            ...
            iForecastFrame.loc[
                N : N + iHorizon, lQuantileName + str(q)
            ] = lForcastValues.values + np.array(lQuants)
        return iForecastFrame
    ...
    def plotResidues(self, name=None, format="png", iOutputDF=None):
        ...
        if name is not None:
            ...
            tsplot.decomp_plot(
                ...
                lTime,
                self.mSignal,
                lPrefix + "TransformedForecast",
                lPrefix + "TransformedResidue",
                name=name + "_transformed_forecast",
                format=format,
                horizon=self.mTimeInfo.mHorizon,
            )
            tsplot.decomp_plot(
    ...
    def standardPlots(self, name=None, format="png"):
        ...
        tsplot.prediction_interval_plot(
            lOutput,
            lTime,
            self.mOriginalSignal,
            lForecastColumn,
    ...
    def getPlotsAsDict(self):
        ...
        lPrefix = self.mSignal + "_"
        lPrefix2 = str(self.mOriginalSignal) + "_"
        lDict["Trend"] = tsplot.decomp_plot_as_png_base64(
            df,
            lTime,
            self.mSignal,
            lPrefix + "Trend",
        ...
        lDict["Cycle"] = tsplot.decomp_plot_as_png_base64(
            df,
            lTime,
            lPrefix + "Trend_residue",
            lPrefix + "Cycle",
        ...
        lDict["AR"] = tsplot.decomp_plot_as_png_base64(
            df,
            lTime,
            lPrefix + "Cycle_residue",
            lPrefix + "AR",
        ...
        lDict["Forecast"] = tsplot.decomp_plot_as_png_base64(
            df,
            lTime,
            lSignalColumn,
            lPrefix2 + "Forecast",
    ...
    def getPredictionIntervalPlot(self, df=None):
        ...
        lTitle = "Prediction Intervals\n\nModel = " + self.mOutName + " [ "
        lTitle = lTitle + "MAPE = " + str(self.mForecastPerf.mMAPE) + " ]"
        return tsplot.prediction_interval_plot_as_png_base64(
            lOutput,
            lTime,
            self.mOriginalSignal,
            lForecastColumn,
    ...
    def getForecastQuantilesPlot(self, df=None):
        ...
        lQuantiles = sorted(lQuantiles)
        return tsplot.quantiles_plot_as_png_base64(
            lOutput,
            lTime,
            self.mOriginalSignal,
            lForecastColumn,
