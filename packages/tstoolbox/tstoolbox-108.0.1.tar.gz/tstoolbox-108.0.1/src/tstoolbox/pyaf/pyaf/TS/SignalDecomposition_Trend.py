...
class cAbstractTrend:
    ...
    def check_not_nan(self, sig, name):
        ...
        if np.isnan(sig[:-1]).any() or np.isinf(sig[:-1]).any():
            ...
            raise tsutil.Internal_PyAF_Error(
                "INVALID_COLUMN _FOR_TREND_RESIDUE ['" + name + "'"
            )
        pass
    ...
    def computePerf(self):
        if self.mOptions.mDebug:
            self.check_not_nan(
                self.mTrendFrame[self.mOutName + "_residue"], self.mOutName + "_residue"
            )
        ...
        self.mTrendFitPerf.compute(
            lFrameFit[self.mSignal], lFrameFit[self.mOutName], self.mOutName
        )
        self.mTrendForecastPerf.compute(
            lFrameForecast[self.mSignal], lFrameForecast[self.mOutName], self.mOutName
        )
...
class cLag1Trend(cAbstractTrend):
    ...
    def dump_values(self):
        logger = tsutil.get_pyaf_logger()
        logger.info(
            "LAG1_TREND " + self.mFormula + " " + str(round(self.mDefaultValue, 6))
        )
...
class cMovingAverageTrend(cAbstractTrend):
    ...
    def fit(self):
        ...
        # real lag1
        target = self.mTrendFrame[self.mSignal].values
        self.mTrendFrame[self.mOutName] = (
            self.mTrendFrame[self.mSignal]
            .shift(1)
            .rolling(self.mWindow)
            .mean()
            .fillna(method="bfill")
        )
...
class cLinearTrend(cAbstractTrend):
    ...
    def compute(self):
        ...
        lTimeAfterSomeStepsNormalized = self.mTimeInfo.normalizeTime(
            lTimeAfterSomeSteps
        )
        df = pd.DataFrame(
            [lTimeAfterSomeStepsNormalized, lTimeAfterSomeStepsNormalized**2]
        )
    ...
    def dump_values(self):
        ...
        logger.info(
            ...
            + " "
            + str(
                (self.mTrendRidge.intercept_.round(6), self.mTrendRidge.coef_.round(6))
            )
        )
...
class cPolyTrend(cAbstractTrend):
    ...
    def addTrendInputVariables(self):
        ...
        self.mTrendFrame = pd.DataFrame(index=self.mTimeInfo.mSignalFrame.index)
        self.mTimeInfo.addVars(self.mTrendFrame)
        self.mTrendFrame[self.mTimeInfo.mNormalizedTimeColumn + "_^2"] = (
            self.mTrendFrame[self.mTimeInfo.mNormalizedTimeColumn] ** 2
        )
        self.mTrendFrame[self.mTimeInfo.mNormalizedTimeColumn + "_^3"] = (
            self.mTrendFrame[self.mTimeInfo.mNormalizedTimeColumn] ** 3
        )
    ...
    def fit(self):
        ...
        est_inputs = lTrendEstimFrame[
            [
                ...
                self.mTimeInfo.mNormalizedTimeColumn + "_^3",
            ]
        ].values
        self.mTrendRidge.fit(est_inputs, est_target)
        self.mTrendRidge.score(est_inputs, est_target)
    ...
    def transformDataset(self, df):
        df[self.mTimeInfo.mNormalizedTimeColumn + "_^2"] = (
            df[self.mTimeInfo.mNormalizedTimeColumn] ** 2
        )
        df[self.mTimeInfo.mNormalizedTimeColumn + "_^3"] = (
            df[self.mTimeInfo.mNormalizedTimeColumn] ** 3
        )
        target = df[self.mSignal].values
        inputs = df[
            [
...
class cTrendEstimator:
    ...
    def check_residue(self, trend, sig, name):
        ...
        if np.isnan(sig).any():
            raise tsutil.Internal_PyAF_Error(
                "Invalid residue_is_nan '"
                + str(
                    (
                        name,
                        trend.mDecompositionType,
                        sig.min(),
                        sig.max(),
                        sig.mean(),
                        sig.std(),
    ...
    def estimateTrends(self):
        ...
        if self.mOptions.mDebugProfile:
            lTimer = tsutil.cTimer(
                ("TRAINING_TRENDS", {"Signal": self.mTimeInfo.mSignal})
            )
        self.mTime = self.mTimeInfo.mTime
        self.mSignal = self.mTimeInfo.mSignal
        ...
        for trend in self.mTrendList:
            ...
            self.mTrendFrame[trend.mOutName + "_residue"] = trend.mTrendFrame[
                trend.mOutName + "_residue"
            ]
            if self.mOptions.mDebug:
                self.check_residue(
                    trend,
                    self.mTrendFrame[trend.mOutName + "_residue"].values[:-1],
                    trend.mOutName + "_residue",
                )
        pass
