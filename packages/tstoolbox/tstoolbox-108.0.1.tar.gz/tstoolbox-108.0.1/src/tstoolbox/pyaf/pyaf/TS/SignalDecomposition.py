...
class cSignalDecompositionOneTransform:
    ...
    def updatePerfsForAllModels(self, iModels):
        ...
        for name, model in iModels.items():
            ...
            self.mPerfsByModel[
                (self.mSignal, self.mDecompositionType, lSplit, model.mOutName)
            ] = [
                (self.mSignal, self.mDecompositionType, model),
                lComplexity,
                lFitPerf,
                lForecastPerf,
                lTestPerf,
    ...
    def train(
        ...
        if self.mExogenousInfo is not None:
            ...
            if self.mOptions.mDebugProfile:
                lTimer2 = tsutil.cTimer(
                    ("TRAINING_EXOGENOUS_DATA", {"Signal": iSignal})
                )
        ...
        for trend in lAREstimator.mTrendList:
            for cycle in lAREstimator.mCycleList[trend]:
                ...
                for autoreg in lAREstimator.mARList[cycle_residue]:
                    lModel = tsmodel.cTimeSeriesModel(
                        self.mTransformation,
                        self.mDecompositionType,
                        trend,
                        cycle,
                        autoreg,
                    )
...
class cModelSelector_OneSignal:
    ...
    def dump_all_model_perfs_as_json(self):
        ...
        lPerfDump = json.dumps(
            lDict, default=lambda o: o.__dict__, indent=4, sort_keys=True
        )
    ...
    def collectPerformanceIndices_ModelSelection(self, iSignal, iSigDecs):
        ...
        lTimer = tsutil.cTimer(
            (
                "MODEL_SELECTION",
                {"Signal": iSignal, "Transformations": sorted(list(iSigDecs.keys()))},
            )
        )
        rows_list = []
        lPerfsByModel = {}
        for lName, sigdec in iSigDecs.items():
            for model, value in sorted(sigdec.mPerfsByModel.items()):
                ...
                row = [
                    ...
                    lForecastPerf.getCriterionValue(
                        self.mOptions.mModelSelection_Criterion
                    ),
                    lTestPerf.getCriterionValue(
                        self.mOptions.mModelSelection_Criterion
                    ),
                ]
        ...
        lHigherIsBetter = tsperf.cPerf.higher_values_are_better(
            self.mOptions.mModelSelection_Criterion
        )
        if lHigherIsBetter:
        ...
        self.mTrPerfDetails.sort_values(
            by=[lIndicator, "Complexity", "Model"],
        ...
        if lHigherIsBetter:
            lInterestingModels = self.mTrPerfDetails[
                self.mTrPerfDetails[lIndicator] >= (lBestPerf - 0.01)
        ...
        self.mModelShortList = lInterestingModels[
            ["Transformation", "DecompositionType", "Model", lIndicator, "Complexity"]
        ]
    ...
    def perform_model_selection_cross_validation(self):
        ...
        lPerfByCategory = (
            self.mTrPerfDetails[lColumns]
            .groupby(by=["Category"], sort=False)[lIndicator]
            .mean()
        )
        ...
        lPerfByCategory_df.sort_values(
            by=[lIndicator, "Category"],
        ...
        if lHigherIsBetter:
            lInterestingCategories_df = lPerfByCategory_df[
                lPerfByCategory_df[lIndicator] >= (lBestPerf - 0.01)
            ].reset_index(drop=True)
        else:
            lInterestingCategories_df = lPerfByCategory_df[
                lPerfByCategory_df[lIndicator] <= (lBestPerf + 0.01)
        ...
        self.mTrPerfDetails["IC"] = self.mTrPerfDetails["Category"].apply(
            lambda x: 1 if x in lInterestingCategories else 0
        )
        ...
        self.mModelShortList = lInterestingModels[
            ["Model", "Category", "Split", lIndicator, "IC"]
        ]
...
def run_finalize_training(arg):
    ...
    return (
        lSignal,
        lModelSelector.mPerfsByModel,
        lModelSelector.mBestModel,
        lModelSelector.mTrPerfDetails,
        lModelSelector.mModelShortList,
    )
...
class cSignalDecompositionTrainer:
                ...
                "FINALIZE_TRAINING",
                {
                    "Signals": [
                        ...
                        for (
                            lSignal,
                            sigdecs,
                        ) in self.mSigDecBySplitAndTransform.items()
                    ],
                    "Transformations": [
                        (lSignal, sorted(list(lSigDecs.keys())))
                        for (
                            lSignal,
                            lSigDecs,
                        ) in self.mSigDecBySplitAndTransform.items()
    ...
    def train_all_transformations(self, iInputDS, iSplits, iTimes, iSignals, iHorizons):
        ...
        lActiveDecompositionTypes = [
            decomp_type
            for (
                decomp_type,
                active_status,
            ) in self.mOptions.mActiveDecompositionTypes.items()
    ...
    def cleanup_after_model_selection(self):
        lSigDecByTransform = {}
        for lSignal, sigdecs in self.mSigDecBySplitAndTransform.items():
            lBestTransformationName = self.mBestModels[
                lSignal
            ].mTransformation.get_name("")
...
def forecast_one_signal(arg):
    ...
    lInputDS = lMissingImputer.apply(
        lInputDS, lBestModel.mTime, lBestModel.mOriginalSignal
    )
...
class cSignalDecompositionForecaster:
    ...
    def merge_frames(self, iFullFrame, iOneSignalFrame, iTimeInfo):
        ...
        lOneSignalCommonColumns = [
            col for col in iOneSignalFrame.columns if col in iFullFrame.columns
        ]
        lOneSignalCommonColumns = [
            col
            for col in lOneSignalCommonColumns
            if col not in [lTime, iTimeInfo.mTime]
        ]
        lOneSignalFrame = iOneSignalFrame.drop(lOneSignalCommonColumns, axis=1)
        lForecastFrame = iFullFrame.merge(
            lOneSignalFrame, how="left", left_on=lTime, right_on=iTimeInfo.mTime
        )
    ...
    def forecast(self, iDecomsposition, iInputDS, iHorizons):
        ...
        if iDecomsposition.mOptions.mParallelMode and NCores > 1:
            ...
            for res in pool.imap(forecast_one_signal, args):
                ...
                lForecastFrame = self.merge_frames(
                    lForecastFrame, lForecastFrame_i, lTimeInfo
                )
...
class cSignalDecomposition:
    ...
    def checkData(self, iInputDS, iTime, iSignal, iHorizon, iExogenousData):
        ...
        if iHorizon < 1:
            raise tsutil.PyAF_Error(
                "PYAF_ERROR_NEGATIVE_OR_NULL_HORIZON " + str(iHorizon)
            )
        ...
        for lSignal in [iSignal]:
            if lSignal not in iInputDS.columns:
                raise tsutil.PyAF_Error(
                    "PYAF_ERROR_SIGNAL_COLUMN_NOT_FOUND " + str(lSignal)
                )
            type2 = iInputDS[lSignal].dtype
            # print(type2)
            if type2.kind != "i" and type2.kind != "u" and type2.kind != "f":
                raise tsutil.PyAF_Error(
                    "PYAF_ERROR_SIGNAL_COLUMN_TYPE_NOT_ALLOWED '"
                    + str(lSignal)
                    + "' '"
                    + str(type2)
                    + "'"
        ...
        if iExogenousData is not None:
            ...
            if iTime not in lExogenousDataFrame.columns:
                raise tsutil.PyAF_Error(
                    "PYAF_ERROR_TIME_COLUMN_NOT_FOUND_IN_EXOGENOUS " + str(iTime)
                )
            ...
            if type1 != type3:
                raise tsutil.PyAF_Error(
                    "PYAF_ERROR_INCOMPATIBLE_TIME_COLUMN_TYPE_IN_EXOGENOUS '"
                    + str(iTime)
                    + "' '"
                    + str(type1)
                    + "' '"
    ...
    def train(self, iInputDS, iTimes, iSignals, iHorizons, iExogenousData=None):
        ...
        lTimer = tsutil.cTimer(
            ("TRAINING", {"Signals": self.mSignals, "Horizons": self.mHorizons})
        )

        for sig in self.mSignals:
            self.checkData(
                iInputDS,
                self.mDateColumns[sig],
                sig,
                self.mHorizons[sig],
                self.mExogenousData[sig],
            )
        ...
        lTrainer.train(
            iInputDS, lSplits, self.mDateColumns, self.mSignals, self.mHorizons
        )
    ...
    def forecast(self, iInputDS, iHorizon):
        lTimer = tsutil.cTimer(
            ("FORECASTING", {"Signals": self.mSignals, "Horizon": iHorizon})
        )
