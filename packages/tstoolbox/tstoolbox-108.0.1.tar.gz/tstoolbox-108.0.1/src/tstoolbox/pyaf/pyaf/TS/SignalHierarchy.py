...
class cSignalHierarchy:
    ...
    def create_SummingMatrix(self):
        lNbNodes = sum(
            [len(self.mStructure[level]) for level in self.mStructure.keys()]
        )
                ...
                for col in sorted(self.mStructure[level].keys()):
                    ...
                    for col1 in sorted(self.mStructure[level][col]):
                        ...
                        for j in range(lBaseLevelCount):
                            self.mSummingMatrix[i][j] = (
                                self.mSummingMatrix[ii][j] + self.mSummingMatrix[i][j]
                            )
            else:
                for col in sorted(self.mStructure[level].keys()):
    ...
    def checkData(self, df):
        if self.mHorizon != int(self.mHorizon):
            raise tsutil.PyAF_Error(
                "PYAF_ERROR_NON_INTEGER_HORIZON " + str(self.mHorizon)
            )
        if self.mHorizon < 1:
            raise tsutil.PyAF_Error(
                "PYAF_ERROR_NEGATIVE_OR_NULL_HORIZON " + str(self.mHorizon)
            )
        if self.mDateColumn not in df.columns:
            raise tsutil.PyAF_Error(
                "PYAF_ERROR_HIERARCHY_TIME_COLUMN_NOT_FOUND " + str(self.mDateColumn)
            )
        ...
        if (
            ...
            raise tsutil.PyAF_Error(
                "PYAF_ERROR_TIME_COLUMN_TYPE_NOT_ALLOWED '"
                + str(self.mDateColumn)
                + "' '"
                + str(type1)
                + "'"
            )
        ...
        for k in self.mStructure[0]:
            if k not in df.columns:
                raise tsutil.PyAF_Error(
                    "PYAF_ERROR_HIERARCHY_BASE_COLUMN_NOT_FOUND " + str(k)
                )
            # print(type2)
            type2 = df[k].dtype
            if type2.kind != "i" and type2.kind != "u" and type2.kind != "f":
                raise tsutil.PyAF_Error(
                    "PYAF_ERROR_HIERARCHY_BASE_SIGNAL_COLUMN_TYPE_NOT_ALLOWED '"
                    + str(k)
                    + "' '"
                    + str(type2)
                    + "'"
    ...
    def create_all_levels_models_with_one_engine(
        self, iAllLevelsDataset, H, iDateColumn
    ):
        ...
        if self.discard_nans_in_aggregate_signals():
            lEngine.mOptions.mMissingDataOptions.mTimeMissingDataImputation = (
                "DiscardRow"
            )
            lEngine.mOptions.mMissingDataOptions.mSignalMissingDataImputation = (
                "DiscardRow"
            )
    ...
    def fit(self):
        ...
        self.create_all_levels_models_with_one_engine(
            lAllLevelsDataset, self.mHorizon, self.mDateColumn
        )
        ...
        self.computePerfOnCombinedForecasts(
            lForecast_DF.head(lForecast_DF.shape[0] - self.mHorizon)
        )
        ...
        if lHasModels:
            ...
            for level in sorted(self.mStructure.keys()):
                for signal in sorted(self.mStructure[level].keys()):
                    ...
                    lMAPE = (
                        "MAPE = %.4f"
                        % self.mValidPerfs[str(signal) + "_Forecast"].mMAPE
                    )
                    lReconciledMAPEs = []
                    for lPrefix in lPrefixes:
                        lMAPE_Rec = self.mValidPerfs[
                            str(signal) + "_" + lPrefix + "_Forecast"
                        ].mMAPE
                        lReconciledMAPEs.append(
                            "MAPE_" + lPrefix + " = %.4f" % lMAPE_Rec
                        )
    ...
    def plot_as_png_base64(self, name=None):
        ...
        lAnnotations = self.get_plot_annotations()
        lBase64 = tsplot.plot_hierarchy_as_png_base64(
            self.mStructure, lAnnotations, name
        )
        return lBase64
    ...
    def forecastAllModels_with_one_engine(self, iAllLevelsDataset, H, iDateColumn):
        ...
        if self.discard_nans_in_aggregate_signals():
            ...
            lForecast_DF.loc[0 : N - H, signal] = lForecast_DF.loc[
                0 : N - H, signal
            ].fillna(0.0)
            lForecast_DF[str(signal) + "_Forecast"] = lForecast_DF[
                str(signal) + "_Forecast"
            ].fillna(0.0)
    ...
    def getEstimPart(self, df):
        lEngine = self.mModels
        lFrameFit = (
            lEngine.mSignalDecomposition.mBestModel.mTimeInfo.mSplit.getEstimPart(df)
        )
        return lFrameFit
    ...
    def getValidPart(self, df):
        lEngine = self.mModels
        lFrameFit = (
            lEngine.mSignalDecomposition.mBestModel.mTimeInfo.mSplit.getValidPart(df)
        )
        return lFrameFit
    ...
    def computeTopDownHistoricalProportions(self, iAllLevelsDataset):
        logger = tsutil.get_pyaf_hierarchical_logger()
        logger.info(
            "TRAINING_HIERARCHICAL_MODEL_COMPUTE_TOP_DOWN_HISTORICAL_PROPORTIONS"
        )
        self.mAvgHistProp = {}
        self.mPropHistAvg = {}
        ...
        for level in sorted(self.mStructure.keys()):
            if level > 0:
                for col in sorted(self.mStructure[level].keys()):
                    ...
                    for col1 in sorted(self.mStructure[level][col]):
                        self.mAvgHistProp[col][col1] = (
                            lEstim[col1] / lEstim[col]
                        ).mean()
                        self.mPropHistAvg[col][col1] = (
                            lEstim[col1].mean() / lEstim[col].mean()
                        )
        # print("AvgHitProp\n", self.mAvgHistProp);
        # print("PropHistAvg\n", self.mPropHistAvg);
        pass
        ...
        for level in sorted(self.mStructure.keys()):
            if level > 0:
                for col in sorted(self.mStructure[level].keys()):
                    ...
                    for col1 in sorted(self.mStructure[level][col]):
                        self.mForecastedProp[col][col1] = (
                            iForecast_DF[col1] / iForecast_DF[col]
                        ).mean()
    ...
    def computeBottomUpForecast(self, iForecast_DF, level, signal, iPrefix="BU"):
        ...
        for col1 in sorted(self.mStructure[level][signal]):
            if new_BU_forecast is None:
                ...
                new_BU_forecast = (
                    new_BU_forecast + iForecast_DF[col1 + "_" + iPrefix + "_Forecast"]
                )
        if new_BU_forecast is None:
    ...
    def computeBottomUpForecasts(self, iForecast_DF):
        ...
        for level in sorted(self.mStructure.keys()):
            for signal in sorted(self.mStructure[level].keys()):
                new_BU_forecast = self.computeBottomUpForecast(
                    lForecast_DF_BU, level, signal
                )
    ...
    def get_clean_signal_and_forecasts(self, iForecast_DF, signal, iPrefixes):
        ...
        lForecasts = lForecasts + [
            str(signal) + "_" + lPrefix + "_Forecast" for lPrefix in iPrefixes
        ]
        lColumns = [
            lEngine.mSignalDecomposition.mDateColumns[signal],
            signal,
        ] + lForecasts
    ...
    def computePerfOnCombinedForecasts(self, iForecast_DF):
        ...
        for level in sorted(self.mStructure.keys()):
            logger.info(
                "STRUCTURE_LEVEL "
                + str((level, sorted(list(self.mStructure[level].keys()))))
            )
            for signal in sorted(self.mStructure[level].keys()):
                lForecast_DF = self.get_clean_signal_and_forecasts(
                    iForecast_DF, signal, lPrefixes
                )
                lFrameFit = self.getEstimPart(lForecast_DF)
                lFrameValid = self.getValidPart(lForecast_DF)
                lColumns = [signal, str(signal) + "_Forecast"] + [
                    str(signal) + "_" + lPrefix + "_Forecast" for lPrefix in lPrefixes
                ]
                ...
                lPerfFit = lEngine.computePerf(
                    lFrameFit[signal], lFrameFit[str(signal) + "_Forecast"], signal
                )
                lPerfValid = lEngine.computePerf(
                    lFrameValid[signal], lFrameValid[str(signal) + "_Forecast"], signal
                )
                ...
                for iPrefix in lPrefixes:
                    ...
                    lPerfFit_Combined = lEngine.computePerf(
                        lFrameFit[signal], lFrameFit[lName], lName
                    )
                    lPerfValid_Combined = lEngine.computePerf(
                        lFrameValid[signal], lFrameValid[lName], lName
                    )
                    lPerfs[str(signal) + "_" + iPrefix] = (
                        lPerfFit,
                        lPerfValid,
                        lPerfFit_Combined,
                        lPerfValid_Combined,
        ...
        for sig, perf in sorted(lPerfs.items()):
            ...
            logger.info(
                "REPORT_COMBINED_FORECASTS_VALID_PERF " + str(perf[3].to_dict())
            )
        return lPerfs
    ...
    def computeTopDownForecasts(self, iForecast_DF, iProp, iPrefix):
        ...
        for signal in sorted(self.mStructure[lHighestLevel].keys()):
            lForecast_DF_TD[str(signal) + "_" + iPrefix + "_Forecast"] = iForecast_DF[
                str(signal) + "_Forecast"
            ]
        for level in lLevelsReversed:
            for signal in sorted(self.mStructure[level].keys()):
                for col in sorted(self.mStructure[level][signal]):
                    new_TD_forecast = (
                        lForecast_DF_TD[str(signal) + "_" + iPrefix + "_Forecast"]
                        * iProp[signal][col]
                    )
                    lForecast_DF_TD[
                        str(col) + "_" + iPrefix + "_Forecast"
                    ] = new_TD_forecast
    ...
    def computeMiddleOutForecasts(self, iForecast_DF, iProp, iPrefix):
        ...
        levels_below = sorted(
            [level for level in self.mStructure.keys() if (level <= lMidLevel)],
            reverse=True,
        )
        ...
        for signal in sorted(self.mStructure[lMidLevel].keys()):
            lForecast_DF_MO[str(signal) + "_" + iPrefix + "_Forecast"] = iForecast_DF[
                str(signal) + "_Forecast"
            ]
        #
        for level in levels_below:
            for signal in sorted(self.mStructure[level].keys()):
                for col in sorted(self.mStructure[level][signal]):
                    new_MO_forecast = (
                        lForecast_DF_MO[str(signal) + "_" + iPrefix + "_Forecast"]
                        * iProp[signal][col]
                    )
                    lForecast_DF_MO[
                        str(col) + "_" + iPrefix + "_Forecast"
        ...
        for level in range(lMidLevel + 1, len(self.mStructure.keys())):
            for signal in sorted(self.mStructure[level].keys()):
                new_MO_forecast = self.computeBottomUpForecast(
                    lForecast_DF_MO, level, signal, iPrefix
                )
                lForecast_DF_MO[
    ...
    def internal_forecast(self, iInputDS, iHorizon):
        ...
        lForecast_DF = self.forecastAllModels_with_one_engine(
            lAllLevelsDataset, iHorizon, self.mDateColumn
        )
        lCombinationMethods = self.mOptions.mHierarchicalCombinationMethod
        if type(lCombinationMethods) is not list:
            lCombinationMethods = [lCombinationMethods]
        logger = tsutil.get_pyaf_hierarchical_logger()
        logger.info(
            "FORECASTING_HIERARCHICAL_MODEL_COMBINATION_METHODS "
            + str(lCombinationMethods)
        )
        ...
        for lMethod in lCombinationMethods:
            ...
            if lMethod == "TD":
                lForecast_DF_TD_AHP = self.computeTopDownForecasts(
                    lForecast_DF, self.mAvgHistProp, "AHP_TD"
                )
                lForecast_DF = lForecast_DF_TD_AHP

                lForecast_DF_TD_PHA = self.computeTopDownForecasts(
                    lForecast_DF, self.mPropHistAvg, "PHA_TD"
                )
                lForecast_DF = lForecast_DF_TD_PHA

            if lMethod == "MO":
                lForecast_DF_MO = self.computeMiddleOutForecasts(
                    lForecast_DF, self.mPropHistAvg, "MO"
                )
