...
class cMissingDataImputer:
    ...
    def apply_time_imputation_method(self, iInputDS, iTime):
        ...
        if not self.has_missing_data(iInputDS[iTime]):
            ...
            self.mOptions.mMissingDataOptions.mTimeMissingDataImputation == "DiscardRow"
        ):
            iInputDS = iInputDS[iInputDS[iTime].notnull()]

        elif (
            self.mOptions.mMissingDataOptions.mTimeMissingDataImputation
            == "Interpolate"
        ):
    ...
    def apply_signal_imputation_method(self, iInputDS, iSignal):
        ...
        if not self.has_missing_data(iInputDS[iSignal]):
            ...
            self.mOptions.mMissingDataOptions.mSignalMissingDataImputation
            == "DiscardRow"
        ):
            ...
            self.mOptions.mMissingDataOptions.mSignalMissingDataImputation == "Constant"
        ):
            lSignal = iInputDS[iSignal].fillna(
                self.mOptions.mMissingDataOptions.mConstant, method=None
            )
            iInputDS[iSignal] = lSignal
    ...
    def interpolate_time_if_needed(self, iInputDS, iTime):
        ...
        if type1.kind == "M":
            ...
            lTime = iInputDS[iTime].interpolate(
    ...
    def interpolate_signal_if_needed(self, iInputDS, iSignal):
        lSignal = iInputDS[iSignal].interpolate(
            method="linear", limit_direction="both", axis=0
        )
