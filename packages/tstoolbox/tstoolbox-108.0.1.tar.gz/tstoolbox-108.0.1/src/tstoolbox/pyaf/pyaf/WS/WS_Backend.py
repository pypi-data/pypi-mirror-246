...
class cWSModel:
    def __init__(self, name=""):
        ...
        self.mTimeVar = None
        self.mPresentTime = None
        # "1960-08-01"; # start forecasting after this date.
        self.mHorizon = 1
        self.mMaxHistoryForDisplay = 1000
        # max length of data frames returned in json
    ...
    def guess_Columns_if_needed(self):
        ...
        if len(self.mFullDataFrame.columns) == 1:
            ...
            self.mSignalVar = (
                lLastColumn if (self.mSignalVar == "") else self.mSignalVar
            )
    ...
    def readData(self):
        self.mFullDataFrame = pd.read_csv(
            self.mCSVFile, sep=r",", engine="python", nrows=1000
        )
        self.guess_Columns_if_needed()
        self.mFullDataFrame[self.mTimeVar] = self.mFullDataFrame[self.mTimeVar].apply(
        ...
        if self.mExogenousDataFile is not None:
            ...
            self.mExogenousDataFrame = pd.read_csv(
                self.mExogenousDataFile, sep=r",", engine="python", nrows=1000
            )
            self.mExogenousDataFrame[self.mTimeVar] = self.mExogenousDataFrame[
                self.mTimeVar
            ].apply(self.convert_string_to_date)
            if self.mExogenousVars is None:
                self.mExogenousVariables = [
                    col
                    for col in self.mExogenousDataFrame.columns
                    if ((col != self.mTimeVar) and (col != self.mTimeVar))
                ]
            else:
    ...
    def trainModel(self):
        ...
        print(
            "TRAIN_PARAMS",
            self.mTrainDataFrame.shape,
            self.mTrainDataFrame.columns,
            self.mTimeVar,
            self.mSignalVar,
            self.mHorizon,
        )
        ...
        self.mForecastEngine.train(
            self.mTrainDataFrame,
            self.mTimeVar,
            self.mSignalVar,
            self.mHorizon,
            lExogenousData,
    ...
    def applyModel(self):
        ...
        self.mDetailedForecast_DataFrame = self.mForecastEngine.forecast(
            self.mApplyIn, self.mHorizon
        )
        self.mForecast_DataFrame = self.mDetailedForecast_DataFrame
        # [[self.mTimeVar , self.mSignalVar, self.mSignalVar + '_Forecast']];
        self.mForecastData = self.mForecast_DataFrame.tail(self.mHorizon)
        lForecastName = self.mSignalVar + "_Forecast"
        self.mForecast = self.mForecastData[
            [
                self.mTimeVar,
                lForecastName,
    ...
    def get_dataset_info(self):
        ...
        lDatasetInfo["Time_Stats"] = {
            "Min": str(self.mFullDataFrame[self.mTimeVar].min()),
    ...
    def as_dict(self):
        ...
        if self.mForecastData is not None:
            ...
            lForecastData["Forecast" + "_Lower_Bound"] = self.mForecast[
                lForecastName + "_Lower_Bound"
            ].tolist()
            lForecastData["Forecast" + "_Upper_Bound"] = self.mForecast[
                lForecastName + "_Upper_Bound"
            ].tolist()
...
class cFlaskBackend:
    ...
    def add_yahoo_symbol(self, symbol):
        lSymbol = symbol
        lYahooURL = (
            "http://ichart.finance.yahoo.com/table.csv?g=d&f=2015&e=12&c=2014&b=10&a=7&d=7&s="
            + lSymbol
        )
        lYahoo = {
            "CSVFile": lYahooURL,
    ...
    def fillSampleModels(self):
        ...
        lAirline = {
            "CSVFile": "https://raw.githubusercontent.com/hawk31/nnet-ts/master/nnet_ts/AirPassengers.csv",
