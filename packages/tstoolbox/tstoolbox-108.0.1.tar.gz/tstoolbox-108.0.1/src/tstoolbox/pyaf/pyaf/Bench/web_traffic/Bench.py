...
class cProjectData:
    ...
    def add_article(
        self, article_id, full_name, name, project, access, agent, article_series
    ):
    ...
    def dump(self):
        ...
        logger.info(
            "PROJECT_DUMP_ARTICLE_NAMES "
            + str([(k, self.mArticleInfo[k][2]) for k in lArticles])
        )
        logger.info(
            "PROJECT_DUMP_ARTICLE_PROJECTS"
            + str([(k, self.mArticleInfo[k][3]) for k in lArticles])
        )
...
class cDataExtractor:
    ...
    def read_projects_data(self, filename):
        ...
        self.mOriginalDataTransposed = self.mOriginalData.drop(
            "Page", axis=1
        ).transpose()
        ...
        self.mDate_var = self.mOriginalDataTransposed["index"].apply(
