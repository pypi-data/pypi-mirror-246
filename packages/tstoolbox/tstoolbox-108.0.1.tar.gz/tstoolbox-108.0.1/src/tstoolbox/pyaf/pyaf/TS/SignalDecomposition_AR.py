...
class cAbstractAR:
    ...
    def compute_ar_residue(self, df):
        ...
        df[self.mOutName + "_residue"] = df[self.mOutName + "_residue"].astype(
            target.dtype
        )
            ...
            for ex in self.mExogenousInfo.mEncodedExogenous:
                if self.mLagsForSeries.get(ex):
                    for p in self.mLagsForSeries[ex]:
                        name = ex + "_Lag" + str(p)
                        lShiftedSeries = self.shift_series(
                            df[ex], p, self.mDefaultValues[ex]
                        )
                        lDict[name] = lShiftedSeries
...
class cAutoRegressiveEstimator:
    ...
    def preselect_exog_vars(self, df, cycle_residue):
        ...
        lPreselectionFeatureSelector = SelectKBest(
            f_regression, k=max(1, lMaxFeatures // P)
        )
    ...
    def estimate_ar_models_for_cycle(self, cycle_residue):
        ...
        for autoreg in self.mARList[cycle_residue]:
            self.mARFrame[autoreg.mTrend.mOutName] = autoreg.mCycle.mTrendFrame[
                autoreg.mTrend.mOutName
            ]
            self.mARFrame[autoreg.mCycle.mOutName] = self.mCycleFrame[
                autoreg.mCycle.mOutName
            ]
    ...
    def check_not_nan(self, sig, name):
        ...
        if np.isnan(sig[:-1]).any():
            ...
            raise tsutil.Internal_PyAF_Error(
                "INVALID_COLUMN _FOR_CYCLE_RESIDUE ['" + name + "'"
            )
        pass
    ...
    def add_model_if_activated(
        self, cycle_residue, model_type_str, model_class, iLags, iAddExogenous=True
    ):
        ...
        if (
            iAddExogenous
            and (self.mExogenousInfo is not None)
            and self.mOptions.mActiveAutoRegressions[model_type_str + "X"]
        ):
    ...
    def estimate(self):
        ...
        if self.mOptions.mDebugProfile:
            lTimer = tsutil.cTimer(
                ("TRAINING_AR_MODELS", {"Signal": self.mTimeInfo.mSignal})
            )
        ...
        for trend in self.mTrendList:
            for cycle in self.mCycleList[trend]:
                ...
                lKeep = (self.mCycleFrame[cycle_residue].shape[0] > 12) and (
                    self.mCycleFrame[cycle_residue].std() > 0.00001
                )
                ...
                if lKeep:
                    self.add_model_if_activated(
                        cycle_residue, "AR", tsscikit.cAutoRegressiveModel, lLags, True
                    )
                    self.add_model_if_activated(
                        cycle_residue, "SVR", tsscikit.cSVR_Model, lLags, True
                    )
                    if (
                        self.mOptions.mActiveAutoRegressions["LSTM"]
                        or self.mOptions.mActiveAutoRegressions["LSTMX"]
                    ):
                        lLSTMClass = self.mOptions.getPytorchOrKerasClass("LSTM")
                        if lLSTMClass is not None:
                            self.add_model_if_activated(
                                cycle_residue, "LSTM", lLSTMClass, lLags, True
                            )
                    if (
                        self.mOptions.mActiveAutoRegressions["MLP"]
                        or self.mOptions.mActiveAutoRegressions["MLPX"]
                    ):
                        lMLPClass = self.mOptions.getPytorchOrKerasClass("MLP")
                        if lMLPClass is not None:
                            self.add_model_if_activated(
                                cycle_residue, "MLP", lMLPClass, lLags, True
                            )
                    if (
                        self.mOptions.mActiveAutoRegressions["XGB"]
                        or self.mOptions.mActiveAutoRegressions["XGBX"]
                    ):
                        if self.mOptions.canBuildXGBoostModel("XGB"):
                            self.add_model_if_activated(
                                cycle_residue,
                                "XGB",
                                tsscikit.cXGBoost_Model,
                                lLags,
                                True,
                            )
                    if (
                        self.mOptions.mActiveAutoRegressions["LGB"]
                        or self.mOptions.mActiveAutoRegressions["LGBX"]
                    ):
                        if self.mOptions.canBuildLightGBMModel("LGB"):
                            self.add_model_if_activated(
                                cycle_residue,
                                "LGB",
                                tsscikit.cLightGBM_Model,
                                lLags,
                                True,
                            )
                    if self.mOptions.mActiveAutoRegressions["CROSTON"]:
                        ...
                        lIsSignalIntermittent = interm.is_signal_intermittent(
                            self.mCycleFrame[cycle_residue], self.mOptions
                        )
        ...
        if len(self.mSkippedARList) > 0:
            ...
            lSkipInfo = (
                ...
                len(self.mSkippedARList),
                10,
                lTenFirst,
            )
            logger.info(
                "SKIPPING_AR_MODELS_WITH_LOW_VARIANCE_FOR_CYCLE_RESIDUES "
                + str(lSkipInfo)
            )
        ...
        if lNeedExogenous:
            if self.mOptions.mDebugProfile:
                logger.info(
                    "AR_MODEL_ADD_EXOGENOUS "
                    + str(self.mCycleFrame.shape[0])
                    + " "
