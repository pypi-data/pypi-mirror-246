...
def _pg_date_diff(element, compiler, **kw):  # pragma: no cover
    ...
    return "(extract(epoch from {}) - extract(epoch from {}))".format(
...
def _sl_date_diff(element, compiler, **kw):  # pragma: no cover
    return "julianday({}) - julianday({})".format(
        compiler.process(element.clauses.clauses[0]),
        compiler.process(element.clauses.clauses[1]),
    )
...
def _my_date_add(element, compiler, **kw):  # pragma: no cover
    return "DATE_ADD({}, INTERVAL {} SECOND)".format(
        compiler.process(element.clauses.clauses[0]),
        compiler.process(element.clauses.clauses[1]),
    )
...
def _pg_date_add(element, compiler, **kw):  # pragma: no cover
    arg1, arg2 = list(element.clauses)
    return "({} + ({}) * INTERVAL '1 SECOND')".format(
        compiler.process(element.clauses.clauses[0]),
        compiler.process(element.clauses.clauses[1]),
    )
...
def _sl_date_add(element, compiler, **kw):  # pragma: no cover
    return "datetime({}, '+{} seconds')".format(
        compiler.process(element.clauses.clauses[0]),
        compiler.process(element.clauses.clauses[1]),
    )
...
class cDatabaseBackend:
    def __init__(self, iDSN=None, iDialect=None):
        ...
        print(
            "CREATING_DATABASE_BACKEND_DSN_DIALECT",
    ...
    def generate_Sql(self, statement):
        ...
        lResult = statement.compile(
            bind=self.mEngine, compile_kwargs={"literal_binds": True}
        )
        print("GENERATE_SQL_RESULT", lResult.string[0:200])
        return lResult.string

    def createLogicalTable(
        self, iTableName, iDateName, iSignalName, iDateType, iExogenousVariables=[]
    ):
        lTestTableName = iTableName
        lPrimaryKeyName = iDateName

        lTestTable = Table(lTestTableName, self.mMeta)
    ...
    def hasAnalyticalRowNumber(self):
        lDialectName = self.getDialectName()
        if ("sqlite" == lDialectName) or (
            ("mysql" == lDialectName) and not self.isNewMySQL()
        ):
            return False
        return True
    ...
    def materializeTable(self, iDataFrame, iTableName):
        lTestTableName = iTableName
        iDataFrame.to_sql(
            lTestTableName, self.mConnection, if_exists="replace", index=False
        )
        lTestTable = Table(
            lTestTableName, self.mMeta, autoload=True, autoload_with=self.mEngine
        )
        return lTestTable
    ...
    def getDateTimeLiteral(self, iValue):
        ...
        if "sqlite" == lDialectName:
            return sqlalchemy.sql.expression.literal_column(
                "'" + str(iValue) + "'", sqlalchemy.types.DATETIME
            )
        # return func.cast(str(iValue), sqlalchemy.types.TIMESTAMP);
        return sqlalchemy.sql.expression.literal_column(
            "TIMESTAMP '" + str(iValue) + "'", sqlalchemy.types.DATETIME
        )
        # .strftime("%Y-%m-%d %H:%M:%S")

    def getIntervalLiteral(self, iValue, h):
        return sqlalchemy.sql.expression.literal_column(
            str(h) + " * INTERVAL '" + str(iValue) + "'", sqlalchemy.types.Interval
        )
...
class cDecompositionCodeGenObject:
    ...
    def generateCode_Internal(self, iTableName=None):
        ...
        lNeedTransformation = (
            self.mBestModel.mTransformation.__class__
            != pyaf.TS.Signal_Transformation.cSignalTransform_None
        )
        ...
        if lTableName is None:
            lTableName = self.mBackEnd.generateRandomTableName()
        base_table = self.mBackEnd.createLogicalTable(
            lTableName,
            self.mDateName,
        ...
        self.generateRowNumberCode(table)
        # => RowNumber_CTE
        self.generateTransformationInputCode(table)
        # => Transformation_CTE
        self.generateTransformationCode(table)
        # => Transformation_CTE

        self.generateTrendInputCode()
        # => Trend_Inputs_CTE
        self.generateTrendCode()
        # => Trend_CTE
        self.generateCycleInputCode()
        # => Cycle_Inputs_CTE
        self.generateCycleCode()
        # => Cycle_CTE
        self.generateCycleResidueCode()
        # => Cycle_Residue_CTE
        self.generateARInputCode()
        # => AR_Inputs_CTE
        self.generateARCode()
        # => AR_CTE
        self.generateModelCode()
        # => Model_CTE
        if lNeedTransformation:
            self.generateModelInvTransformationInputCode()
            self.generateInverseTransformationCode()
            # => Model_CTE
    ...
    def addNormalizedTime(self, table):
        ...
        if lTimeInfo.isPhysicalTime():
            ...
            normalized_time = table.c[self.mDateName] - self.as_float(
                lTimeInfo.mTimeMin
            )
    ...
    def addRowNumber_analytical(self, table):
        ...
        row_number_column = func.row_number().over(
            order_by=asc(table.c[self.mDateName])
        )
    ...
    def generateRowNumberCode(self, table):
        ...
        self.mRowNumber_CTE = self.mBackEnd.generate_CTE(
            lMainColumns + exprs1 + exprs2 + exprs3, "TS_CTE_RowNumber"
        )
    ...
    def addOriginalSignalAggreagtes(self, table):
        ...
        TS1 = alias(
            select(
                [
                    table.columns[self.mOriginalSignal],
                    table.columns[self.mRowNumberAlias],
                ]
            ),
            "t_SigRowNum",
        )
        sig_expr = TS1.c[self.mOriginalSignal]
        # original signal
        ...
        if lTrClass == pyaf.TS.Signal_Transformation.cSignalTransform_None:
            ...
            cumulated_signal = select([func.sum(sig_expr)]).where(
                rn_expr_1 >= rn_expr_2
            )
        elif lTrClass == pyaf.TS.Signal_Transformation.cSignalTransform_Differencing:
            previous_expr = select([func.sum(sig_expr)]).where(
                rn_expr_1 == (rn_expr_2 + 1)
            )
        elif (
            lTrClass
            == pyaf.TS.Signal_Transformation.cSignalTransform_RelativeDifferencing
        ):
            ...
            relatve_diff = (lNormalizedSignal - lNormalizedLag1) / (
                lNormalizedLag1 + 1.0
            )
        else:
            assert 0
        ...
        for exog in lExogenousVariables:
            ...
            if lList is not None:
                for lCat in lList:
                    ...
                    expr1 = case([(cond, self.as_float(1.0))], else_=self.as_float(0.0))
    ...
    def generateTransformationInputCode(self, table):
        ...
        self.mTransformationInputs_CTE = self.mBackEnd.generate_CTE(
            self.mRowNumber_CTE.columns + exprs1 + exprs2, "TS_CTE_TransformationInput"
        )
    ...
    def addTransformedSignal(self, table):
        ...
        if lTrClass == pyaf.TS.Signal_Transformation.cSignalTransform_None:
            ...
            previous_expr = func.coalesce(
                table.c["Lag1_" + self.mOriginalSignal], lDefault_expr
            )
            ...
            lDefault_expr = self.as_float(tr.mTransformation.mFirstValue)
            lDefault_expr = (
                lDefault_expr - tr.mTransformation.mMinValue
            ) / tr.mTransformation.mDelta
            lRelDiff = func.coalesce(
                table.c["RelDiff_" + self.mOriginalSignal], lDefault_expr
            )
    ...
    def generateTransformationCode(self, table):
        ...
        self.mTransformation_CTE = self.mBackEnd.generate_CTE(
            [self.mTransformationInputs_CTE] + signal_exprs, "TS_CTE_Transformation"
        )
    ...
    def addTrendInputs(self, table):
        ...
        if self.mTrend.__class__ == pyaf.TS.SignalDecomposition_Trend.cLag1Trend:
            TS1 = alias(
                select(
                    [table.columns[self.mSignal], table.columns[self.mRowNumberAlias]]
                ),
                "t_SigRowNum3",
            )
    ...
    def generateTrendInputCode(self):
        ...
        self.mTrend_inputs_CTE = self.mBackEnd.generate_CTE(
            [self.mTransformation_CTE] + trend_inputs, "TS_CTE_Trend_Input"
        )
    ...
    def generateTrendExpression(self, table):
        ...
        if self.mTrend.__class__ == pyaf.TS.SignalDecomposition_Trend.cConstantTrend:
            ...
            trend_expr = self.as_float(lCoeffs[0]) * table.c["NTime"] + self.as_float(
                self.mTrend.mTrendRidge.intercept_
            )
            pass
    ...
    def generateTrendCode(self):
        ...
        self.mTrend_CTE = self.mBackEnd.generate_CTE(
            [self.mTrend_inputs_CTE] + trends, "TS_CTE_Trend"
        )
    ...
    def addCycleInputs(self, table):
        ...
        if self.mCycle.__class__ == pyaf.TS.SignalDecomposition_Cycle.cSeasonalPeriodic:
            ...
            date_parts = [
                extract("year", date_expr).label(lTime + "_Year"),
    ...
    def generateCycleInputCode(self):
        ...
        self.mCycle_input_CTE = self.mBackEnd.generate_CTE(
            [self.mTrend_CTE] + cycle_inputs, "TS_CTE_CylceInputs"
        )
    ...
    def generateCycleSpecificExpression(self, iExpr, iDict, iDefault):
        ...
        expr_2 = self.generateCaseWhen(
            iExpr, self.mBackEnd.getIntegerLiteral(key), valueexpr, expr_1
        )
        return expr_2
    ...
    def generateCycleExpression(self, table):
        ...
        if self.mCycle.__class__ == pyaf.TS.SignalDecomposition_Cycle.cZeroCycle:
            ...
            self.mCycle.__class__ == pyaf.TS.SignalDecomposition_Cycle.cSeasonalPeriodic
        ):
            lExpr = table.c[self.mDateName + "_" + self.mCycle.mDatePart]
            cycle_expr = self.generateCycleSpecificExpression(
                lExpr, self.mCycle.mEncodedValueDict, self.mCycle.mDefaultValue
            )
            pass
        elif (
            self.mCycle.__class__
            == pyaf.TS.SignalDecomposition_Cycle.cBestCycleForTrend
        ):
            lExpr = table.c[self.mRowNumberAlias] - 1
            lExpr = func.mod(
                lExpr,
                self.mBackEnd.getIntegerLiteral(int(self.mCycle.mBestCycleLength)),
            )
            cycle_expr = self.generateCycleSpecificExpression(
                lExpr,
    ...
    def generateCycleCode(self):
        ...
        self.mCycle_CTE = self.mBackEnd.generate_CTE(
            [self.mCycle_input_CTE] + cycles, "TS_CTE_Cycle"
        )
    ...
    def addCycleResidues(self, table):
        ...
        cycle_residue_expr = cycle_residue_expr.label(
            self.shorten(self.mCycle.getCycleResidueName())
        )
    ...
    def generateCycleResidueCode(self):
        ...
        self.mCycle_residues_CTE = self.mBackEnd.generate_CTE(
            [self.mCycle_CTE] + cycle_resdiues, "TS_CTE_CycleResidue"
        )
    ...
    def createLags(self, table, P, cols, index_col):
        ...
        for p in range(1, P + 1):
            for col in cols:
                ...
                case1 = case(
                    [(index_expr == (index_expr_1 + p), col_expr_1)], else_=null()
                )
        ...
        if self.mAR.mFormula != "NoAR":
            ...
            if lExogenousInfo is not None:
                for exog in lExogenousInfo.mEncodedExogenous:
                    lVars = lVars + [exog]
            exprs = exprs + self.createLags(
                table, self.mAR.mNbLags, lVars, self.mRowNumberAlias
            )
        return exprs
    ...
    def generateARInputCode(self):
        ...
        self.mAR_input_CTE = self.mBackEnd.generate_CTE(
            [self.mCycle_residues_CTE] + self.mAR_inputs, "TS_CTE_AR_Inputs"
        )
    ...
    def generateARCode(self):
        ...
        self.mAR_CTE = self.mBackEnd.generate_CTE(
            [self.mAR_input_CTE] + ars, "TS_CTE_AR"
        )
    ...
    def generateModelCode(self):
        ...
        self.mModel_CTE = self.mBackEnd.generate_CTE(
            [self.mAR_CTE] + model_vars, "TS_CTE_Model"
        )
    ...
    def addModelAggreagtes(self, table):
        ...
        TS1 = alias(
            select([table.c[lModelName], table.c[self.mRowNumberAlias]]), "t_ModelComp"
        )
        model_expr = TS1.c[lModelName]
        # original signal
        rn_expr_1 = table.c[self.mRowNumberAlias]
        rn_expr_2 = TS1.c[self.mRowNumberAlias]
        ...
        if lTrClass == pyaf.TS.Signal_Transformation.cSignalTransform_None:
            ...
            previous_model = select([func.sum(model_expr)]).where(
                rn_expr_1 == (rn_expr_2 + 1)
            )
            pass
        elif lTrClass == pyaf.TS.Signal_Transformation.cSignalTransform_Differencing:
            cumulated_model = select([func.sum(model_expr)]).where(
                rn_expr_1 >= rn_expr_2
            )
            pass
        elif (
    ...
    def generateModelInvTransformationInputCode(self):
        ...
        self.mModelInvTransformationInputs_CTE = self.mBackEnd.generate_CTE(
            self.mModel_CTE.columns + exprs1, "TS_CTE_ModelInvTransformationInput"
        )
    ...
    def addTransformedModel(self, table):
        ...
        transformed_model_residue = transformed_model_residue.label(
            "OriginalForecast_Residue"
        )
    ...
    def generateInverseTransformationCode(self):
        ...
        self.mModelInvTransformation_CTE = self.mBackEnd.generate_CTE(
            [self.mModelInvTransformationInputs_CTE] + model_exprs,
            "TS_CTE_InvTransformedModel",
        )
    ...
    def generate_next_time(self, current, h):
        ...
        if lTimeInfo.isPhysicalTime():
            ...
            lIntervalExpr = sqlalchemy.sql.expression.literal_column(
                lNbSeconds, sqlalchemy.types.String
            )
    ...
    def generateForecasts(self):
        ...
        for h in range(1, H + 1):
            ...
            table2 = alias(
                select([lCTEs[h - 1]]), "base_table_with_horizon_" + str(h - 1) + "_1"
            )
            ...
            lSignal = func.coalesce(
                table.c[self.mOriginalSignal], table.c["PredictedSignal"]
            )
