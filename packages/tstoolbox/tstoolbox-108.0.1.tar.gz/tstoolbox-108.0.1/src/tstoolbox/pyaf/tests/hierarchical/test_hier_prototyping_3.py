...
def read_dataset():
    ...
    df[lDateColumn] = df[lDateColumn].apply(
...
def reportOnBottomUpForecasts(iStructure, iModels, iForecast_DF, iDateColumn):
    ...
    for level in iStructure.keys():
        for signal in iStructure[level].keys():
            ...
            lPerf = lEngine.computePerf(
                lForecast_DF_BU[signal], lForecast_DF_BU[signal + "_Forecast"], signal
            )
            lPerf_BU = lEngine.computePerf(
                lForecast_DF_BU[signal],
                lForecast_DF_BU[signal + "_BU_Forecast"],
                signal + "_BU",
            )
...
lForecast_DF_BU = reportOnBottomUpForecasts(
    lStructure, lModels, lForecast_DF, lDateColumn
)
