...
def draw(iEngine, iFormula, iSignal):
    ...
    b64_string = iEngine.mSignalDecomposition.mBestModels[
        iSignal
    ].getPredictionIntervalPlot()
    import base64
    import io
...
def plot_model(arg):
    ...
    try:
        ...
        print("PLOT_MODEL_END", lSpec, lFormula, lMAPE)
...
class cModelEstheticsVisualizer:
    ...
    def gen_all(self):
        ...
        lSpecs = []
        lSpec = {}
        lKnownAutoRegressions = [
            x for x in lEngine.mOptions.mKnownAutoRegressions if not x.endswith("X")
        ]
    ...
    def generate_video(self, iDataset):
        ...
        writer = iio.get_writer(
            lSignal + "_models_H_" + str(self.mDataset.mHorizon) + ".mp4", fps=20
        )
        for im in images:
