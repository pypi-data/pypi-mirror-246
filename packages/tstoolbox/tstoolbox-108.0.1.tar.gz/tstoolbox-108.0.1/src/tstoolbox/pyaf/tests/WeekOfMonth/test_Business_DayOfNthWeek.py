...
df["Week"] = df[lTimeVar + "_Hourly"].dt.day // 7
...
df[lSignalVar] = (
    ...
    + np.random.randn(N)
    + 10
    * df["Week"].apply(lambda x: x if (x < 2) else 4)
    * df["Day"].apply(lambda x: x if (x < 4) else 3)
)
...
for k in [1]:
    for timevar in [lTimeVar + "_Hourly"]:
        ...
        lEngine.standardPlots(name="outputs/ozone_DayOfNthWeek_" + timevar)
