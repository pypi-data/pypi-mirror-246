...
class STIG:
    ...
    def refresh_implementation_objectives(self) -> None:
        ...
        for result in results:
            for obj_list in result["controlImplementations"]["items"]:
                for obj in obj_list["objectives"]:
                    self.all_implementation_objectives.append(
                        ImplementationObjective(**obj)
                    )
    ...
    def post_options(self, options: list[ImplementationOption]) -> None:
        ...
        new_options = []
        for opt in options:
            if opt not in self.all_implementation_options:
                new_options.append(opt.dict())
        if new_options:
            with component_progess as progress:
