...
def get_catalog(catalog_id: int) -> dict:
    """
    Get a catalog from RegScale

    :param int catalog_id: The ID of the catalog to retrieve
    :return: The catalog as a dictionary
    """
    api_handler = APIHandler()
    catalog = api_handler.fetch_record(
        endpoint=f"/api/catalogues/getCatalogWithAllDetails/{catalog_id}"
    )
    if not catalog:
        error_and_exit(f"Unable to find catalog with ID: {catalog_id}")
    return catalog
