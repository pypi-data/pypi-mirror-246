...
def parse_inventory_item_to_asset(
    ...
    # logger.info(f"inserting: {asset.dict()}")
    response = api_handler.insert_record("/api/assets", json=asset.dict())
    if response:
                ...
                for key in item.get("props").keys():
                    ...
                    properties_results.append(
                        api_handler.insert_record(
                            "/api/properties", json=prop_obj.dict()
                        )
                    )

                property_failure_results = [r for r in properties_results if not r]

                # Check how we did on properties and report.
                if len(property_failure_results) == 0:
                    trv.log_info(
                        {
                            "model_layer": "system-implementation",
                            "record_type": "inventory-item",
                            "event_msg": f"{len(properties_results)} created.",
                        }
                    )
                else:
                    trv.log_error(
            ...
            if implemented_components:
                ...
                for imp_component in implemented_components:
                    if not isinstance(components_dict, dict):
                        ...
                        logger.info("components do no not exist yet")
                        continue
                    ...
                    asset = Asset.fetch_asset_by_id(comp_id)
                    logger.info("Fetched Linked Asset")

                    asset["parentId"] = new_parent_asset.id
