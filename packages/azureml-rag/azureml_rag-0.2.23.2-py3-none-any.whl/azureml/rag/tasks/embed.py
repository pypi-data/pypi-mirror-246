        ...
        with track_activity(logger, "init.load_embeddings_container") as activity_logger:
            ...
            if snapshot_dir_name:
                embeddings_container = EmbeddingsContainer.load(
                    snapshot_dir_name,
                    pathlib.Path(embeddings_container_path),
...
def chunk_embedder(
    ...
    snapshot_dir_name: Optional[str] = None,
    metadata_only: bool = False,
    logger_queue: Optional[Any] = None,
):
...
def _merge_and_log_worker_results(
    results_queue, merged_results: dict, activity_logger, results_callback: Optional[Callable[[Dict], None]] = None
):
    got_results = False
    while True:
        try:
    ...
    with mp.Manager() as manager:
        try:
            ...
            with mp.Pool(processes=num_workers) as pool:
                ...
                embedder_futures = None

                def create_workers():
                    nonlocal embedder_futures
                    embedder_futures = [
                        pool.apply_async(
                            chunk_embedder,
                            (
                ...
                batch_id = 0
                for chunk in chunks:
                    # Delay creating embedding workers until we know there's chunks to process.
                    if embedder_futures is None:
                        create_workers()
                    ...
                    if len(chunk_batch) == batch_size:
                        ...
                        while True:
                            ...
                            merged_results = _merge_and_log_worker_results(
                                results_queue, merged_results, activity_logger, results_callback
                            )
                            try:
                ...
                for _ in range(len(embedder_futures)):
                    while True:
                        _check_workers(embedder_futures, activity_logger)
                        merged_results = _merge_and_log_worker_results(
                            results_queue, merged_results, activity_logger, results_callback
