...
from core.check_reponse import check_response
...
class git_assistente:
        ...
        try:
            ...
            if "use \"git pull\" to merge the remote branch into yours" in cmd_result:
                ...
                if check_response("Deseja trazer as atualizações para sua máquina?"):
                    return True
                print("Não é possivel subir as atualizações para o GitHub sem antes trazer as modificações.")
        except:
    ...
    # verifica se tem alguma coisa para subir
    def verify_push(self):
        try:
            cmd_result = str(cmd("git status").stdout, encoding="utf-8")
            if "use \"git push\" to publish your local commits" in cmd_result:
                return True
            print("Seu projeto não tem nenhuma atualização para subir.")
        except:
            exit("Erro ao verificar se tem alguma coisa para dar commit.")
