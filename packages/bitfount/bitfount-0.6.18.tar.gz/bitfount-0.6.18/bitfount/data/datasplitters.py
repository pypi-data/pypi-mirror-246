...
class SplitterDefinedInData(DatasetSplitter):
    ...
    def get_filenames(
        ...
        """
        if split == DataSplit.TRAIN:
            filenames = datasource.data[
                datasource.data[self.column_name] == self.training_set_label
            ]["_original_filename"].tolist()
        elif split == DataSplit.VALIDATION:
            filenames = datasource.data[
                datasource.data[self.column_name] == self.validation_set_label
            ]["_original_filename"].tolist()
        elif split == DataSplit.TEST:
            filenames = datasource.data[
                datasource.data[self.column_name] == self.test_set_label
            ]["_original_filename"].tolist()
        else:
            raise ValueError("Split not recognised")
        return [i for i in filenames if i in datasource.selected_file_names]
