...
def parse_request(request):
    ...
    if "json" in request.content_type:
        ...
        if isinstance(request_body,QueryDict):
...
class Validator:
    ...
    def __init__(self, rules):
        ...
        # reinitilize self.rules with every init
        self.rules = dict()
        ...
        for attribute,rules in self.rules.items():
            for rule in rules:
                ...
                if isinstance(rule,(present,required,required_if,required_unless,required_with,required_with_all,required_without,required_without_all)):
                    ...
                    if has_value and isinstance(value,list) and "*" in rule.attribute :
    ...
    def dec(func):
        def new_view_function(*args, **kwargs):
            ...
            if v.valid:
                ...
                return JsonResponse(v.errors,safe=False, status = 422)
