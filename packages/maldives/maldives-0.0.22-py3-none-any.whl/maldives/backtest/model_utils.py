...
def ConvergenceTrades(data, observed_column='ClosingPrice', pred_column='Prediction'):
# FIXME: look ahead error. Specify train_period and test_period

    df = data[[observed_column,pred_column]].dropna()
    df.loc[:,'Difference'] = df.eval('Prediction-ClosingPrice')
    ...
    df_trades.loc[:,'PnL'] = df_trades.eval('Side*(ClosingPrice2-ClosingPrice1)')
