...
def lemmatize_lux_text(text: str) -> str:
    try:
        ...
        raise Exception("ModuleNotFoundError: No module named 'spellux'. \n"
                        "If you want to use the Luxembourgish preprocessing and are using the pip version of BBF, "
                        "please install the spellux library manually.\n"
                        "If uncertain how to do so, look up the GitHub repository of the BBF.")
    nlp = Luxembourgish()
    doc = nlp(text)
    tokens = []
    for token in doc: 
