...
class ProxyPool:
    def __init__(self, nodes: list, rotate=5, detect=30, check_url="http://checkip.amazonaws.com", proxy_timeout=5, proxy_file='proxy.p'):
        ...
        self.detect_interval = detect * 60
...
def main():
    ...
    parser.add_argument('-p', type=int, help='指定socks服务监听端口，默认为 18000', default=18000)
    parser.add_argument('-r', type=int, help='代理轮换的间隔时间（单位：秒），默认为 5', default=5)
    parser.add_argument('-d', type=int, help='代理健康检查的间隔时间（单位：分钟），默认为 30', default=30)
    parser.add_argument('-u', type=str, help='用于测试连通性的URL，默认为 http://checkip.amazonaws.com', default="http://checkip.amazonaws.com")
    parser.add_argument('-t', type=int, help='上游代理连接的超时时间（单位：秒），默认为 5', default=5)
    parser.add_argument('-i', type=str, help='需导入的代理节点文件路径，默认为 node.list', default='node.list')
    parser.add_argument('-s', type=str, help='可用代理节点存储文件路径，默认为 .s5p', default='.s5p')
    ...
    async def func(loop, args):
        ...
        try:
            ...
            logger.debug(f'读取 {len(all_nodes)} 个节点')
