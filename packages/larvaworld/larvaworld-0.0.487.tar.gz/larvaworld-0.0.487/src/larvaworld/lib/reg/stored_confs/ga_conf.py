...
def Ga_dict():
    d = aux.AttrDict({
        **ga_conf('interference', dt=1 / 16, dur=3, refID=reg.default_refID, m0='explorer',
                  m1='NEU_PHI',
                  cycle_curve_metrics=['fov', 'foa', 'rov'],
                  mkeys=['interference', 'turner'],
                  Nel=2, N=6, env='arena_200mm'),
        **ga_conf('exploration', dur=0.5, dt=1 / 16, refID=reg.default_refID, m0='explorer',
                  m1='NEU_PHI',
        ...
        **ga_conf('realism', dur=1, dt=1 / 16, refID=reg.default_refID, m0='explorer', m1='PHIonSIN',
                  eval_metrics=aux.AttrDict({'angular kinematics': ['run_fov_mu', 'pau_fov_mu', 'b', 'fov', 'foa'],
