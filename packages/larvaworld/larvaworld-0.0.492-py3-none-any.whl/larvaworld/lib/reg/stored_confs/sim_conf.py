...
def Trial_dict():
    ...
    return AttrDict({
        ...
        'odor_preference': AttrDict({'epochs' : trial_conf(
            [5.0] * 8,
            [1.0, 0.0] * 4)}),
        'odor_preference_short': AttrDict({'epochs' : trial_conf(
            [0.125] * 8,
            [1.0, 0.0] * 4)})
    })
    ...
    def d():
        ...
        def game_groups(dim=0.1, N=10, x=0.4, y=0.0, mode='king'):
            ...
            if mode == 'king':
                ...
                l = {**lg(id='Left', N=1, loc=(-0.01, 0.0), mID='follower-L', c='darkblue',
                          odor=Odor.oD(id='Left_odor')),
                     **lg(id='Right', N=1, loc=(+0.01, 0.0), mID='follower-R', c='darkred',
        ...
        d = {
            ...
            'tactile': {
                ...
                'multi_tactile_detection': tE('multi_tactile_detection', env='multi_patch',
                                                     l=lgs(mIDs=['RLtoucher_2', 'RLtoucher', 'toucher'],
                                                           ids=['RL_3sensors', 'RL_1sensor', 'control'], N=4)),
            },

            'growth': {'growth': dE('growth', dur=24 * 60.0, age=0.0),
...
def Ga_dict():
    def bb(name, env='arena_200mm', ks=['interference', 'turner'], refID=None, fit_kws={},
                cyc=[], ev=aux.AttrDict(), m='explorer',fID=None,**kwargs):
        if refID is not None :
            kwargs['dt']=reg.conf.Ref.getRef(refID).dt
        # if m0 is None:
        #     m0=reg.conf.Exp.getID(name).larva_groups.keylist[0]
        return reg.gen.Ga(ga_select_kws=reg.gen.GAselector(base_model=m, space_mkeys=ks),
                          ...
                          env_params=env,experiment=name,**kwargs).nestedConf

    ev1 = AttrDict({'angular kinematics': ['run_fov_mu', 'pau_fov_mu', 'b', 'fov', 'foa'],
                       'spatial displacement': ['v_mu', 'pau_v_mu', 'run_v_mu', 'v', 'a'],
                       'temporal dynamics': ['fsv', 'ffov', 'run_tr', 'pau_tr']})
    dID=reg.default_refID
    l = [bb('interference', refID=dID,cyc=['fov', 'foa', 'rov']),
        bb('exploration',refID=dID, ev=ev1),
        bb('realism', refID=dID, ev=ev1,cyc=['sv', 'fov', 'foa', 'b']),
        bb('chemorbit',  m='navigator', ks=['olfactor'],
           fID='dst2source', fit_kws={'source_xy': None}, env='odor_gaussian_square'),
        bb('obstacle_avoidance',  m='obstacle_avoider', ks=['sensorimotor'],
           fID='cum_dst',env='dish_40mm',scene='obstacle_avoidance_700')]
...
def Batch_dict():
    def bb(exp, proc=[], ss={}, o=None, N=5, abs=False, min=True, thr=0.001, **kwargs):
        return AttrDict(exp=exp,
                            exp_kws={'enrichment': reg.gen.EnrichConf(proc_keys=proc).nestedConf, 'experiment': exp},
                            optimization=OptimizationOps(fit_par=o, max_Nsims=N, absolute=abs, minimize=min,
                                                         threshold=thr).nestedConf,
                            space_search=ss,
                            **kwargs)

    l = [
        bb('PItest_off', ss={'gain_dict.CS': [[-100.0, 100.0], 4], 'gain_dict.UCS': [[-100.0, 100.0], 4]}, proc=['PI']),
        bb('patchy_food', ss={'EEB': [[0.0, 1.0], 3], 'freq': [[1.5, 2.5], 3]}, o='ingested_food_volume'),
        ...
        bb('anemotaxis',
           ...
           o='anemotaxis', N=100, thr=1000.0, min=False, proc=['wind'])
        ]
    l += [bb(exp, ss={'Odor.mean': [[300.0, 1300.0], 3], 'decay_coef': [[0.1, 0.5], 3]},
             o='final_dst_to_Source', proc=['angular', 'spatial', 'source']) for exp in ['chemotaxis', 'chemorbit']]
    l += [bb(exp, ss={'input_noise': [[0.0, 0.4], 2], 'decay_coef': [[0.1, 0.5], 2]}, proc=['PI']) for exp in
          ['PItrain_mini', 'PItrain']]
