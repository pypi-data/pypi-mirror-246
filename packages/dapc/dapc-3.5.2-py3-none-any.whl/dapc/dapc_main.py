...
def reverb(reverb_SampleRate, reverb_left, reverb_right, echoFunction=reverb_funDefault, reverb_numberOfEcho=3,
           reverb_maxVolumeDb=2.0, reverb_offsetSecond=0.11, CONSTANT_REVERB_RANGE=12.0):
    print("[STEPS]Reverb Processing...")
...
def pitch(pitch_left, pitch_right, pitch_dataType, input_sampleRate, output_sampleRate, pitch_pitchFactor=0.85,
          pitch_speedFactor=1.0, sonic_library_path=r"./sonic.dll"):
    """
    @Description:
...
def changeBitRate(cbr_left, cbr_right, original_dataType, current_dataType):
    if original_dataType == current_dataType:
        ...
        print("[STEPS]Changing bit depth...")
...
def transcoding_to_tempDir(inputFile, outputFile):
    ...
    # subprocess.call(os.path.join(os.path.abspath(os.path.dirname(os.path.realpath(sys.path[0]))), 'ffmpeg') + " -y -i " + '"' + inputFile + '"' + " " + '"' + outputFile + '"',stderr=device,shell=True)
