...
class PybulletEnv(gym.Env):
    ...
    def load_env(self):
        ...
        if self.arena == "arena2":
            ...
            p.loadURDF('plane.urdf')
            p.loadURDF('urdf/arena/track.urdf')
    ...
    def get_image(self, cam_height=0, dims=[512, 512]):
        ...
        if self.arena == "arena1":
            orn_cam = [-0.1, 0.13, orn[2]]
            camera_eye = [pos[0] - 1 * np.cos(orn[2]), pos[1] - 1 * np.sin(orn[2]), pos[2] + 1.2 * np.cos(orn[0])]
            target_pos = [pos[0] - 1 * np.cos(orn[2]), pos[1] - 1 * np.sin(orn[2]), pos[2] + 0.4 * np.cos(orn[0])]
            view_matrix = p.computeViewMatrix(camera_eye, target_pos, orn_cam)
            print("done")
        else:
