...
def grahamConvexHull(points: list[Point]) -> Polygon:
    cPolygon = _cgeo.libAlgorithms_2D.ConvexHullUtils().grahamConvexHull([p.cPoint for p in points])
    return Polygon.fromCPolygon(cPolygon)


def giftWrapConvexHull(points: list[Point]) -> Polygon:
    cPolygon = _cgeo.libAlgorithms_2D.ConvexHullUtils.giftWrapConvexHull([p.cPoint for p in points])
    return Polygon.fromCPolygon(cPolygon)
