...
class AHEventFilter:
    ...
    def get_AH_with_OD(self, row, od_eps=45):
        self.od_events['End'] = self.od_events['Start'] + self.od_events['Duration']
        if row['Type'] == 'Hypopnea':
            ...
            con_a = ((hypopnea_start <= self.od_events['Start']) & (hypopnea_end + od_eps > self.od_events['Start']))
            # 氧降事件>50%在Hypopnea内
            con_b = ((np.minimum(hypopnea_end, self.od_events['End']) -
                      np.maximum(hypopnea_start, self.od_events['Start'])) / self.od_events['Duration'] > 0.5)
            condition = (con_a | con_b) & (~self.od_events['Processed_od'])
