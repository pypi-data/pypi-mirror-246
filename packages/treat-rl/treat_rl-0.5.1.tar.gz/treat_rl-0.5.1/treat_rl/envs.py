...
class DiseaseTreatmentEnv(gym.Env):
    ...
    def generate_diseases(self, rng):
        ...
        for i in range(self.n_diseases):
            ...
            std_devs = rng.uniform(self.symptom_std_range[0],
        ...
        for treatment in all_treatments:
            num_diseases_for_treatment = rng.randint(*self.num_diseases_for_treatment_range)
            diseases_for_treatment = rng.choice(self.n_diseases, size=num_diseases_for_treatment, replace=False)
            for disease in diseases_for_treatment:
                disease = f"Disease_{rng.randint(0, self.n_diseases)}"
                self.diseases[disease]['treatments'] = np.append(self.diseases[disease]['treatments'], treatment)
                self.diseases[disease]['remission_probs'][treatment] = rng.uniform(*self.remission_prob_range)
        ...
        for disease in self.diseases.values():
            if len(disease['treatments']) == 0:
                ...
                disease['remission_probs'][disease['treatments'][0]] = rng.uniform(*self.remission_prob_range)
    ...
    def apply_treatment_and_transition_disease(self, action, treatment):
        ...
        if self.use_symptom_rewards:
            symptom_cost = (self.current_symptoms.sum() / self.n_symptoms) * (self.remission_reward / (2 * self.max_visits))
