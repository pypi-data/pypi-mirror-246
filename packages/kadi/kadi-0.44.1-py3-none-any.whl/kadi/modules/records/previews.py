...
def _get_file_encoding(file):
    ...
    with open_file(file, mode="rb") as f:
        # Check if we should read the whole file or only parts of it, depending on its
        # size. Note that partial reading of the file might product incorrect results
        # for multibyte encodings.
        if file.size <= 10 * const.ONE_MB:
            results = from_fp(f)
        else:
            results = from_bytes(f.read(const.ONE_MB))
