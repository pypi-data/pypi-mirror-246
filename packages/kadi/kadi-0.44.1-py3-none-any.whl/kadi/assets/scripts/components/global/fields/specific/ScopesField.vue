    ...
    <div class="card">
      <div class="card-body px-3 pt-3 pb-2">
        <div class="d-lg-flex justify-content-between">
          <div :class="{'mt-4 mt-lg-0': index > 0}" v-for="(actions, object, index) in scopes" :key="object">
            <input type="checkbox"
                   :id="object"
                   :checked="objectScopesChecked(object)"
                   @click="checkObjectScopes(object, $event)">
            <label :for="object">
              <strong>{{ object | capitalize }}</strong>
            </label>
            <div v-for="action in actions" :key="getScopeValue(object, action)">
              <input type="checkbox" :id="getScopeValue(object, action)" v-model="scopesModel[object][action].checked">
