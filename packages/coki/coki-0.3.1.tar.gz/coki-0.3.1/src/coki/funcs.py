...
def get_account_id(token):
    if os.path.exists("coki_token.txt"):
        ...
        r = requests.get(
            "https://api.cocos.capital/api/v1/users/me", headers=parse_headers(token, 1)
        )
...
def parse_headers(token, account_id=1):
    headers = {
...
def orders(account_id):
    ...
    token = open("coki_token.txt", "r").read()
    headers = parse_headers(token, account_id)
...
def gen_table_orders(account_id, ticker="ALL", filtro="cancelado"):
    ...
    for i in range(0, len(orders)):
        ...
        table = {
            "Fecha": datetime.datetime.strptime(data["date"], "%Y-%m-%dT%H:%M:%S.%f%z")
            .time()
            .strftime("%H:%M:%S"),
            "Ticker": ticker_name,
            "Estado": data["status"],
            "Precio": round(data["set_price"], 2),
            "Cantidad": round(data["set_quantity"], 0),
            "Monto": round(data["set_amount"], 2),
            "Ejecutados": data["result_quantity"],
            "code_op": data["order_id"],
        }
        tables.append(table)

    df = pd.DataFrame(tables)
    df = df.copy().sort_values(by=["Fecha"], ascending=True).reset_index(drop=True)
    df["Num_op"] = df.index
    df = df.copy().set_index("Num_op")

    if filtro == "cancelado":
        df = df[df["Estado"] != "CANCELLED"].copy()
    elif filtro == "ejecutado":
        df = df[df["Estado"] == "EXECUTED"].copy()
    elif filtro == "mercado":
        df = df[
            (df["Estado"] == "PARTIALLY_EXECUTED") | (df["Estado"] == "MARKET")
        ].copy()

    if ticker != "ALL":
        df = df[df["Ticker"] == ticker].copy()

    df = df.sort_values(by=["Ticker", "Fecha"], ascending=False)
