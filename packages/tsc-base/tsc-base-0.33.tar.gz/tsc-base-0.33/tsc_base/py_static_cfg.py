import inspect
from typing import Any, Iterator, Union, TypeVar, Tuple, Callable, List, Optional
...
class Cfg:
    ...
    def _is_config_class_(cls, v: Any) -> bool:
        """判断是否是合法的配置类"""
        return inspect.isclass(v) and issubclass(v, Cfg) and v.__name__ != Cfg.__name__
    ...
    def _get_(cls, 
        ...
        if isinstance(key, str):
            ...
            value = getattr(cls, key, default) if allow_default else getattr(cls, key)
        else:
    ...
    def model_dump_code(cls, 
        ...
        annotations = getattr(cls, '__annotations__', {})  # 用于补充类型注释
        # 遍历类字典
        for name, value in cls._ikv_:
            ...
            if cls._is_config_class_(value):  # 嵌套类
                ...
                annotation_str = f": {type_annotation.__name__}" if type_annotation else ''
...
if __name__ == '__main__':
    ...
    print(MyClass._get_(1234, allow_default=True))
