...
@click.command(epilog='make it easy', help='LAN file sharing {0}'.format(version.VERSION))
@click.option('-i', '--ip', type=str, show_default=True, default=utils.get_ip(), help="Local IP")
@click.option('-p', '--port', type=int, show_default=True, default=8000, help='Local port')
@click.option('-d', '--dir', type=str, show_default=True, default=os.getcwd(), help='Shared directory path')
@click.option('-q', '--qr', is_flag=True, help='Show QRCode')
@click.option('-u', '--upload', is_flag=True, help='Support upload')
@click.option('-m', '--mkdir', is_flag=True, help='Support mkdir')
@click.option('-z', '--zip', is_flag=True, help='Support zip')
@click.option('-r', '--rm', is_flag=True, help='Support rm')
@click.option('--hidden', is_flag=True, help='Show hidden')
@click.option('--user', type=str, help='Basic Auth User')
@click.option('--password', type=str, help='Basic Auth Password')
@click.option('--start', is_flag=True, help='daemon start')
@click.option('--stop', is_flag=True, help='daemon stop')
@click.option('--pid_file', type=str, show_default=True, default=utils.get_pid(), help='pid_file')
@click.option('--log_file', type=str, show_default=True, default=utils.get_log(), help='log_file')
def cmd_http_server(ip, port, dir, qr, upload, mkdir, zip, rm, hidden, user, password, start, stop, pid_file, log_file):
    """
    http server
    :param args:
    :return:
    ...
    if stop:
        daemon.daemon_exec(command='stop',
                           log_file=log_file,
                           pid_file=pid_file)
    app.config[key_www] = dir
    app.config[key_upload] = upload
    app.config[key_user] = user
    app.config[key_pass] = password
    app.config[key_mkdir] = mkdir
    app.config[key_rm] = rm
    app.config[key_hidden] = hidden
    app.config[key_zip] = zip
    if qr:
        utils.qr_code_show('http://{0}:{1}/'.format(ip, port))
    if start:
