...
from pih.const import CheckableSections, Actions, JournalType, Tag
...
from pih.console_api import LINE, SPLITTER
import re
...
class MobileOutput(Output):
    ...
    @contextmanager
    def personalized(self, enter: bool = True):
        if enter:
            try:
                while_not_do(lambda: e(self.message_buffer))
    ...
    @contextmanager
    def set_allow_show_error(self, value: bool):
...
class MobileInput(Input):
    def __init__(
        ...
        self.data_input_timeout: int | None = (
            ...
            if data_input_timeout == -1
                ...
                data_input_timeout
                or A.S.get(MOBILE_HELPER_USER_DATA_INPUT_TIMEOUT)
            )
        )
    ...
    def index(
        ...
        return super().index(
            caption, data, label_function, use_zero_index, use_free_input, text_after
        )
...
class MobileHelper(OutputStub):
    ...
    def get_file_arg_information(self, value: str) -> tuple[int, int]:
        file_index: int = -1
        file_pattern_index: int = -1
        for file_pattern_index, pattern in enumerate(FILE_PATTERN_LIST):
            file_index = value.find(pattern)
            if file_index == 0:
                break
        return (file_index, file_pattern_index)

    def check_for_arg_is_file(self, value: str) -> bool:
        return self.get_file_arg_information(value)[0] != -1
    ...
    def __init__(self, pih: PIH, stdin: Stdin):
        ...
        hr_unit_node: CommandNode = CommandNode(
            ...
            text=lambda: f"Руководитель: {b(A.R.get_first_item(A.R.filter(lambda item: not item.samAccountName.startswith(A.CT_AD.TEMPLATED_USER_SERACH_TEMPLATE[0]) and not item.samAccountName.endswith(A.CT_AD.TEMPLATED_USER_SERACH_TEMPLATE[-1]), A.R_U.by_job_position(A.CT_AD.JobPositions.HR))).name)}.\nТелефон: {b('706')}.",
        )
        self.menu.unit = [
            it_unit_node,
        ...
        create_user_node: CommandNode = CommandNode(
            ...
            lambda: [
                "Создание",
                j(("Создать", (" пользователя" if self.in_choice_command else ""))),
            ],
        ...
        find_user_node: CommandNode = CommandNode(
            ...
            lambda: [
                "Поиск пользователя",
                j(("Найти", (" пользователя" if self.in_choice_command else ""))),
            ],
        ...
        polibase_person_card_registry_folder_qr_code_create_node: CommandNode = (
            CommandNode(
                ...
                filter_function=lambda: (
                    self.argless and (not self.is_all or self.in_main_menu)
                )
                or (
                    not self.argless
                    and not self.is_all
        ...
        polibase_person_card_registry_folder_statistics_node: CommandNode = CommandNode(
            ...
            and (not self.is_all or self.in_main_menu),
        )
        ...
        self.main_menu_node: CommandNode = CommandNode(
            ...
            text=lambda: b("Все команды:") if self.is_all else nl(b("Список разделов:")),
        )
        #######################
        ...
        def get_note_title() -> list[str]:
            ...
            gkeep_item: GKeepItem | None = one(
                A.R_N.find_gkeep_item(value, value, not self.is_all)
            )
            if e(gkeep_item):
        ...
        self.execute_file("@on_before_command_node_tree_creation")
        ...
        self.command_node_tree = {
            ...
            CommandNode(
                ...
                CommandNode(["card", "карт", ""], [""]): A.D.to_dict(
                    self.menu.card_registry
                )
            },
    ...
    def check_for_arg_list_include_file(self) -> bool:
        return (len(self.arg_list) - len(self.arg_list_exclude_file)) != 0
    ...
    def arg(
        ...
        result_source: str = result

        if as_file and ne(result):
            result = result.lower()

            def label_function(note: Note, _) -> str:
                note_title_list: list[str] = note.title.split(SPLITTER)
                if len(note_title_list) == 3:
                    return j(
                        (
                            [b(note_title_list[0].upper()), " ", A.CT_V.ARROW, " "]
                            if n(filter_function)
                            else []
                        )
                        + [
                ...
                return b(note_title_list[1])

            file_index: int = -1
            file_pattern_index: int = -1
            file_index, file_pattern_index = self.get_file_arg_information(result)
            if file_index == 0:
                file_list: list[Note] = sorted(
                    A.R.filter(
                        lambda item: item.title.find(j((SPLITTER, "@"))) == -1,
                        A.R_N.files(force_update=True),
                    ).data,
                    key=lambda note: note.title,
                )
                file_name_index: int = file_index + len(
                    FILE_PATTERN_LIST[file_pattern_index]
                )
                if len(result) != len(FILE_PATTERN_LIST[file_pattern_index]):
                    splitter_index: int = result.find(SPLITTER)
                    file_list_temp: list[Note] | None = (
                        file_list
                        if splitter_index == -1
                        else A.D.filter(
                            lambda note: note.title.lower().startswith(
                                result[file_name_index:splitter_index]
                            ),
                            file_list,
                        )
                    )
                    if splitter_index != -1:
                        result = result[splitter_index + len(SPLITTER) :]
                    file_list_temp = A.D.filter(
                        lambda note: note.title.lower().find(
                            result[file_name_index:].lower()
                        )
                        != -1,
                        file_list_temp,
                    )
                ...
                title_list: list[str] = file_item.title.split(SPLITTER)
                self.output.separated_line()
                self.output.head1(title_list[-1])
                command_type: A.CT_CMDT | None = None
                for item in A.CT_CMDT:
                    if title_list[0].lower() in A.D.map(
                        lambda item: item.lower(), item.value[1:]
                    ):
                        command_type = item
                        break
                self.arg_list.remove(result_source)
                return (command_type, file_item.text, file_item.title)
        return result
    ...
    def get_joke_handler(self) -> None:
    ...
    def robocopy_job_run_handler(self) -> None:
        ...
        if ne(source_job_name) and is_active(source_job_name) and not forced:
            ...
            job_name: str = source_job_name or self.input.item_by_index(
                f"Пожалуйста, выберите Robocopy-задание, которое необходимо запустить",
                job_name_list,
    ...
    def workstation_action_handler(
        self,
        search_value: str | User | None = None,
        ...
        try:
            ...
            if non_search_action:
                ...
                if e(workstations_result):
                    if A.C_R.accessibility_by_ping(search_value, None, 2):
                        if self.is_forced:
                            ...
                            self.output.error(
                                js(("Компьютер", search_value, "нельзя перезагрузить"))
                            )
                    else:
                        self.output.error(f"Компьютер {search_value} не найден")
                else:
                ...
                try:
                    if not A.C_WS.name(search_value):
                        search_value = A.D.get_first_item(
                            self.input.user.by_any(search_value)
                        )
                except NotFound as _:
                    pass
                workstations_result = (
                    A.R_WS.all() if all else A.R_WS.by_any(search_value)
                )
    ...
    def shutdown_workstation_handler(self, all: bool = False) -> None:
        self.workstation_action_handler(None, 1, all)
    ...
    def where_handler(self) -> None:
        ...
        with self.output.set_allow_show_error(False):
            value: str = self.arg() or self.input.input("Введите поисковый запрос")
            try:
                self.workstation_action_handler(value, 2, False)
                was_found = True
            except NotFound as _:
                pass
            try:
                if A.C_P.person_pin(value):
                    ...
                    result: str = self.get_polibase_person_card_place_label(
                        polibase_person,
                        A.CR.persons_pin_by_folder(polibase_person.ChartFolder),
                    )
                    if ne(result):
                        self.write_line(
    ...
    def who_handler(self) -> None:
        ...
        with self.output.set_allow_show_error(False):
            value: str = self.arg() or self.input.input("Введите поисковый запрос")
            try:
                self.find_user_handler(value)
                was_found = True
            except NotFound as _:
                pass
            try:
                if A.C_P.person_pin(value):
                    self.polibase_person_information_handler(value)
                    was_found = True
            except NotFound as _:
                pass
        if not was_found:
    ...
    def run_command_handler(self, command_type: A.CT_CMDT | None = None) -> None:
        ...
        args: Any | None = self.arg(
            as_file=True,
            filter_function=None if n(command_type) else filter_function,
        )
        if isinstance(args, tuple):
            self.run_command(*args, test=self.is_test)
        else:
            self.run_command(command_type, args, test=self.is_test)
    ...
    def run_command(
        ...
        if command_type == A.CT_CMDT.CMD:
            ...
            if ne(command_text):
                command_text = extract_parameters(command_text)
            command_text_list: list[str] = A.D.filter(
                lambda item: not item.startswith("::"),
                ([None] if e(command_text) else command_text.splitlines()),
            )
            for command_text in command_text_list:
                command_text = get_command(command_text if ne(command_text) else None)
                if n(use_psexec_executor):
                    use_psexec_executor = e(
                        [
                            value
                            for value in A.D.map(
                                lambda item: item.lower(), command_text
                            )
                ...
                if use_psexec_executor:
                    if e(host):
                        if self.yes_no(
                            f"Использовать хост {b(default_host)}, для выполнения команды",
            ...
            if self.session.is_mobile:
                while True:
                    ...
                    command_part_item: str = extract_parameters(
                        command_text or jnl(command_line_list)
                    )
                    command_result: str | None = None
                    try:
                        command_result = A.R_N.execute(
                            command_part_item, {"self": self}, True, True
                        )
                ...
                for command_part_index, command_part_item in enumerate(
                    ...
                    while True:
                        try:
                            ...
                            with self.output.make_loading(0.5, "Выполнение запроса"):
                                result = (
    ...
    def find_note_handler(self, root: bool, show_all: bool = False) -> None:
        argument: str | None = self.arg()
        note_name: str | None = argument
        ...
        full_equaliment: bool = not self.is_all and (
            not has_argument or A.C_N.exists(note_name, note_name, not self.is_all)
        )
        if full_equaliment or A.C_N.exists(note_name, note_name, False):
            ...
            gkeep_item_list_result: Result[list[GKeepItem]] = A.R.sort(lambda item: item.title, A.R_N.find_gkeep_item(
                note_name, note_name, not self.is_all
            ))
            if e(gkeep_item_list_result):
                ...
                gkeep_item_list: list[GKeepItem] = (
                    ...
                    if not self.is_all or show_all or has_argument
                    else gkeep_item_list_result.data
                )
                for gkeep_item in gkeep_item_list:
                    note_result: Result[None] = A.R_N.get(
                        gkeep_item.id, force_update=True
                    )
                    ...
                    if ne(note.images):
                        self.write_line(
                            i(js(("Ожидайте загрузку изображений:", len(note.images))))
                        )
    ...
    def choice_journal(self, name: str | None) -> JournalType:
        ...
        journal_list: list[JournalType] = A.D.to_list(A.CT_J)[1:]
        return one(
            A.D_J.type_by_any(
                (
                    ...
                    if n(name)
                        ...
                        A.D_J.type_by_any(name, not self.is_all) or journal_list
                    )
                )
            )
        )
    ...
    def add_journal_record_handler(self) -> None:
        ...
        tag: Tag = A.D_J.tag_by_id(
            1
            + self.input.index(
                "Выберите тэг записи журнал",
                A.D.to_list(A.CT_Tag),
                lambda item, _: js((item[2], item[1])),
            )
        )
    ...
    def show_journal_handler(self) -> None:
        with self.output.make_separated_lines():
            self.execute_file("show_journal_records", redirect_to_output=True)
    ...
    def find_user_handler(self, value: str | None = None) -> None:
    ...
    def exitable(self, interruption: InternalInterrupt) -> bool:
        return interruption.type in (
            InteraptionTypes.EXIT,
            InteraptionTypes.NEW_COMMAND,
        )
                        ...
                        with self.output.make_exit_line("для выбора месяца"):
                            while True:
                                try:
                                    ...
                                    if interruption.type == InteraptionTypes.BACK:
                                        break
                                    if self.exitable(interruption):
                                        raise interruption
                    except InternalInterrupt as interruption:
                        if interruption.type == InteraptionTypes.BACK or self.exitable(
                            interruption
                        ):
                            raise interruption
        else:
    ...
    def untill_exit(self, action: Callable[[None], None]) -> None:
        while True:
            try:
                ...
                if interruption.type in (InteraptionTypes.BACK, InteraptionTypes.EXIT):
                    raise interruption
    ...
    def polibase_person_information_handler(self, value: str | None = None) -> None:
        ...
        if self.get_language_index(line):
            if self.wait_for_input():
                ...
                source_line_part_list: list[str] = A.D.map(
                    lambda item: item.lower(), list(action_line_part_list)
                )
