...
def training(GRNdir,Input_dir,method,outdir):
    if method=='LINGER':
        ...
        for i in range(23):
            ...
            warnings.filterwarnings("ignore")
            res=Parallel(n_jobs = n_jobs)(delayed(sc_nn)(ii,gene_chr,TFindex,TFindex_bulk,REindex,REindex_bulk,REindex_bulk_match,Target,netall,adj_matrix_all,Exp,TF_match,input_size_all,fisherall,Opn,l1_lambda,fisher_w) for ii in range(N))
            warnings.resetwarnings()
            for ii in tqdm(range(N)):
                ...
                if res[ii][4]==1:
                    ...
                    shapall_s[index_all]=res[ii]s[1]
