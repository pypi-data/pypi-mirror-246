...
def bulk_reg(Input_dir,GRNdir,genome,chrN,outdir):    
...
import numpy as np
import pandas as pd
def regulon(Input_dir,RNA_file,GRNdir,network,genome,outdir):
...
# Apply quantile normalization to 'trans_reg_n'
        ...
        for i in tqdm(range(23)):
            chrN=chrom[i]
            TF_RE,RE_TG=bulk_reg(Input_dir,GRNdir,genome,chrN,outdir)
        ...
        TGset=cis.columns
        from LingerGRN.LL_net import load_TF_TG
...
def master_regulator(regulon_score,Input_dir,labels,celltype):
    ...
    label=pd.read_csv(labels,sep='\t',header=None)
...
def heatmap_cluster(regulon_score,labels,save,outdir):
...
def box_comp(TFName,labels,celltype1,celltype2,datatype,RNA_file,regulon_score,save,outdir):
    import numpy as np
    data=np.zeros(regulon_score.shape[1])
    if datatype=='activity':
        TFexp=regulon_score.loc[TFName].values
    if datatype=='expression':
        data0=pd.read_csv(Input_dir+RNA_file,sep='\t',header=0,index_col=0)
