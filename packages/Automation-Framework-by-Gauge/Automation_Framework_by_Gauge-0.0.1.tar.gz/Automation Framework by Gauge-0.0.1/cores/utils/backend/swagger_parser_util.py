...
from cores.utils.common.store_util import StoreUtil, GetUtil
from cores.utils.backend.gitlab_util import GitLabService
from cores.utils.backend.request_util import RequestUtil
...
class SwaggerUtil:
    ...
    def swagger_parser_factory(request_type: str, **kwargs):
        ...
        if not GetUtil.suite_get(__service_name):
            ...
            __env = GetUtil.suite_get(EnvironmentConst.Environment.ENV_OBJ).env
            if __env == 'stg':
        ...
        if is_gitlab:
            ...
            if not prj_name:
                prj_name = GetUtil.suite_get(
                    EnvironmentConst.Environment.ENV_OBJ).PROJECT_NAME
            try:
                swagger_content = gl.parsing_swagger_content(projects=prj_name)
                StoreUtil.suite_store(
                    EnvironmentConst.Swagger.SWAGGER_OBJ, swagger_content)
