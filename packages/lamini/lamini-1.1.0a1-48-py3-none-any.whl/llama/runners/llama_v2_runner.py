...
{user} [/INST]"""
        ...
        # if sys prompt passed as "", do not override with default prompt.
        self.system_prompt = (
            DEFAULT_SYSTEM_PROMPT if (system_prompt == None) else system_prompt
        )
    ...
    def call(
        ...
        final = [
            self.prompt_template.format(system=system_prompt, user=p) for p in prompt
        ]
        output = self.llm(
            prompt=final,
            output_type=output_type,
