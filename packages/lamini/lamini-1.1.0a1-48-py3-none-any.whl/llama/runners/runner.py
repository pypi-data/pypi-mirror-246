    ...
    def __call__(self, *args, **kwargs):
        ...
        with cf.ThreadPoolExecutor(max_workers=num_servers) as executor:
            ...
            for i in range(0, init_task_len):
                print(
                    f"Submitting input group {i+1} out of {len(input_groups)} for inference"
                )
                future = self.submit_sub_group(
                    executor, input_groups[i], *args, **kwargs
                )
            ...
            while num_completed_futures < len(input_groups):
                completed_futures, _ = cf.wait(
                    futures, return_when=cf.FIRST_COMPLETED, timeout=1200
                )
