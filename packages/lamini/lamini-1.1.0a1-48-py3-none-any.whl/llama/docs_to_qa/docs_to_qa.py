...
class DocsToQA:
    def __init__(
        ...
        self.question_llm = LlamaV2Runner(model_name=model_name)
        self.answer_llm = LlamaV2Runner(model_name=model_name)
        self.qa_llm = LlamaV2Runner(model_name=model_name)
...
{user_prompt} [/INST]"""
    ...
    def _get_question_prompt(self, num_examples=3):
        prompt_question_examples = sample(
            self.question_examples, min(num_examples, len(self.question_examples))
        )
    ...
    def _get_answer_prompt(self, num_examples=3):
        prompt_answer_examples = sample(
            self.answer_examples, min(num_examples, len(self.answer_examples))
        )
        ...
        for doc_id, qas in self.qa.items():
            for qa in qas:
                ...
                if use_retrieval:
                    ...
                    prompt_with_doc = self._make_prompt(
                        self.qa_system_prompt, question_with_doc
                    )
        ...
        for dirpath, _, filenames in os.walk(self.docs_dirpath):
            for filename in filenames:
                ...
                if docs_path_type == ".csv":
                    ...
                    docs_path_type == ".jsonl"
                    or docs_path_type == ".jsonlines"
                    or docs_path_type == ".json"
                ):
    ...
    def _run_questions(
        ...
        for docs_id, chunk in tqdm(list(self.docs.items())[start_index:]):
            prompt = (
                f"{chunk}{self.prompt_sep}{self.question_prompt_suffix}"
                if self.question_prompt_suffix
                else chunk
            )
            # if self.qa_examples:
    ...
    def _run_answers(
        ...
        for docs_id, chunk in self.docs.items():
            ...
            for question in question_list:
                prompt = (
                    f"{chunk}{self.prompt_sep}{question}{self.prompt_sep}{self.answer_prompt_suffix}"
                    if self.answer_prompt_suffix
                    else f"{chunk}{self.prompt_sep}{question}"
                )
                # if self.qa_examples:
                # prompt = self._get_qa_prompt() + prompt
...
def finetune_qa(
    qa_dirpath, docs_dirpath=None, model_name=None, is_public=False, use_retrieval=True
):
    """
