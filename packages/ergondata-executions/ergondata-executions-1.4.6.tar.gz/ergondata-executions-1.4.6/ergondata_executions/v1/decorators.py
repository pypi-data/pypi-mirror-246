...
def run_decorator(wrapped, instance, args, kwargs):
    ...
    try:
        ...
        self.ergon.write_tk_exec_log(message=f"Task execution failed due to TaskException {str(e.args)}", level="error")
...
def get_queue_item_decorator(wrapped, instance, args, kwargs):
    ...
    while True:
        ...
        except PerformerQIException as e:
            self.ergon.write_tk_exec_log(message=f"Queue item raised an exception: {str(e.args)}", level="error")
        except PerformerQISysError as e:
            self.ergon.write_tk_exec_log(message=f"Queue item raised a system exception {str(e.args)}", level="error")
        except PerformerQIBusException as e:
            self.ergon.write_tk_exec_log(message=f"Queue item raised a business exception {str(e.args)}", level="error")
        except BaseException as e:
            self.ergon.write_tk_exec_log(message=str(e), level="error")
            self.ergon.update_queue_item(
