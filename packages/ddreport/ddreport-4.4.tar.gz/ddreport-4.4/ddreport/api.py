...
class CustomQuery():
    ...
    def query(self, method=None, url=None, **kwargs):
        ...
        data = kwargs.copy()
        if self.headers:
            data.update({"headers": self.headers})
        if self.cookies:
            data.update({"cookies": self.cookies})
        if self.verify is not None:
            data.update({"verify": self.verify})
        if self.proxies:
            data.update({"proxies": self.proxies})
        if self.cert:
            data.update({"cert": self.cert})
        Q.query = data
        try:
