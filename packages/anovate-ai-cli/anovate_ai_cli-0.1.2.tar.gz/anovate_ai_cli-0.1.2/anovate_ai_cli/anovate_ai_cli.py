...
DOWNLOAD_FOLDER = os.path.join(os.getcwd(), "datasets")
...
def get_images(access_token: str, dataset_id: int):
    response = requests.get(
...
def handle_image(image_info: dict):
    ...
    image_path = os.path.join(image_info["dataset_path"], image_name)
    image_hash = get_image_hash(image_path)
    if image_hash != image_info["image"]["hash"]:
        img_url = get_image_url(image_info["access_token"], image_info["image"]["id"])
        r = requests.get(img_url, allow_redirects=True)
        open(os.path.join(image_info["dataset_path"], image_name), "wb").write(
            r.content
        )
...
def download_dataset(
    ...
    if not dataset:
        ...
        try:
            ...
            rprint(f"[red bold] Dataset not selected")
            return
    try:
    ...
    images = get_images(access_token, dataset_id)
