...
try:
    ...
    def cmaplist(index):
        ...
        if type(index) == list:
            ...
            str(index)
            if index in ['-28','28','-27','-26','-25','-24','-23','-22','22','23','24','25','26','27','12','13','14','15','-12','-13','-14','-15','33','34','35','36','37','38','39','-33','-34','-35','-36','-37','-38','-39','43','44','45','46','47','48','49','-43','-44','-45','-46','-47','-48','-49','59','58','57','-57','-58','-59','-69','69','-79','79','-89','89','-99','99','-109','109','-119','119','-129','129']:
                try:
                    if theme == 'default':
                        if index == '15':
                        ...
                        if index == '119':
                            return LinearSegmentedColormap.from_list('mycmap',['#FFFFFF','#CECECE','#A1A1A1','#717171','#444444','#7747D0','#380C66'])
                        if index == '-119':
                            return LinearSegmentedColormap.from_list('mycmap',['#380C66','#7747D0','#444444','#717171','#A1A1A1','#CECECE','#FFFFFF'])
    ...
    # 刻度
    def xticks(ticks, labels=None, xlim=True ,color='fg', font=fontpath, fontsize=fontsize.ticklabel, **kwargs):
        if labels == None: # 將ticks當作labels
            # 對ticks的每個元素轉換成str，若為float則至多取到小數點後3位
            labels = []
            for i in ticks:
                if type(i) == float:
    ...
    def yticks(ticks, labels=None, ylim=True, color='fg', font=fontpath, fontsize=fontsize.ticklabel, **kwargs):
        if labels == None: # 將ticks當作labels
            # 對ticks的每個元素轉換成str，若為float則至多取到小數點後3位
            labels = []
            for i in ticks:
    ...
    # font
    def fontprop(fontsize, weight='normal', **kwargs):
        if kwargs == {}:
            if weight == 'normal':
                return fm.FontProperties(fname=fontpath, size=fontsize)
            elif weight == 'bold':
                return fm.FontProperties(fname=fontpath_bold, size=fontsize)
            else:
                print(bcolors.WARNING+'[WARNING] Invalid argument given when using fontprop(weight=...), using default fontpath.'+bcolors.ENDC)
                return fm.FontProperties(fname=fontpath, size=fontsize)
        else:
    ...
    class initsubplots():
        ...
        def set_facecolor(self, color):
            if self.ndim == 1:
                self.ax[self.row].set_facecolor(colorlist(color))
            else:
                self.ax[self.row, self.col].set_facecolor(colorlist(color))
