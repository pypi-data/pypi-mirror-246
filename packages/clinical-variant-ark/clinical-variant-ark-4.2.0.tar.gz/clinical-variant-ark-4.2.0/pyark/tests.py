...
from protocols.protocol_7_4.cva import Assembly, PedigreeInjectRD, CancerParticipantInject, \
     EvidenceEntry, Property, EvidenceSource, Actions, Therapy, DrugResponse, GenomicFeature, \
    FeatureTypes, VariantCoordinates, VariantsCoordinates, Penetrance, DrugResponseClassification, Transaction, \
    TransactionStatus, VariantInterpretationLog, EvidenceEntryAndVariants, ClinicalReportInject
from protocols.protocol_7_4.reports import Program
...
class TestPyArk (TestCase):
    ...
    def setUpClass(cls):
        ...
        if 'prod' in cls.CVA_URL_BASE:
            logging.error("You cannot run tests against prod because it modifies the data")
        ...
        cls.random_clinical_reports = [
            cls.cases.get_clinical_report(case['caseIdentifier'], case['version'])
            for case in cls.random_closed_cases
        ]
    ...
    def _get_random_case_and_clinical_report_id_and_version(self):
        cr = self._get_random_clinical_report()
        return cr['interpretationRequestId'], cr['interpretationRequestVersion'], cr['id'], cr['version']

    def _get_random_clinical_report(self):
        return random.choice(self.random_clinical_reports)
...
class TestCases(TestPyArk):
    ...
    def test_count_cases(self):
        # Limit to GRCh37 cases to speed up the query
        count = self.cases.count(assembly='GRCh37')
        self.assertIsInstance(count, int)
...
class TestOthers(TestPyArk):
    ...
    def test_get_clinical_report(self):
        ...
        # gets clinical report as json
        result = self.cases.get_clinical_report(case_id, case_version)
        self.assertTrue(result is not None)
        self.assertTrue(isinstance(result, dict))

        # gets clinical report as dataframe
        result = self.cases.get_clinical_report(case_id, case_version, as_data_frame=True)
        self.assertTrue(result is not None)
        self.assertTrue(isinstance(result, pd.DataFrame))


    def test_get_clinical_report_version(self):

        case_id, case_version, report_id, report_version = self._get_random_case_and_clinical_report_id_and_version()

        # gets clinical report as json
        result = self.cases.get_clinical_report_version(case_id, case_version, report_id, report_version)
        self.assertTrue(result is not None)
        self.assertTrue(isinstance(result, dict))

        # gets clinical report as dataframe
        result = self.cases.get_clinical_report_version(
            case_id, case_version, report_id, report_version, as_data_frame=True
        )
        self.assertTrue(result is not None)
        self.assertTrue(isinstance(result, pd.DataFrame))
    ...
    def test_post_clinical_report(self):
        transaction = self._test_post(ClinicalReportInject, self.data_intake.post_participant)
        self.assertTrue(isinstance(transaction, Transaction))
        self.assertTrue(transaction.id is not None)
        self.assertTrue(transaction.status == TransactionStatus.PENDING)
        self.assertTrue(transaction.compressedData is None)
