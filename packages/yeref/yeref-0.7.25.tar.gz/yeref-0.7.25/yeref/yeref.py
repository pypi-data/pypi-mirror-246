        ...
        document.addEventListener("DOMContentLoaded", async () => {{
            ...
            async function updateMedia() {{
                ...
                if (mediaElement && mediaNumber) {{
                    const isVideo = mediaElement.src.endsWith('.mp4')
                    
                    if (isVideo) {{
                        mediaElement.outerHTML = `<video class="media" src="${{mediaList[currentIndex]}}" controls autoplay loop muted></video>`;
                    }}
                    else {{
                        mediaElement.outerHTML = `<img class="media" src="${{mediaList[currentIndex]}}" alt="Media">`;
                    }}
        ...
        if not is_web:
            ...
            while i > 0:
                ...
                if (0 < r_i < len(tmp_arr) - 1 and len(word) >= 4 and not word.startswith(
                    ...
                    word_pattern = re.escape(word)
                    result = re.sub(rf'\b{word_pattern}\b', f"```\n{word}```", result)
                    break
...
async def outsource_handle(url, lst):
    result = []
    try:
        data_ = []
            ...
            try:
                if item['type'] == 'tgph':
                    ...
                    if item['prompt'] == 'variation':
                        ...
                        src = os.path.abspath(os.path.join(os.path.dirname(__file__), item['file_path']))
                        dst = datetime.datetime.utcnow().strftime(f"%f-{os.path.basename(item['file_path'])}")
                        dst = os.path.abspath(os.path.join(os.path.dirname(__file__), dst))
                        ...
                        prompt = f"{item['prompt']} {extra_prompt}"
                        file_content = ''
                        file_name = 'en' if 'file_name' not in item else item['file_name']
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(url=url, json={'lst': data_}) as response:
                ...
                for i in range(len(result)):
                    if result[i]['type'] == 'tts':
                        ...
                        dst_mp3 = os.path.abspath(os.path.join(os.path.dirname(__file__), dt_))
