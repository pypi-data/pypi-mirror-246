...
def _build_test_graph1():
    conv0 = make_node("Conv", inputs=["a", "w0"], outputs=["c"], groups=2, name="conv0")
    conv1 = make_node("Conv", inputs=["c", "w1"], outputs=["d"], groups=1, name="conv1")
    graph = make_graph(
        [conv0, conv1],
        name="graph",
...
def test_single_node_match():
    ...
    pattern = SingleNodePattern("Conv").with_attr("groups", 1)
    nodes = list(pattern.match(graph))
    assert len(nodes) == 1
    for i in nodes:
        assert i.attribute[0].i == 1

    pattern1 = SingleNodePattern("Conv").with_attr("groups", 1)
    pattern2 = SingleNodePattern("Conv").with_attr("groups", 2)
