...
{% macro extrica__get_columns_in_relation(relation) -%}
  {%- set sql -%}
    describe {{ relation }}
  {%- endset -%}
...
{% macro extrica__list_relations_without_caching(relation) %}
...
{% macro extrica__reset_csv_table(model, full_refresh, old_relation, agate_table) %}
    {{ adapter.drop_relation(old_relation) }}
    {{ return(create_csv_table(model, agate_table)) }}
{% endmacro %}


{% macro extrica__create_csv_table(model, agate_table) %}
...
{% macro extrica__create_table_as(temporary, relation, sql) -%}
  {%- set _properties = config.get('properties') -%}
...
{% macro extrica__create_view_as(relation, sql) -%}
...
{%- macro extrica__get_drop_sql(relation) -%}
...
{% macro extrica__information_schema_name(database) -%}
  {%- if database -%}
...
{# On extrica, 'cascade' is not supported so we have to manually cascade. #}
{% macro extrica__drop_schema(relation) -%}
...
{% macro extrica__rename_relation(from_relation, to_relation) -%}
...
{% macro extrica__alter_relation_comment(relation, relation_comment) -%}
  comment on {{ relation.type }} {{ relation }} is '{{ relation_comment | replace("'", "''") }}';
{% endmacro %}


{% macro extrica__alter_column_comment(relation, column_dict) %}
...
{% macro extrica__list_schemas(database) -%}
...
{% macro extrica__check_schema_exists(information_schema, schema) -%}
...
{% macro extrica__get_binding_char() %}
...
{% macro extrica__alter_relation_add_remove_columns(relation, add_columns, remove_columns) %}
...
{% macro extrica__alter_column_type(relation, column_name, new_column_type) %}
  {#
