{% macro extrica__get_show_grant_sql(relation) -%}
    select
        grantee,
...
{%- macro extrica__get_grant_sql(relation, privilege, grantees) -%}
    grant {{ privilege }} on {{ relation }} to {{ adapter.quote(grantees[0]) }}
{%- endmacro %}

{%- macro extrica__support_multiple_grantees_per_dcl_statement() -%}
    {#
        -- This macro should return true or false depending on the answer to
        -- following question:
...
{% macro extrica__call_dcl_statements(dcl_statement_list) %}
