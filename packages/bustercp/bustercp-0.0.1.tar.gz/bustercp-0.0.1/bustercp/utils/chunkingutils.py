...
class ChunkingPipeline:
    ...
    def __init__(self, conf:ChunkinConfig):
        self.conf = conf

    def transform(self, documents) -> List[Chunk]:
        ...
        for doc_id, text in docs_new_dict.items():
            ...
            for i,t in enumerate(sentences):
                max_length = self.conf.chunk_max_length
...
def write_chunks(chunks: list[any], candidates: list[any], datasources_path: str, datasource: str):

    now = datetime.now().strftime("%Y-%m-%dT%H_%M_%S")
    chunk_dir_path = os.path.join(datasources_path, datasource, 'chunks')
    os.makedirs(chunk_dir_path, exist_ok=True)
    chunk_file_path = os.path.join(chunk_dir_path, 'chunks-%s.txt' % now)
    # buster input format: source | country | title | content  | link | year
    with open(chunk_file_path, 'w+', newline='', encoding='utf-8') as out_file:
        writer = csv.writer(out_file, delimiter='\t')
        writer.writerow(["source", "country", "title", "doc_id", "p_id", "content", "link", "year"])
        candidates_dict = {cand.id:cand for cand in candidates}

        for chunk in chunks:
            document = candidates_dict[chunk.doc_id]
            source = "%s" % datasource
            # title = "%s/%s/%d/%d" % (document.country, document.group, chunk.doc_id, chunk.p_id)

            country = iso_to_country_name(document.country)
            
            writer.writerow([source, country, document.title, chunk.doc_id, chunk.p_id, chunk.text, document.link, document.year])