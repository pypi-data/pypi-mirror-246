'''
### Aiv 的 ship 模块管理
* 此模块的 _aiv_ship_init()函数,在ship 模块中,必须是最先运行的代码(不一定是顺序排前面),
    相反,run应该写在api开头的函数声明后面。
* 本模块是 AivM 端的代表。负责初始化所有ship模块,并管理ship的Api函数列表、参数列表等
* Aiv 的ship模块,以api开头的函数,将自动注册为Api函数
* run()负责初始化应用的路径,日志工具等,必须优选运行，否则容易报 import错误,找不到依赖包
* 如果需要在进程间数据交换,使用进程间的内存共享技术。数据收、发经不同的途径。实现数据双向通讯。
* 根据AivC端用户的设置,匹配ship列表中的api函数,并把参数拼接和格式化,最后成功调用api函数。
* 这个AivM模块由 ship 模块使用,它连接AivC前端,AivC与js端(AivW)交互
* AivM 与 AivC 共用的工具函数,统一放在 aivtool.py 模块
'''
import sys,os,time
...
## 1、初始化Aiv Ship 的第一个函数=======================================================================
def run(shipName,shipId=None,callFun=None,logLevel=None) -> None:
    '''
        ### 注册Ship模块函数
        - param
            - name:     用户自定义的ship名称,如果此ship模块公开发布在Aiv平台,则需要在Aiv平台注册的name
                        Aiv平台将分配一个唯一的uid ,name与uid匹配使用
            - shipId:   Aiv平台分配小程序唯一的token
            - callfun:  程序初始化后,调用的函数
            - writeyaml:是否把Api参数等写入conf目录下yaml文件,适合开发期使用
        - desc
        * 此函数,必须是在用户空间中执行的第一行代码(但应该在api函数<以aiv开头>声明之后)
        * 当前模块下,所有以 api 开头的函数,都自动注册为aiv系统的api函数
            其它不用作为api函数输出的函数,可以用"_"开头,或用其它命名开头
        * 在Api函数内,必须调用 regParser() 注册api函数调用的参数,包含默认值、数据类型、可选值等等
    ...
    frame = inspect.stack()
    if frame[1][3] != '<module>':
        raise Exception('run() 函数必须在模块"__main__"下调用,并且不能包裹在函数中调用')
    ...
    from aiv.aivapp import _aiv_app_init #初始化全局路径、环境变量、logger
    _aiv_app_init(globalvar['__file__'],False,loglvl=logLevel)
    ...
    global aivShip
    aivShip = AivShip()
    aivShip.shipInfo.path = os.path.dirname(os.path.abspath(globalvar['__file__'])) #文件执行路径
    aivShip.shipInfo.loglevel = logLevel
    aivShip.shipInfo.shipName = shipName
    aivShip.shipInfo.shipId = shipId

    aivShip.regApi(globalvar)

    from aiv.aivmmap import AivShipMmap
    global aivShipMmap
    # print('AivShip收到的 taskMMAPName333 ==> {}'.format(aivShip.taskMMAPName))
    aivShipMmap = AivShipMmap(aivShip.shipInfo.shipName,aivShip.shipInfo.shipId,aivShip.shipInfo.taskMMAPName)
    # aivShipMmap.aivShip = aivShip
    aivShipMmap.onStartTask = aivShip.onStartTask

    if callFun is not None:
        callFun()  #调用用户设置的回调函数----------

    aivShip.initParam() # 加载 Api的参数,在这里,可以选择性加载 2023.11

    ''' 2023.11
        用线程检测进程 wcPid 是否退出
        用线程检测 wcPid 进程是否退出(不是用 asyncio协程,是用threading检测),如果wcPid进程退出,线程也跟着退出 
        原因是 asyncio的所有子协程都是在进程的主线程运行, 当Aiv平台的ship模块运行任务时,基本是上独占模式(死循环)
        这样如果用户中止任务,虽然ship模块也收到 TaskState.taskProExit 信号,但 aivShipMmap.run() 阻塞在运行任务上
        没办法响应  TaskState.taskProExit 信号, 但线程是可以的.因此,在 aivShipMmap.run()检测任务是否中止信号上,
        另外用线程 threading 建立一个独立于主线程的子线程,用于检测wcPid是否退出(这里是检测aivwc.py 的进程)。这样,
        当协程失灵时,threading的线程仍然可用,可以保证用户下达的中止指令可以执行。
    '''
    aivShipMmap.aivSys.createCheckPidThread([aivShip.shipInfo.ppid,aivShip.shipInfo.wcpid],aivShip.shipInfo.shipName)

    logger.debug('ship: 模块 {} 初始化完成！'.format(aivShip.shipInfo.shipName))
    # 协程函数
    async def main():
        # aivShipMmap的功能主要是检测是否有新任务(抢单),二是检测任务的状态,如果任务被前端取消,则马上退出进程
        # 另外,也要不断检测任务是否超时,超时就自动中止进程 2023.11
        asyncio.create_task(aivShipMmap.run())  
        while True:
...
class AivShipInfo:
    ...
    def getShipInfo(self):
        ...
        return {
            ...
            'modePath' : self.modelPath
        }

class AivShip:
    def __init__(self) -> None:
        self.shipInfo = AivShipInfo()
        # self.state = None
        # self.taskMMAPName = ''
        # self.shipId = ''
        # self.shipName = ''
        ...
        # self.loglevel = None
        # self.ppid = None #父进程的pid,根据父进程pid,如果它退出,自己也退出
        # self.wcpid = None # 同级子进程 wc的进程pid,如果在命令行参数中传入此值,则可以根据此值,同步退出进程 2023.11
        # self.task = None
        self.api = [] # 记录当前Ship模块所有的Api信息

        if len(sys.argv)>1:
            self.shipInfo.ppid = int(sys.argv[1]) #如果命令行第一个参数传应用的主进程pid过来
        if len(sys.argv)>2:
            self.shipInfo.taskMMAPName = sys.argv[2]
        if len(sys.argv)>3:
            self.shipInfo.wcpid = int(sys.argv[3])  # 同级子进程 wc的进程pid,如果在命令行参数中传入此值,则可以根据此值,同步退出进程

        # print('AivShip收到的 taskMMAPName ==> {}'.format(self.taskMMAPName))

    # async def run(self):
    #     from loguru import logger
    #     import asyncio
    #     logger.warning('asyncio已经开始运行!')
    #     # asyncio.create_task(self.setParam())
    #     while True:
        ...
        if self.api is not None:
            ...
            for aivApi in self.api:
                ...
                if aivApi['paramIn'] is None:
                    aivApi['fun']('param') #执行api的初始化函数, 传递 cmd = 'param' 指令进去
            ...
            if haveParam:
                from aiv.aivmmap import TaskState
                # self.state = TaskState.taskReady
                logger.debug('Ship模块 {} 载入完成!'.format(self.shipInfo.shipName))
                # break
    ...
    def onStartTask(self,task):
        ''' 2023.09
            ### 生成调用函数的参数
            * 前端返回服务器,服务器调用 ship 的参数,是一个dict列表,本函数用 parser.parse_args 把dict转换成 Namespace 命名空间
            * 此函数把多余的数据删除,只留下类似格式dict： {'参数名1': '参数值1','参数名2': '参数值2', ... } 
        '''
        from loguru import logger
        from aiv.aivtool import getFileInfo
        logger.debug('aivShip收到的启动任务参数是: task ==> {}'.format(task))

        # =====================================================================
        # ======= 调api的参数准备 ============================================== 1
        # 检测是否有输入参数 'paramIn'
        if task['paramIn'] is None:
            logger.error('任务的 ParamIn 为空！')
            return

        # 检查是否有调用的 api 函数名
        apiName = task.get('apiName',None)
        if apiName is None:
            logger.error('任务没有指定要调用的Api函数名! ')

        # 根据任务给定的api函数名,找出ship对应的真实函数 (通过 apiName)
        apiFun = None
        parser = None
        for api in self.api:
            if api['name'].strip().lower() == apiName.strip().lower():
        ...
        if apiFun is None:
            logger.error('还没初始化完成! Api函数 == {} 为空!'.format(apiFun))
            return
        # if parser is  None:
        #     logger.error('还没初始化完成! parser == {} 为空!'.format(parser))
        #     return

        args = None
        if parser is not None:
            logger.debug('当前 api 函数 {} 运行时不依赖 parser 对象, 直接传 dict 参数!'.format(apiName))
            ...
            for param in task['paramIn']:
                if param['bsonType'] == 'file':
                    file = param['files'][0] #暂时取第一项内容
                    filename = aivShipMmap.getFilePath(file)
                    params['--'+param['name']] = filename

                elif param['bsonType'] == 'bool':
                    if param['value'] is None:
                        param['value'] = False    

                    params['--'+param['name']] = param['value']
                elif param['bsonType'] == 'string':
                    params['--'+param['name']] = param['value']
                elif param['bsonType'] == 'int':
                    params['--'+param['name']] = param['value']
                elif param['bsonType'] == 'float':
                    params['--'+param['name']] = param['value']
            ...
            files = []
            for param in paramOut: # 把输出参数与变成 args的一个参数(cmd 命令行下也是用 --xxxx xxxx 传给ship模块) 2023.12
                if param['bsonType'] == 'file':  
                    #根据当前时间生成一个文件名
                    file = str(int(time.time()))+'.mp4' 
                    fileName =os.path.join(aivShipMmap.aivSys.aivOutDir,file)
            ...
            # params['--outfile'] = os.path.join(aivShipMmap.aivSys.aivTempDir,'outfile.mp4')

            # logger.warning('生成的参数列表是：{}'.format(params)) 
        ...
        try:
            # 运行api函数----------------------------------------------
            apiFun('run',args)
            # logger.warning('ship: 999 args参数的类型是: {}, 内容是：{}'.format(type(args)))
            # logger.warning('apiFun finally 运行完成：{}'.format(task))
            # --------------------------------------------------------

            # 根据生成的文件,生成文件信息---------------------------------
            paramOut = task['paramOut']
            for param in paramOut:
                if (param['bsonType'] == 'file') and (param['files'] is not None):  
                    for i in range(len(param['files'])):
                        file = param['files'][i]
                        # 根据文件名生成文件信息（如果文件没有生成在磁盘,则部分信息为空<如文件大小、md5码等>)
                        param['files'][i] = getFileInfo(file['path']) #用函数getFileInfo 生成文件的信息 更新原来的文件信息
                        # print('文件信息是:',fileInfo)
            # ---------------------------------------------------------

            aivShipMmap.setTaskResultCode(TaskResultCode.taskOk) # 设置为OK状态
        except Exception as e:
            logger.warning('ship: 模块{} 的Api函数 {} 执行出现错误: {}'.format(self.shipInfo.shipName,api['name'],e))
            aivShipMmap.setTaskResultCode(TaskResultCode.taskSvr) # 设置为服务器出错状态
        finally:
            # self.task = None
            aivShipMmap.endTask()
    ...
    def regApi(self,glob:dict): #利用 globals() 读取指定模块的所有函数名
        '''
        ### 注册ship模块
        ...
        * 导入的函数也可以改名 : from xxx.xxx import fun01 as api_fun01
        ...
        # 模块初始化时,系统默认初始化一些ship的信息。并保存在 aivm.yaml 文件中,用户可以修改。


        logger.aiv('Ship模块所有参数如下 (包含模块内所有函数、方法名称和内存地址): \n{}'.format(glob))
        #--------------------------------------------------------------------------
        #循环模块的所有函数,把aiv 开头的函数自动导入----------------------------------
        for fun_name in lst:
            
            if fun_name.startswith('api') : #找到以aiv 开头的函数才能导出
                    fun = glob[fun_name]
                    
                    if (type(fun) == types.FunctionType) or (type(fun) == types.MethodType): #判断是方法（而不是属性)
                        # apidict 只记录静态的信息
                        apidict = {'ship':self.shipInfo.shipName, 'name':fun_name,'fun':fun,'paramIn':None,'parser':None}
                        self.api.append(apidict)
                        fun('init') #初始化Api函数
                    

        logger.info('\n成功注册Ship模块:[ {} ]'.format(self.shipInfo.shipName))

        logger.debug('\n_aiv_ship_init() 成功注册ship模块:{} \n它的Api函数列表是: \n{}'.format(self.shipInfo.shipName,self.api))


## 2、注册Api的调用参数 (必须由ship模块在api函数内调用 )============================================================================
def regParser(parser :argparse.ArgumentParser,modify=None,
                    dev=None):
    '''
        ### 注册命令行参数
        * 参数
        ...
        * Api函数默认有两个参数 (cmd,args=None) ,AivM 调用 Api函数时,有三种情况：
             cmd='init' 时, 只在ship初始化时调用一次
    ...
    if parser is None: #ship初始化时,也可以传空的 parser进来 (这意味着调用此 api 函数不依赖 parser) 2023.12
        return
    ...
    def _change_type(param_list,fileparam=None,devfield=None):
        ...
        * 转换成 tuple：
        ...
        for param in param_list:
            ...
            #根据 dest 的字段名和用户传入的参数组: devfield 修改开发者字段---------------------
            # if devfield is not None and  param['dest'] in devfield:
            #     param['dev'] = True
    ...
    if aivShip is None or aivShip.api is None:
        return
    ...
    if not fun_name.startswith('api'):
        logger.error('_aiv_reg_parser() 必须在以 api 开头的函数中调用!' )
        return
    
    #根据函数名,找出对应的运行时信息,更新它
    for api_run in aivShip.api:
