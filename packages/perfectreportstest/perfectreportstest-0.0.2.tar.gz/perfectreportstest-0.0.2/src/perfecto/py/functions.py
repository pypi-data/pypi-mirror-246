...
def topFailedReasons(failed_df):
    # top failed reasons
    failed = failed_df[~failed_df.name.isin(["Interactive session"])]
    topfailedTCNames = (
...
def df_formatter(df, jobName, jobNumber):
    ...
    if "Duration" not in df.columns:
        df["Duration"] = pandas.to_datetime(df["endTime"], format='mixed') - pandas.to_datetime(df["startTime"], format='mixed')
