...
def validate_kubectl_context(kube_ctrl: KubectlController, cloud: str) -> None:
    ...
    if not identified_cloud_context == cloud:
        logger.error(f"Kubectl context({identified_cloud_context}) doesn't match the one on CAST ({cloud})")
        raise RuntimeError(f"Kubectl context({identified_cloud_context}) doesn't match the one on CAST ({cloud})")
