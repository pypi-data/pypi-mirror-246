...
Last Update: 17/12 (December)/2023 <GMT+7>                              
...
Args:
  ...
  if style == 'fg':
    ...
    raise OptionNotFoundError(f"'{style}' The style type is not found. Only 'fg' (foreground) or 'bg' (background)")
...
Args:
  ...
  if (typefmt == 'table') or (typefmt == 'table_fancy-grid'):
    ...
    header = borders[1] + borders[1].join(f" {header.center(width)} " for header, width in zip(headers, column_widths)) + borders[1]+'\n'
    ...
    raise OptionNotFoundError(f"'{typefmt}' The type is not found.")

  return table_main

# -- func: make progress bar ascii terminal | yield [str] -- #
def progress_bar(typefmt='line', width=50, max=100, bar_progress="#", bar_space=".", bar_box="[]", text="Hello World! - asciiTUI ", isdone=" ")->str:
  """
yield: `str`
...
Args:
  `typefmt`      : Type of progress model (`line` or `circle`)
  ...
  """
  typefmt, bar_progress, bar_space, bar_box, text, isdone = map(str, [typefmt, bar_progress, bar_space, bar_box, text, isdone])
  if not (isinstance(width, int) and isinstance(max, int)):
    raise TypeError(f"width, max is int not width:{type(width).__name__}, max:{type(max).__name__}")
  if len(bar_box) != 2:
    raise ValueError(f"bar_box need 2 characters not {len(bar_box)}")

  if 'line' in typefmt.lower():
    total = 100
    progress = 0
    bar_start = bar_box[0]
    bar_end = bar_box[-1]
  ...
  elif 'circle' in typefmt.lower():
    circle_keys = ['|', '/', '-', '\\']
    count = 0
    while max >= 0:
      yield text + circle_keys[int(count)]
      count += 0.5
      max -= 1
      if count > 3:
        count = 0
    ...
    raise OptionNotFoundError(f"'{typefmt.lower()}' The type is not found.")

# -- func: make justify func for text | return [str] -- #
def justify(content:str, width:int, make='center', height=50, fill=' ', align=False)->str:
  """
return: `str`
    ...
    if make.lower() == 'center':
      ...
      if len(remove_ansi(coline)) <= width:
        ...
        for item in content_pieces:
          contents += item + '\n'
        extra_space = width - len(remove_ansi(content_end))
        contents += fill * (extra_space) + content_end

    else:
      raise OptionNotFoundError(f"'{make.lower()}' The type is not found.")
...
Args:
  ...
  `param_char`     : Parameter character. To separate parameters
    ...
    if (len(esc_char) != 1) or (len(quotes_char) != 1) or (len(ln_char) != 1) or (len(backslash_char) != 1) or (len(param_char) != 1):
