...
class CanWriteTest(TestCase):
    ...
    def test_can_write_timer(self):
        ...
        sio.emit(
            "set_can_write",
            {"manipulator_id": 1, "can_write": True, "hours": 0.00027777777},
        )
        sio.emit(
            "set_can_write",
            {"manipulator_id": 2, "can_write": True, "hours": 0.00027777777},
        )
        ...
        while self.mock.call_count != 4:
            pass
        self.assertEqual(
            self.mock.call_args.args[0]["error"], "Manipulator movement canceled"
        )
