...
cdef object numa_nodemask_to_set(nodemask_t *p_mask):
    """
    ...
    for i in range(0, get_max_node()+1):
        if nodemask_isset(p_mask, i):
            result.add(i)

    return result

cdef set_to_numa_nodemask(object mask, nodemask_t *p_result):
    """
    Convert Python set to NUMA nodemask.
    """
    cdef int i

    nodemask_zero(p_result)

    for i in range(0, get_max_node()+1):
        if i in mask:
            nodemask_set(p_result, i)
...
def get_interleave_mask():
    ...
    @rtype: C{set}
    """
    cdef nodemask_t mask = numa_get_interleave_mask()
    return numa_nodemask_to_set(&mask)
...
def get_membind():
    ...
    @rtype: C{set}
    """
    cdef nodemask_t mask = numa_get_membind()
...
def get_run_on_node_mask():
    ...
    @rtype: C{set}
    """
    cdef nodemask_t mask = numa_get_run_node_mask()
