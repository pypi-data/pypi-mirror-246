...
def run(sat_folder=None, is_file=None, AC=None, out_file=None, to_log = True):
    ...
    for sat_i, sat_file in enumerate(sat_files): 
        ...
        print('{}/{}'.format(sat_i, len(sat_files)))
        ...
        for ind, row in enumerate(is_matches):
            ...
            for ws in window_size: 
                ...
                # Column names 
                columns_mean    = ['mean_'+item+'_ws'+str(ws) for item in sat_data['bands']]
                columns_median  = ['median_'+item+'_ws'+str(ws) for item in sat_data['bands']]
                ...
                df_is.loc[row, columns_mean]    = rhow_data_mean
                df_is.loc[row, columns_median]  = rhow_data_median
    ...
    # Output dataframe 
    df_is.to_csv(out_file, index=False)
