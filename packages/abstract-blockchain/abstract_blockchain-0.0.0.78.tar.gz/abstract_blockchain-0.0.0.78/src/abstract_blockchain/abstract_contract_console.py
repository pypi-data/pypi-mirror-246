    ...
    try:
        ...
        if len(output_keys)>0:
            if len(output_keys) == 1:
                window_mgr.update_value(key=output_keys[0], value=str(get_type(output_keys[0].split('_')[-1][:-1],result,output=True)))
            if len(output_keys)>1:
                ...
                for each_output_key in output_keys:
                    ...
                    window_mgr.update_value(key=each_output_key, value=str(get_type(each_output_key.split('_')[-1][:-1],result[num],output=True)))
...
def contract_win_while(event,values,window):
...
def win_while(event,values,window):
    rpc_win_while(event,values,window)
    ...
    if event == "-GET_ABI-":
        ...
        function_js = {}
        if new_bridge_global["abi_manager"].abi:
            for each in new_bridge_global["abi_manager"].abi:
                if each["type"] == "function":
        ...
        new_window = window_mgr.add_window(title=f"{window_mgr.get_values(window=new_bridge_global['main_window'])['-NETWORK_NAME-']} contract {contract_address}", layout=all_layouts, event_handlers=[contract_win_while])
    ...
    if event == "-DERIVE_RPC-":
        ...
        if isinstance(rpc,dict):
            for key,value in rpc.items():
                window_mgr.update_value(key=get_rpc_js()[key],value=value)
    if event == "-ASSOCIATE_ACCOUNT-":
        new_bridge_global["account_manager"] = ACCTBridge(env_key=values["-ACCOUNT_ENV_KEY-"],rpc=get_rpc())
        window_mgr.update_value(key="-ACCOUNT_ADDRESS-",value=new_bridge_global["account_manager"].account_address)
def _parse_io(io_data,function_name:str, is_output=False):
    ...
    for i, io_type in enumerate(io_data):
        ...
        if is_output:
            layout.append([make_component("Text", args={"text": text}),get_push(), make_component(component_type, args={"size": size,"key": key, "disabled": True})])
        else:
            layout.append([make_component("Text", args={"text": text}),get_push(),  make_component(component_type, args={"size": size, "key": key})])
    return layout
def get_account_layout():
    frame_layout = [[make_component("Text", args={"text": "ENV Key:"}),get_push(),make_component("Input",args={"key":"-ACCOUNT_ENV_KEY-"})],
                    [make_component("Text", args={"text": "Account Address"}),get_push(),make_component("Input",args={"default_text":"No Address Found","key":"-ACCOUNT_ADDRESS-","disabled":True})],
                    [make_component("Button", args={"button_text":"Associate Account","enable_events":True,"key":"-ASSOCIATE_ACCOUNT-"})]]
    return [make_component("Frame", "Account", args={"layout":frame_layout})]
# If you need the ABI helper function in the new script, define it here 
def get_abi():
    frame_layout = [make_component("Input",args={"key":"-CONTRACT_ADDRESS-"}),
                    make_component("Button", args={"button_text":"GET ABI","enable_events":True,"key":"-GET_ABI-"}),
                    make_component("Button", args={"button_text":"DERIVE RPC","enable_events":True,"key":"-DERIVE_RPC-"})]
...
def abstract_contract_console_main(rpc_list:list=None):
    ...
    rpc_layout= [[make_component("Frame", "RPC_LAY",layout=get_rpc_layout(rpc_list=new_bridge_global["rpc_list"]))]]
    # Construct the final layout
    new_layout = [[get_account_layout()],get_abi(),rpc_layout]
    # Create and run the window
    
    new_bridge_global["main_window"] = window_mgr.add_window(title="New Blockchain Console", layout=[new_layout], event_handlers=[win_while])
    window_mgr.while_window(window=new_bridge_global["main_window"])
