...
def show_cmd(ctx, **kwargs):
    ...
    nodes, dependencies = mu.get_nodes_and_dependencies(manifest=ctx.obj['manifest'])
...
def merge_cmd(ctx, **kwargs):
    ...
    nodes, dependencies = mu.get_nodes_and_dependencies(manifest=ctx.obj['manifest'], default_seed=existing_node_ids[0] if ctx.params['default_seed'] is None else ctx.params['default_seed'])
...
def delete_cmd(ctx, **kwargs):
    ...
    nodes, _ = mu.get_nodes_and_dependencies(manifest=ctx.obj['manifest'])
