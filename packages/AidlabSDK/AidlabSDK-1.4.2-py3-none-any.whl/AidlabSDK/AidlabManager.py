...
class AidlabManager:
    ...
    def __init__(self, delegate, aidlab_address):
        ...
        if 'linux' in sys.platform:
            if os.uname()[4][:3] == 'arm':
                self.lib = cdll.LoadLibrary(cwd+"/aidlabsdk-linux-gnueabihf.so") # arm(raspberry pi) version
            else:
                self.lib = cdll.LoadLibrary(cwd+"/aidlabsdk-linux-gnu.so")
        elif 'win32' in sys.platform:
            os.add_dll_directory(cwd+"/libmingw64/")
    ...
    def did_receive_firmware_revision(self, firmware_revision):
        len_with_null_terminator = len(firmware_revision) + 1  # +1 for null terminator
        firmware_revision_utf8 = firmware_revision.encode('utf-8')

        # Create c_uint8 array from encoded string
        firmware_revision_array = (c_uint8 * len_with_null_terminator)(*firmware_revision_utf8)

        self.lib.setFirmwareRevision(firmware_revision_array, len_with_null_terminator, self.aidlab_sdk_ptr)
    ...
    def did_receive_error_callback(self, context, log_text):
        try:
            logger.info(" [DLL] " + log_text.decode("utf-8"))
        except UnicodeDecodeError:
            logger.info(" [DLL] " + str(log_text))
