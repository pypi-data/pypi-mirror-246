...
from jf_ingest.config import IngestionConfig, JiraConfig, JiraAuthMethod
...
def load_and_push_jira_to_s3(ingest_config: IngestionConfig):
    ...
    jira_atlas_connect_connection = (
        get_jira_connection(
            config=jira_config, auth_method=JiraAuthMethod.AtlassianConnect
        )
    ...
    if jira_config.feature_flags.get(
    ...
    if not jira_config.only_issues:
        ...
        ingest_io_helper.write_json_data_to_local(
            object_name=JiraObject.JiraBoards.value, subdirectory=SubDirectory.JIRA, json_data=boards
        )

        ingest_io_helper.write_json_data_to_local(
            object_name=JiraObject.JiraSprints.value, subdirectory=SubDirectory.JIRA, json_data=sprints
        )

        ingest_io_helper.write_json_data_to_local(
            object_name=JiraObject.JiraBoardSprintLinks.value, subdirectory=SubDirectory.JIRA, json_data=links
        )
