...
from jf_ingest.jf_jira.auth import get_jira_connection, JiraConfig, JiraAuthMethod
...
class IngestionHealthCheckResult:
    ...
    def __post_init__(self):
        ...
        """
        jira_successful = self.jira_connection_healthcheck.successful if self.jira_connection_healthcheck else False

        # If we have a non-empty git healthcheck result list, ensure all its members are successful.
        git_successful = bool(self.git_connection_healthcheck) and all([x.successful for x in self.git_connection_healthcheck])
    ...
    def from_json(cls, body: str):
        ...
        healthcheck_dict['jira_connection_healthcheck'] = JiraConnectionHealthCheckResult.from_dict(healthcheck_dict['jira_connection_healthcheck']) if healthcheck_dict['jira_connection_healthcheck'] else None
...
def validate_jira(config: JiraIngestionConfig) -> JiraConnectionHealthCheckResult:
    """
    ...
    """
    auth_config: JiraConfig = config.jira_config
