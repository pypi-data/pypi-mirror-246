...
def _generate_spoofed_user_data(
    user_count: int, char: str = "default", gdpr_active: bool = True
):
    return [
        {
...
@contextmanager
def _spoof_gdpr_server(
    page_size: int, total_users: int, server_page_size: int = DEFAULT_PAGE_SIZE
):
...
def test_search_users_data_less_than_page_size():
    ...
    with _mock_search_by_user_api_that_can_page(
        test_data=test_data, page_size=DEFAULT_PAGE_SIZE
    ):
        users = _search_users_by_letter(
            get_jira_mock_connection(), None, page_size=DEFAULT_PAGE_SIZE
        )
...
def test_search_users_data_exact_page_size():
    test_data = {
        "a": _generate_spoofed_user_data(
            char="a", user_count=DEFAULT_PAGE_SIZE, gdpr_active=False
        )
    }
    with _mock_search_by_user_api_that_can_page(
        test_data=test_data, page_size=DEFAULT_PAGE_SIZE
...
def test_search_users_data_all_test_cases():
    test_data = {
        ...
        "f": _generate_spoofed_user_data(
            char="f", user_count=DEFAULT_PAGE_SIZE, gdpr_active=False
        ),
...
def test_search_users_data_recursive_case():
    test_data = {
        ...
        "b": _generate_spoofed_user_data(
            char="b", user_count=DEFAULT_PAGE_SIZE, gdpr_active=False
        ),
        ...
        # Test some numbers
        "4": _generate_spoofed_user_data(
            char="4", user_count=DEFAULT_PAGE_SIZE, gdpr_active=False
        ),
        "42": _generate_spoofed_user_data(
            char="42", user_count=DEFAULT_PAGE_SIZE, gdpr_active=False
        ),
        "426": _generate_spoofed_user_data(
            char="426", user_count=DEFAULT_PAGE_SIZE // 2, gdpr_active=False,
        ),
        "429": _generate_spoofed_user_data(
            char="4267", user_count=DEFAULT_PAGE_SIZE // 3, gdpr_active=False,
        ),
...
def test_get_all_users_for_gdpr_active_instance():
    ...
    with _spoof_gdpr_server(
        page_size=page_size, total_users=total_users
...
def test_augment_with_email():
    ...
    with requests_mock.Mocker() as m:
        ...
        users = augment_jira_user_with_email(get_jira_mock_connection(), jira_users)
        for user in users:
