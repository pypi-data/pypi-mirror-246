...
class IngestIOHelper:
    ...
    def get_signed_urls(self, filenames: list[str], timestamp: str) -> dict[str, dict]:
        ...
        payload = {'files': [f'{name}.gz' for name in filenames]} # the files will be gzipped before uploading
    ...
    def upload_file_to_s3(self, file_obj: bytes, s3_obj_path: str, signed_url: dict, dry_run=False) -> None:
        """
    ...
    def wrapped_upload_file_to_s3(self, filepath: str, s3_obj_path: str, signed_url: dict) -> bool:
        ...
        try:
            ...
            if _is_auth_error(str(e).lower()):
                logger.error(f"Authentication error encountered when uploading {filepath} to S3! Error: \n{e} "
    ...
    def upload_local_files_to_s3_threaded(self, filenames: list[str], prefix: str = "") -> bool:
        ...
        signed_urls = self.get_signed_urls(filenames_for_s3, self.ingest_config.timestamp)
        threads = [
            threading.Thread(
                target=self.wrapped_upload_file_to_s3, args=[
        ...
        if any(thread_exceptions):
            ...
            for exception in thread_exceptions:
                logger.error(f"A thread exception was encountered when uploading files to S3: {exception}")
            logger.error(
