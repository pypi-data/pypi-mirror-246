...
def worklogs_context_manager(existing_ids: list[int] = []):
    with requests_mock.Mocker() as m:
        ...
        worklog_updated_data_str = get_fixture_file_data(
            "api_responses/worklog_updated.json"
        )
        _register_jira_uri(
        ...
        worklog_filtered = [
            wl
            for wl in json.loads(worklog_list_data_str)
            if int(wl["issueId"]) in existing_ids
        ]
        _register_jira_uri(
            m,
        ...
        # Get deleted
        worklog_deleted_data_str = get_fixture_file_data(
            "api_responses/worklog_deleted.json"
        )
        _register_jira_uri(
