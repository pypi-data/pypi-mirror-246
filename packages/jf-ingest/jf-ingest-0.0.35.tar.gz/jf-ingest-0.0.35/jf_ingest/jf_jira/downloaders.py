...
from jf_ingest.config import IngestionConfig, JiraConfig, IssueMetadata
...
def download_fields(
    ...
    logger.info("Done downloading Jira Fields!")
    return fields


def _project_is_accessible(
    jira_connection: JIRA, project_id: str, project_key: str
) -> bool:
...
def download_resolutions(jira_connection: JIRA) -> list[dict]:
    ...
    logger.info("Done downloading Jira Resolutions!")
    return result
...
def download_issuetypes(jira_connection: JIRA, project_ids: list[str],) -> list[dict]:
    ...
    """
    logger.info("Downloading IssueTypes...",)
    result = []
    for it in retry_for_429s(jira_connection.issue_types):
        if "scope" in it.raw and it.raw["scope"]["type"] == "PROJECT":
            ...
            result.append(it.raw)
    logger.info("Done downloading IssueTypes!")
    return result
...
def download_issuelinktypes(jira_connection: JIRA) -> list[dict]:
    ...
    logger.info("Done downloading IssueLinkTypes!")
    return result
...
def download_priorities(jira_connection: JIRA) -> list[dict]:
    ...
    logger.info("Done downloading Jira Priorities!")
    return result
...
def download_issues(
    ...
    deleted_issue_ids = get_ids_from_difference_of_issue_metadata(
        source=jellyfish_issue_metadata, dest=issue_metadata_from_jira,
    )
    ...
    missing_issue_ids = get_ids_from_difference_of_issue_metadata(
        source=issue_metadata_from_jira, dest=jellyfish_issue_metadata,
    )
    ...
    issue_ids_to_download = sorted(
        [*missing_issue_ids, *out_of_date_issue_ids, *issue_ids_to_redownload,], key=int
    )
...
def generate_project_pull_from_jql(
    project_key: str, earliest_issue_dt: datetime
) -> str:
...
def get_jira_search_batch_size(
    jira_connection: JIRA, optimistic_batch_size: int = 1000
) -> int:
            ...
            if "histories" in changelog:
                ...
                for history in histories:
                    ...
                    for item in history.get("items", []):
                        ...
                        if (
                            include_fields
                            and item.get(item_field_identifier) not in include_fields
                        ):
                            continue
...
def pull_jira_issues_by_jira_ids(
    ...
    with ThreadPoolWithTqdm(
        ...
        for issue_batch in batch_iterable(
            sorted(jira_ids, key=int), batch_size=batch_size
        ):
...
def pull_all_jira_issues_by_date(
    ...
    total_expected_issues: int = sum(
        [count for count in project_issue_count_map.values()]
    )
...
def download_all_issue_metadata(
    ...
    issue_meta_data_ids = set(
        [issue_metadata.id for issue_metadata in issue_metadata_by_date_set]
    )
    ...
    missing_parent_id_set: Set[str] = set(
        [
            ...
            if issue_metadata.parent_id
            and issue_metadata.parent_id not in issue_meta_data_ids
        ]
    )
...
def get_out_of_date_issue_ids(
    ...
    jira_ids_to_updated_date: dict[str, datetime] = {
        issue_metadata.id: issue_metadata.updated
        for issue_metadata in issue_metadata_from_jira
    }
...
def get_ids_from_difference_of_issue_metadata(
    source: list[IssueMetadata], dest: list[IssueMetadata],
) -> set[str]:
...
def detect_issues_needing_re_download(
    ...
    jf_issue_metadata_lookup = {
        issue_metadata.id: issue_metadata
        for issue_metadata in issue_metadata_from_jellyfish
    }
    ...
    for issue_id, jf_issue_metadata in jf_issue_metadata_lookup.items():
        ...
        if jf_issue_metadata.epic_link_field_issue_key:
            issues_by_epic_link_field_issue_key[epic_link_field_issue_key].append(
                issue_id
            )
...
def download_worklogs(
    ...
    logger.info("Done downloading Worklogs!")
...
def download_statuses(jira_connection: JIRA) -> list[dict]:
    ...
    logger.info("Done downloading Jira Statuses!")
    return result
