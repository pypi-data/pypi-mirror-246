...
from jf_ingest.config import IngestionConfig, JiraConfig
from jf_ingest.utils import ReauthSession, retry_session
...
class IngestIOHelper:
    ...
    def _write_file(self, json_data: dict | list[dict], full_file_path: str):
        ...
        with open(full_file_path, "wb") as f:
            f.write(
                json.dumps(json_data, indent=2, cls=StrDefaultEncoder).encode("utf-8")
            )
            logger.debug(
    ...
    def get_signed_urls(self, filenames: list[str], timestamp: str) -> dict[str, dict]:
        """
    ...
    def wrapped_upload_file_to_s3(
        self, filepath: str, s3_obj_path: str, signed_url: dict
    ) -> bool:
        """
        ...
        try:
            ...
            if _is_auth_error(str(e).lower()):
                ...
                logger.error(
                    f"Exception encountered when uploading {filepath} to S3! Error: {e}"
                )
            logger.debug(traceback.format_exc())

            return False

    def upload_local_files_to_s3_threaded(
        self, filenames: list[str], prefix: str = ""
    ) -> bool:
        """
        ...
        signed_urls = self.get_signed_urls(
            filenames_for_s3, self.ingest_config.timestamp
        )
        threads = [
            threading.Thread(
        ...
        logger.info(
            f"Uploading {len(filenames)} files to S3 using {len(threads)} threads..."
        )
    ...
    def upload_all_directory_output_to_s3(self, directory: str) -> bool:
        ...
        success = False
        try:
            _, directories, filenames = next(
                os.walk(f"{self.local_file_path}/{directory}/")
            )
            filenames = [
                f"{self.local_file_path}/{directory}/{filename}"
                for filename in filenames
            ]
            success = self.upload_local_files_to_s3_threaded(
                filenames, prefix=f"{directory}/"
            )
        except StopIteration:
            logger.warning(f"No {directory} output to upload, skipping.")
        if success:
            logger.info(
                f"Successfully compressed and uploaded {directory} output to S3"
            )
    ...
    def upload_all_locals_to_s3(self) -> None:
        ...
        success_log = self.upload_local_files_to_s3_threaded(
            [logging_helper.LOG_FILE_NAME]
        )
        if success_log:
            logger.info("✓")
        else:
        ...
        if success_jira and success_git and success_remaining:
            done_file_dict = self.get_signed_urls([".done"])[".done"]
            self.upload_file_to_s3(
                b".done", done_file_dict["s3_path"], done_file_dict["url"]
            )
