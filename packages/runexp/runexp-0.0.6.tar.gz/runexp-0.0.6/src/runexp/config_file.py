...
def run_no_sweep(
    ...
    job_out_dir = state.valid_job_out_dir(state.no_dry_run, name, now.isoformat())
    if not state.no_dry_run:
        # save a copy for consistency with the sweep run
        config_copy_path = job_out_dir / pathlib.Path(config_path).name
...
def run_sweep(
    ...
    if not state.use_slurm:
        if state.no_dry_run:
            run_proc.run(fun, config_list, max_concurrency=state.max_concurrent_proc)
        else:
            run = DryRun()
    ...
    config_path_ = job_out_dir / pathlib.Path(config_path).name
    config_prefix = config_path_.stem
    ...
    key = "--runexp-slurm-array-index"
    command_list[path_idx] = str(config_path_.with_stem(config_prefix + key))
    command = shlex.join(command_list)
    # it's just an integer so no need to quote the argument
    command = command.replace(key, "-${SLURM_ARRAY_TASK_ID}")

    concurrent_limit = state.max_concurrent_proc
    if concurrent_limit is None:
        concurrent_limit = len(config_list)
    slurm_utils.run_from_template(
        ...
        state.slurm_args,
        sbatch_options=[
            "#SBATCH --array=1-{:d}%{:d}".format(len(config_list), concurrent_limit)
        ],
    )
...
def _runexp_body(fun: Fun, dtype: type[TypedDictLike], args: argparse.Namespace):
    ...
    # find sweep options
    sweep_groups: list[dict[str, typing.Any]] = config_data.get("sweep", None) or []
