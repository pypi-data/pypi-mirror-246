...
class SummaryYAML:
    ...
    def get_entities_hashes_in_current_file(self):
        if not self.current_file:
            raise ValueError("Current file is not set.")
        return [{'name': e['name'], 'entity_hash': e['entity_hash']} for e in self.current_file.get('entities', [])]
    ...
    def get_entity_code(self, reduced=True, reduce_empty_lines=False):
        ...
        return remove_empty_lines(entity_code) if reduce_empty_lines else entity_code
    ...
    def get_code_from_path(self, start_line=None, code=None, show_nested=True):
        if start_line:
            ...
            if not code:
                ...
                for entity_info in entities_lines:
                    self.set_bookmark(entity_info)
                    code += self.get_entity_code(reduced=not show_nested)
