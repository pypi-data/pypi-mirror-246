...
class PostCMD(install):
        ...
        def  run(self):
            ...
            # pypi.write_file("cmd_html","setupPIP")   ### "git config --global file.{name}
            # SS= pypi.read_file("cmd_html.setupPIP")
            # print("@ SS @",SS)
            # install.run(self) ################ 加上這個 看看???
            
            print("#"*60, "安裝前:B ","#"*60 )
            # install.run(self) ############################################
            pypi.Var.run(self,install,Var.nameA) ## del[cmd_pop] --->>> 版本
            print("#"*60, "安裝後:A ","#"*60 )
