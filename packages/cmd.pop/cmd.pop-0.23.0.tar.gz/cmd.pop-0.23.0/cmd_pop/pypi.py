    ...
    ########################
        ...
        if len(NN)==2:
            ...
            if ".git" in os.listdir():
                ...
                #         print(f"檔案已成功複製: {source_path} 到 {destination_path}")
                #     except FileNotFoundError:
                #         print(f"找不到檔案: {source_path}")
                #     except PermissionError:
                #         print(f"無法複製檔案，權限不足: {source_path}")
                ...
                text = os.popen("git config user.home").read().strip()
                os.system(f"pip setup.py {text} 0.0.1")
                del text
