...
class Var:
    ...
    def  clear(cls):
            ...
            ################################################################################
    @classmethod
    def run(cls,  this="self" ,nameA="cmd.pop" ):
...
if len(sys.argv)==3:
    ...
    if   sys.argv[1]=="name":
        ...
        os._exit(0)
    elif   sys.argv[1]=="moon" and sys.argv[2]!="get":
        import os
        name=str(sys.argv[2])
        ...
        if os.name=="nt": #### 在 win 11 使用
            ##########################################
            name = sys.argv[2]
            value = os.popen(f"pip setup.py cmd.pop  {name} && pip install cmd.pop").read().strip()
            ##########################################
            print(f"## [save-cmd.pop]","##"*60,f"\n\n\n{value}") 
            os._exit(0)
