...
class StrangeworksBinaryQuadraticModelHitachiDictConverter(StrangeworksConverter):
    ...
    def convert(self) -> tuple[HitachiModelList, dict[Any, Any]]:
        ...
        def get_target_list_from_bqm(bqm):
            # factor of 4 in quadratic term and 2 in linear term:
            # ising variables {-1/2, 1/2} - target variables {-1, 1}
            out_list = [
                [*k[0], *k[1], 4 * v] for k, v in bqm.quadratic.items()
            ]  # should this include `if quadratic[k] != 0`?
            out_list += [[*k, *k, 2 * v] for k, v in bqm.linear.items() if v != 0]
            return out_list
        ...
        # May need to play around with chain strength
        target_bqm = embed_bqm(
            bqm, embedding, target_graph, chain_strength=2 * np.max([np.abs(v) for v in list(bqm.linear.values())])
        )

        # Convert to Ising
        target_h, target_J, _ = target_bqm.to_ising()

        # Convert to Hitachi list
        ising_bqm = BinaryQuadraticModel(target_h, target_J, vartype="BINARY")
        target_list = get_target_list_from_bqm(ising_bqm)
