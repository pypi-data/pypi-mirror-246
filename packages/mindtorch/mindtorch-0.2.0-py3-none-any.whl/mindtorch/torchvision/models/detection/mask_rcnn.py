...
model_urls = {
    'maskrcnn_resnet50_fpn_coco':
        model_path_name + 'maskrcnn_resnet50_fpn_coco-bf2d0c1e.pth',
}
...
class MaskRCNNHeads(nn.Sequential):
    ...
    def forward(self, x):
        if x.numel() == 0:
            Hout = (x.shape[2] + 2 * self.dilation - self.dilation * (3 - 1) - 1) / 1 + 1
            Wout = (x.shape[3] + 2 * self.dilation - self.dilation * (3 - 1) - 1) / 1 + 1
            x = torch.empty([x.shape[0], self.layer_features, Hout, Wout])
        else:
            x = super().forward(x)
        return x
...
class MaskRCNNPredictor(nn.Sequential):
    ...
    # TODO: Conv2d and ConvTranspose2d currently not support null input with shape (0, a, b, c),
    # manually construct empty tensor as return value
    def forward(self, x):
        if x.numel() == 0:
            output_shape = (x.shape[2] - 1) * 2 + 2
            x = torch.empty([x.shape[0], self.num_classes, output_shape, output_shape])
        else:
            x = super().forward(x)
        return x
