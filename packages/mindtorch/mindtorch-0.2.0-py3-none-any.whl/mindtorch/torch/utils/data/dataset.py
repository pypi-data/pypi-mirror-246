...
# from mindtorch.torch import default_generator, randperm
#
...
class ConcatDataset(Dataset[T_co]):
    ...
    def cummulative_sizes(self):
        warning("cummulative_sizes attribute is renamed to " \
                "cumulative_sizes")
...
def random_split(dataset: Dataset[T], lengths: Sequence[int],
    ...
    if math.isclose(sum(lengths), 1) and sum(lengths) <= 1:
        ...
        for i, length in enumerate(lengths):
            if length == 0:
                warning(f"Length of split at index {i} is 0. This might result in an empty dataset.")

    if sum(lengths) != len(dataset):    # type: ignore[arg-type]
        raise ValueError("Sum of input lengths does not equal the length of the input dataset!")

    if generator is not None:
        warning("Don't support generator now.")
