...
def integrate_dos_bins(dos:DensityOfStates,
    ...
    intdos = np.cumsum(totaldos) * dos.get_energy_interval()
...
def dos_generate_sets(atomlist, orbs):
    sets = []
    for a in atomlist:
        for o in orbs:
            sets.append('%i-%s' % (a,o))
...
def __plot_dos(ax:matplotlib.axes._axes.Axes,
    ...
    if style is None or style == 'line':
        ...
        if stack is not None:
            ax.fill_betweenx(energies, stack, stack + dos, color=color, label=label)
        else:
