...
def deploy(
    ...
    # TODO: print logs based on event_id. This requires the CP prod to be updated, will be done in a follow up PR
    ...
    with console.status("Checking backend ..."):
        for _ in range(constants.Hosting.BACKEND_POLL_RETRIES):
            if backend_up := hosting.poll_backend(backend_deploy_response.url):
                break
            time.sleep(1)
    if not backend_up:
    ...
    with console.status("Checking frontend ..."):
        for _ in range(constants.Hosting.FRONTEND_POLL_RETRIES):
            if frontend_up := hosting.poll_frontend(frontend_deploy_response.url):
                break
            time.sleep(1)
    if not frontend_up:
