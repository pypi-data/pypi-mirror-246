...
# @Last Modified time: 2023-11-14 10:52:09
...
class PluginSuite:
    ...
    def __init__(self, dict_conf: Dict, abs_path: str):
        ...
        self.dependency_map, self.plugins, self.hidden = self._load_from_config(
            dict_conf, abs_path
        )
    ...
    def get_hidden(self):
        """
        Gets a list of the hidden nodes.
        """
        return self.hidden

    def __call__(
        self, base_input: Any, methods: Methods, dependency_map: Dict[str, List[str]]
    ) -> Dict:
        """
        ...
        components = {
            k: PluginComponent(self.plugins[k]()) for k in dependency_map.keys()
        }
    ...
    # PRIVATE
    ##########
    def _load_from_config( self, dict_config, abs_path: str
        ...
        for conf in dict_config["plugins"]:
            ...
            hidden_dict[clazz_name] = hidden
        logger.info(f"plugin dependency map {dependency_map}")
        logger.info(f"pluins {plugins}")
        return dependency_map, plugins, hidden_dict  # used to generate pipeline
