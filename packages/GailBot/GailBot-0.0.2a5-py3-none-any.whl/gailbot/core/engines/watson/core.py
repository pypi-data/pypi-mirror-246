...
class WatsonCore:
    ...
    def transcribe( self, 
        ...
        logger.info("transcribing the audio file with watson STT")
        try:
        ...
        try:
            if get_size(audio_path) >= WATSON_CONFIG.max_file_size_bytes:
                ...
                logger.info(f"compressed auido_path is {audio_path}")
        except Exception as e: 
...
##############
    ...
    def _prepare_utterance(self, audio_name: str,  closure: Dict[str, Any]) -> List:
        ...
        """
        logger.info(f"preparing utterance for audio {audio_name}")
        utterances = list()
