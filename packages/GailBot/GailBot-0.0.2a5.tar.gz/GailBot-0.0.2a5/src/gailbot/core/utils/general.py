...
# @Last Modified time: 2023-08-14 14:14:06
    ...
    try:
        if only_dirs:
            ...
            return list(
                itertools.chain(
                    *[
                        glob.glob(f"{path}/*.{ext}", recursive=recursive)
...
def filepaths_in_dir(
    dir_path: str, extensions: List[str] = ["*"], recursive: bool = False
) -> List[str]:
    """
    Get paths of files with specified extension in the directory.
    ...
    Returns:
        List[str]: List of paths to files in the directory
    """
    try:
        return paths_in_dir(
            path=dir_path, extensions=extensions, recursive=recursive, only_dirs=False
        )
    except Exception as e:
        logger.error(e)
        return False
...
def subdirs_in_dir(dir_path: str, recursive: bool = False) -> List[str]:
    ...
    try:
        return paths_in_dir(
            path=dir_path, extensions=None, recursive=recursive, only_dirs=True
        )
    except Exception as e:
        logger.error(e)
        return False
...
def make_dir(path: str, overwrite: bool = False):
    ...
    Returns:
        None
    """
    logger.info(f"make_dir is called to create the directory at {path}")
    try:
...
def run_cmd(cmd: List[str]) -> int:
    ...
    Returns:
        int: The process ID that can be used to identify the process the command runs on.
    """
