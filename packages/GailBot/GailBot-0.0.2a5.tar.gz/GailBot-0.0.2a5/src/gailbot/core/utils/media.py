...
# @Last Modified time: 2023-12-04 19:31:26
...
class VideoStream(Stream):
    ...
    def __repr__(self):
        print(f"video stream {self.name}\n")
...
class VideoHandler:
    ...
    @staticmethod
    def is_supported(path : str) -> bool:
        return get_extension(path) in ["mp4"]
    ...
    def read_file(self, path : str) -> VideoStream:
        ...
        Returns:
            VideoStream: return the video file data as a VideoStream object 
                         if the file type is unsupported, return nothing 
        """
        if not VideoHandler.is_supported(path):
            return
        format = get_extension(path)
    ...
    def write_stream(
        ...
        # Construct output file path
        name = stream.name if name == None else name
        extension = stream.extension if format == None else format
        make_dir(out_dir,overwrite=False)
        path = f"{out_dir}/{name}.{extension}"
        stream.clip.write_videofile(path)
        return path
    ...
    def extract_audio(self, stream : VideoStream) -> AudioStream:
        audioClip = stream.clip.audio
        path = f"{stream.name}.mp3"
        audioClip.write_audiofile(path)
        audioHandler = AudioHandler()
        audioStream = audioHandler.read_file(path)
        os.remove(path)
        return audioStream


    def remove_audio(self, stream : VideoStream) -> VideoStream:
        stream.clip = stream.clip.withoutaudio()
        return stream
