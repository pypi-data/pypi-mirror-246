...
# @Last Modified time: 2023-11-28 19:53:02
...
class Pipeline:
    ...
    def __call__(
        ...
        """
        Execute the pipeline by running all components in order of the
        dependency graph. This wraps data as DataStream before passing it b/w
        components. Additionally, each component receives the output of its
        dependencies.
        ...
        while True:
            ...
            for executable in executables:
                ...
                if dependency_resolved:
                    ...
                    logger.info(
                        f" the component {executable} is getting the thread key {key}"
                    )
    ...
    def _generate_dependency_graph(
        ...
        for name, dependencies in dependency_map.items():
            if name not in components:
                logger.warning(f"Component {name} not found in components, skipping...")
                continue
            ...
            for dep in dependencies:
                if dep not in components:
                    logger.warning(
                        f"Dependency {dep} for {name} not found, skipping..."
                    )
                    continue
        ...
        self.component_to_name: Dict[Component, str] = {
