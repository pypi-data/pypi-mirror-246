...
# @Last Modified time: 2023-12-07 12:45:23
import time
import os
...
class AnalysisComponent(Component):
    ...
    def analyze_payload(self, payload: PayLoadObject):
        ...
        # try applying plugin
        try:
            for suite_name, dependency_graph in suites.items():
                suite_result = dict()
                ...
                res: Dict[str, ComponentState] = plugin_suite(
                    base_input=None, methods=method, dependency_map=dependency_graph
                )
            ...
            logger.error(f"fail to apply the plugin suite, error {e}", exc_info=e)
