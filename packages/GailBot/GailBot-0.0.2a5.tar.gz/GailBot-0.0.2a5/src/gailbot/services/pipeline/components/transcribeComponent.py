...
# @Last Modified time: 2023-11-16 17:42:44
import copy
import time
import threading
...
DEFAULT_NUM_THREAD = service_config_loader().thread.transcriber_num_threads
...
class TranscribeComponent(Component):
    ...
    def __init__(self, num_thread: int = DEFAULT_NUM_THREAD, *args, **kwargs):
    ...
    def __call__(
        self, dependency_output: Dict[str, ComponentResult], *args, **kwargs
    ) -> ComponentResult:
        """
    ...
    def _transcribe_one_payload(self, payload: PayLoadObject) -> bool:
        ...
        utt_map = dict()
        threadpool = ThreadPool(DEFAULT_NUM_THREAD)
    ...
    def _transcribe_single_file(
        ...
        Args:
            engine_name: engine to perform the transcription
        ...
        """
        engine = EngineManager.init_engine(engine_name, **init_kwargs)
