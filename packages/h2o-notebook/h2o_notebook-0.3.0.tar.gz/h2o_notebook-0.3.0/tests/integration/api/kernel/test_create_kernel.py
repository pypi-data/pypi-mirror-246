...
def test_create_kernel_default_values(kernel_client_super_admin: KernelClient, kernels_cleanup_after):
    ...
    assert k.type == KernelType.TYPE_PYTHON
    assert k.display_name == ""
    assert k.image == "something"
    assert k.cpu_request == 0
    assert k.cpu_limit == 0
    assert k.gpu_resource_name == "nvidia.com/gpu"
    assert k.gpu_request == 0
    assert k.gpu_limit == 0
    assert k.memory_bytes_request == "0"
    assert k.memory_bytes_limit == "0"
    assert k.yaml_pod_template_spec == ""
...
def test_create_kernel_full_params(kernel_client_super_admin: KernelClient, kernels_cleanup_after):
...
def test_create_kernel_already_exists(kernel_client_super_admin: KernelClient, kernels_cleanup_after):
