...
def load_image(fp:str, graphics_format:GraphicsFormat=GraphicsFormat.UNKNOWN) -> torch.Tensor:
    ...
    denom = 1.
    if graphics_format & GraphicsFormat.UINT8:     denom = 255.
    elif graphics_format & GraphicsFormat.UINT16:  denom = 65535.
    elif graphics_format & GraphicsFormat.UINT32:  denom = 4294967295.
...
def save_image(im:torch.Tensor, fp:str, graphics_format:GraphicsFormat=GraphicsFormat.UNKNOWN) -> bool:
    ...
    fac = 1.
    dtype = np.uint8
    if graphics_format & GraphicsFormat.UINT8:     
        fac = 255.
        dtype = np.uint8
    elif graphics_format & GraphicsFormat.UINT16:  
        fac = 65535.
        dtype = np.uint16
    elif graphics_format & GraphicsFormat.UINT32:  
        fac = 4294967295.
        dtype = np.uint32
