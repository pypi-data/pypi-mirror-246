...
from .token import count_token, sub_tokenize
...
def make_chunk(text: str, max_length: int, overlap_ratio: float = 0.2) -> list[str]:
    ...
    for i in range(0, length, int(max_length * (1 - overlap_ratio))):
        yield sub_tokenize(text, i, i + max_length)
...
def get_browser_functions(ai: "OpenAI"):
    visit_page_model_name = "gpt-3.5-turbo-16k"  # "gpt-3.5-turbo-1106"
        ...
        try:
            ...
            while True:
                ...
                for chunk in make_chunk(text, visit_page_max_context_length):
                    ret = _page_summarize(query_text, chunk)
                    rets.append(ret)
                text = "\n".join(rets).strip()
