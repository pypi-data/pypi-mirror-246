...
class PD2:
    ...
    def recv_led_radiation_intensity(self, led:LED):
        ...
        Return 
            LOS链路LED辐射强度
        """
        Vt_r = self.tp_pos - led.pos           # LED到所有测试点处的PD方向向量 (n,3)
        d = np.linalg.norm(Vt_r, axis=-1)      # LED到PD的方向向量的模 (n)
        cos_led_t_angle_rad_rad = np.sum(Vt_r*led.Nv, axis=-1) / d          # LED发射角的余弦值 (n)
    ...
    def recv_los_led_signal(self, led:LED):
        ...
        H0 = np.where((pd_r_angle>=0) & (pd_r_angle<=self.fov), H0, 0)  # 视场内信道增益 (n) <=> np.place(H0,(phi>=0)&(phi<=pd.fov),0)
    ...
    def recv_nlos_led_signal(self, led:LED, room:Room):
        ...
        for i, (w_pos, angle) in enumerate(room.get_reflect_wall_args()):
            ...
            Hw = np.where((pd_r_angle>=0) & (pd_r_angle<=self.fov), Hw, 0)  # 视场内信道增益 (n,l1*w1)

            Hw = np.sum(Hw, axis=-1)   # 反射(NLOS)链路信道增益求和 (n)
            H0 += Hw                   # 墙壁反射(NLOS)链路信道增益(n)
