...
def resolve_payload_extended(**kwargs):
    ...
    try:
        ...
        # print("current assets: {}".format(current_assets))
        for item in os.listdir(blender_folder):
            item_path = os.path.join(blender_folder, item)
            if os.path.isdir(item_path) or os.path.isfile(item_path):
                item_path.replace("\\", "/")
                if item_path not in current_assets:
                    current_assets.append(item_path)
