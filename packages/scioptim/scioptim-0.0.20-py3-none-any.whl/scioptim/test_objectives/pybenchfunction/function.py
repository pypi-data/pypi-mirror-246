...
class PyBenchFunction(ABC):
    ...
    def is_dim_compatible(cls, d: int) -> bool:
        """
        Checks if the dimension is compatible with the function
        Args:
            d: int, the dimension to check
        Returns:
            bool, True if the dimension is compatible with the function
        """

    @abstractmethod
    def get_global_minimum(self) -> Tuple[np.ndarray, float]:
        """
        Computes the global minimum of the function
        Returns:
            X: array of shape (input_dim,n), the coordinates of the global minimum
            where n is the number of global minimas
            y: float, the value of the global minimum
        """

    @abstractmethod
    def _eval(self, X: np.ndarray) -> float:
        """
        Evaluates the function at the given point(s)
        Args:
    ...
    def __call__(self, X: np.ndarray) -> float:
        ...
        X = X.flatten()
        if not self.dimensionality == X.shape[0]:
            raise ValueError(
                f"X must have the same dimension as the function for {self.__class__.__name__} but got {X.shape[0]} instead of {self.dimensionality}"
            )
    ...
    def dimensionality(self):
        return self.input_domain.shape[0]
...
class Thevenot(PyBenchFunction):
    ...
    @classmethod
    def is_dim_compatible(cls, d: int):
        if not isinstance(d, int):
            raise TypeError("The dimension d must be an integer")
        if d <= 0:
            raise ValueError("The dimension d must be positive")
        return d > 0
    ...
    def get_global_minimum(self):
        X = np.zeros(self.dimensionality)
        return (X, self(X))
...
class AckleyN3(PyBenchFunction):
    ...
    def get_global_minimum(self):
        X = np.array([-2.86557969e-10, -6.77345523e-03])  # correcteed minima
        return (X, self(X))
...
class Adjiman(PyBenchFunction):
    ...
    latex_formula_global_minimum = r"f(x1_max, 0)=-2.02181"
    continuous = True
    convex = False
    separable = False
    differentiable = True
...
class BohachevskyN2(PyBenchFunction):
    ...
    def _eval(self, X) -> float:
        x, y = X
        res = (
            x**2 + 2 * y**2 - 0.3 * np.cos(3 * np.pi * x) * np.cos(4 * np.pi * y) + 0.3
        )
        return res
...
class DeJongN5(PyBenchFunction):
    ...
    def __init__(self, d, a=None):
        ...
        if a is None:
            _as = [-32.0, -16.0, 0.0, 16.0, 32.0]
            self.a = np.array([[x, y] for x in _as for y in _as])
        else:
            self.a = a
...
class HappyCat(PyBenchFunction):
    ...
    def __init__(self, d, alpha=0.125):
    ...
    def get_global_minimum(self):
        X = np.ones(self.dimensionality) * -1
        return (X, self(X))
...
class Langermann(PyBenchFunction):
    ...
    def get_global_minimum(self):
        d = self.dimensionality
        if d == 1:
            X = np.array([1.98886066])
        elif d == 2:
            X = np.array([2.79340221, 1.59723251])
        return (X, self(X))
...
class Michalewicz(PyBenchFunction):
    ...
    latex_formula_global_minimum = r"\text{at $d=2$, }f(2.203, 1.57) \approx -1.8013"
    continuous = True
    convex = False
    separable = True
    differentiable = True
...
class Qing(PyBenchFunction):
    ...
    def get_global_minimum(self):
        d = self.dimensionality
        X = np.sqrt(np.arange(1, d + 1))[None, :]  # here the root was forgotten
        for i in range(d):
            neg = X.copy()
            neg[:, i] *= -1
            X = np.vstack((X, neg))
...
class Quartic(PyBenchFunction):
    ...
    def _eval(self, X) -> float:
        ...
        res = (
            np.sum(np.arange(1, d + 1) * X**4)
            + (
                np.random.RandomState(norm.astype(np.float32).view(np.uint32)).random()
                + 1e-8
            )
            * norm
        )  # needed reproducibility and a fixed zero
        return res
...
class Rosenbrock(PyBenchFunction):
    ...
    def is_dim_compatible(cls, d: int):
        ...
        return d > 1  # Rosenbrock is not defined for d=1...
...
class SchaffelN1(PyBenchFunction):
    ...
    def _eval(self, X) -> float:
        ...
        res = (
            0.5
            + (np.sin((x**2 + y**2) ** 2) ** 2 - 0.5) / (1 + 0.001 * (x**2 + y**2)) ** 2
        )
        return res
...
class Schwefel(PyBenchFunction):
    ...
    def get_global_minimum(self):
        X = np.ones(self.dimensionality) * 420.9687
        return (X, self(X))
...
class StyblinskiTank(PyBenchFunction):
    ...
    def get_global_minimum(self):
        X = np.ones(self.dimensionality) * -2.903534
        return (X, self(X))
...
class XinSheYang(PyBenchFunction):
    ...
    def _eval(self, X) -> float:
        ...
        rand = np.random.RandomState(
            np.abs(X.sum()).astype(np.float32).view(np.uint32)
        ).random(d)
