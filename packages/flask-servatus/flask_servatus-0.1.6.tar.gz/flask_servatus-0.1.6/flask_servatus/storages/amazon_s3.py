...
def encode(aws_secret_access_key, str, urlencode=False):
    b64_hmac = (
        base64.encodebytes(
            hmac.new(
                aws_secret_access_key.encode("utf-8"), str.encode("utf-8"), sha
            ).digest()
        )
        .decode("utf-8")
        .strip()
...
class AWSAuthConnection:
    ...
    def _make_request(
        ...
        while True:
            ...
            # (close connection)
            resp.read()
            scheme, host, path, params, query, fragment = urllib.parse.urlparse(
                location
            )
            if scheme == "http":
                is_secure = True
                ...
                raise IOError("Not http/https: " + location)
            if query:
                path += "?" + query
            # retry with redirect
