...
dir_path = os.path.dirname(os.path.abspath(__file__))+'/results/'
os.makedirs(dir_path, exist_ok=True)
model_name = os.path.dirname(os.path.abspath(__file__))+'/model/best.pt'
...
class GetInput(QWidget):
    ...
    def showDialog(self):
        text, ok = QInputDialog.getMultiLineText(
            self, 'Camera Info', 'Enter Camera URLs and Camera Names:')
        if ok:
            return text
...
class CaptureIpCameraFramesWorker(QThread):
    ...
    def run(self) -> None:
        ...
        # logging.info(self.fps)
        print(f"{self.camera_name} FPS: {self.fps}")
        self.max_length = int(self.fps) * 5

        max_threads = self.idealThreadCount()
                    ...
                    if ret:
                        try:
                            ...
                            if len(results[0].boxes.data) > 0:
                                ...
                                self.detected = True
                                self.counter += 1
                                # print(self.camera_name, self.counter)

                                if self.counter >= self.max_length:
                                    self.out.release()

                                    jsonResponse = requests.post(api_url, json=json.dumps({'video_file_path': video_output_path,
                                                                                           'img_file_path': img_output_path,
                                                                                           'camera_name': self.camera_name,
                                                                                           'acc': self.acc}))

                                    # print(f"Req posted: Response {jsonResponse}")
                                    self.out = None
                                    self.detected = False
                                    self.counter = 0
                                    self.acc = 0.0
...
class Ui_Form(object):
    ...
    def setupUi(self, Form):
        ...
        self.label_2.setGeometry(QtCore.QRect(100, 80, 111, 81))
        font = QtGui.QFont()
    ...
    def check_password(self):
        ...
        if not login:
            ...
            try:
                os.environ["SCHOOLD_ID"] = login['localId']
                Popen(f'python {main_path}/api.py')

                Form.close()
                self.inputs = self.open_dialog()
