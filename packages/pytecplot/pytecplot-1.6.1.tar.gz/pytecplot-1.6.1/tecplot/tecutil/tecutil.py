#
# This file was auto-generated on 2023-04-28 by tecgen.
...
class TecUtil(object):
    ...
    def configure_tecutil_argtypes(self):
        ...
        if self.handle is not None:
            ...
            try:
                self.handle.tecUtilIsBusy.restype = c_bool
            except AttributeError:
                error_list.append('TecUtilIsBusy')
            ...
            # int32_t TecUtilLicenseCheckOut(char const * Feature)
            try:
                self.handle.tecUtilLicenseCheckOut.argtypes = [c_char_p]
                self.handle.tecUtilLicenseCheckOut.restype = c_int32
            except AttributeError:
                error_list.append('TecUtilLicenseCheckOut')
            # int32_t TecUtilLicenseGetInfo(char const * Feature, TP_GIVES char * * VendorData, TP_OUT time_t * ExpirationDate)
            try:
                self.handle.tecUtilLicenseGetInfo.argtypes = [c_char_p, POINTER(c_char_p), POINTER(c_int64)]
                self.handle.tecUtilLicenseGetInfo.restype = c_int32
            except AttributeError:
                error_list.append('TecUtilLicenseGetInfo')
    ...
    # void TecUtilFinishInteractiveViewChange()
    @tecutil_preamble
    def FinishInteractiveViewChange(self):
        self.handle.tecUtilFinishInteractiveViewChange()
    ...
    # Boolean_t TecUtilIsBusy()
    @tecutil_preamble
    def IsBusy(self):
        result = self.handle.tecUtilIsBusy()
        return result
    ...
    # int32_t TecUtilLicenseCheckOut(char const * Feature)
    @tecutil_preamble
    def LicenseCheckOut(self, feature):
        feature_proxy = feature.encode('utf-8') if feature is not None else feature
        result = self.handle.tecUtilLicenseCheckOut(feature_proxy)
        return result

    # int32_t TecUtilLicenseGetInfo(char const * Feature, TP_GIVES char * * VendorData, TP_OUT time_t * ExpirationDate)
    @tecutil_preamble
    def LicenseGetInfo(self, feature):
        feature_proxy = feature.encode('utf-8') if feature is not None else feature
        vendor_data_out = c_char_p()
        expiration_date_out = c_int64()
        result = self.handle.tecUtilLicenseGetInfo(feature_proxy, byref(vendor_data_out), byref(expiration_date_out))
        vendor_data_out_value = None if vendor_data_out.value is None else vendor_data_out.value.decode('utf-8')
        self.handle.tecUtilStringDealloc(byref(vendor_data_out))
        return result, vendor_data_out_value, expiration_date_out.value
    ...
    # void TecUtilStartInteractiveViewChange()
    @tecutil_preamble
    def StartInteractiveViewChange(self):
        self.handle.tecUtilStartInteractiveViewChange()
