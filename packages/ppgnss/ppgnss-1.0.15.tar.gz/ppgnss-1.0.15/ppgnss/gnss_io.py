...
def read_anubis(filename):
    with open(filename, "rb") as fread:
        ...
        _ = fread.readline()
        line = fread.readline()
        first_epoch = np.datetime64(gnss_time.strtime2datetime(line[8:27].decode("utf-8")))
        last_epoch = np.datetime64(gnss_time.strtime2datetime(line[28:47].decode("utf-8")))
