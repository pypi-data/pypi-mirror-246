...
def download_brdc_data(year, doy, out_dir="."):
    ...
    try:
        ...
        print(f"Downloaded BRDC data for {year}-{doy_str} to {filename}")
...
def download_gim_data(year, doy, out_dir="."):
    ...
    filename = os.path.join(out_dir, f"CODG{doy_str}0.{yr}I.Z")

    try:
        wget.download(url, out=filename)
        print(f"Downloaded GIM data for {year}-{doy_str} to {filename}")
    except Exception as e:
...
def download_sp3_data(year, doy, out_dir="."):
    ...
    try:
        wget.download(url, out=filename)
        print(f"Downloaded SP3 data for {year}-{doy_str} to {filename}")
    except Exception as e:
        print(f"Failed to download SP3 data for {year}-{doy_str}. Error: {e}")
# ftp://igs.gnsswhu.cn//pub/gps/products/2264/COD0MGXFIN_20231480000_01D_30S_CLK.CLK.gz
def download_clk_data(year, doy, out_dir="."):
    gpsw, gpsd = gnss_time.doy2gpsw(year, doy)
    base_url = "ftp://igs.gnsswhu.cn/pub/gps/products"
    doy_str = f"{doy:03d}"  # 使用 zfill 将 doy 格式化为三位数字
    url = f"{base_url}/{gpsw}/IGS0OPSFIN_{year}{doy_str}0000_01D_05M_CLK.CLK.gz"
    filename = os.path.join(out_dir, f"IGS0OPSFIN_{year}{doy_str}0000_01D_05M_CLK.CLK.gz")

    try:
        wget.download(url, out=filename)
        print(f"Downloaded CLK data for {year}-{doy_str} to {filename}")
    except Exception as e:
...
def main():
    ...
    parser.add_argument('--from', dest='start_date', type=parse_year_and_doy, help='Specify the start date for data download. Format: year,doy')

    # 添加--to参数，用于指定结束时间的年份和年积日
    parser.add_argument('--to', dest='end_date', type=parse_year_and_doy, help='Specify the end date for data download. Format: year,doy')
