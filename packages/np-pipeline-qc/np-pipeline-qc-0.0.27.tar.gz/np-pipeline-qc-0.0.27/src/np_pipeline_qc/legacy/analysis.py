...
def plot_sample_LFP(
    ...
    for p in lfp_dict:
        if p not in agar_chan_dict:
            continue
        lfp = lfp_dict[p]['lfp']
        lfp_time = lfp_dict[p]['time']
        probeSampleRate = 1.0 / np.median(np.diff(lfp_time))
        samplesBefore = int(round(windowBefore * probeSampleRate))
        samplesAfter = int(round(windowAfter * probeSampleRate))
        midpoint = int(len(lfp) * 0.5)
        agarChRange = np.array(agar_chan_dict[p]).astype(int)
        print('Using agar range {} for probe {}'.format(agarChRange, p))
        ll = processLFP(
            lfp[midpoint - samplesBefore : midpoint + samplesAfter],
            baseline_samps=samplesBefore,
            agarChRange=agarChRange,
        )
        ll_filt = filterTrace(ll, highcutoff=1)

        ll = np.nanmean(ll, axis=0) * 0.195  # convert to uV
        ll_filt = np.nanmean(ll_filt, axis=0) * 0.195
        mtime = np.linspace(-windowBefore, windowAfter, ll.size)

        fig, axes = plt.subplots(3, 1)
        fig.set_size_inches([12, 8])
        fig.suptitle(
            p
            + 'Sample LFP at mid-point in recording'
        )
        im_raw = axes[0].imshow(ll.T, aspect='auto')
        im_filt = axes[1].imshow(ll_filt.T, aspect='auto')
        ...
        axes[2].plot(np.mean(ll, axis=1), 'k')
        for a in axes[:-1]:
            a.set_xticks(
        ...
        save_figure(
            fig,
            os.path.join(
                FIG_SAVE_DIR, prefix + 'Probe' + p + ' sample LFP'
            ),
        )
