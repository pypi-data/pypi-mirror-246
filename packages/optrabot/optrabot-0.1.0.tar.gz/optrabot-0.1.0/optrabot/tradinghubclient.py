...
class TradinghubClient():
		...
		try:
			...
			if response.text != '\"\"' and response.text != '':
				...
				try:
					...
					logger.info("Qualified Contract")
					ib.reqMarketDataType(self.getMarketDataType())
					...
					if not self._checkContractIsQualified(shortPutContract):
						return
					...
					if not self._checkContractIsQualified(shortCallContract):
						return
					...
					if not self._checkContractIsQualified(longPutContract):
						return
					...
					if not self._checkContractIsQualified(longCallContract):
						return
					...
					limitPrice = self._roundToTickSize(ironFlyMidPrice)
	...
	async def _placeTakeProfitAndStop(self, entryTrade: Trade):
		...
		limitPrice = self._roundToTickSize(fillPrice - profit)
		now = datetime.now()
		...
		self._ironFlyStopPrice = self._roundToTickSize(fillPrice * -1.16)
	...
	async def _monitorPosition(self):
		...
		desiredStopPrice = self._roundToTickSize(self._ironFlyStopPrice - longLegsValue)
		currentStopPrice = self._roundToTickSize(self._slShortTrade.order.auxPrice)
	...
	def getMarketDataType(self):
		if self._marketDataType is None:
			try:
				confMarketData = self._config.get('tws.marketdata')
			except KeyError as keyError:
				confMarketData = 'delayed'
			
			if confMarketData == 'live':
				self._marketDataType = 1 # Live
			else:
				self._marketDataType = 4 # Delayed
	...
	def _roundToTickSize(self, value: float):
		roundBase = 5
		return (roundBase * round(value*100/roundBase)) / 100
	
	def _checkContractIsQualified(self, contract):
		if contract.conId == 0:
