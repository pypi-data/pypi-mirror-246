...
def get_signal_ee(preffix, year, trigger):
    ...
    total_ee_shape = zfit.pdf.SumPDF(dscbs, [f0_corrected, f1_corrected])
    return total_ee_shape, constraints


def get_signal_mm(preffix, year, trigger):
    scale_reader = CacheMSReader()
    all_mu = scale_reader.get_scale("mu")
    dmu_value = all_mu.loc[f"{year}", f"v_{trigger}"]
    dmu_error = all_mu.loc[f"{year}", f"e_{trigger}"]
...
def get_signal_shape(preffix='', year="2018", trigger="ETOS"):
    if trigger in ["ETOS", "GTIS"]:
        return get_signal_ee(preffix, year, trigger)
    elif trigger == "MTOS":
        return get_signal_mm(preffix, year, trigger)
    else:
