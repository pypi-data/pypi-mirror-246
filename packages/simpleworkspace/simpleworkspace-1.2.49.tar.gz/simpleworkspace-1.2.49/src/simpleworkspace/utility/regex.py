...
def Replace(pattern: str, replacement: str, message:str) -> str:  
    ...
    return Regex(pattern).Replace(replacement, message)
...
def Test(pattern:str, string:str) -> bool:
    """
...
class Regex:
    ...
    def Test(self, string:str) -> bool:
        ...
        :return: True if at least one match is found, False otherwise.
        """
        return _re.search(self.pattern, string, flags=self.flags) is not None

    def Match(self, string: str) -> (list[list[str]] | None):  
        """
        Finds all matches of the regex pattern in the string, default flag is multiline
        ...
        iterator = _re.finditer(self.pattern, string, flags=self.flags)
        results = [[i.group(0), *i.groups()] for i in iterator]
        if len(results) == 0:
            return None
        return results  

    def Replace(self, replacement: str, message:str) -> str:  
        """
        Replaces all occurrences of the regex pattern in the message with the given replacement.
        ...
        return  _re.sub(self.pattern, replacement, message, flags=self.flags)

    def _ParsePattern(self, pattern:str) -> tuple:
        """
