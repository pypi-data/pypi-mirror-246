...
class DDSP(nn.Module):
    ...
    def __init__(self, hidden_size, n_harmonic, n_bands, n_conditional_params, sampling_rate,
        ...
        self.n_conditional_params = torch.tensor(n_conditional_params + 2)
    ...
    def forward(self, pitch, conditional_parameters, loudness):
        ...
        for i in range(self.n_conditional_params - 2):
            hidden = torch.cat([
                hidden, 
                self.in_mlps[i + 2](conditional_parameters[:, i, :, :])
            ], -1)
        ...
        for i in range(self.n_conditional_params - 2):
            hidden = torch.cat([hidden, conditional_parameters[:, i, :, :]], -1)
