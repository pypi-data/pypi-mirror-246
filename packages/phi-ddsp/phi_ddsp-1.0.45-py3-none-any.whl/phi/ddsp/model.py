...
class DDSP(nn.Module):
    ...
    def forward(self, pitch, conditional_parameters, loudness):
        ...
        # Process pitch and loudness
        hidden_parts.append(self.in_mlps[0](pitch))
        hidden_parts.append(self.in_mlps[1](loudness))

        # Process conditional parameters in key-value format
        for i, cond_param in enumerate(conditional_parameters.unbind(1)):
            hidden_parts.append(self.in_mlps[2 + i](cond_param))

        # Concatenate processed inputs
        hidden = torch.cat(hidden_parts, -1)

        # Process the concatenated inputs using GRU
        hidden = torch.cat([
            self.gru(hidden)[0],
            pitch,
            loudness,
            *conditional_parameters.unbind(1)
        ], -1)
