...
class DDSP(nn.Module):
    ...
    def forward(self, pitch, conditional_parameters, loudness):
        # Concat pitch and loudness
        hidden_list = [
            self.in_mlps[0](pitch),
            self.in_mlps[1](loudness),
        ]

        # Concat each conditional parameter
        for i in range(2, self.n_conditional_params):
            hidden_list.append(self.in_mlps[i](conditional_parameters[:, i - 2, :, :]))

        # Concatenate all intermediate results
        hidden = torch.cat(hidden_list, dim=-1)

        # Concat pitch and loudness again
        gru_output = self.gru(hidden)[0]
        hidden_list = [gru_output, pitch, loudness]

        # Concat each conditional parameter ... again
        for i in range(2, self.n_conditional_params):
            hidden_list.append(conditional_parameters[:, i - 2, :, :])
