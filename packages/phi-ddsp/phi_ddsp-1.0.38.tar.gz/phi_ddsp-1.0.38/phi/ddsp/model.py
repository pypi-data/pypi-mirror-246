...
class DDSP(nn.Module):
    ...
    def forward(self, pitch, conditional_parameters, loudness):
        ...
        for i, conditional_param_mlp in enumerate(self.in_mlps):
            # Omit pitch and loudness
            if i >= 2:
                hidden = torch.cat([hidden, conditional_param_mlp(conditional_parameters[:, i - 2, :, :])], -1)
