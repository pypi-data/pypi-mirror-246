...
class DDSP(nn.Module):
    ...
    def __init__(self, hidden_size, n_harmonic, n_bands, n_conditional_params, sampling_rate,
                 block_size):
        super().__init__()
        self.register_buffer("sampling_rate", torch.tensor(sampling_rate))
        self.register_buffer("block_size", torch.tensor(block_size))

        self.n_conditional_params = n_conditional_params + 2

        self.in_mlps = nn.ModuleList([mlp(1, hidden_size, 3)] * self.n_conditional_params)
        self.gru = gru(self.n_conditional_params, hidden_size)
        self.out_mlp = mlp(hidden_size + self.n_conditional_params, hidden_size, 3)
    ...
    def forward(self, pitch, conditional_parameters, loudness):
        # Concatenate pitch and loudness
        hidden = torch.cat([
            self.in_mlps[0](pitch),
            self.in_mlps[1](loudness),
        ], -1)

        slice_end = self.n_conditional_params

        # Concatenate conditional parameters
        conditional_param_list = [conditional_param_mlp(conditional_parameters[:, i, :, :]) for i,
                                  conditional_param_mlp in
                                  enumerate(self.in_mlps[2:slice_end])]

        hidden = torch.cat([hidden, *conditional_param_list], -1)
                    
        # Concatenate pitch, loudness, and conditional parameters again
        hidden = torch.cat([
            self.gru(hidden)[0], 
            pitch, 
            loudness,
            *conditional_parameters.unbind(dim=1)
        ], -1)
