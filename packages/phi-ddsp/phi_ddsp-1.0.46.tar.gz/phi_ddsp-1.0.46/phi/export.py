...
class ScriptDDSP(torch.nn.Module):
    ...
    def forward(self, pitch, conditional_parameters, loudness):
        ...
        return self.ddsp(pitch, conditional_parameters, loudness)


def export(config):
    # Check if the directory exists, and create one if not
    export_dir = path.join(config["model"]["model_dir"], "export")
    ...
    pretrained = torch.load(path.join(config["model"]["model_dir"], "state.pth"), map_location="cpu")
    state.update(pretrained)
    ddsp.load_state_dict(state)

    name = path.basename(path.normpath(config["model"]["model_dir"]))
