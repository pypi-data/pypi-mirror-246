...
class PromptTemplateManager(WMLResource):
    ...
    **Example**
    ...
    def _create_request_body(self, prompt_template: PromptTemplate) -> Dict:
        ...
        if prompt_template.model_version is not None:
            json_data.update({"model_version": {"number": prompt_template.model_version}})
    ...
    def _from_json_to_prompt(self, response: Dict) -> PromptTemplate:
        ...
        prompt_template = PromptTemplate(name=response.get('name'),
                                         ...
                                         model_id=next((model for model in ModelTypes
                                                        if model.value == prompt_field.get('model_id')), None),
                                         model_params=prompt_field.get('model_parameters'),
                                         task_ids=response.get("task_ids"),
                                         model_version=response.get("model_version", dict()).get("number"))
        ...
        prompt_template.instruction = prompt_field.get('data', dict()).get('instruction')
        ...
        prompt_template._lock = PromptTemplateLock(**response.get('lock', {}))
