...
from cubicweb_web.views import uicfg, autoform
...
simplified_afs.tag_attribute(
    ("SEDASeqAppraisalRuleRule", "user_cardinality"), "main", "hidden"
)
simplified_afs.tag_attribute(
    ("SEDASeqAccessRuleRule", "user_cardinality"), "main", "hidden"
)
...
# XXX TODO the same with SEDAHoldEndDate


class StartDateAutomaticEntityForm(autoform.AutomaticEntityForm):
    __select__ = is_instance("SEDAStartDate") & simplified_profile()

    def editable_attributes(self, strict=False):
        """return a list of (relation schema, role) to edit for the entity"""
        attributes = list(super().editable_attributes(strict))
...
class RuleAutomaticEntityForm(autoform.AutomaticEntityForm):
    __select__ = (
        is_instance("SEDAAppraisalRule", "SEDAAccessRule") & simplified_profile()
    )
    ...
    def should_display_inline_creation_form(self, rschema, existing, card):
        ...
        if not existing and rschema in (
            ...
            return True
        return super().should_display_inline_creation_form(rschema, existing, card)
    ...
    def should_display_add_new_relation_link(self, rschema, existing, card):
        ...
        if rschema in ("seda_seq_appraisal_rule_rule", "seda_seq_access_rule_rule"):
            return False
        return super().should_display_add_new_relation_link(rschema, existing, card)


class SeqRuleRuleAutomaticEntityForm(autoform.AutomaticEntityForm):
    __select__ = (
        is_instance("SEDASeqAppraisalRuleRule", "SEDASeqAccessRuleRule")
        & simplified_profile()
    )
...
def simplified_rule_rule(cls, req, rtype=None, pform=None, **kwargs):
    ...
    if isinstance(pform, RuleAutomaticEntityForm) and rtype in (
        "seda_seq_appraisal_rule_rule",
        "seda_seq_access_rule_rule",
    ):
        return 1
...
class RuleRuleInlineEntityEditionFormView(autoform.InlineEntityEditionFormView):
    __select__ = (
        autoform.InlineEntityEditionFormView.__select__ & simplified_rule_rule()
    )
    removejs = None
...
class RuleRuleInlineEntityCreationFormView(autoform.InlineEntityCreationFormView):
    __select__ = (
        autoform.InlineEntityCreationFormView.__select__ & simplified_rule_rule()
    )
    removejs = None
...
class StartDateInlineEntityEditionFormView(autoform.InlineEntityEditionFormView):
    __select__ = (
        autoform.InlineEntityEditionFormView.__select__ & simplified_start_date()
    )
    removejs = None
...
class StartDateInlineEntityCreationFormView(autoform.InlineEntityCreationFormView):
    __select__ = (
        autoform.InlineEntityCreationFormView.__select__ & simplified_start_date()
    )
    removejs = None
    form_renderer_id = "notitle"
...
for rule_type in RULE_TYPES:
    etype = f"SEDA{rule_type.capitalize()}Rule"
    rtype = f"seda_alt_{rule_type}_rule_prevent_inheritance"
...
for rtype, role in archivetransfer.at_ordered_fields:
    if not rtype.endswith("agency"):
        ...
        simplified_pvs.tag_object_of(
            ("*", rtype, "SEDAArchiveTransfer"), simplified_section
        )


# SEDAArchiveUnit
simplified_au_rtypes = {
    rtype
    for rtype, role, targets in archiveunit.SimplifiedContentMainView.rtype_role_targets
}
...
simplified_afs.tag_object_of(
    ("SEDADataObjectReference", "seda_data_object_reference_id", "*"), "main", "inlined"
)
simplified_afs.tag_object_of(
    ("SEDACustodialHistoryFile", "seda_data_object_reference_id", "*"), "main", "hidden"
)
...
simplified_pvs.tag_object_of(
    ("*", "seda_date_created_by_application", "SEDABinaryDataObject"), "attributes"
)
...
class DataObjectSimplifiedAutomaticEntityForm(widgets.SimplifiedAutomaticEntityForm):
    ...
    # edition
    __select__ = (
        widgets.SimplifiedAutomaticEntityForm.__select__
        & is_instance("SEDABinaryDataObject", "SEDAPhysicalDataObject")
        & simplified_profile()
    )

    def inlined_form_views(self):
        views = list(super().inlined_form_views())
        ref_forms = [
            v.form for v in views if v.rtype == "seda_data_object_reference_id"
        ]
...
simplified_afs.tag_attribute(
    ("SEDADataObjectReference", "user_cardinality"), "inlined", "hidden"
)


class DataObjectReferenceNoTitleEntityInlinedFormRenderer(
    ...
    __select__ = (
        widgets.NoTitleEntityInlinedFormRenderer.__select__
