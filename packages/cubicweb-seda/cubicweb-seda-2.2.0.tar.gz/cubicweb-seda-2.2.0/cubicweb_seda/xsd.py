...
* `XSDMMapping` which holds a structure mapping the previous SEDA XSD representation to the
  cubicweb data model.
"""
...
SEDA_NS = "{fr:gouv:culture:archivesdefrance:seda:v2.2}"
SKIP_ELEMENTS = {
    "DataObjectGroupId",
    "DataObjectGroupReferenceId",  # XXX simplification
    ...
    (
        "Content",
        "Signature",
    ),  # keep signature on ArchiveTransfer, skip it on content
}
JUMP_ELEMENTS = {
    # ArchiveTransfer
    "CodeListVersions",
...
XSD2YAMS = {
    ...
    "token": "String",  # string and start and final whitespaces
    "algorithm": "Concept",
    "language": "Concept",
    ...
    "HoldRuleCodeListVersion": "ConceptScheme",
}
...
def seda_xsd():
    """Use this function to return the XSD representation of SEDA 2.2 (singleton)."""
    global _SEDA_XSD
    ...
    if _SEDA_XSD is None:
        xsd_path = osp.join(osp.dirname(__file__), "xsd", "seda-2.2-main.xsd")
...
def _xsiterate(xselement):  # noqa
    ...
    `child occ` holds the reachable XSD child elements and `entities path` the associated list of
    ...
    while _stack:
        ...
        if isinstance(parent, graph_nodes.XMLElement):
            ...
            if (
                parent.local_name not in XSD2YAMS
                ...
                if (
                    etype != "Bytes"
                ):  # Bytes element are usually to be specified by the end user
            ...
            for occ in _children_occurences(parent):
                ...
                if isinstance(child, graph_nodes.XMLElement):
                    if child.local_name in JUMP_ELEMENTS:
                        ...
                        etype = (
                            XSD2YAMS.get(child.local_name)
                            or (
                                not child.attributes
                                and XSD2YAMS.get(_content_types(child))
                            )
                            or etype_name(child)
                        )
                        rtype = base_rtype_name(child, etype)
                        composite = not (
                            child.local_name in XSD2YAMS
                            or (
                                not child.attributes
                    ...
                    alt_etype = None
                    if (
                        isinstance(child, graph_nodes.Alternative)
                        or isinstance(parent, graph_nodes.Alternative)
                        or not (
                            occ.maximum == 1
                            and (
                                occ.minimum == 1
                                or all(cocc.minimum == 0 for cocc in child.children)
                            )
                        )
                    ):
                        # use first children to build then entity type for the element
                        sample_target = child.children[0].target
                        sample_target_name = getattr(
                            sample_target,
                            "local_name",
                            sample_target.__class__.__name__,
                        )
                        alt_etype = "SEDA{}{}{}".format(
                            _shorten(occ.target.__class__.__name__),
...
def _attributes_paths(element):
    ...
    for occ in element.attributes.values():
        ...
        if xattr.local_name == "lang":
            continue
        etype = XSD2YAMS.get(xattr.local_name) or XSD2YAMS[_content_types(xattr)]
...
def _occ_path(occ, etype, rtype, composite):
    ...
    relations.append(
        (ref_rtype, "object", rtype_etype, {"composite": "object", "inlined": True})
    )
    ...
    if etype in BASE_TYPES:
        ...
        relations.append(
            (
                ...
                "subject",
                etype,
                {"composite": composite, "inlined": inlined},
            )
        )
    return relations
...
def _children_occurences(element):
    ...
    if isinstance(
        element, (graph_nodes.Alternative, graph_nodes.Group, graph_nodes.Sequence)
    ):
