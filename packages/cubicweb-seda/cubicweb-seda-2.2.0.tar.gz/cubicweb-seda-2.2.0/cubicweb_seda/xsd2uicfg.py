...
RTYPES_IN_TAB = {
    ...
    "seda_service_level",
    "seda_archival_profile",
    "seda_acquisition_information_from",
    "seda_legal_status_from",
...
class UICFGGenerator(CodeGenerator):
    ...
    def _generate(self, mapping, stream):
        ...
        for etype in all_etypes:
            stream.write(f"ives['{etype}'] = 'subobject'\n")
        # autoform / primary view relation rtags
        for rtag, rtag_info in sorted(self.rtags_info.items()):
            alias = rtag_info["shortname"]
            stream.write(f"\n\n{rtag_info['shortname']} = uicfg.{rtag}\n")
            for mapping_element in mapping.ordered:
                for rtype, role, value in self._callback(
                    rtag + "_for", mapping_element
                ):
        ...
        for mapping_element in mapping.ordered:
            for etype, attributes in self._callback("order_for", mapping_element):
                stream.write(f"affk.set_fields_order('{etype}', {attributes})\n")
                stream.write(f"pvds.set_fields_order('{etype}', {attributes})\n")
        # fields documentation
        stream.write("\nETYPE_ATTR_DOC = {}\n")
        template = (
            "ETYPE_ATTR_DOC[('{etype}', '{rtype}', '{role}')] = ('{element}', {doc})\n"
        )
        for mapping_element in mapping.ordered:
            for etype, rtype, role, element, doc in self._callback(
                "doc_for", mapping_element
            ):
                stream.write(
                    template.format(
                        etype=etype, rtype=rtype, role=role, element=element, doc=doc
                    )
                )
...
def ordered_attributes(mapping):
    """Given an ETypeMapping, return a list of its attributes sorted by desired
    order of appearance"""
