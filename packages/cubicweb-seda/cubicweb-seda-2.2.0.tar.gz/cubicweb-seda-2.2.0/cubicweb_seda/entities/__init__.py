    ...
    if entity.has_eid():
        ...
        if container is None:
            ...
            if entity.cw_adapt_to("IContainer") is None:
                entity.debug(
                    "cannot adapt %s as IContainer in parent_and_container()", entity
                )
                return None, None
            container = entity
    else:
...
@objectify_predicate
def wrap_dataobjects(cls, req, rset=None, entity=None, **kwargs):
    """Predicate returning 1 score if context entity is VITAM compliant
    (BinaryDataObject and PhysicalDataObject must be wrapped in a DataObjectGroup
    object .
    ...
    if container.cw_etype == "SEDAArchiveUnit":
        # XXX archive unit component, for now suppose it's "simplified"
        return 1
    return 1 if container.wrap_dataobjects else 0
...
def rule_type_from_etype(etype):
    ...
    assert rule_type in RULE_TYPES, f"unhandled etype {etype}"
    return rule_type
...
def registration_callback(vreg):
    ...
    vreg.register(
        IContainer.build_class("SEDAArchiveUnit")
    )  # archive unit may also be a container
