...
UNEXPORTED_ELEMENTS = {"CodeListVersions"}
...
def content_types(content_type):
    """Return an ordered tuple of content types from pyxst
    `textual_content_type` that may be None, a set or a string value."""
    if content_type:
...
def minmax_cardinality(string_cardinality, _allowed=("0..1", "0..n", "1", "1..n")):
    ...
    '1' or '1..n').
    """
    assert string_cardinality in _allowed, "{} not allowed {}".format(
        string_cardinality,
        _allowed,
    )
...
class SEDA2ExportAdapter(EntityAdapter):
    ...
    def exportable_type(self, cwetype):
        return True
    ...
    def _process(self, entity, profile_element, child_defs, to_process):
        for occ, path in child_defs:
            ...
            # export elements in UNEXPORTED_ELEMENTS but not their children
            if profile_element.get("name", None) in UNEXPORTED_ELEMENTS:
                continue
            ...
            if not path:
                ...
                for card_entity, target_value in _path_target_values(entity, path):
                    if (
                        card_entity
                        and hasattr(card_entity, "cw_etype")
                        and not self.exportable_type(card_entity.cw_etype)
                    ):
                        continue
                    self.dispatch_occ(
                        profile_element,
...
class AbstractSEDA2RelaxNGExport(RNGMixin, SEDA2ExportAdapter):
    """Abstract Adapter to build a Relax NG representation of a SEDA profile, using SEDA 2.X
    specification."""
    ...
    def fill_organization_element(self, parent_element, value):
        ...
        if value:
            uri_attribute = self.element(
                "rng:attribute", target_element, {"name": "schemeURI"}
            )
            self.element(
                "rng:value",
                uri_attribute,
                attributes={"type": "string"},
                text=value.absolute_url(),
            )
            self.element(
                "rng:value",
                target_element,
...
class SEDA21RelaxNGExport(AbstractSEDA2RelaxNGExport):
    ...
    def exportable_type(self, cwetype):
        return cwetype not in SEDA_2_2_ONLY_ETYPES


class SEDA21WarpDataObjectRelaxNGExport(SEDA2WrapObjectMixin, SEDA21RelaxNGExport):
    __select__ = SEDA21RelaxNGExport.__select__ & wrap_dataobjects()


class SEDA2NoWrapObjectMixin:
    def postprocess_dataobjects(self, root, namespaces):
...
class SEDA21NoWarpDataObjectRelaxNGExport(SEDA2NoWrapObjectMixin, SEDA21RelaxNGExport):
    __select__ = SEDA21RelaxNGExport.__select__ & ~wrap_dataobjects()


class SEDA22RelaxNGExport(AbstractSEDA2RelaxNGExport):
    """Abstract Adapter to build a Relax NG representation of a SEDA profile, using SEDA 2.2
    specification."""

    __regid__ = "SEDA-2.2.rng"

    namespaces = {
        None: "fr:gouv:culture:archivesdefrance:seda:v2.2",
        "seda": "fr:gouv:culture:archivesdefrance:seda:v2.2",
        "xml": "http://www.w3.org/XML/1998/namespace",
        "xsd": "http://www.w3.org/2001/XMLSchema",
        "xlink": "http://www.w3.org/1999/xlink",
        "rng": "http://relaxng.org/ns/structure/1.0",
    ...
    _root_attributes = {
        "ns": "fr:gouv:culture:archivesdefrance:seda:v2.2",
        "datatypeLibrary": "http://www.w3.org/2001/XMLSchema-datatypes",
    }


class SEDA22WarpDataObjectRelaxNGExport(SEDA2WrapObjectMixin, SEDA22RelaxNGExport):
    __select__ = SEDA22RelaxNGExport.__select__ & wrap_dataobjects()


class SEDA22NoWarpDataObjectRelaxNGExport(SEDA2NoWrapObjectMixin, SEDA22RelaxNGExport):
    __select__ = SEDA22RelaxNGExport.__select__ & ~wrap_dataobjects()
    ...
    * `fixed_value`, optional fixed value for the attribute.
    ...
    def __new__(cls, name, qualified_type, cardinality="0..1", fixed_value=None):
        ...
        if fixed_value:
            ...
            fixed_value = None
        return super().__new__(cls, name, qualified_type, cardinality, fixed_value)
...
class SEDA1XSDExport(SEDA2ExportAdapter):
    ...
    namespaces = {
        ...
        "clmIANACharacterSetCode": "urn:un:unece:uncefact:codelist:standard:IANA:CharacterSetCode:2007-05-14",  # noqa
        "clmIANAMIMEMediaType": "urn:un:unece:uncefact:codelist:standard:IANA:MIMEMediaType:2008-11-12",  # noqa
    ...
    def xsd_content_dates(self, parent, content):
        for seda2_name, seda1_name in (("end", "latest"), ("start", "oldest")):
            date_entity = getattr(content, f"{seda2_name}_date")
            if date_entity:
                self.element_schema(
                    parent,
                    f"{seda1_name.capitalize()}Date",
                    "udt:DateType",
...
class SEDA02XSDExport(SEDA1XSDExport):
    ...
    def xsd_archive_object(self, parent, archive_unit):
        """Append XSD elements for the archive object to the given parent node."""
        ao_node = super().xsd_archive_object(parent, archive_unit)
