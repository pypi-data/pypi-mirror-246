...
GRAPH_SKIP_RTYPES = {"container", "clone_of"}
...
def seda_profile_container_def(schema):
    ...
    return structure_def(
        schema,
        "SEDAArchiveTransfer",
        skipetypes=GRAPH_SKIP_ETYPES,
        skiprtypes=GRAPH_SKIP_RTYPES,
    ).items()
...
def iter_all_rdefs(schema, container_etype):
    ...
    graph = CompositeGraph(
        schema, skipetypes=GRAPH_SKIP_ETYPES, skiprtypes=GRAPH_SKIP_RTYPES
    )
