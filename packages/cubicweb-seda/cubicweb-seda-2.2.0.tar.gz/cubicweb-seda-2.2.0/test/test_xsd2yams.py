...
from cubicweb import devtools, Binary  # noqa
...
def readable_edef(edef):
    ...
    if len(edef) == 3:
        return (
            name(edef[0]),
            edef[1],
            [(name(rdef[0].target), [x[:-1] for x in rdef[1]]) for rdef in edef[2]],
        )
    else:
        return (
            edef[0],
            [(name(rdef[0].target), [x[:-1] for x in rdef[1]]) for rdef in edef[1]],
        )
    ...
    def test_base(self):
        ...
        for line in code.splitlines():
            if line.startswith("class "):
                ...
                self.assertLess(len(name), 64, f"too long name {name!r} ({len(name)}))")
...
class XSYMappingTC(unittest.TestCase):
    ...
    def assertRMappingEqual(self, rmapping, rdefs):
        self.assertEqual(
            sorted((rdef.subjtype, rdef.objtype) for rdef in rmapping.rdefs), rdefs
        )
    ...
    def test_binarydataobject(self):
        ...
        self.assertEMappingEqual(emapping, {"compressed": "String"})

    def test_hardcoded_cardinalities(self):
        mapping = xsy_mapping("Content")
        seda_title_card = [
            rdefmapping.card
            for rdefmapping in mapping.ordered
            if getattr(rdefmapping, "rtype", None) == "seda_title"
        ][0]
        self.assertEqual(seda_title_card, "11")
        seda_description_card = [
            rdefmapping.card
            for rdefmapping in mapping.ordered
            if getattr(rdefmapping, "rtype", None) == "seda_description"
        ][0]
...
class XSIterateTC(unittest.TestCase):
            ...
            (
                ...
                [
                    (
                        "when",
                        [
                            ("seda_when", "object", "SEDAwhen"),
                            ("when", "subject", "TZDatetime"),
                        ],
                    ),
                    (
                        "CustodialHistoryItem",
        ...
        self.assertEqual(
            ...
            (
                ...
                [
                    (
                        "id",
                        [("seda_id", "object", "SEDAid"), ("id", "subject", "String")],
                    ),
                    ("KeywordContent", []),
                    (
                        "KeywordReference",
                    ...
                    (
                        "KeywordType",
                        [
                            ("seda_keyword_type_from", "object", "SEDAKeywordType"),
    ...
    def test_rule(self):
        ...
        edef = next(element_defs)
        self.assertEqual(readable_edef(edef), ("StartDate", "SEDAStartDate", []))
        edef = next(element_defs)
        self.assertEqual(
            readable_edef(edef), ("PreventInheritance", "SEDAPreventInheritance", [])
        )
        self.assertRaises(StopIteration, next, element_defs)
    ...
    def test_binarydataobject(self):
        ...
        children = [
            getattr(e[0].target, "local_name", e[0].target.__class__.__name__)
            for e in edef[2]
        ]
        self.assertEqual(
            children,
        ...
        digest_element = edef[2][7][0].target
        self.assertEqual(edef[2][7][1], [])
        ...
        self.assertEqual(
            ...
            (
                ...
                [
                    ...
                    (
                        "ArchiveUnit",
                        [("seda_archive_unit", "object", "SEDAArchiveUnit")],
                    ),
                    (
                        "DataObjectReference",
                        [
    ...
    def test_dataobjectpackage(self):
        ...
        self.assertEqual(
            ...
            (
                ...
                [
                    ...
                    (
                        "MessageIdentifier",
                        [("message_identifier", "subject", "String")],
                    ),
                    ...
                    (
                        "ArchivalAgency",
                        [("seda_archival_agency", "subject", "AuthorityRecord")],
                    ),
                    (
                        "TransferringAgency",
        ...
        descr_element = dao_edef[2][4][0].target
        ...
        mgmt_element = dao_edef[2][5][0].target
            ...
            (
                ...
                [
                    ...
                    (
                        ...
                        [
                            (
                                "seda_acquisition_information_from",
                                "object",
                                "SEDAAcquisitionInformation",
                            ),
                    ...
                    (
                        ...
                        [
                            (
                                "seda_originating_agency_identifier",
                                "object",
                    ...
                    (
                        ...
                        [
                            (
                                "seda_submission_agency_identifier",
                                "object",
                    ...
                    (
                        "StorageRule",
                        [("seda_storage_rule", "object", "SEDAStorageRule")],
                    ),
                    (
                        "AppraisalRule",
                        [("seda_appraisal_rule", "object", "SEDAAppraisalRule")],
                    ),
                    ...
                    ("LogBook", [("seda_log_book", "object", "SEDALogBook")]),
                    (
                        "NeedAuthorization",
                        [
                            (
                    ...
                    ("HoldRule", [("seda_hold_rule", "object", "SEDAHoldRule")]),
                ],
            ),
        )
    ...
    def test_elements_by_name(self):
        ...
        self.assertEqual(
            mapping.element_by_name("CodeListVersions").local_name, "CodeListVersions"
        )
