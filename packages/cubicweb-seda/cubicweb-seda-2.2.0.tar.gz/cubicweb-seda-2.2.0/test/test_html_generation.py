...
class SEDAHTMLExportFuncTC(SEDAExportFuncTCMixIn, XmlTestMixin, CubicWebTC):
    adapter_id = "SEDA-2.2.html"

    def setup_database(self):
        super().setup_database()
        with self.admin_access.cnx() as cnx:
            scheme = cnx.find("ConceptScheme", title="Keyword Types").one()
    ...
    def get_element(self, profile, name):
        elements = self.xpath(profile, f'//h3[text()="{name}"]')
    ...
    def element_values(self, element):
        ...
        for div in self.xpath(element, "div"):
            ...
            for span in self.xpath(div, "span"):
                if len(span):
                    value = etree.tostring(span[0], encoding=str)
                else:
                    value = span.text
    ...
    def test_profile1(self):
        with self.admin_access.cnx() as cnx:
            ...
            self.assertEqual(
                [
                    ann
                    for ann in self.xpath(profile, '//div[@class="description"]/text()')
            ...
            for attr_div in attr_divs:
                attr_def = AttrDef(
                    self.xpath(attr_div, 'span[@class="label"]')[0].text,
            ...
            self.assertEqual(
                attr_defs,
                {
                    ...
                    AttrDef(
                        label="id",
                        card="mandatory",
                        value=f"id{self.au_eid}",
                    ),
                    AttrDef(
                        label="id",
                        card="mandatory",
                        value=f"id{self.bdo_eid}",
                    ),
            ...
            self.assertEqual(
                el_defs,
                [
                    {"label": "listVersionID", "value": "edition 2009"},
            ...
            concept_link = f'<a href="{some_concept.absolute_url()}">md5</a>'
            scheme = some_concept.scheme
            scheme_link = f'<a href="{scheme.absolute_url()}">Keyword Types</a>'
            self.assertEqual(
                el_defs,
                [
                    {"label": "schemeURI", "value": scheme_link},
                    {"label": "XSD content type", "value": "xsd:token"},
            ...
            self.assertEqual(
                el_defs,
                [
                    {"label": "XSD content type", "value": "xsd:language"},
                    {"label": "fixed value", "value": concept_link},
                ],
            )
