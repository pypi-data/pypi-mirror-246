...
class XmlTestMixin:
    ...
    def check_xsd_profile(self, root, sample_file, **substitutions):
        ...
        if os.environ.get("TEST_WRITE_SEDA_FILES"):
            ...
            print(f"Generated profile saved to {fname}")
...
class XMLSchemaTestMixin(XmlTestMixin):
    ...
    def get_elements(self, profile, name):
        return self.xpath(profile, f'//xs:element[@name="{name}"]')

    def get_attributes(self, profile, name):
        return self.xpath(profile, f'//xs:attribute[@name="{name}"]')
...
class RelaxNGTestMixin(XmlTestMixin):
    ...
    def get_elements(self, profile, name):
        return self.xpath(profile, f'//rng:element[@name="{name}"]')

    def get_attributes(self, profile, name):
        return self.xpath(profile, f'//rng:attribute[@name="{name}"]')
...
class PathTargetValuesTC(CubicWebTC):
    def test_keyword_path(self):
        ...
        with self.admin_access.cnx() as cnx:
            ...
            kw = create("SEDAKeyword", user_cardinality="0..n")
            ...
            kt_scheme = testutils.scheme_for_rtype(cnx, "seda_keyword_type_to", "theme")
    ...
    def test_integrity_cardinality(self):
        with self.admin_access.cnx() as cnx:
            transfer = cnx.create_entity("SEDAArchiveTransfer", title="test profile")
            bdo = cnx.create_entity(
                "SEDABinaryDataObject",
                user_annotation="I am mandatory",
                seda_binary_data_object=transfer,
            ...
            _, _, unit_alt_seq = testutils.create_archive_unit(
                None, cnx=cnx, user_cardinality="1..n"
            )
            bdo = testutils.create_data_object(unit_alt_seq, user_cardinality="0..1")
            assert pg.integrity_cardinality(bdo) == "0..n"
            ...
            _, _, unit_alt_seq2 = testutils.create_archive_unit(
                unit_alt_seq, cnx=cnx, user_cardinality="0..n"
            )
            bdo = testutils.create_data_object(unit_alt_seq2, user_cardinality="1")
            assert pg.integrity_cardinality(bdo) == "0..n"

            _, _, unit_alt_seq = testutils.create_archive_unit(
                None, cnx=cnx, user_cardinality="1"
            )
            _, _, unit_alt_seq2 = testutils.create_archive_unit(
                unit_alt_seq, cnx=cnx, user_cardinality="1..n"
            )
            bdo = testutils.create_data_object(unit_alt_seq2, user_cardinality="0..1")
            assert pg.integrity_cardinality(bdo) == "0..n"

            _, _, unit_alt_seq = testutils.create_archive_unit(
                None, cnx=cnx, user_cardinality="0..1"
            )
            _, _, unit_alt_seq2 = testutils.create_archive_unit(
                unit_alt_seq, cnx=cnx, user_cardinality="1"
            )
            bdo = testutils.create_data_object(unit_alt_seq2, user_cardinality="1..n")
            assert pg.integrity_cardinality(bdo) == "0..n"
...
class SEDA2RNGExportTC(RelaxNGTestMixin, CubicWebTC):
    def test_skipped_mandatory_simple(self):
        with self.admin_access.cnx() as cnx:
            profile = self.profile_etree(
                cnx.create_entity("SEDAArchiveTransfer", title="test profile")
            )
    ...
    def test_skipped_mandatory_complex(self):
        with self.admin_access.cnx() as cnx:
            transfer = cnx.create_entity("SEDAArchiveTransfer", title="test profile")
            testutils.create_data_object(transfer, filename="fixed.txt")
    ...
    def test_fileinfo_card(self):
        with self.admin_access.cnx() as cnx:
            ...
            appname.cw_set(user_cardinality="1")
            profile = self.profile_etree(transfer)
            fileinfo = self.get_element(profile, "FileInfo")
            self.assertElementDefinition(fileinfo, {"name": "FileInfo"})

            appname.cw_set(user_cardinality="0..1")
            bdo.cw_set(filename="fixed.txt")
            profile = self.profile_etree(transfer)
            fileinfo = self.get_element(profile, "FileInfo")
            self.assertElementDefinition(fileinfo, {"name": "FileInfo"})
    ...
    def test_object_package_group(self):
        with self.admin_access.cnx() as cnx:
            ...
            bdo = cnx.create_entity(
                "SEDABinaryDataObject",
                user_annotation="I am number one",
                seda_binary_data_object=transfer,
            )
            cnx.create_entity(
                "SEDABinaryDataObject",
                user_annotation="I am number two",
                seda_binary_data_object=transfer,
            )
            cnx.create_entity(
                "SEDAPhysicalDataObject",
                user_annotation="I am number three",
    ...
    def test_object_data_object_with_group(self):
        with self.admin_access.cnx() as cnx:
            transfer = cnx.create_entity(
                "SEDAArchiveTransfer", title="test profile", wrap_dataobjects=True
            )
            cnx.create_entity(
                "SEDABinaryDataObject",
                user_cardinality="1",
    ...
    def test_object_data_object_without_group(self):
        with self.admin_access.cnx() as cnx:
            transfer = cnx.create_entity(
                "SEDAArchiveTransfer", title="test profile", wrap_dataobjects=False
            )
            cnx.create_entity(
                "SEDABinaryDataObject",
                user_cardinality="1",
        ...
        with self.admin_access.cnx() as cnx:
            profile = self.profile_etree(
                cnx.create_entity(
                    "SEDAArchiveTransfer",
                    title="test profile",
                    user_annotation="some description",
                )
            )
    ...
    def test_keyword(self):
        with self.admin_access.cnx() as cnx:
            ...
            scheme = testutils.scheme_for_rtype(cnx, "seda_keyword_type_to", "theme")
            kw_type = scheme.reverse_in_scheme[0]

            transfer = create("SEDAArchiveTransfer", title="test profile")
            unit, unit_alt, unit_alt_seq = testutils.create_archive_unit(
                transfer, user_cardinality="0..n"
            )

            kw = create(
                "SEDAKeyword", seda_keyword=unit_alt_seq, keyword_content="kwick"
            )
    ...
    def test_code_list(self):
        with self.admin_access.cnx() as cnx:
            transfer = cnx.create_entity("SEDAArchiveTransfer", title="test profile")
            scheme = cnx.create_entity("ConceptScheme", title="Keyword Types")
            cnx.create_entity(
            ...
            profile = self.profile_etree(transfer)
            elements = self.get_elements(profile, "MimeTypeCodeListVersion")
    ...
    def test_multiple_concepts(self):
        with self.admin_access.cnx() as cnx:
            ...
            scheme = testutils.scheme_for_type(
                cnx, None, None, "application/msword", "application/pdf"
            )
            cnx.create_entity(
                "SEDAMimeTypeCodeListVersion",
                seda_mime_type_code_list_version_from=transfer,
            ...
            bdo.mime_type.cw_set(
                user_cardinality="1", seda_mime_type_to=scheme.reverse_in_scheme
            )
  ...
  <rng:choice>
    ...
    def test_seda2_concept(self):
        with self.admin_access.cnx() as cnx:
            create = cnx.create_entity
            scheme = create("ConceptScheme", title="Digest algorithm")
            some_concept = scheme.add_concept(label="md5 algorithm", language_code="en")
            transfer = create("SEDAArchiveTransfer", title="test profile")
            cnx.create_entity(
    ...
    def test_seda_2_1(self):
        with self.admin_access.cnx() as cnx:
            transfer = cnx.create_entity(
                "SEDAArchiveTransfer", title="test profile", wrap_dataobjects=False
            )
            cnx.create_entity("SEDAHoldRule", seda_hold_rule=transfer)
            archiveunitseq = cnx.create_entity(
                "SEDASeqAltArchiveUnitArchiveUnitRefIdManagement",
                reverse_seda_title=cnx.create_entity("SEDATitle", title="poulet"),
            )
            cnx.create_entity("SEDADateLitteral", seda_date_litteral=archiveunitseq)

            archiveunitalt = cnx.create_entity(
                "SEDAAltArchiveUnitArchiveUnitRefId",
                seda_seq_alt_archive_unit_archive_unit_ref_id_management=archiveunitseq,
            )
            cnx.create_entity(
                "SEDAArchiveUnit",
                seda_archive_unit=transfer,
                user_annotation="poulet",
                seda_alt_archive_unit_archive_unit_ref_id=archiveunitalt,
            )

            cnx.commit()
            adapter = transfer.cw_adapt_to(self.adapter_id)
            generated_xsd = adapter.dump(_encoding=str)
            self.assertIn(
                'ns="fr:gouv:culture:archivesdefrance:seda:v2.1"', generated_xsd
            )
            self.assertNotIn("HoldRule", generated_xsd)
            self.assertNotIn("DateLitteral", generated_xsd)


class SEDA22RNGExportTC(RelaxNGTestMixin, CubicWebTC):
    """Mixin extending XmlTestMixin with implementation of some assert methods for RelaxNG."""

    adapter_id = "SEDA-2.2.rng"

    def test_seda_2_2(self):
        with self.admin_access.cnx() as cnx:
            transfer = cnx.create_entity(
                "SEDAArchiveTransfer", title="test profile", wrap_dataobjects=False
            )
            ...
            generated_xsd = adapter.dump(_encoding=str)
            self.assertIn(
                'ns="fr:gouv:culture:archivesdefrance:seda:v2.2"', generated_xsd
            )
            self.assertIn("HoldRule", generated_xsd)
            self.assertIn("DateLitteral", generated_xsd)


class SEDAExportFuncTCMixIn:
    """Test that SEDA profile export works correctly."""

    def setup_database(self):
        with self.admin_access.cnx() as cnx:
            create = cnx.create_entity
            scheme = create("ConceptScheme", title="Keyword Types")
            # ensure we're able to export concept with unexpected language code
            some_concept = scheme.add_concept(label="md5", language_code="de")
            some_other_concept = scheme.add_concept(
                label="hash/md5", language_code="de"
            )
            ...
            access_rule = create(
                "SEDAAccessRule", user_cardinality="0..1", seda_access_rule=transfer
            )
            ...
            bdo = testutils.create_data_object(
                transfer, user_cardinality="0..n", seda_algorithm=some_concept
            )
            bdo.mime_type.cw_set(seda_mime_type_to=[some_concept, some_other_concept])
            create(
                "SEDAFormatLitteral", user_cardinality="0..1", seda_format_litteral=bdo
            )
            create("SEDAEncoding", user_cardinality="0..1", seda_encoding_from=bdo)
            create("SEDAUri", seda_uri=bdo.seda_alt_binary_data_object_attachment)
            # first level archive unit
            unit, unit_alt, unit_alt_seq = testutils.create_archive_unit(
                transfer, user_cardinality="0..n", user_annotation="Composant ISAD(G)"
            )
            # management rules
            ...
            # content
            kw = create(
                "SEDAKeyword", user_cardinality="0..n", seda_keyword=unit_alt_seq
            )
            ...
            create("SEDAwhen", user_cardinality="0..1", seda_when=history_item)
    ...
    def test_profile1(self):
        """Check a minimal SEDA profile validating BV2.1_min.xml."""
        with self.admin_access.cnx() as cnx:
            transfer = cnx.entity_from_eid(self.transfer_eid)
            root = self.profile_etree(transfer)
        self.check_xsd_profile(root, self.datapath("BV2.1_min.xml"))
...
class SEDARNGExportFuncTC(SEDAExportFuncTCMixIn, RelaxNGTestMixin, CubicWebTC):
    def assertProfileDetails(self, root):
        ...
        references = self.xpath(
            root, f'//rng:element[@a:defaultValue="{self.bdo_xmlid}"]'
        )
        ...
        self.assertEqual(
            {elmt.attrib["type"] for elmt in ddt},
            {
                "date",
                "dateTime",
                "gDay",
    ...
    def setup_database(self):
        with self.admin_access.cnx() as cnx:
            ...
            for rtype, etype, labels in [
                ("file_category", None, ["fmt/123", "fmt/987"]),
                ("seda_encoding_to", None, ["6"]),
                ("seda_type_to", None, ["CDO"]),
                ("seda_description_level_to", None, ["file"]),
                ("seda_algorithm", "SEDABinaryDataObject", ["md5"]),
                ("seda_rule", "SEDASeqAppraisalRuleRule", ["P10Y"]),
                ("seda_rule", "SEDASeqAccessRuleRule", ["AR038"]),
                ("seda_final_action", "SEDAAppraisalRule", ["detruire"]),
            ]:
            ...
            # ensure we're able to export concept with unexpected language code
            concepts["md5"].preferred_label[0].cw_set(language_code="de")

            agent = testutils.create_authority_record(
                cnx, "bob", record_id="a_record_id"
            )
            ...
            create(
                "SEDAComment",
                user_cardinality="1",
                comment="my profile description &&",
            ...
            appraisal_rule_rule = create(
                ...
                user_annotation="C'est dans 10ans je m'en irai",
                reverse_seda_start_date=create("SEDAStartDate"),
            )
            create(
                "SEDAAppraisalRule",
                ...
                user_annotation="detruire le document",
            )

            unit, _, unit_alt_seq = testutils.create_archive_unit(
                transfer, user_cardinality="1..n"
            )

            unit_alt_seq.cw_set(
                reverse_seda_start_date=create(
                    "SEDAStartDate", user_cardinality="0..1"
                ),
                reverse_seda_end_date=create("SEDAEndDate", user_cardinality="0..1"),
                # XXX, value=date(2015, 2, 24)),
                reverse_seda_description=create(
                    "SEDADescription", user_cardinality="0..1"
                ),
            )
            ...
            create(
                "SEDAKeywordType", user_cardinality="0..1", seda_keyword_type_from=kw
            )
    ...
    def _test_profile(self, adapter_id, expected_file):
        with self.admin_access.cnx() as cnx:
            ...
            if os.environ.get("TEST_WRITE_SEDA_FILES"):
                ...
                for value, key in [
                    (file_concept.cwuri, "concept-uri"),
                    (file_concept.scheme.cwuri, "scheme-uri"),
                    (str(agent.eid), "agent-id"),
                    (agent.dc_title(), "agent-name"),
                ]:
                    orig_content = orig_content.replace(value, f"%({key})s")
                with open(self.datapath(expected_file + ".new"), "w") as stream:
                    stream.write(orig_content)
                print(f"Regenerated expected file as {expected_file}.new")
            root = etree.fromstring(generated_xsd.encode("utf-8"))
            self.assertXmlValid(root)
            with open(self.datapath(expected_file), "r") as f:
                expected = f.read() % {
                    "unit-eid": str(self.unit_eid),
                    "subunit1-eid": str(self.subunit1_eid),
                    "subunit2-eid": str(self.subunit2_eid),
                    "bdo-eid": str(self.bdo_eid),
                    "kw-eid": str(self.kw_eid),
                    "chi-eid": str(self.chi_eid),
                    "concept-uri": file_concept.cwuri,
                    "scheme-uri": file_concept.scheme.cwuri,
                    "agent-id": str(agent.eid),
...
class OldSEDAXSDExportTC(XMLSchemaTestMixin, OldSEDAExportMixin, CubicWebTC):
    ...
    def _test_profile(self, adapter_id, expected_file):
        adapter, root = super()._test_profile(adapter_id, expected_file)
...
class OldSEDAExportTC(RelaxNGTestMixin, CubicWebTC):
    def test_seda_0_2_bordereau_ref(self):
        ...
        with self.admin_access.cnx() as cnx:
            ...
            transfer = create(
                "SEDAArchiveTransfer", title="test profile", simplified_profile=True
            )
            ...
            kw = create(
                "SEDAKeyword", user_cardinality="0..n", seda_keyword=subunit_alt_seq
            )
            create("SEDAKeywordReference", seda_keyword_reference_from=kw)

            create("SEDASystemId", seda_system_id=subunit_alt_seq)

            bdo = testutils.create_data_object(transfer, user_cardinality="0..1")
            create(
                "SEDADataObjectReference",
            ...
            bdo.mime_type.cw_set(user_cardinality="1")
    ...
    def test_xsd_children_order(self):
        with self.admin_access.cnx() as cnx:
            ...
            self.assertEqual(
                [node.attrib["name"] for node in root], ["Document", "Contains"]
            )
            ...
            adapter.xsd_children(root, unit)
            self.assertEqual(
                [node.attrib["name"] for node in root], ["ArchiveObject", "Document"]
            )

    def test_duplicated_format_id(self):
        with self.admin_access.cnx() as cnx:
            scheme = testutils.scheme_for_type(
                cnx, "file_category", None, "fmt/123", "fmt/123"
            )
            concepts = scheme.reverse_in_scheme

            transfer = cnx.create_entity(
                "SEDAArchiveTransfer", title="my profile", simplified_profile=True
            )
            bdo = testutils.create_data_object(transfer)
            ...
            self.assertEqual(
                [node.text for node in self.xpath(root, "//rng:value")], ["fmt/123"]
            )
