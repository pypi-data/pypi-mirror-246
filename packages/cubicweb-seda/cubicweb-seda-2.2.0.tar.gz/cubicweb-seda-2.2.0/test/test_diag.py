...
class CompatAnalyzerTC(CubicWebTC):
    def test_rules(self):
        with self.admin_access.cnx() as cnx:
            ...
            transfer = create("SEDAArchiveTransfer", title="diagnosis testing")
            doctor = transfer.cw_adapt_to("ISEDACompatAnalyzer")
            cnx.commit()
            self.assertFailingRules(
                doctor, ["SEDA 2.2", "SEDA 2.1", "SEDA 1.0", "SEDA 0.2", "simplified"]
            )
            ...
            self.assertFailingRules(
                doctor,
                ["SEDA 2.2", "SEDA 2.1", "SEDA 0.2", "simplified"],
            ...
            cnx.commit()
            self.assertFailingRules(
                doctor, ["SEDA 2.2", "SEDA 2.1"], "rule_without_rule"
            )
            ...
            access_rule_seq.cw_set(user_cardinality="1")
            start_date = create(
                "SEDAStartDate",
            ...
            cnx.commit()
            self.assertFailingRules(
                doctor, ["SEDA 2.2", "SEDA 2.1"], "rule_start_unsupported_card"
            )

            start_date.cw_set(user_cardinality="1")
            start_date.cw_clear_all_caches()
            access_rule_seq2 = create(
                "SEDASeqAccessRuleRule",
                user_cardinality="1..n",
                reverse_seda_seq_access_rule_rule=access_rule,
            )
            ...
            cnx.commit()
            self.assertFailingRules(
                doctor, ["SEDA 2.2", "SEDA 2.1"], "rule_with_too_much_rules"
            )
            ...
            cnx.commit()
            self.assertFailingRules(
                doctor, ["SEDA 2.2", "SEDA 2.1"], "rule_ref_non_rule_id"
            )
    ...
    def test_custodial_history(self):
        with self.admin_access.cnx() as cnx:
            ...
            create(
                "SEDAStartDate", user_cardinality="1", seda_start_date=access_rule_seq
            )
            ...
            self.assertFailingRules(
                doctor,
                ["SEDA 2.2", "SEDA 2.1", "SEDA 1.0", "simplified"],
    ...
    def test_archive_unit_reference_in_transfer(self):
        with self.admin_access.cnx() as cnx:
            ...
            cnx.commit()
            self.assertFailingRules(
                doctor, ["SEDA 2.1", "SEDA 2.2"], "use_archive_unit_ref"
            )
    ...
    def assertFailingRules(self, doctor, expected_formats, *expected_rule_ids):
        ...
        rule_ids = {rule_id for rule_id, entity in doctor.failing_rules()}
    ...
    def test_multiple_child_unhandled_cardinality_archive_unit(self):
        with self.admin_access.cnx() as cnx:
            ...
            unit2 = testutils.create_archive_unit(transfer, user_cardinality="0..1")[0]
            testutils.create_archive_unit(transfer, user_cardinality="1..n")
            cnx.commit()

            self.assertIsRNGAmbiguous(transfer, True)
            self.assertErrorOn(
                doctor, "rng_ambiguity", [(transfer, "seda_archive_units_tab")]
            )

            unit2.cw_set(user_cardinality="1")
            cnx.commit()
            self.assertIsRNGAmbiguous(transfer, False)

            unit3 = testutils.create_archive_unit(transfer, user_cardinality="0..1")[0]
            cnx.commit()
            self.assertIsRNGAmbiguous(transfer, True)
            self.assertErrorOn(
                doctor, "rng_ambiguity", [(transfer, "seda_archive_units_tab")]
    ...
    def test_multiple_child_unhandled_cardinality_sub_archive_unit(self):
        with self.admin_access.cnx() as cnx:
            ...
            doctor = transfer.cw_adapt_to("ISEDACompatAnalyzer")
            unit, unit_alt, unit_alt_seq = testutils.create_archive_unit(transfer)
            testutils.create_archive_unit(unit_alt_seq, user_cardinality="1..n")
            testutils.create_archive_unit(unit_alt_seq, user_cardinality="1..n")
            cnx.commit()

            self.assertIsRNGAmbiguous(transfer, True)
            self.assertErrorOn(
                doctor, "rng_ambiguity", [(unit, "seda_archive_units_tab")]
            )
    ...
    def test_multiple_child_unhandled_cardinality_document(self):
        with self.admin_access.cnx() as cnx:
            ...
            doctor = transfer.cw_adapt_to("ISEDACompatAnalyzer")
            unit, unit_alt, unit_alt_seq = testutils.create_archive_unit(transfer)
            testutils.create_data_object(
                unit_alt_seq, user_cardinality="0..n", seda_binary_data_object=transfer
            )
            bdo = testutils.create_data_object(
                unit_alt_seq, user_cardinality="1..n", seda_binary_data_object=transfer
            )
            cnx.commit()
            self.assertIsRNGAmbiguous(transfer, True)
            self.assertErrorOn(
                doctor,
                "rng_ambiguity",
                [(unit, "seda_data_objects_tab"), (transfer, "seda_data_objects_tab")],
            )
            ...
            bdo = testutils.create_data_object(
                unit2_alt_seq, user_cardinality="0..n", seda_binary_data_object=transfer
            )
            cnx.commit()
            self.assertIsRNGAmbiguous(transfer, True)
            self.assertErrorOn(
                doctor, "rng_ambiguity", [(transfer, "seda_data_objects_tab")]
            )
            _delete_data_object(bdo)
            cnx.commit()
            self.assertIsRNGAmbiguous(transfer, False)
    ...
    def test_multiple_child_unhandled_cardinality_keyword(self):
        with self.admin_access.cnx() as cnx:
            ...
            unit, unit_alt, unit_alt_seq = testutils.create_archive_unit(transfer)
            cnx.create_entity(
                "SEDAKeyword", seda_keyword=unit_alt_seq, user_cardinality="1..n"
            )
            cnx.create_entity(
                "SEDAKeyword", seda_keyword=unit_alt_seq, user_cardinality="1..n"
            )
    ...
    def test_single_cardinality_not_first(self):
        with self.admin_access.cnx() as cnx:
            transfer = cnx.create_entity("SEDAArchiveTransfer", title="test profile")
            doctor = transfer.cw_adapt_to("ISEDACompatAnalyzer")
            u1 = testutils.create_archive_unit(transfer, user_cardinality="1..n")
            u2 = testutils.create_archive_unit(transfer, user_cardinality="1")
            cnx.commit()
