...
def rtags_from_rtype_role_targets(etype, rtype_role_targets):
    ...
    for rtype, role, target in rtype_role_targets:
        if role == "subject":
            ...
            rec.tag_subject_of(("*", rtype, "*"), {"rvid": "seda.reledit.simplelink"})
        else:
            ...
            rec.tag_object_of(("*", rtype, "*"), {"rvid": vid, "novalue_label": " "})
...
def add_subobject_link(entity, rtype, role, extraurlparams, msg=None, klass=None):
    ...
    assert len(targets) == 1, f"expecting a single {role} for relation {rschema}"
    target_etype = targets[0].type
    if not has_rel_perm("add", entity, rtype, role, target_etype=target_etype):
        return ""
    linkto = f"{rtype}:{entity.eid}:{neg_role(role)}"
    urlparams = {"__linkto": linkto, "__redirectpath": entity.rest_path()}
...
def add_etype_link(req, etype, urlparams, msg="", klass=None):
...
def dropdown_button(text, links):
    ...
    data = UStringIO()
    w = data.write
    w('<div class="btn-group pull-right clearfix">')
    w(
        tags.button(
            " ".join([text, tags.span(klass="caret")]),
            escapecontent=False,
...
def _rel_label(self, entity, rschema, role, dispctrl):
    ...
    relkey = (entity.cw_etype, rschema.type, role)
    try:
        element_name, desc = ETYPE_ATTR_DOC[relkey]
    except KeyError:
        return label
    description = xml_escape(desc[0]) if desc else ""
    if rschema.type in (
        ...
        warning = self._cw._("Only available in SEDA 2.2.")
        description = f"{description} {warning}"
        label = f"{label} <span class='badge badge-warning'>SEDA 2.2</span>".strip()
    title = xml_escape(element_name)
    self._cw.add_onload("$('.popOverLabel').popover();")
    return (
        '<div class="popOverLabel" '
        'data-toggle="popover" data-placement="top" '
        'title="{}" data-content="{}">{}</div>'.format(title, description, label)
    )
