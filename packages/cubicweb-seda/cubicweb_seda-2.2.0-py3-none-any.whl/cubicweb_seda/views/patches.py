...
def __init__(self, *args, **kwargs):
    ...
    if "peid" not in kwargs and self.edited_entity.cw_etype.startswith(
        "SEDA"
    ):  # main form
        parent = None
        if self.edited_entity.has_eid():
            ...
            parent = self._cw.entity_from_eid(
                int(self._cw.form["__linkto"].split(":")[1])
            )
        if parent is not None:
            ...
            self.add_hidden(
                name="sedaContainerEID", value=container.eid, id="sedaContainerEID"
            )
