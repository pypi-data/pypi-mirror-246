...
class ProfileTreeComponent(BaseTreeComponent):
    __select__ = BaseTreeComponent.__select__ & is_instance(
        "SEDAArchiveTransfer",
        "SEDAArchiveUnit",
...
class ArchiveTransferIJQTreeAdapter(SEDAIJQTreeAdapter):
    __select__ = SEDAIJQTreeAdapter.__select__ & is_instance("SEDAArchiveTransfer")

    def maybe_parent_of(self):
        return ["SEDAArchiveUnit", "SEDAPhysicalDataObject", "SEDABinaryDataObject"]
...
class ArchiveUnitIJQTreeAdapter(SEDAIJQTreeAdapter):
    __select__ = SEDAIJQTreeAdapter.__select__ & is_instance("SEDAArchiveUnit")
    ...
    def reparent(self, peid, index):
        ...
        if parent.cw_etype == "SEDAArchiveUnit":
            ...
            assert (
                parent.cw_etype == "SEDAArchiveTransfer"
            ), f"cannot re-parent to entity type {parent.cw_etype}"
...
class SimplifiedArchiveUnitIJQTreeAdapter(ArchiveUnitIJQTreeAdapter):
    ...
    def maybe_parent_of(self):
        return (
            []
            if self.entity.is_archive_unit_ref
            else ["SEDAArchiveUnit", "SEDABinaryDataObject"]
        )
...
class DataObjectIJQTreeAdapter(SEDAIJQTreeAdapter):
    __select__ = SEDAIJQTreeAdapter.__select__ & is_instance(
...
class SimplifiedBinaryDataObjectIJQTreeAdapter(SEDAIJQTreeAdapter):
    __select__ = (
        SEDAIJQTreeAdapter.__select__
        & is_instance("SEDABinaryDataObject")
        & simplified_profile()
    )
