...
class ContainedRelationFacetWidget(rwdg.RelationFacetWidget):
    def _render_triggers(self, w, domid, form, field, rtype):
        ...
        if not req.execute("Any CS WHERE " + rql_expr, {"container": container.eid}):
            ...
            if not scheme_relations:
                w(
                    req._(
                        "there is no scheme available for this relation. "
            ...
            w(
                req._("you must specify a scheme for {0} to select a value").format(
                    scheme_relation
                )
            )
        else:
            return super()._render_triggers(w, domid, form, field, rtype)
    ...
    def trigger_search_url(self, entity, url_params):
        ...
        return super().trigger_search_url(entity, url_params)
...
class ContainedSearchForRelatedEntitiesView(skos.SearchForRelatedConceptsView):
    __select__ = skos.SearchForRelatedConceptsView.__select__ & match_form_params(
        "container"
    )

    def constrained_rql(self):
        container = self._cw.entity_from_eid(int(self._cw.form["container"]))
        rql_expr = scheme_rql_expr(
            container, self.schema_rdef.subject, self.schema_rdef.rtype
        )
        return rql_expr + ", O in_scheme CS", {"container": container.eid}


pvs.tag_object_of(("*", "seda_data_object_reference_id", "*"), "relations")
pvdc.tag_object_of(
...
afs.tag_object_of(("*", "seda_data_object_profile", "*"), "main", "hidden")
pvs.tag_object_of(("*", "seda_data_object_profile", "*"), "attributes")
rec.tag_object_of(
    ("*", "seda_data_object_profile", "*"),
    {"rvid": "seda.reledit.text", "novalue_label": " "},
)
...
for rtype in (
    ...
    rec.tag_object_of(
        ("*", rtype, "*"), {"rvid": "seda.reledit.complexlink", "novalue_label": " "}
    )

rec.tag_subject_of(
    ("SEDABinaryDataObject", "seda_alt_binary_data_object_attachment", "*"),
    {"rvid": "seda.reledit.alternative", "novalue_label": " "},
)

rec.tag_object_of(
    ("*", "seda_compressed", "*"), {"rvid": "seda.reledit.text", "novalue_label": " "}
)
...
affk.tag_attribute(
    ("SEDAUri", "user_cardinality"), {"choices": uri_cardinality_vocabulary}
)
...
bdo_ordered_fields = [
    ...
    ("seda_data_object_profile", "object"),
]
...
def _setup_format_tab(rsection, display_ctrl):
    ...
    # gen time ?)
    rsection.tag_object_of(
        ("*", "seda_format_id_from", "SEDABinaryDataObject"), "hidden"
    )
    rsection.tag_object_of(
        ("*", "seda_mime_type_from", "SEDABinaryDataObject"), "hidden"
    )
    rsection.tag_subject_of(
        ("SEDABinaryDataObject", "file_category", "*"), "attributes"
    )
    # dunno why it crashes without this
    display_ctrl.tag_subject_of(
        ("SEDABinaryDataObject", "file_category", "*"), {"order": 0}
    )
...
class BinaryDataObjectFormatIdentificationTab(viewlib.PrimaryTabWithoutBoxes):
    ...
    rsection, display_ctrl = rtags_from_xsd_element(
        "SEDABinaryDataObject", "FormatIdentification"
    )
...
class SimplifiedBinaryDataObjectFormatIdentificationTab(
    ...
    __select__ = (
        BinaryDataObjectFormatIdentificationTab.__select__ & simplified_profile()
    )
    ...
    _setup_format_tab(rsection, display_ctrl)
    rsection.tag_object_of(
        ("*", "seda_format_litteral", "SEDABinaryDataObject"), "hidden"
    )
...
def file_category_vocabulary(form, field):
    ...
    for category, category_label, extension, extension_label in rset:
        ...
        values.append((f"{category_label} > {extension_label}", str(extension)))
    return values


affk.tag_subject_of(
    ("SEDABinaryDataObject", "file_category", "*"),
    {"choices": file_category_vocabulary},
)
rec.tag_subject_of(
    ("SEDABinaryDataObject", "file_category", "*"), {"novalue_label": " "}
)
...
class PhysicalDataObjectDimensionsTab(viewlib.PrimaryTabWithoutBoxes):
    ...
    rsection, display_ctrl = rtags_from_xsd_element(
        "SEDAPhysicalDataObject", "PhysicalDimensions"
    )
...
class DataObjectRelationsTab(viewlib.PrimaryTabWithoutBoxes):
    ...
    def entity_call(self, entity):
        ...
        if rset:
            self.w(f"<h2>{self._cw._('Relationship target of')}</h2>")
            self.w(
                "<div>{}</div>".format(
                    self._cw._(
            ...
            self._cw.view(
                "list", rset=rset, subvid="seda.relationship.reverse", w=self.w
            )
...
class CompressedBusinessValueEntityView(viewlib.BusinessValueEntityView):
    ...
    def entity_value(self, entity):
        ...
        if entity.seda_algorithm:
            ...
            value += self._cw._(", using {algorithm}").format(algorithm=algorithm)
        return value
