...
def _climb_rule_holders(transfer_or_archive_unit):
    ...
    while transfer_or_archive_unit is not None:
        if (
            transfer_or_archive_unit.cw_etype
            == "SEDASeqAltArchiveUnitArchiveUnitRefIdManagement"
        ):
            yield transfer_or_archive_unit
            transfer_or_archive_unit = itree.parent_archive_unit(
                transfer_or_archive_unit
            )
        ...
        transfer_or_archive_unit = transfer_or_archive_unit.cw_adapt_to(
            "ITreeBase"
        ).parent()
...
def _inherited_rule(self, rule_type):
    ...
    rtype = f"reverse_seda_{rule_type}_rule"
...
class SEDAArchiveUnit(generated.SEDAArchiveUnit):
    ...
    def first_level_choice(self):
        ...
        """
        return self.related(
            "seda_alt_archive_unit_archive_unit_ref_id", "subject"
        ).one()
...
class SEDABinaryDataObject(generated.SEDABinaryDataObject):
    ...
    def format_id(self):
        return (
            self.reverse_seda_format_id_from[0]
            if self.reverse_seda_format_id_from
            else None
        )
    ...
    def encoding(self):
        return (
            self.reverse_seda_encoding_from[0]
            if self.reverse_seda_encoding_from
            else None
        )
    ...
    def mime_type(self):
        return (
            self.reverse_seda_mime_type_from[0]
            if self.reverse_seda_mime_type_from
            else None
        )
    ...
    def referenced_by(self):
        ...
        for ref in self.reverse_seda_data_object_reference_id:
            yield from ref.seda_data_object_reference
...
class SEDAAltArchiveUnitArchiveUnitRefId(generated.SEDAAltArchiveUnitArchiveUnitRefId):
    ...
    def content_sequence(self):
        """Return the sequence element holding content for an archive unit which
        is not a reference"""
        rset = self.related(
...
class SEDASeqAltArchiveUnitArchiveUnitRefIdManagement(
    ...
    def language(self):
        return (
            self.reverse_seda_language_from[0]
            if self.reverse_seda_language_from
            else None
        )
    ...
    def description_level_concept(self):
        return (
            self.reverse_seda_description_level_from[0]
            if self.reverse_seda_description_level_from
            else None
        )
    ...
    def description(self):
        return (
            self.reverse_seda_description[0] if self.reverse_seda_description else None
        )
...
class SEDAKeyword(generated.SEDAKeyword):
    ...
    def reference(self):
        return (
            self.reverse_seda_keyword_reference_from[0]
            if self.reverse_seda_keyword_reference_from
            else None
        )
    ...
    def type(self):
        return (
            self.reverse_seda_keyword_type_from[0]
            if self.reverse_seda_keyword_type_from
            else None
        )
...
class SEDAKeywordReference(generated.SEDAKeywordReference):
    ...
    def scheme(self):
        return (
            self.seda_keyword_reference_to_scheme[0]
            if self.seda_keyword_reference_to_scheme
            else None
        )
    ...
    def concept(self):
        return (
            self.seda_keyword_reference_to[0]
            if self.seda_keyword_reference_to
            else None
        )
...
class RuleMixIn:
    ...
    @property
    def rules(self):
        return getattr(self, f"seda_seq_{self._rule_type}_rule_rule")

    @property
    def inheritance_control(self):
        alt = getattr(self, f"seda_alt_{self._rule_type}_rule_prevent_inheritance")
...
class SEDAHoldRule(RuleMixIn, generated.SEDAHoldRule):
    pass
...
class SEDASeqClassificationRuleRule(
    RuleRuleMixIn, generated.SEDASeqClassificationRuleRule
):
    pass


class SEDASeqDisseminationRuleRule(
    RuleRuleMixIn, generated.SEDASeqDisseminationRuleRule
):
    pass
...
class SEDADescriptionLevel(generated.SEDADescriptionLevel):
    @property
    def concept(self):
        return (
            self.seda_description_level_to[0]
            if self.seda_description_level_to
            else None
        )
...
class SEDAOriginatingAgency(generated.SEDAOriginatingAgency):
    ...
    def agency(self):
        return (
            self.seda_originating_agency_to[0]
            if self.seda_originating_agency_to
            else None
        )
