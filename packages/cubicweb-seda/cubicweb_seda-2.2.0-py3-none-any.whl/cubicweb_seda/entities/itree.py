...
class IContainedToITreeBase(EntityAdapter):
    """Map IContained adapters to ITreeBase, additionaly configured with a list
    of relations leading to contained's children.
    ...
    def iterchildren(self):
        ...
        for rel, role in self._children_relations:
            yield from self.entity.related(rel, role, entities=True)
...
class ITreeBaseArchiveUnitAdapter(IContainedToITreeBase):
    ...
    def iterchildren(self):
        ...
        if seq is None:
            ...
            return
        for rel, role in self._children_relations:
            yield from seq.related(rel, role, entities=True)
...
def move_child_at_index(cnx, parent_eid, child_eid, index, reparenting=False):
    ...
    assert reparenting or child.ordering not in (
        index,
        index + 1,
    ...
    if reparenting:
        rql = (
            "SET X ordering XO + 1 WHERE X ordering XO, "
            "X ordering > %(index)s, "
...
def reparent(entity, new_parent_eid, index):
    ...
    move_child_at_index(entity._cw, new_parent_eid, entity.eid, index, reparenting=True)
