...
def attribute_cardinality_as_string(occ, card_entity):
    """Return 'optional' or 'mandatory' for the given pyxst attribute's
    Occurence. Cardinality may be overriden by the data model's user_cardinality
    value."""
    cardinality = getattr(card_entity, "user_cardinality", None)
    if cardinality is None:
        minimum = occ.minimum
...
class SEDA2HTMLExport(SEDA2ExportAdapter):
    ...
    def element_alternative(
        ...
        div = self.element("div", profile_element, attributes={"class": "choice"})
    ...
    def element_sequence(
        ...
        div = self.element("div", profile_element, attributes={"class": "sequence"})
    ...
    def element_xmlattribute(
        ...
        div = self.element("div", profile_element, attributes={"class": "attribute"})
        card = self._cw._(attribute_cardinality_as_string(occ, card_entity))
        self.element(
            "span", div, text=occ.target.local_name, attributes={"class": "label"}
        )
    ...
    def element_xmlelement(
        ...
        if annotation:
            self.element(
                "div", div, text=annotation, attributes={"class": "description"}
            )
    ...
    def fill_element(self, xselement, div, target_value, card_entity, xstypes=None):
        if xselement.local_name == "KeywordType":
            ...
            self.element(
                "span", list_div, text="listVersionID", attributes={"class": "label"}
            )
            if target_value:
                list_value = (
                    target_value.scheme.description or target_value.scheme.dc_title()
                )
            else:
                list_value = "edition 2009"
            self.element(
                "span", list_div, text=list_value, attributes={"class": "value"}
            )
        ...
        if xstypes:
            ...
            self.element(
                "span",
                ct_div,
                text=self._cw._("XSD content type"),
                attributes={"class": "label"},
            )
            xstypes = self._cw._(" ALT_I18N ").join(
                "xsd:" + xstype for xstype in xstypes
            )
        ...
        for value in values:
            ...
            if value_div is None:
                ...
                self.element(
                    "span",
                    value_div,
                    text=self._cw._("fixed value"),
                    attributes={"class": "label"},
                )
            if isinstance(fixed_value, str):
                self.element(
                    "span", value_div, text=fixed_value, attributes={"class": "value"}
                )
            else:
    ...
    def concept_scheme_attribute(self, xselement, div, scheme):
        ...
        self.element(
            "a",
            self.element("span", div, attributes={"class": "value"}),
            {"href": scheme.absolute_url()},
            text=scheme.dc_title(),
        )
    ...
    def title(self, div, label, occ, card_entity):
        ...
        self.element(
            ...
            text=f"[{self._cw._(element_uml_cardinality(occ, card_entity))}]",
        )
