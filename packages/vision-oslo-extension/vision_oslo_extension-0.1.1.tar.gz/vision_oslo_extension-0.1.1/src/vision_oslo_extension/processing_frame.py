...
from vision_oslo_extension import post_processing
from vision_oslo_extension import batch_processing
from vision_oslo_extension import average_load
from vision_oslo_extension import protection_if
    ...
    def run_excution_in_thread(self, sim_name, main_option, time_start, time_end, option_select, text_input, low_v, high_v, time_step):
        try:    
            ...
            if not continue_process:
                ...
                print("ERROR: Process terminated unexpectly. Please Contact Support. You can continue use other options...")
...
class P05(BasePage):
    def __init__(self, parent, controller):
        ...
        explain2 = tk.Label(master=self.headframe, text = 'NOTE: Import "Excel - For Protection" if needed ',font = controller.text_font)
        explain2.pack()
...
class P07(BasePage):
    def __init__(self, parent, controller):
        ...
        # add widgets here
        head = tk.Label(master=self.headframe, text = 'Page 7: Substation TRU Assessment Data Prepare',font = controller.sub_title_font)
        head.pack()
...
class P08(BasePage):
    def __init__(self, parent, controller):
        ...
        # add widgets here
        head = tk.Label(master=self.headframe, text = 'Page 8: Substation Protection/Track CB & ETE Assessment Data Prepare',font = controller.sub_title_font)
        head.pack()
...
class F11(BasePage):
    def __init__(self, parent, controller):
        ...
        button = tk.Button(master=self.excuteframe, text="RUN!", command = lambda: self.run_post_processing(option1,input1,input3),width = 20, height =2)
        button.pack()
    ...
    def run_post_processing(self,option1,input1,input3):
        try:
            # so that sim_name is updated when clicked
            sim_name = SharedVariables.sim_variable.get() # call variables saved in a shared places.
            #main_option = SharedVariables.main_option.get()
