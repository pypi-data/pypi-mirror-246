...
async def get_project_folder_entity(
    metadata_url: str, name: str, _type: str, zone: str, project_code: str, query_path: str, status: str
):
    payload = {
        'name': name,
        query_path: 'shared',
...
async def has_file_permission(
    ...
    if root_folder == 'shared':
        ...
        project_folder = await get_project_folder_entity(
            ...
            project_code=project_code,
            query_path=path_for_permissions,
            zone=zone,
            status=file_entity.get('status', 'ACTIVE'),
        )
