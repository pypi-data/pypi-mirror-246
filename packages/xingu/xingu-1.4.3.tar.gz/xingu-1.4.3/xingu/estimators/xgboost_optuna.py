...
class XinguXGBoostClassifier(xingu.Estimator):
    ...
    def fit(self, datasets, features, target, model=None):
        ...
        for task in concurrent.futures.as_completed(tasks):
            ...
            val = (
                pandas.DataFrame(index=tasks[task])
                .assign(
                    **{
                        f'estimation_class_{model.dp.proba_class_index}':
                        xgb.predict_proba(
                            datasets['train'].iloc[tasks[task]][features]
                        )[:, model.dp.proba_class_index]
                    }
                )
            )
            ...
            if model.sets_estimations['validation'] is None:
                model.sets_estimations['validation'] = val
            else:
