...
from trytond.model import (
    Check, DeactivableMixin, Index, Model, ModelSQL, ModelView, Unique, fields)
...
from .exceptions import (
    CalendarNameValidationError, EventInvalidRecurrenceError,
    InvalidRRuleError)
...
class Calendar(DeactivableMixin, ModelSQL, ModelView):
    "Calendar"
    ...
    owner = fields.Many2One('calendar.resource', 'Owner',
        states={
            'readonly': Bool(Eval('events', [0])),
            },
        depends=['events'], ondelete='RESTRICT')
    ...
    events = fields.One2Many('calendar.event', 'calendar', 'Events')

    _get_name_cache = Cache('calendar_calendar.get_name', context=False)
    ...
    def __setup__(cls):
        ...
        cls._sql_constraints = [
            ('owner_type_uniq', Unique(table, table.owner, table.type),
                'calendar.msg_owner_type_unique'),
            ]
    ...
    def write(cls, calendars, values, *args):
        super().write(calendars, values, *args)
        # Restart the cache for get_name
        cls._get_name_cache.clear()
    ...
    def check_name(self):
        ...
        if self.name.endswith('.ics'):
            raise CalendarNameValidationError(
                gettext('calendar.msg_calendar_invalid_name',
                    name=self.name))
            ...
            if event_ical.vevent.rruleset:
                for freebusy_dtstart in event_ical.vevent.rruleset:
                    ...
                    if not ((f_dtstart_tz <= dtstart
                                and f_dtend_tz >= dtstart)  # noqa: E127
...
class CalendarResource(ModelSQL, ModelView):
    "Calendar Resource"
    __name__ = 'calendar.resource'

    email = fields.Char("Email", required=True)
    resource = fields.Reference("Resource", selection='get_resource',
        states={
            'readonly': Bool(Eval('calendars', [0])),
            },
        depends=['calendars'],
        help='The resource owning the calendar.')
    calendars = fields.One2Many('calendar.calendar', 'owner',
        'Calendars')
    ...
    def get_rec_name(self, name):
        ...
        if self.resource:
            if isinstance(self.resource, str):
                model_name, name = self.resource.split(',')
                rec_name = Model.get_name(model_name)
            else:
                rec_name = self.resource.rec_name
            rec_name += ' | '
        rec_name += self.email
        return rec_name
    ...
    def search_rec_name(cls, name, clause):
        return [('resource.rec_name',) + tuple(clause[1:])]

    @classmethod
    def _get_resource(cls):
        'Return list of model names for resource reference'
        CalendarModel = Pool().get('calendar.model')
        models = CalendarModel.search([
                ('company', '=', Transaction().context.get('company')),
                ])
        return [m.model.model for m in models]
    ...
    def resource_name(self):
        return self.resource.rec_name if self.resource else None


class CalendarModel(ModelSQL, ModelView):
    'Calendar Model per Company'
    __name__ = "calendar.model"
...
class Event(ModelSQL, ModelView):
    ...
    dtstart = fields.DateTime('Start Date', required=True,
        domain=[If((Eval('dtstart')) & (Eval('dtend')),
                ('dtstart', '<=', Eval('dtend')),
                (),
                )
            ],
    ...
    def __setup__(cls):
        super().__setup__()
        cls.__access__.add('calendar')
        table = cls.__table__()
        cls._sql_constraints = [
            ('uuid_recurrence_uniq',
                Unique(table, table.uuid, table.calendar, table.recurrence),
                'UUID and recurrence must be unique in a calendar.'),
            ]
        cls._sql_indexes.update({
                Index(table, (table.dtstart, Index.Range())),
                Index(table, (table.dtend, Index.Range())),
                Index(table, (table.recurrence, Index.Range())),
                })
    ...
    def check_recurrence(self):
        ...
        if self.parent:
            if self.rdates \
                    ...
                    or self.exrules \
                    or self.occurences:
                raise EventInvalidRecurrenceError(
                    gettext('calendar.msg_calendar_event_invalid_recurrence',
                        name=self.rec_name))
    ...
    def copy(cls, events, default=None):
        ...
        for event in events:
            ...
            new_events.extend(super().copy([event],
...
class EventAttendee(AttendeeMixin, ModelSQL, ModelView):
    ...
    def create(cls, vlist):
        ...
        if to_write:
            Event.write(Event.browse(to_write), {})
        event_attendees = super().create(vlist)
        for event_attendee in event_attendees:
            event = event_attendee.event
            if (event.calendar.owner
                    and (event.organizer == event.calendar.owner.email
                        or (event.parent
                            and event.parent.organizer ==   # noqa: W504
                            event.parent.calendar.owner.email))):
                if event.organizer == event.calendar.owner.email:
                    attendee_emails = [x.email for x in event.attendees
                            if x.email != event.organizer]
        ...
        for attendee in event_attendees:
            ...
            if (event.calendar.owner
                ...
                if attendee_emails:
                    ...
                    and ((event.organizer
                            or (event.parent and event.parent.organizer))  # noqa: E127, E501
...
class DateMixin:
    ...
    def get_rec_name(self, name):
        ...
        if self.datetime:
            lang = Lang.get()
            return lang.strftime(self.datetime)
        return ''
...
class RRuleMixin(Model):
    ...
    freq_string = freq.translated('freq')
    ...
    def get_rec_name(self, name):
        return self.freq_string if self.freq_string else ''
            ...
            for second in self.bysecond.split(','):
                ...
                if not (second >= 0 and second <= 59):
                    raise InvalidRRuleError(
                        gettext('calendar.msg_calendar_rrule_invalid_bysecond',
                            name=self.rec_name))
        ...
        if self.byminute:
            for minute in self.byminute.split(','):
                ...
                if not (minute >= 0 and minute <= 59):
                    raise InvalidRRuleError(
                        gettext('calendar.msg_calendar_rrule_invalid_byminute',
                            name=self.rec_name))
        ...
        if self.byhour:
            for hour in self.byhour.split(','):
                ...
                if not (hour >= 0 and hour <= 23):
                    raise InvalidRRuleError(
                        gettext('calendar.msg_calendar_rrule_invalid_byhour',
                            name=self.rec_name))
        ...
        if self.byday:
            for weekdaynum in self.byday.split(','):
                ...
                if weekday not in ('SU', 'MO', 'TU', 'WE', 'TH', 'FR', 'SA'):
                    raise InvalidRRuleError(
                        gettext('calendar.msg_calendar_rrule_invalid_byday',
                            name=self.rec_name))
                ordwk = weekday[:-2]
                if not ordwk:
                    continue
        ...
        if self.bymonthday:
            for monthdaynum in self.bymonthday.split(','):
                ...
                if not (abs(monthdaynum) >= 1 and abs(monthdaynum) <= 31):
                    raise InvalidRRuleError(
                        gettext(
                            'calendar.msg_calendar_rrule_invalid_bymonthday',
                            name=self.rec_name))
        ...
        if self.byyearday:
            for yeardaynum in self.byyearday.split(','):
                ...
                if not (abs(yeardaynum) >= 1 and abs(yeardaynum) <= 366):
                    raise InvalidRRuleError(
                        gettext(
                            'calendar.msg_calendar_rrule_invalid_byyearday',
                            name=self.rec_name))
        ...
        if self.byweekno:
            for weeknum in self.byweekno.split(','):
                ...
                if not (abs(weeknum) >= 1 and abs(weeknum) <= 53):
                    raise InvalidRRuleError(
                        gettext('calendar.msg_calendar_rrule_invalid_byweekno',
                            name=self.rec_name))
        ...
        if self.bymonth:
            for monthnum in self.bymonth.split(','):
                ...
                if not (monthnum >= 1 and monthnum <= 12):
                    raise InvalidRRuleError(
                        gettext('calendar.msg_calendar_rrule_invalid_bymonth',
                            name=self.rec_name))
        ...
        if self.bysetpos:
            for setposday in self.bysetpos.split(','):
                ...
                if not (abs(setposday) >= 1 and abs(setposday) <= 366):
                    raise InvalidRRuleError(
                        gettext('calendar.msg_calendar_rrule_invalid_bysetpos',
                            name=self.rec_name))
