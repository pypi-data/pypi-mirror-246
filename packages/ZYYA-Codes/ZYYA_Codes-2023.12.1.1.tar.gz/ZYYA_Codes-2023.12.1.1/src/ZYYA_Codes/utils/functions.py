    ...
    while kwargs.get("stoptime", 10) >= run >= 1:
        try:
            for Folder in conn.list()[1]:
                ...
                if imapclient.imap_utf7.decode(Folder).split('"|"')[0].split(' ')[-1].split('"')[1] not in ["草稿箱", "垃圾邮件",
                                                                                                 "已删除"]:
                    Exist = [x["序号"] for x in
                             Email.find(
                                 {"文件夹": imapclient.imap_utf7.decode(Folder).split('"|"')[0].split(' ')[-1].split('"')[1]},
                                 {"序号": 1}
                             )
                             ]
                    ...
                    for index in To_fetch[::-1]:
                        ...
                        Email.insert_one({"序号": index,
                                          ...
                                          "附件": [
                                              ...
                                              if item.get_param("name") and (type(item.get_param("name")) == str or type(
                                                  item.get_param("name")
                                              ) == bytes)],
                                          "文件夹": imapclient.imap_utf7.decode(Folder).split('"|"')[0].split(' ')[-1].split('"')[
                                              1]})

                        print("---邮件数据更新中---",
                              imapclient.imap_utf7.decode(Folder).split('"|"')[0].split(' ')[-1].split('"')[1],
                              index,
                              to_datetime_localize(decode_str(msg.get("date")).split(", ")[-1].split(" (")[0]) if msg.get(
                                  "date") else "",
