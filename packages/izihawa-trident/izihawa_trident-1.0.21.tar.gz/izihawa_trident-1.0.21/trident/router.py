...
@router.get("/kv/{key}")
async def read(request: TridentFastAPIRequest, key: str) -> Response:
    points = list(
...
async def exists(request: TridentFastAPIRequest, key: str) -> dict:
    for point in request.app.state.hash_ring.range(key, config["replicas"].get(int), unique=True):
        file_shard: FileShard = point["instance"]
