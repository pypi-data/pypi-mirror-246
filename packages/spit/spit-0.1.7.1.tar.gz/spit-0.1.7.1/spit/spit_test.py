...
def jitter_matrix(m):
    m = m + np.random.uniform(-0.005, 0.005, m.shape)
    m = np.clip(m, 0, 1)
    return m
...
def run_mann_whitney_u(m_1, m_2):
    ...
    mwu_method = 'auto'
    print(m_1.shape, len(m_1))
    if((len(m_1)<=8) or (len(m_2)<=8)):
        print('loyloy')
        mwu_method = 'exact'
    mwu_results = sts.mannwhitneyu(m_1, m_2, method = mwu_method, alternative='two-sided', axis = 0, nan_policy = 'omit')
    if(np.isnan(mwu_results[1]).all()):
        min_p_val = 1
    else:
        min_p_val = np.nanmin(mwu_results[1])
    return mwu_results[1], round(min_p_val, 32)
...
def mannwhitneyu_permutation(IFs, samples, num_of_it, n_small):
    min_perm_p_arr = []
    perm_p_arr = []
    sampled_txs = set()
...
def main(args):
    ...
    if (len(samples) < 8):
        samples = pheno.id.to_list()
    min_perm_p_arr, perm_p_medians = mannwhitneyu_permutation(IFs, samples, args.n_iter, args.n_small)
    if(args.exp):
        write_dir = args.exp
    else:
