...
def fit_kde_and_mannwhitney(ctrl_subgroup, case_subgroup, b):
    ...
    mwu_method = 'auto'
    print(case_subgroup.shape, len(case_subgroup))
    if((len(case_subgroup)<=8) or (len(ctrl_subgroup)<=8)):
        print('hey')
        mwu_method = 'exact'
    r = sts.mannwhitneyu(case_subgroup, ctrl_subgroup, method = mwu_method, alternative='two-sided')
