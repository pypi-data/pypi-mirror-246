...
def jitter_matrix(m):
    m = m + np.random.uniform(-0.001, 0.001, m.shape)
    m = np.clip(m, 0, 1)
    return m
...
def run_mann_whitney_u(m_1, m_2):
    ...
    if(((len(m_1)) <= 8) or ((len(m_2)) <= 8)):
        contrast_method = 'exact'
    m_1 = jitter_matrix(m_1)
    m_2 = jitter_matrix(m_2)
    mwu_results = sts.mannwhitneyu(m_1, m_2, method = contrast_method, alternative='two-sided', axis = 0, nan_policy = 'omit')
