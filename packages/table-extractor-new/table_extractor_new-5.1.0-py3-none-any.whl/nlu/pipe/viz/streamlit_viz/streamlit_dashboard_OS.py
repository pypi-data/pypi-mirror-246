...
from nlu.pipe.viz.streamlit_viz.viz_building_blocks.sentence_embedding_manifold import \
    SentenceEmbeddingManifoldStreamlitBlock
from nlu.pipe.viz.streamlit_viz.viz_building_blocks.entity_embedding_manifold import \
    EntityEmbeddingManifoldStreamlitBlock
...
class StreamlitVizBlockHandler():
    ...
    def viz_streamlit_dashboard(
        ...
        for viz in active_visualizers:
            if 'ner' == viz:
                ...
                NERStreamlitBlock.visualize_ner(ner_pipe, text, generate_code_sample=show_code_snippets,
                                                key=key + '_ner', show_model_select=False, show_text_input=True,
                                                show_logo=False, show_infos=False)
            if 'dependency_tree' == viz:
                tree_pipe = StreamlitUtilsOS.get_pipe(
                    'en.dep.typed')  # if not ValidateVizPipe.viz_tree_satisfied(component_list) else component_list
                DepTreeStreamlitBlock.visualize_dep_tree(tree_pipe, text, generate_code_sample=show_code_snippets,
                                                         key=key + '_dep', show_infos=False, show_logo=False)
            if 'token_features' == viz:
                ner_pipe = pipe if pipe.nlu_ref == ner_model_2_viz else StreamlitUtilsOS.get_pipe(ner_model_2_viz)
                TokenFeaturesStreamlitBlock.visualize_tokens_information(ner_pipe, text,
                                                                         generate_code_sample=show_code_snippets,
                                                                         key=key + '_tok',
                                                                         model_select_position=model_select_position,
                                                                         show_infos=False, show_logo=False, )
            if 'classification' == viz:
                ner_pipe = pipe if pipe.nlu_ref == ner_model_2_viz else StreamlitUtilsOS.get_pipe(ner_model_2_viz)
                ClassifierStreamlitBlock.visualize_classes(ner_pipe, text, generate_code_sample=show_code_snippets,
                                                           key=key + '_class',
                                                           model_select_position=model_select_position,
                                                           show_infos=False, show_logo=False)
            if 'similarity' == viz:
                ...
                WordSimilarityStreamlitBlock.display_word_similarity(ner_pipe, similarity_texts,
                                                                     ...
                                                                     model_select_position=model_select_position,
                                                                     show_infos=False, show_logo=False,
                                                                     num_cols=num_similarity_cols, key=key + '_sim')
            if 'manifold' == viz:
                ner_pipe = pipe if ner_model_2_viz in pipe.nlu_ref.split(' ') else StreamlitUtilsOS.get_pipe(
                    ner_model_2_viz)
                WordEmbeddingManifoldStreamlitBlock.viz_streamlit_word_embed_manifold(ner_pipe, similarity_texts,
                                                                                      generate_code_sample=show_code_snippets,
                                                                                      model_select_position=model_select_position,
                                                                                      show_infos=False, show_logo=False,
    ...
    def viz_streamlit_entity_embed_manifold(
            ...
            default_texts: List[str] = ("Donald Trump likes to visit New York", "Angela Merkel likes to visit Berlin!",
    ...
    def viz_streamlit_sentence_embed_manifold(
        ...
        SentenceEmbeddingManifoldStreamlitBlock.viz_streamlit_sentence_embed_manifold(
            pipe,
            default_texts,
            title,
            sub_title,
