    ...
    for i in range(len(device_types)):
        if device_types[i] < -0.5 and device_types[i] > -1.5:
            ...
            if not isForbiddens[i] and reachable_check_flag and sigma_mask[-1, i] and not human_mask[i]:
                tmp_rc_data_diff = rc_data_res_diff[np.abs(
                    rc_data_res_diff[:, i]) > 0.001, i]  # 把不为0的数提取出来
                if tmp_rc_data_diff.shape < 3:
                    rc_bound = [0, 0]
                else:
                    rc_bound = np.percentile(
                        tmp_rc_data_diff, [percentile_down, percentile_up])  # 把不为0的数提取出来
                rc_mask = np.logical_and(np.logical_or(
                    rc_data_res_diff[-(ab2normal_times[i]-1):, i] < rc_bound[0], rc_data_res_diff[-(ab2normal_times[i]-1):, i] > rc_bound[1]), mad[i] > ab_mad_threshold)
