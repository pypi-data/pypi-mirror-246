...
def is_display_math(x):
    if x is None:
        return False
    display_envs = ['$$', 'align', 'align*', 'eqnarray', 'eqnarray*', 'equation', 'equation*', 'gather', 'gather*',
                    'multline', 'multline*', 'displaymath', 'flalign', 'flalign*', 'alignat', 'alignat*']
...
def traverse_tex(tex, parent_node_list=None, depth=0):
    ...
    if isinstance(tex, str):
        ...
        if AGG_STRINGS and len(parent_node_list) > 0 and parent_node_list[-1].cat == LatCat.STR:
            ...
            parent_node_list.append(Treenode(
                cat=LatCat.STR,
                token=tex
            ))
        return
...
def perform_checks(source: str, language: SupportedLanguages = SupportedLanguages.EN, debug_mode=False):
    ...
    re_nonsymbolic_reference = re.compile(r"(рис(унок|унка|унке|\.)|формул(а|е|ой|у|ы)|(равенств|тождеств)(о|а|е|у|ами|ах)|(соотношени|выражени)(е|ю|и|я|ями|ях|ям))\s+\(?\d+\)?", re.IGNORECASE)
    ...
    for node in all_nodes:
        ...
        if is_display_math(node):
            ...
            if node.prev_sibling and (node.prev_sibling.cat == LatCat.STR) and node.prev_sibling.token.startswith(
                    '\\\\'):
        ...
        if (node.cat == LatCat.CMD and node.token in ['in', 'notin', 'ni', 'subset', 'subseteq']
                and node.next_node and node.next_node.token and (node.next_node.token.strip()+' ')[0] in 'NRZQC'):
        ...
        if not node.is_in_math and node.cat == LatCat.STR and not is_pure_argument(node):
            ...
            if re_capitalization_after_comma.search(node.token) or (re_starts_with_uppercase.match(node.token) and node.prev_node and node.prev_node.cat == LatCat.STR and node.prev_node.token.strip().endswith(',')):
                add_error('CAPITALIZATION_AFTER_PUNCTUATION_MARK', node)
            if re_capitalization_after_period.search(node.token) or (re_starts_with_lowercase.match(node.token) and node.prev_node and node.prev_node.cat == LatCat.STR and node.prev_node.token.strip().endswith('.')):
        ...
        if node.cat == LatCat.ENV and node.token == 'math' and node.next_sibling and node.next_sibling.cat == LatCat.ENV and node.next_sibling.token == 'math':
            add_error('UNNECESSARY_FORMULA_BREAK', node)
        ...
        if node.cat == LatCat.CMD and node.token == 'cite' and (
                ...
                or
                node.next_sibling and is_math(node.next_sibling)
                ):
    ...
    for node in all_nodes:
        if (node.cat == LatCat.ENV) and is_display_math(node.token) and node.next_sibling and (
                node.next_sibling.cat == LatCat.ENV) and is_display_math(node.next_sibling.token):
            add_error('CONSECUTIVE_DISPLAY_FORMULAE', node.next_sibling.pos or node.pos)
    ...
    for node in all_nodes:
        if node.cat == LatCat.CMD and node.token in ['label'] and node.args and len(node.args) == 1 and node.args[0].tex not in used_labels:
            add_error('NUMBERED_MATH_NEEDS_REFERENCING', node)
