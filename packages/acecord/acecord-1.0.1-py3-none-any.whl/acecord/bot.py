...
class Bot(discord.Bot):
    ...
    **kwargs: Any (optional) Any keyword arguments to pass to the `discord.Client` class
    ...
    async def _sync_cmds(self) -> None:
        ...
        if self._debug:
            self.logger.logger("Syncing slash commands", "acecord", "debug")
    ...
    async def _ready_func(self) -> None:
        """Not to be used by the user"""
        self.logger.logger(f"Bot is ready", "acecord", "info")
        self.logger.logger(f"Guild(s): {len(self.guilds)}", "acecord", "info")
        self.logger.logger(
            f"acecord version: {_version} | Pycord version: {discord.__version__}", "acecord", "debug")
    ...
    def run(self, token: str = None) -> None:
        ...
        self.logger.logger("Starting bot...", "acecord", "info")
    ...
    def _load_config(self) -> dict:
        ...
        if not os.path.exists("acecord.json"):
            with open("acecord.json", "w") as f:
                json.dump({
                    "token": "",
                    "debug": True,
                    "log_file": False,
            ...
            self.logger.logger("Created config file", "acecord", "info")
            exit(0)

        with open("acecord.json", "r") as f:
            config = json.load(f)
    ...
    def get(self, key: str, default: Any = None) -> Union[Any, None]:
        """Get a key from the `acecord.json`
    ...
    def start_time(self) -> float:
        ...
        if self._start_time is None:
            return self.logger._force_logger("Bot is not running, start_time will be None", "acecord", "error")
    ...
    def uptime(self) -> float:
        ...
        if self._start_time is None:
            return self.logger._force_logger("Bot is not running, uptime will be None", "acecord", "error")
