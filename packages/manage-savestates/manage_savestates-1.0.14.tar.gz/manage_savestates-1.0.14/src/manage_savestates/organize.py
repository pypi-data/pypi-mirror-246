...
from manage_savestates import utilities
from manage_savestates.utilities import Directory
...
def organize():
    ...
    for directory in dirs:
        if not os.path.exists(directory.path):
            print(f"{dir.path} could not be found. Please verify path in settings.")
        elif dir.action is not None:
            log_path = f"{directory}/log.txt"
            raw_gz_files = sorted([x for x in os.listdir(directory)
                                   if (x.endswith(".gzs") or x.endswith(".gzm"))
                                   and not x.startswith(".")])
            states, macros = [], []
            if not os.path.exists(f"{directory}/_other"):
                os.mkdir(f"{directory}/_other")
            ...
            for file in raw_gz_files:
                ...
                if (len(packaged_file.prefix) == 4
                    ...
                    os.remove(f"{directory}/{packaged_file.original_name}")
                else:
            ...
            if directory.action == "reorder":
                reorder_files(directory, states, macros)
            elif directory.action == "trim":
                trim_files(directory, states + macros)
