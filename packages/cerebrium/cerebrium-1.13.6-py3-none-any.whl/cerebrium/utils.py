...
def confirm_deployment(config: datatypes.CerebriumConfig, cerebrium_function):
    """
...
def legacy_to_toml_structure(
    ...
    hardware = datatypes.CerebriumHardware(
        gpu=legacy_config.get("gpu", datatypes.DEFAULT_GPU_SELECTION),
    ...
    if (
        ...
        dependencies = update_from_file("requirements.txt", dependencies, "pip")
    else:
    ...
    if (
        ...
        dependencies = update_from_file("pkglist.txt", dependencies, "apt")
    ...
    if (
        ...
        dependencies = update_from_file("conda_pkglist.txt", dependencies, "conda")
    else:
    ...
    if save_to_file:
        save_config_to_toml_file(new_config, config_file, overwrite=overwrite)
...
def load_config(
    config_file: str, disable_confirmation: bool = False
) -> datatypes.CerebriumConfig:
    """
    ...
    def toml_dict_to_config(config: dict) -> datatypes.CerebriumConfig:
        config_dict = copy.deepcopy(config)
        if "local_files" in config_dict:
            config_dict.pop("local_files")
        if "cerebrium_version" in config_dict:
        ...
        return datatypes.CerebriumConfig(
    ...
    if os.path.exists(config_file):
        if config_file.endswith(".yaml") or config_file.endswith(".yml"):
            ...
            config = legacy_to_toml_structure(config, config_file, save_to_file=False)
            legacy = True
        else:
            with open(config_file, "r") as f:
                config = toml.load(f)
                config = toml_dict_to_config(config=config)
        ...
        if os.path.exists(config_toml_file):
            with open(config_toml_file, "r") as f:
                config = toml.load(f)
                config = toml_dict_to_config(config_dict=config)
        else:
            # try load from yaml
            ...
            if os.path.exists(config_yaml_file):
                ...
                config = legacy_to_toml_structure(
                    config, config_yaml_file, save_to_file=False
                )
                legacy = True
            else:
                cerebriumLog(
    ...
    if os.path.exists(os.path.join(base_dir, "requirements.txt")):
        deps = update_from_file(os.path.join(base_dir, "requirements.txt"), deps, "pip")
    if os.path.exists(os.path.join(base_dir, "pkglist.txt")):
        deps = update_from_file(os.path.join(base_dir, "pkglist.txt"), deps, "apt")
    if os.path.exists(os.path.join(base_dir, "conda_pkglist.txt")):
        deps = update_from_file(
            os.path.join(base_dir, "conda_pkglist.txt"), deps, "conda"
        )

    if deps:
        original_deps = config.dependencies.to_dict()
        ...
        if original_deps != deps and not legacy and not disable_confirmation:
            if not disable_confirmation:
                if typer.confirm(
                    ...
                    config.dependencies = datatypes.CerebriumDependencies(**deps)
                    save_config_to_toml_file(config, config_file, overwrite=True)

    if legacy and not disable_confirmation:
        # Confirm and upgrade to toml.
        # If the user doesn't confirm, we just return the legacy config
        if typer.confirm(
            colored(
            ...
            config = legacy_to_toml_structure(
                config, "cerebrium.toml", save_to_file=True, legacy=True
            )
...
def update_from_file(file: str, requirements: dict, key: str, confirm=False) -> dict:
    ...
    if confirm:
        if typer.confirm(
            ...
            requirements[key] = parse_requirements(file)
        else:
            return requirements
    else:
        requirements[key] = parse_requirements(file)
