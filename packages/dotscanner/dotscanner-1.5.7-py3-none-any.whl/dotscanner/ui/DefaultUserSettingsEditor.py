...
class DefaultUserSettingsEditor:
    def __init__(self, rootWindow, userSettings):
        self.window = tk.Toplevel(rootWindow)
        self.window.title(strings.DEFAULT_CONFIGURATIONS_EDITOR_WINDOW_TITLE)
        width, _ = ui.getWindowDimensions(
            self.window.winfo_screenheight(), self.window.winfo_screenwidth())
        if width > 650:
            width = 650
        self.window.geometry(
            f"{width}x170+{cfg.WINDOW_X + 20}+{cfg.WINDOW_Y + 20}")
        ...
        self.labelSelectedPath = tk.Label(
            self.window, text="No default filepath selected", bg="white", fg="lightgray")
        ...
        self.buttonClearDefaultPath = tk.Button(
            self.window, text="Clear", fg="red", command=self.clearDefaultPath)
        self.buttonSaveDefaultPath = tk.Button(
            self.window, text="Save", fg="blue", command=self.saveDefaultPath)
        ...
        self.buttonEdit = tk.Button(
            self.window, text="Edit...", command=cm.showEditConfigFileDialog)
        self.buttonReset = tk.Button(
            self.window, text="Reset", fg="red", command=cm.showResetConfigFileDialog)
    ...
    def browseFolders(self):
        chosenFolder = filedialog.askdirectory(
            initialdir=self.filepath, title="Select a default starting folder")
        if chosenFolder not in EMPTY_DIRECTORY_SET:
            ...
            self.labelSelectedPath.configure(
                text=displayedFolder, bg="white", fg="black")
    ...
    def showFilepath(self):
        if self.filepath in EMPTY_DIRECTORY_SET:
            self.labelSelectedPath.configure(
                text="No default filepath selected", fg="lightgray", bg="white")
        else:
