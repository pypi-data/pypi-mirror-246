...
class ThresholdAdjuster:
    def __init__(self, rootWindow, microscopeImage, userSettings, skipButton=True, drawUi=True):
        self.index = 0
        ...
        self.window.title(strings.THRESHOLD_ADJUSTER_WINDOW_TITLE)
        height, _ = ui.getWindowDimensions(
            self.window.winfo_screenheight(), self.window.winfo_screenwidth())
        self.window.geometry(
            f"{height + 88}x{height}+{cfg.WINDOW_X}+{cfg.WINDOW_Y}")

        self.windowScaling = height / 550

        self.figure, self.axes = pl.subplots()
        self.dataPlot = self.axes.imshow(self.image.data, cmap="gray",
                                         vmin=userSettings.lowerContrast,
                                         vmax=userSettings.upperContrast * np.std(self.image.data))
        self.dotScatter = self.axes.scatter([None], [None], s=5 * userSettings.dotSize * self.windowScaling,
                                            facecolors="none", edgecolors=cfg.DOT_COLOR,
                                            linewidths=cfg.DOT_THICKNESS)
        self.blobScatter = self.axes.scatter([None], [None], s=2 * userSettings.blobSize * self.windowScaling,
