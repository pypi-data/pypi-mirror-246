...
class UserSettings:
    def __init__(self, rootWindow):
        ...
        _, width = ui.getWindowDimensions(
            self.window.winfo_screenheight(), self.window.winfo_screenwidth())
        self.window.title(strings.CONFIGURATIONS_WINDOW_TITLE)
        self.window.geometry(f"{width}x280+{cfg.WINDOW_X}+{cfg.WINDOW_Y}")
        ...
        self.buttonEditDefaults = tk.Button(self.window, text="Edit defaults...", command=partial(
            self.editDefaults, rootWindow), fg="red")
    ...
    def editDefaults(self, rootWindow):
        DefaultUserSettingsEditor(rootWindow, self)
