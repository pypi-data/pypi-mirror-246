...
class gateFutures(baseFuturesExchange.BaseFuturesExchange):
    ...
    def setQuantMultiplier(self, ticker):
        self.quanto_multiplier = float(self.fetch_ticker(self._getSymbol(ticker))["quanto_multiplier"])
    ...
    def create_order(self, symbol, type, side, amount, price=None, params={}):
        try:
            ...
            if not self.quanto_multiplier:
                self.setQuantMultiplier(ticker=ticker)
            amount = float(amount) / self.quanto_multiplier
            ...
            response = self._signedRequest(method='POST', request_path=apiUrl, queryString=None, body=body)
