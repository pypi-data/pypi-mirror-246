...
This module provides pump and probe focus characterization.

Acquire a set of through-focus images of the pump and probe beams by moving a
stage-mounted camera. The camera is controlled via FLIR Spinnaker API, and the
...
def read_setup(par_name=None, file_name='setup.xml'):
    """Read setup XML file."""
    try:
        setup = json.load(open(file_name))
...
def cam_focus_test(
        ...
        use_shutters=True, dry_run=False):
    ...
    if use_cam:
        ...
        if cam_id is not None:
            ...
            flir_sn_vid_bug = False
            print("Number of cameras detected: {:d}".format(len(cam_list)))
            for cam_ind, cam2 in enumerate(cam_list):
                list_cam_sn = cam2.GetUniqueID()
                print("\t{:d}: {:s}".format(cam_ind, list_cam_sn))
                if list_cam_sn.find('VID_') != -1:
                    flir_sn_vid_bug = True
                if list_cam_sn == cam_sn:
                    cam = cam2

            if flir_sn_vid_bug:
                print("WARNING: USB VID returned instead of camera SN. This is a known "
                      "FLIR Spinaker API bug. The requested camera will likely be not "
                      "detected even if the requested SN is corrected. You can pass an "
                      "explicit camera index using 'cam_id'. In a system with two "
                      "cameras pass cam_id=1 if the default behaviour opens the "
                      "incorrect camera. Another workaround is to replug all "
                      "cameras or reboot the system.")

        if cam is None:
            cam = cam_list[0]
            print("WARNING: Camera with SN {:s} not found. ".format(cam_sn) +
                  "Using first available camera with SN {:s}".format(cam.GetUniqueID()))
