...
# -*- coding: utf-8 -*-
#==========================================================================
# HARPIA Microscopy Module Python routines
#--------------------------------------------------------------------------
#
# Stock dependencies: os, xmltodict, numpy, matplotlib, scipy, PIL
# Custom dependencies: lklib
#
...
def GetXYZOverlapScanData(path='.', mode='pinhole', verbose=False, WithCube=False):
    ...
    try:
        ...
        for ind,file_name in enumerate(file_names):
            ...
            try:
                plt.figure()
                fr = FitXYZScanXYProfile(I=I, X=X, Y=Y, pxsz=pxsz, crop_area=fit_crop_area, \
...
def FitXYZScanXYProfile(**kwargs):
    return fit_gaussian_2d(**kwargs, return_fwhm=True)
