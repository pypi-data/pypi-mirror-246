...
from .assets import (
    AssetList,
    get_portfolio_mean_return,
    get_portfolio_risk,
    annualize_return,
    annualize_risk,
    rebalanced_portfolio_return_ts,
    approx_return_risk_adjusted,
)
...
class EfficientFrontierReb(AssetList):
    def __init__(self, symbols=[default_ticker], ccy="USD", period="year"):
        if len(symbols) < 2:
            raise ValueError("The number of symbols cannot be less than two")
        super().__init__(symbols, curr)
        self.period = period
    ...
    def max_return(self):
        ...
        def objective_function(w, ror, per):
            ...
            accumulated_return = (returns + 1.0).prod() - 1.0
    ...
    def _maximize_return(self, target_risk: float) -> float:
        ...
        # Calculate points of EF given optimal weights
        cagr = (1.0 - objective_function(weights.x, ror, period)) ** (12 / ror.shape[0]) - 1.0
