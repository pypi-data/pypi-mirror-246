...
def build_script():
    ...
    parser.add_argument("--skip", action="store_true", help="Ignore les pdf déjà présents.")
    args = parser.parse_args()

    # configuration file #
    config = load_config(args.config)
    build(config, skip_pdf=args.skip)


def build(config, skip_pdf=False, skip_latex=False):
    ####################
    # open students db #
    ####################
    database = FileHandler(config["paths"].get("database", "./etudiants.json"))
    ...
    for etudiant_i, e in enumerate([v for k, v in etudiants.items()]):
        ...
        if bool(FileHandler(pdf_to)) and skip_pdf:
            print(" déjà compilé")
            continue
        ...
        replacements = [("SED_VARIABLES", "".join([f"{chr(92)}def{chr(92)}{k}{{{v}}}\n" for k, v in variables.items()])), ("SED_FICHE_NUMBER", str(e["id"])), ("SED_ADDRESS", "\\\\\n    ".join(config["latex"]["address"])), ("SED_CITY", config["latex"]["city"]), ("SED_NAME", config["latex"]["name"])]
