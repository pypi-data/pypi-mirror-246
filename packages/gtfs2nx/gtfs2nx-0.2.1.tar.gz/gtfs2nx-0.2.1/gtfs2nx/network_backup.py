...
# logger.info('STEP X/12 - Determine route variations (e.g. inward & outward)...')
# stop_times = _determine_route_variations(stop_times)
# def _determine_route_variations(stop_times):
#     """Determine route variations (e.g. inward & outward & express) based on stop sequence."""
#     stop_times = stop_times.sort_values(['trip_id', 'stop_sequence'])
#     trip_stops = stop_times.groupby('trip_id')['stop_id'].apply(tuple).rename('stop_sequence').reset_index().set_index('trip_id')
#     route_variation = trip_stops.groupby('stop_sequence').ngroup().rename('route_variation_id').reset_index()
#     stop_times = pd.merge(stop_times, route_variation, on='trip_id')
#     return stop_times


# def determine_trip_direction(trips, stop_times):
#     stop_times = stop_times.sort_values(['route_id', 'trip_id', 'stop_sequence'])
#     trip_stops = stop_times.groupby(['route_id', 'trip_id'])['stop_id'].apply(tuple).rename('stop_sequence').reset_index().set_index('trip_id')
#     directions = trip_stops.groupby('route_id')['stop_sequence'].unique().apply(determine_direction).values
#     directions_flat = [[k, int(v)] for d in directions for k, v in d.items()]
#     directions_df = pd.DataFrame(directions_flat, columns=['stop_sequence', 'direction'])
#     trip_direction = pd.merge(trip_stops.reset_index(), directions_df, how='left', on='stop_sequence')
#     trips_w_direction = pd.merge(trips, trip_direction[['trip_id', 'stop_sequence', 'direction']], how='left', on='trip_id')
#     return trips_w_direction


# def same_direction(ref_seq, seq):
#     i = list(set(ref_seq) & set(seq))

#     inward_ref = ref_seq.index(i[0]) - ref_seq.index(i[1]) > 0
#     inward = seq.index(i[0]) - seq.index(i[1]) > 0

#     return inward_ref == inward



# def determine_direction(unique_trips, ref_seq=None):
#     directions = {}
#     later = set()

#     # heuristic: start the search for overlapping trips with the longest trip
#     if ref_seq is None:
#         ref_seq = list(max(unique_trips, key=len))

#     # determine direction of trips with at least 2 common stops
#     for seq in unique_trips:

#         if len(set(ref_seq) & set(seq)) > 1:
#             directions[seq] = same_direction(ref_seq, seq)
#         else:
#             later.add(seq)

#     # print(f'{len(directions.keys())} identified, {len(later)} missing.')

#     # determine direction of remaining trips
#     for new_ref, d in directions.copy().items():

#         if not later:
#             break

#         if not d:
#             # reverse direction to align with original ref_seq
#             new_ref = new_ref[::-1]

#         # print('Recursion.')
#         directions |= determine_direction(later, new_ref)
#         later -= set(directions.keys())

#     # if later:
#         # SHOULD ONLY BE CALLED OF OUTERMOST FUNCTION CALL
#     #     raise Exception(f'Direction of {len(later)} trips could not be identified. Likely due to missing overlap with all other trips.')
...
# def _create_unique_route_stop_ids_old(routes, trips, stops, stop_times):
    ...
    # else:
    # stop_times = pd.merge(stop_times, trips[['trip_id', 'route_id', 'route_type', 'route_short_name']], on='trip_id')
    # stop_times['new_stop_id'] = stop_times['stop_id'] + ID_SEP + stop_times['route_id'] + ID_SEP + stop_times['route_variation_id'].astype(str)

    #####
    # stop_times = pd.merge(stop_times, trips, on='trip_id')
    # stop_times['new_stop_id'] = stop_times['stop_id'] + ID_SEP + stop_times['route_id']
    # stops = pd.merge(stops, stop_times[['stop_id', 'new_stop_id', 'route_id', 'route_type', 'route_short_name']].drop_duplicates(), on='stop_id')
