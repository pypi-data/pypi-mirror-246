...
from .ChannelConstants import (EMAIL_CHANNEL_ID, SMS_MESSAGE_CHANNEL_ID,
                               WHATSAPP_CHANNEL_ID)
...
class MessageLocal(Item, GenericCRUD):
    ...
    def get_message_template_dict_by_campaign_id(self, campaign_id: int) -> dict:
        ...
        if campaign_id is None:
            return {}
        # lang_code: sms_body_template, email_subject_template, email_body_html_template, whatsapp_body_template
    ...
    def _set_body_after_text_template(self, body: str = None, to_recipients: List[Recipient] = None) -> None:
        ...
        for recipient in to_recipients:
            if self.campaign_id:
                body = self.message_template_dict_by_campaign_id.get(self.campaign_id).get(recipient.get_preferred_language())
            elif body is None:
