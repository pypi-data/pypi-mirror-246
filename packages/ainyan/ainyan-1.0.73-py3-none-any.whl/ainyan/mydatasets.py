...
def dataset_preprocess(source_folder, dataset_save_path, include, exclude):
    ...
    print(f'Saving data to:{dataset_save_path}')
...
def main():
    ...
    parser.add_argument('--language', choices=['en', 'jp'], help="Keep Only Given Language.", required=False)
    # only required when
    parser.add_argument('--profile', required=False, help='aws profile to use for upload')
    args = parser.parse_args()

    if args.language is not None:
        print(f'Settings language to: {args.language}')
        global language
