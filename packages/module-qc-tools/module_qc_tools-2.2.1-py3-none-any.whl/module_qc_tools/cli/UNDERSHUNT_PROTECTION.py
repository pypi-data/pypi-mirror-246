...
def run(data, USP_config, NOMINAL_config, ps, yr, meter, nt, layer):
    ...
    eye, status = yr.eyeDiagram(skipconfig=False)

    # Configure
    status = yr.configure()

    # Check ADC ground
    if USP_config["use_calib_adc"]:
        check_adc_ground(yr, USP_config)
        ...
        for chip in range(yr._number_of_chips):
            ...
            for v_mux in USP_config["v_mux"]:
                mea_chips = read_vmux(
                    ...
                    v_mux=v_mux,
                    chip_position=chip,
                    use_adc=USP_config["use_calib_adc"],
                )
            ...
            for i_mux in USP_config["i_mux"]:
                mea_chips = read_vmux(
                    meter,
                    yr,
                    USP_config,  # Or should it be nominal?
                    i_mux=i_mux,
                    chip_position=chip,
                    use_adc=USP_config["use_calib_adc"],
                )
...
def main(
    ...
    USP_config = copy.deepcopy(config["tasks"]["GENERAL"])
    USP_config.update(config["tasks"]["UNDERSHUNT_PROTECTION"])
    USP_config["use_calib_adc"] = use_calib_adc
