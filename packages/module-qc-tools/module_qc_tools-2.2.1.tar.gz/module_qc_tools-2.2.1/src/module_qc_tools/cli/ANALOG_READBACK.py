...
def run_tmeas(data, analog_readback_config, ps, yr, meter, nt, layer):
    ...
    max_n_measure = len(data[0]["Vmux14"]) if data[0] else 0
    n_measure = 0
...
def run_vdda_vddd_vs_trim(data, analog_readback_config, ps, yr, meter, layer):
    ...
    for v_mux, trim_name in vmux_to_trim.items():
        ...
        config_trim, status = yr.read_register(name=trim_name)
        ...
        for trim in analog_readback_config[trim_name]:
            ...
            eye, status = yr.eyeDiagram(skipconfig=True, testsize=100000)
        ...
        for chip in range(yr._number_of_chips):
            ...
            yr.write_register(
                name=vmux_to_trim[v_mux], value=config_trim[chip], chip_position=chip
            )
            eye, status = yr.eyeDiagram(skipconfig=True)
    ...
    max_n_measure = len(data[0]["Vmux34"]) if data[0] else 0
