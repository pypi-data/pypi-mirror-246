...
class RequirementsParser:
    def __init__(self, req_tracker, index_url):
        self.session = Session()
        self.finder = get_package_finder(self.session, index_url)
        self.preparer = get_requirement_preparer(self.finder, self.session, req_tracker)
        ...
        if req.link:
            if is_vcs_url(req.link):
                ...
                raise NotImplementedError(
                    "Requirement `{}` is not in a supported format".format(str(req))
                )
