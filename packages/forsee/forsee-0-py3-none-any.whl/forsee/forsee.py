...
import sys
import forsee_functions as c4
from sre_constants import FAILURE, SUCCESS
import args_handler as handler


#cces_log_file = "C:/Users/Nikolaos/cces/2.12.0/.metadata/.plugins/org.eclipse.cdt.ui/testAnalyser_Core0.build.log"

defaultPath = c4.set_def_path()

arguments = sys.argv[1:]

projects = c4.handle_arguments(arguments)
...
for i in range(0,len(projects)):
    ...
    print(projects[i])
    cces_log_file = c4.get_project_path(projects[i],defaultPath)
    if(cces_log_file == FAILURE):
        handler.print_error("Path not Found")
        exit(1)


    #file is too populated...here we filter only the lines we need(command and file)
    filtered_lines = c4.filter_data(cces_log_file)


    directory = c4.get_directory(cces_log_file)
    command = ""
    file = ""
    jsonformat = []
    ...
    if(c4.create_compiledb(output_file_path,jsonformat) == SUCCESS):
        handler.print_success("You can find you compile_commands.json at:")
        print("         "+directory)
    else:
        handler.print_error()
