...
class LoxIntelligentRoomControllerV2:
    ...
    def set_value(self, stateName, value):
        if self._device_type == "IRoomControllerV2" and stateName == "activeMode":
            ...
            _LOGGER.debug("id:'{0}', name:'{1}', [activeMode] - state={2} '{3}'".format(self._id, self._name, value, value_names[value]))
            ...
            _LOGGER.debug("id:'{0}', name:'{1}', [operatingMode] - state={2} '{3}'".format(self._id, self._name, value, value_names[value]))
            ...
            _LOGGER.debug("id:'{0}', name:'{1}', [prepareState] - state={2} '{3}'".format(self._id, self._name, value, value_names[value + 1]))
            ...
            _LOGGER.debug("id:'{0}', name:'{1}', [overrideReason] - state={2} '{3}'".format(self._id, self._name, value, value_names[value]))
            ...
            value = int(value)
            value_names = ["0 = Automatic, heating and cooling allowed","1 = Automatic, only heating allowed","2 = Automatic, only cooling allowed","3 = Manual, heating and cooling allowed","4 = Manual, only heating allowed","5 = Manual, only cooling allowed"]
            _LOGGER.debug("id:'{0}', name:'{1}', [currentMode] - state={2} '{3}'".format(self._id, self._name, value, value_names[value]))
