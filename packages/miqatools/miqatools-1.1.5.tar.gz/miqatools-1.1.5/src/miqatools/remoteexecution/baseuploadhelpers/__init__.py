...
def upload_file(bucket, filepath, server, subfolder, cloud_provider='google', org_config_id=None, api_key=None):
    url = get_upload_url(bucket, filepath, server, subfolder, cloud_provider=cloud_provider, org_config_id=org_config_id)
...
def get_upload_url(bucket, filepath, server, subfolder, cloud_provider='google', org_config_id=None):
    remote_url = get_remote_url(bucket, filepath, server, subfolder, cloud_provider=cloud_provider, org_config_id=org_config_id)
    url = requests.get(remote_url).json().get('url')
    return url
