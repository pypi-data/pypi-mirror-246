...
case_insensitive_collation = "utf8mb4_bin"
...
def match_type(
    ...
    if "oneOf" in schema:
        ...
        for item in oneOfSchema:
            ...
            if not "format" in oneOfElement:
                ...
                all_ip = False
        if all_ip:
            return sqlalchemy.String(length=45)
        return sqlalchemy.dialects.mysql.MEDIUMTEXT()  # type: ignore
    ...
    if type_name == "integer":
        ...
        if "enum" in schema:
            ...
            # ENUM type should be case insensitive
            detected_type.collation = case_insensitive_collation

        elif "format" in schema and schema["format"] == "date-time":
            detected_type = sqlalchemy.DATETIME(timezone=False)

        elif "format" in schema and schema["format"] == "uuid":
            detected_type = sqlalchemy.BINARY(length=16)
        elif "format" in schema and (
            schema["format"] == "ipv4" or schema["format"] == "ipv6"
        ):
            detected_type = sqlalchemy.VARCHAR(length=45)

        elif "maxLength" in schema:
            max_length = typing.cast(int, schema["maxLength"])
            detected_type = sqlalchemy.String(length=max_length)
        else:
            # MySQL TEXT type has a maximum length of 65,535 bytes
            # At this point we don't know the exact length of the string, so we use MEDIUMTEXT
            # which should be enough for our purposes.
            # ref. https://dev.mysql.com/doc/refman/8.0/en/string-type-syntax.html
            detected_type = sqlalchemy.dialects.mysql.MEDIUMTEXT()  # type: ignore
