...
class DAPSession:
    ...
    async def download_resources(
        ...
        downloads = [self.download_resource(resource, output_directory) for resource in resources]
        local_files = await gather_n(downloads, concurrency=10)
        logger.info(f"Files from server downloaded to folder: {output_directory}")
        return local_files

    async def download_resource(self, resource: Resource, output_directory: str) -> str:
        """
        Save a single remote file to a local directory.

        :param resource: Recourse to download.
    ...
    async def download_table_data(
        ...
        downloaded_files = await self.download_resources(list(resources.values()), directory)
