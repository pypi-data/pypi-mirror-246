...
from ..internal import dyConfig, t
...
async def _check_blacklist(interaction: discord.Interaction) -> bool:
    ...
    if interaction.user.id in bans and dyConfig.blacklist:
        if dyConfig.blacklist.raise_error:
            raise Blacklisted()
        else:
...
class Blacklist(Cog, hidden=True):
    ...
    async def cog_check(self, ctx):
        if dyConfig.blacklist.owner_only:
    ...
    if PYCORD:
        admin = discord.SlashCommandGroup(
            t("admin_group"),
            description="accord admin commands",
            guild_ids=dyConfig.admin_guilds,
