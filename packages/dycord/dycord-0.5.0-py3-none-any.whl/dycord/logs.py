...
def _format_log_colors(log_format: str, file: bool, final_colors: dict[int, str]) -> dict[int, str]:
    """Checks if the is sent to a file and formats the colors dycordingly."""
...
class _ColorFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord):
        """Adds colors to log messages and formats them dycordingly.
...
async def _send_discord_log(webhook_url: str, record: logging.LogRecord, msg):
    async with aiohttp.ClientSession() as session:
        ...
        name = "dycord" if record.name == DEFAULT_LOG else record.name
        try:
            await webhook.send(
                content=msg,
                username=f"{name} Log",
