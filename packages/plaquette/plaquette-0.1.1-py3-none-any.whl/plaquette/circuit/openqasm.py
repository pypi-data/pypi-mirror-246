...
def convert_to_openqasm(
    ...
    Raises:
        GateNotSupportedError: If the input circuit contains an unsupported gate.
    """
    ...
    two_qubit_syntax_instructions = {
        "CX": "cx",
        "CZ": "cz",
    }

    measure_syntax_insructions = {
        "M": "measure",
    }
    ...
    gate_mapping = {
        plaquette_instr: openqasm_instr
        for d in gate_mapping_dicts
        for plaquette_instr, openqasm_instr in d.items()
    }
    ...
    for gate, qubits in circuit.gates:
        ...
        if gate in one_qubit_syntax_instructions:
            for qubit in qubits:
                qasm_lines.append(f"{qasm_gate} q[{qubit}];")
        elif gate in two_qubit_syntax_instructions:
            for i in range(0, len(qubits), 2):
                control_qubit, target_qubit = qubits[i], qubits[i + 1]
                qasm_lines.append(f"{qasm_gate} q[{control_qubit}],q[{target_qubit}];")
        elif gate in measure_syntax_insructions:
            for qubit in qubits:
