...
class CodeConfig:
    ...
    def instantiate(self) -> codes.Code:
        ...
        match self.name:
            case "ShorCode":
                return codes.Code.make_shor()
            case "FiveQubitCode":
                return codes.Code.make_five_qubit()
            case "RepetitionCode":
                return codes.Code.make_repetition(self.size)
            case "PlanarCode":
                return codes.Code.make_planar(self.size)
            case "RotatedPlanarCode":
                return codes.Code.make_rotated_planar(self.size)
            case _:
...
class QubitErrorsConfig:
    ...
    @property
    def simulated_errors(self) -> tuple[dict, tuple[bool, str]] | None:
        ...
        return sim_qub_errors_distr, (self.load_file, self.data_path)
...
class _GateErrorMetadata:
    ...
    Raises:
        ValueError: If :attr:`.distribution` is ``"user"`` and ``params``,
            ``induced_errors`` are not empty.
        ...
        ValueError: If distribution is ``"constant"`` and the corresponding params is
            not of length 1.
        ValueError: If distribution is ``"gaussian"`` and the corresponding params is
            not of length 2.
...
class ErrorsConfig:
    ...
    def instantiate(self, code: codes.Code) -> ErrorData:
        """Instantiate a :class:`~.ErrorData` object with the objects' config.

        Args:
            code : The code being simulated.
...
class CircuitConfig:
    ...
    def instantiate(
        ...
        Args:
            ...
            n_rounds: The number of measurement rounds.
...
class DeviceConfig:
    ...
    def instantiate(self, **kwargs) -> Device:
        """Instantiate a :class:`.Device` with the backend in the objects' config.
...
class DecoderConfig:
    ...
    Raises:
        ValueError: If name is not in :attr:`VALID_DECODERS`.

    Examples:
        >>> conf = DecoderConfig(name="FusionBlossomDecoder")
        >>> print(conf)
        DecoderConfig(name='FusionBlossomDecoder')
    """
    ...
    def instantiate(self, code: codes.Code, ed: ErrorData, rounds: int):
        ...
        Args:
            ...
            rounds: The number of measurement rounds.


        Returns:
            A :class:`~plaquette.decoders.PyMatchingDecoder` or
            :class:`~plaquette.decoders.FusionBlossomDecoder` object.

        Examples:
            >>> conf =  DecoderConfig.from_dict(dict(name="PyMatchingDecoder"))
            >>> c = codes.Code.make_five_qubit(n_rounds=1)
            >>> errors = ErrorData.from_lattice(c)
            >>> dec = conf.instantiate(code =c, ed=errors)
        """
        decoder_class = getattr(decoders, self.name)
        ret_val = decoder_class(code, ed.error_data_dict, rounds)
        return ret_val
...
class ExperimentConfig:
    ...
    _decoder: Type[decoders.AbstractDecoder] | None = None
    ...
    def build_errors(self) -> None:
        ...
        """
        self._errors = self.errors_conf.instantiate(self._code)  # type: ignore
    ...
    def generate_empty_error_csv(self, path: str):
        ...
        return generate_empty_qubit_errors_csv(
            code=cast(codes.Code, self._code), csv_path=path
        )
    ...
    @property
    # TODO: update once new API is compatible with visualizer:
    def visualizer(self, code=None) -> visualizer.Visualizer:
        """Get the `~plaquette.visualizer.LatticeVisualizer` object."""
        if code is None:
            code = self._code
        return visualizer.Visualizer(code)
