...
from plaquette.pauli import Factor
...
class QECCircuitGenerator:
    ...
    def measure_op(self, edge_idxs: Sequence[int], with_errors: bool = True):
        """Measure an operator defined on the code.

        .. seealso:: :meth:`measure_stabgens` for the order in which gates
            are laid out for a given operator.

        Args:
            edge_idxs: edges contributing to the measurement operator.
            with_errors: toggle to indicate if gate errors are to be applied
        """
        ancilla = {
            max(self.code.orchestration_graph.nodes_connected_by(i)) for i in edge_idxs
        }
        if len(ancilla) != 1:
            raise ValueError("Incorrect configuration of the orchestration graph!")
        ancilla = ancilla.pop()
        ...
        edges: dict[int, pauli.Factor] = {
            min(self.code.orchestration_graph.nodes_connected_by(edge)): Factor[
                self.code.orchestration_graph.edges[edge].type
            ]
    ...
    def measure_logical_ops(self, logop_indices: Sequence[int]):
        ...
        Args:
            logop_indices:
                ...
                :attr:`~plaquette.codes.Code.logical_ops` for
                details.
    ...
    def _measure_stabgens_seq(self, with_errors: bool):
        ...
        Args:
            with_errors:
                ...
                verification are currently performed without errors.)
        """
        for edge_idxs in cast(list, self.code.orchestration_graph.checks):
            self.measure_op(edge_idxs, with_errors=with_errors)
...
def generate_qec_circuit(
    ...
    .. important::
       ...
       This function doesn't actually simulate the circuit! That's
       the job of the device that uses an underlying backend to perform the
       simulation, e.g. :class:`._errortrackingbackend.ErrorTrackingBackend` when
       choosing the ``"clifford"`` backend.
    ...
    See :meth:`QECCircuitGenerator.get_circuit` for a description of the measurement
