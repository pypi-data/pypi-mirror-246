...
Starting from a :class:`.Code` and an :class:`.ErrorData` object, one can
...
import plaquette
from plaquette import codes, graph
...
def generate_empty_qubit_errors_csv(
    ...
    Args:
        code: :class:`.Code` that is considered
...
def generate_empty_qubit_errors(
    ...
    Args:
        code: The :class:`.Code` that is considered
        error_names: The names of errors to be added to columns to the dataframe
    ...
    qubit_info = [
        (
            node,
            t.cast(graph.Node, code.orchestration_graph.nodes[node]).type,
        )
...
class ErrorData:  # noqa: D101
    ...
    - ``probs`` (list[float]): The probabilities of the effected channels
    """
    ...
    def from_lattice(
        cls,
        code: codes.Code,
        qubit_error_config: t.Optional[tuple[dict, tuple[bool, str]]] = None,
        ...
        if qubit_error_config is None and gate_error_config is None:
            ...
            for node in range(code.num_qubits):
                if (data := code.orchestration_graph.nodes[node]) is not None:
                    qe.append((node, data.type))
    ...
    def update(
        ...
        """Update the dataframes in the object from a CSV file.
                ...
                match row.gate:
                    ...
                    case "H" | "M" | "R":
                        ...
                        for q_id in row.on_qubits:
                            channel = SinglePauliChannelErrorValueDict(  # type: ignore
                                {
                                    str(k).lower(): v
                                    for k, v in zip(row.induced_errors, row.probs)
                                }
                            )
                            if gate_errors_dict[row.gate].get(q_id) is None:
                                gate_errors_dict[row.gate][
                                    q_id
                                ] = SinglePauliChannelErrorValueDict()  # noqa: E501
                            gate_errors_dict[row.gate][q_id].update(channel)
                    # two qubit gate errors
                    case "CX" | "CZ":
                        if gate_errors_dict.get(row.gate) is None:
                            gate_errors_dict[row.gate] = {}
                        induced_errors = {i for i in row.induced_errors}
                        if not set(single_paulis).isdisjoint(induced_errors):
                            raise KeyError("Two qubit channels must be provided!")
                        for i in range(0, len(row.on_qubits), 2):
                            qubit_pair = (row.on_qubits[i], row.on_qubits[i + 1])
                            # fmt: off
                            if gate_errors_dict[row.gate].get(qubit_pair) is None:
                                gate_errors_dict[row.gate][qubit_pair] = TwoPauliChannelErrorValueDict() # noqa: E501
                            # fmt: on
                            two_qubit_channel = TwoPauliChannelErrorValueDict(  # type: ignore # noqa: E501
                                {
                                    str(k).lower(): v
                                    for k, v in zip(row.induced_errors, row.probs)
                                }
                            )
                            gate_errors_dict[row.gate][qubit_pair].update(
                                two_qubit_channel
                            )
    ...
    def _check_twoqubit_gate_error(
        ...
        for qubits, errors in error_dict.items():
            ...
            valid = set(errors.keys()).issubset(
    ...
    # TODO: implement this
    # def toggle_fabrication_error(self, code: Code) -> Code:
...
def assimilate_qubit_errors(
    qubit_error_config: tuple[dict, tuple[bool, str]],
    lattice: codes.Code,
) -> pd.DataFrame:
    ...
    try:
        qubit_csv_df = pd.read_csv(qubit_error_config[1][1])
        qubit_csv_df.set_index(["qubit_id"], drop=True, inplace=True)
    except FileNotFoundError:
        if qubit_error_config[1][0]:
            warnings.warn(
                "The given path to `csv` was invalid, ignoring the errors from csv."
    ...
    for label in qubit_error_config[0].keys():
        ...
        match qubit_error_config[0][label][0]:
            ...
            case "gaussian":
                qubit_pd[label] = generate_gaussian_errors(
                    mean=qubit_error_config[0][label][1],
                    std_dev=qubit_error_config[0][label][2],
                    qubit_list=qubit_pd["qubit_id"],
                )
            case "user":
                ...
                case "H" | "M" | "R":
                    for params in gate_error_config[0][label]:
                        if params[0] == "constant":
                            ...
                            probs = generate_gaussian_errors(
                                mean=params[2],
                                std_dev=params[3],
                                qubit_list=code.ancilla_qubit_indices,
                            )
                            for i, p in zip(
                ...
                case "CX" | "CZ":
                    for params in gate_error_config[0][label]:
                        ...
                        for stab in code.ancilla_qubit_indices:
                            for data in code.orchestration_graph.neighbours_of(stab):
                                on_qubits.extend([stab, data])
                        if params[0] == "constant":
                            gate_pd = pd.concat(
                                [
                                    gate_pd,
                            ...
                            probs = generate_gaussian_errors(
                                mean=params[2],
                                std_dev=params[3],
                                qubit_list=on_qubits[: len(on_qubits) // 2],
                            )
