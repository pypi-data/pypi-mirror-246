...
class VariableMissing(ValueError):
    """Exception raised by :func:`importData` when ``--fail_if_missing`` /
    ``failIfMissing`` is active, and a requested variable is not present in
    the input files.
    """
...
def importData(fileinfo,
    ...
    :arg failIfMissing:     Defaults to ``False``. If ``True``, and a requested
                            variable is not present in the input files, a
                            ``VariableMissing`` error is raised.
    ...
    # Warn/error if any requested variables are not present
    if variables is not None:
        for vid in variables:
            if vid not in fileinfo.allVariables:
                msg = f'Requested data field {vid} is ' \
                       'not present in input file(s).'
                if failIfMissing:
...
def loadChunk(i,
    ...
    pandaspre2  = any([pd.__version__.startswith(f'{v}.') for v in (0, 1)])
    ...
    with warnings.catch_warnings():
        ...
        if pandaspre2:
            dlargs['infer_datetime_format'] = True
