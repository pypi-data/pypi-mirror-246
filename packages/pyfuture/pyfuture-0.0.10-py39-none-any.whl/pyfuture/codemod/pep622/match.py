...
def match_selector(left: cst.BaseExpression, case: cst.MatchCase):
    if isinstance(case.pattern, cst.MatchAs):
        """
                case _
            """
    ...
    elif isinstance(case.pattern, cst.MatchClass):
        """
            class demo:
                pass
    ...
    elif isinstance(case.pattern, cst.MatchValue):
        """
            case "test":
            """
        gen_if = cst.If(
...
def replace_match_node(
    ...
    root_if: Union[cst.If, None] = None,
) -> FunctionDef:
...
class TransformMatchCommand(VisitorBasedCodemodCommand):
    ...
    def visit_FunctionDef(self, node: FunctionDef) -> Union[bool, None]:
    ...
    ) -> Union[Union[BaseStatement, FlattenSentinel[BaseStatement]], RemovalSentinel]:
