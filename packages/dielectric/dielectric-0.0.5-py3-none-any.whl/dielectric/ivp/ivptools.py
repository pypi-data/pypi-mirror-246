...
from dielectric.classes.electrolytes import Electrolyte2, Ion
import matplotlib
import matplotlib.pyplot as plt
matplotlib.use("Qt5Agg")


def calc_kappa_q(e: Electrolyte2):
    sumD = 0.0
    kappa2q = 0.0
    for salt in e.salts:
...
def calc_Aij(e: Electrolyte2, Gamma, sigma):
    Aij = 0.0
    pr = 1.0
    for salt in e.salts:
        for ion in salt.ions:
...
def calc_alpha_k(e: Electrolyte2, kappa_q, Gamma, sigma, Aij, i0, i1):
    pr = 1.0
    for salt in e.salts:
        for ion in salt.ions:
            pr *= ion.z*E_CHARGE
