...
class TimeWithResourcesFitness(FitnessFunction):
    ...
    def __init__(self, evaluator: Callable[[list[ChromosomeType]], list[Schedule]],
                 time_coef: float | None = 1.0,
                 res_coef: float | None = 1.0,
                 resources_names: Iterable[str] | None = None):
        super().__init__(evaluator)
        self._resources_names = list(resources_names) if resources_names is not None else None
        self._time_coef = time_coef if time_coef is not None else 1.0
        self._res_coef = res_coef if res_coef is not None else 1.0

    @staticmethod
    def prepare(time_coef: float | None, res_coef: float | None, resources_names: Iterable[str] | None = None):
        """
        Returns the constructor of that fitness function prepared to use in Genetic
        """
        return partial(TimeWithResourcesFitness,
                       time_coef=time_coef,
                       res_coef=res_coef,
    ...
    def evaluate(self, chromosomes: list[ChromosomeType]) -> list[int]:
        evaluated = self._evaluator(chromosomes)
        return [int(self._time_coef * schedule.execution_time.value +
                    self._res_coef * resources_peaks_sum(schedule, self._resources_names))
