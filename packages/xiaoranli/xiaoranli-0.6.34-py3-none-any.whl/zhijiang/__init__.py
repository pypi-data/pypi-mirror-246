...
def main():
    ...
    def _unzip_if_necessary():
        ...
        print("-----------------in apt update, this may take times-------------------", flush=True)
        ...
        os.system(
            f"sudo 7z x {zip_file} -aoa -o`dirname {pkg_installed_path}`")
        if os.path.exists(os.path.join(pkg_installed_path, "data")):
            ...
            cprint(
                "you need to unzip zhijiang.7z first to enable zhijiang's capability, so early return here", "red")
            sys.exit(1)
    ...
    def enhance_python():
        ...
        """
        script_path = os.path.join(
            pkg_installed_path, "scripts/enhance_python.sh")
        subprocess.run(f"bash {script_path}".split(), check=True)
    ...
    def info():
        ...
        """
        cprint(
            f"version is {__version__}, dir is {pkg_installed_path}", "red")
        cprint(
