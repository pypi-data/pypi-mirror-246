...
mdfu_protocol_version = Version("1.0.0")
...
class ClientInfo():
    ...
    def __init__(self, version: Version, buffer_count: int, buffer_size: int,
                    default_timeout: float, timeouts: dict = None):
...
"""
    ...
    def from_bytes(cls, data):
        ...
        try:
            ...
            raise MdfuClientInfoError("Mandatory client info parameter version is missing.") from exc
        try:
            # pylint: disable=pointless-statement
            buffer_count
            buffer_size
        except NameError as exc:
            raise MdfuClientInfoError("Mandatory client info parameter buffer info is missing.") from exc
        try:
            default_timeout
        except NameError as exc:
            raise MdfuClientInfoError("Mandatory default timeout is missing in client info command timeouts.") from exc
...
class Mdfu():
    ...
    def run_upgrade(self, image):
        ...
        try:
            ...
            self.sequence_number = 0
            self.client = self._get_client_info(sync=True)
    ...
    def get_client_info(self, sync=True):
        ...
        client = self._get_client_info(sync=sync)
        return client
    ...
    def _get_client_info(self, sync=False):
        ...
        try:
            client = ClientInfo.from_bytes(response.data)
        except ValueError as err:
            self.logger.error("Received invalid MDFU Client Info 0x%x", response.data.hex())
            raise MdfuProtocolError from err
        return client
    ...
    def log_error_cause(self, status_packet: MdfuStatusPacket):
        ...
        error_cause = None
        if error == MdfuStatus.PACKET_TRANSPORT_FAILURE:
            try:
                cause = TransportFailureCause(status_packet.data[0])
                error_cause = f"Transport error cause: {cause.description}"
            except KeyError:
                error_cause = f"Unknown transport failure cause {status_packet.data[0]}"
        elif error == MdfuStatus.ABORT_FILE_TRANSFER:
            try:
                cause = FileTransferAbortCause(status_packet.data[0])
                error_cause = f"File transfer abort cause: {cause.description}"
            except KeyError:
                error_cause = f"Uknown file abort cause {status_packet.data[0]}"
