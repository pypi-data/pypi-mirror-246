...
def single_ask(user_prompt, temp_file="", input_file="", temp_is_temp=False, should_print=False):
	if not temp_file:
		temp_is_temp = True
		temp_file = get_temp_file()
	messages = Conversation()
...
def main():
	...
	if (args.ask):
		answer = single_ask(args.ask, temp_file, input_history, temp_is_temp, True)
		if (args.ttsfile):
			tts_answer = openai.audio.speech.create(
				model="tts-1",
				voice="nova",
