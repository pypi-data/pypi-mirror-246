...
def __add_memory(*args, **kwargs):
	...
	if rdict["new_memory_index"] < 0:
		...
		return json.dumps(rdict)
	rdict["function_status"] = f"New memory added at index {rdict['new_memory_index']}"
	return json.dumps(rdict)
...
__add_memory_desc = {
	...
	"parameters": {
		...
		"properties": {
			"new_memory": {
				"type": "string",
				"description": "the memory to remember, as a sentence from the AI's point of view, not from the user's"
			}
		},
		"required": ["new_memory"],
	},
...
def __remove_memory(*args, **kwargs):
	...
	if index_to_delete < 0:
		rdict["function_status"] = "No memory index provided, memory not removed"
		return json.dumps(rdict)
...
def __edit_memory(*args, **kwargs):
	...
	if not rval:
		rdict = {}
		rdict["function_status"] = "Something went wrong, no memory were edited"
		return json.dumps(rdict)
...
__edit_memory_desc = {
	...
	"parameters": {
		...
		"properties": {
			...
			"new_memory": {
				"type": "string",
				"description": "the new memory to be remembered instead of the old one, as a sentence from the AI's point of view, not the user's"
			},
		},
