...
def save_data_vtk_from_csv(df, surf, fname, args, idx):
    ...
    if args.overwrite == 'False':
        ...
        true_vtk_path = new_filename.replace(args.vtk_folder, "")
        predictions_csv_name = os.path.splitext(os.path.basename(args.csv))[0]
        print(predictions_csv_name)

    # Creation of the directory to store the prediction vtk files
    directory = os.path.join(args.out, predictions_csv_name)
    os.makedirs(directory, exist_ok=True)
    output_fn = os.path.normpath(directory + "/" + true_vtk_path)
    print(bcolors.SUCCESS,f"Saving results to {output_fn}", bcolors.ENDC)
    utils.Write(surf , output_fn, print_out=False)
...
def save_csv(predictions, args):
    ...
    if args.overwrite == 'False':
        ...
        os.remove(args.csv)
        predictions_csv_path = os.path.join(args.out, args.csv)
        predictions_df.to_csv(predictions_csv_path, index=False)
        print(bcolors.SUCCESS,f"Saving results to {predictions_csv_path}", bcolors.ENDC)
...
def get_argparse():
    ...
    parser.add_argument('--suffix', type=str, help='Suffix of the predicted', default='pred')
    ...
    parser.add_argument('--device', type=str, help='Device to use for inference', default='cuda:0')
    parser.add_argument('--vtk_folder', type=str, help='Folder containing vtk files', default='')
