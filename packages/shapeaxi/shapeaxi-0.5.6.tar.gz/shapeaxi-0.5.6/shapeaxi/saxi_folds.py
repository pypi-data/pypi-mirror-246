...
def cml():
    ...
    split_group.add_argument('--folds', help='Number of folds', type=positive_int, default=5)
    split_group.add_argument('--valid_split', help='Split float [0-1]', type=positive_float, default=0.2)
    ...
    train_group.add_argument('--nn', help='Neural network name : SaxiClassification, SaxiRegression, SaxiSegmentation, SaxiIcoClassification', type=str, required=True, choices=['SaxiClassification', 'SaxiRegression', 'SaxiSegmentation', 'SaxiIcoClassification'])
    ...
    train_group.add_argument('--lr',  help='Learning rate', type=float, default=1e-4)
    ...
    train_group.add_argument('--tb_name', help='Tensorboard experiment name', type=str, default="tensorboard")
    ...
    train_group.add_argument('--path_ico_right', type=str, default='./3DObject/sphere_f327680_v163842.vtk', help='Path to ico right (default: ../3DObject/sphere_f327680_v163842.vtk)')
    train_group.add_argument('--path_ico_left', type=str, default='./3DObject/sphere_f327680_v163842.vtk', help='Path to ico left (default: ../3DObject/sphere_f327680_v163842.vtk)')

    # Arguments used for prediction
    pred_group = parser.add_argument_group('Prediction group')
    pred_group.add_argument('--crown_segmentation', help='Isolation of each different tooth in a specific vtk file', type=bool, default=False)
    pred_group.add_argument('--fdi', help = 'numbering system. 0: universal numbering; 1: FDI world dental Federation notation', type=int, default=0)

    # Arguments used for testing
    eval_group = parser.add_argument_group('Test group')
    eval_group.add_argument('--csv_true_column', help='Which column to do the stats on', type=str, default="class")
    eval_group.add_argument('--csv_tag_column', help='Which column has the actual names', type=str, default=None)
    eval_group.add_argument('--csv_prediction_column', help='csv true class', type=str, default="pred")
    eval_group.add_argument('--eval_metric', help='Score you want to choose for picking the best model : F1 or AUC', type=str, default='F1', choices=['F1', 'AUC'])
