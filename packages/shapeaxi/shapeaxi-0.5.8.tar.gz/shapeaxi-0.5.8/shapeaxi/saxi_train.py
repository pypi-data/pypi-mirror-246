...
def main(args):
    ...
    if args.nn == "SaxiClassification" or args.nn == "SaxiRegression":
        ...
        model, brain_data, trainer = SaxiIcoClassification_train(args, checkpoint_callback, mount_point, path_train, path_val, path_test, logger, early_stop_callback)
...
def get_argparse():
    ...
    input_group.add_argument('--csv_train', help='CSV with column surf', type=str, required=True)    
    input_group.add_argument('--csv_valid', help='CSV with column surf', type=str)
    ...
    input_group.add_argument('--class_column', help='Class column name', type=str, default=None)
    ...
    hyper_group.add_argument('--nn', help='Neural network name : SaxiClassification, SaxiRegression, SaxiSegmentation, SaxiIcoClassification', type=str, required=True, choices=["SaxiClassification", "SaxiRegression", "SaxiSegmentation", "SaxiIcoClassification"])
    ...
    hyper_group.add_argument('--noise_lvl', help='Noise level (default: 0.01)', type=float, default=0.01)
    hyper_group.add_argument('--ico_lvl', help='Ico level, minimum level is 1 (default: 2)', type=int, default=2)
    hyper_group.add_argument('--pretrained',  help='Pretrained (default: False)', type=bool, default=False)
    hyper_group.add_argument('--dropout_lvl',  help='Dropout level (default: 0.2)', type=float, default=0.2)
    hyper_group.add_argument('--layer', help="Layer, choose between 'Att','IcoConv2D','IcoConv1D','IcoLinear' (default: IcoConv2D)", type=str, default='IcoConv2D')
    ...
    logger_group.add_argument('--tb_name', help='Tensorboard experiment name', type=str, default="tensorboard")
