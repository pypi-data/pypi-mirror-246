...
class Worker(BaseCommand):
    ...
    def run_available_tasks(self):
        ...
        while True:
            ...
            if exc:
                if job:
                    # If the job have already saved itself as failed in this transaction, the below call
                    # to save below would raise an exception since no fields in update_fields are changed.
                    if not job.failed:
                        job.failed = True
