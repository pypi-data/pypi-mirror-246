...
class PIH:
    ...
    class SETTINGS:
        ...
        class USER:
            ...
            def set(login: str, name: str, value: Any) -> bool:
                return PIH.ACTION.DATA_STORAGE.value(value, name, PIH.SETTINGS.USER.get_section(login))

            @staticmethod
            def get(login: str, name: str, class_type_holder: Any | Callable[[Any], Any] | None = None) -> Any:
                return PIH.RESULT.DATA_STORAGE.value(name, class_type_holder, PIH.SETTINGS.USER.get_section(login)).data
    ...
    class DATA(DataTool, StringTool, ListTool, DateTimeTool, EnumTool, FullNameTool):
        class JOURNAL:
            ...
            def type_by_any(
                ...
                try:
                    value = int(value)
                    data: tuple[int, ParamItem] = DataTool.by_index(
                        DataTool.to_list(JournalType), value
                    )
                ...
                for item in JournalType:
                    item_value: tuple[int, ParamItem] = EnumTool.get(item)
                    item_value_value: ParamItem = item_value[1]
                    if (
                        item_value_value.name.lower() == value
                        if full_equaliment
                        else StringTool.contains(item_value_value.name, value)
                    ) or StringTool.contains(item_value_value.caption, value):
                        result.append(item)
                        if full_equaliment:
                            break
            ...
            def tag_by_id(value: int) -> Tag | None:
                data: tuple[int, str, str] | None = DataTool.by_index(
                    DataTool.to_list(Tag), value - 1
                )
        ...
        class FORMAT:
            ...
            @staticmethod
            def format(value: str, parameters: dict[str, Any] | None = None, use_python: bool = False) -> str:
    ...
    class RESULT(ResultTool):
            ...
            def get(
                ...
                def convert(event: EventDS) -> JournalRecord:
                    ...
                    return DataTool.fill_data_from_source(
                        JournalRecord(event.timestamp, applicant_user=user),
                        event.parameters,
                    )
        ...
        class NOTES:
            ...
            def get(id: str, force_update: bool = False) -> Result[Note]:
                return DataTool.to_result(
                    PIH.SERVICE.call_command(ServiceCommands.get_note, (id, force_update)), Note
                )
            ...
            @staticmethod
            def files(
                name: str | None = None, cached: bool = False, force_update: bool = False
            ) -> Result[list[Note]]:
                ...
                if not cached or A.R.is_empty(result):
                    PIH.RESULT.NOTES.cache = PIH.RESULT.NOTES.by_label(
                        CONST.NOTES.FILE_SECTION, force_update
                    )
                    result = ResultTool.filter(
                        search_function, PIH.RESULT.NOTES.cache, True
                    )
            ...
            def execute_file(
                ...
                return PIH.RESULT.NOTES.execute(
                    PIH.RESULT.get_first_item(
                        PIH.RESULT.NOTES.files(file_search_request, cached)
                    ).text,
                    parameters,
                    stdout_redirect,
                    catch_exceptions,
    ...
    class CHECK:
        ...
        class JOURNALS:
            @staticmethod
            def exists(name: str | None, title: str | None = None) -> bool:
                return nn(
                    PIH.DATA.JOURNAL.type_by_any(name)
                    or PIH.DATA.JOURNAL.type_by_any(name)
                )
        ...
        class JOURNAL:
            ...
            def add(
                ...
                PIH.EVENT.send(
                    ...
                    (
                        EnumTool.get(tag)[0],
                        EnumTool.get(journal_type)[0],
                        applicant_user.samAccountName,
                        title,
                        text,
                        js((EnumTool.get(tag)[2], EnumTool.get(tag)[1])),
                        EnumTool.get(journal_type)[1].caption,
                        parameters,
                    ),
                )
