...
class JournalType(tuple[int, ParamItem], Enum):
    TEST: tuple[int, ParamItem] = (0, ParamItem("test", "Тест"))
    COMPUTER: tuple[int, ParamItem] = (1, ParamItem("computer", "Компьютер"))
    MRI_CHILLER: tuple[int, ParamItem] = (
        2,
        ParamItem("mri_chiller", "Чиллер МРТ"),
    )
    MRI_GRADIENT_CHILLER: tuple[int, ParamItem] = (
        3,
        ParamItem("mri_gradient_chiller", "Чиллер градиентов МРТ"),
    )
    MRI_CLOSET_CHILLER: tuple[int, ParamItem] = (
        4,
        ParamItem("mri_closet_chiller", "Чиллер шкафа МРТ"),
    )
    CHILLER: tuple[int, ParamItem] = (
        5,
        ParamItem("chiller", "Чиллер"),
    )

    COMMUNICATION_ROOM: tuple[int, ParamItem] = (
        6,
        ParamItem("communication_room", "Коммутационная комната"),
    )
    SERVER_ROOM: tuple[int, ParamItem] = (
        7,
        ParamItem("server_room", "Серверная комната"),
    )
    MRI_TECHNICAL_ROOM: tuple[int, ParamItem] = (
        8,
        ParamItem("mri_technical_room", "Техническая комната МРТ"),
    )
    MRI_PROCEDURAL_ROOM: tuple[int, ParamItem] = (
        9,
        ParamItem("mri_procedural_room", "Процедурная комната МРТ"),
    )
    PRINTER: tuple[int, ParamItem] = (10, ParamItem("printer", "Принтер"))
    SERVER: tuple[int, ParamItem] = (11, ParamItem("server", "Сервер"))
   


class Tag(tuple[int, str, str], Enum):
    SERVICE: tuple[int, str, str] = (1, "Обслуживание", CONST.VISUAL.GOOD)
    ERROR: tuple[int, str, str] = (2, "Ошибка", CONST.VISUAL.ERROR)
    WARNING: tuple[int, str, str] = (3, "Внимание", CONST.VISUAL.WARNING)
    NOTIFICATION: tuple[int, str, str] = (
        4,
        "Уведомление",
