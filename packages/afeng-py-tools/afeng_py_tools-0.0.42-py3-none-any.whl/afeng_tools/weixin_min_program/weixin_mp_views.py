...
@router.post("/wx/mp/userinfo")
async def weixin_login(user_profile: WxUserProfile):
    """
    小程序登录
    :param user_profile: 授权登录后获取到的用户信息
    :return: token
    """
    return response_tools.create_json_response(weixin_mp_service.decrypt_userinfo(user_profile.app_id, user_profile.token, user_profile))
