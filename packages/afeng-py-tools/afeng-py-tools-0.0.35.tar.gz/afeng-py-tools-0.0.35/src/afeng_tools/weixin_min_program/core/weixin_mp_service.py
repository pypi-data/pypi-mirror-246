...
from afeng_tools.weixin_min_program.model.weixin_mp_open_models import WxMpLoginInfo, WxMpUserInfo
...
def decrypt_userinfo(mp_app_id: str, token: str, user_profile: WxUserProfile) -> ResponseModel:
    ...
    if user_profile.signature == weixin_mp_tools.calc_signature(session_key=session_key, raw_data=user_profile.rawData):
        ...
        # 保存用户信息
        save_userinfo_callback = weixin_mp_settings.get_config(mp_app_id, WeixinMpConfigKeyEnum.mp_save_userinfo_callback)
        if save_userinfo_callback:
            save_userinfo_callback(WxMpUserInfo(
                mp_app_id=login_info.mp_app_id,
                session_key=login_info.session_key,
