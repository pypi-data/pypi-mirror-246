...
def fetch_dependency(module_list, include_list, c_path, f_branch):
    def checkout_code(git_url, tag):
        # 使用 git 命令拉取代码到本地
        command = f'git clone {git_url} --branch {tag}'
        os.system(command)
    ...
    def get_direct_dependencies(podspec_file):
        ...
        with open(podspec_file) as file:
            for line in file:
                if line.startswith('  s.dependency'):
                    dependency = line.split("'")[1]
                    dependencies.append(dependency)

        return dependencies

    def get_all_dependencies(component, all_dependencies, level=0):
        git_url = component.git
        tag = component.tag

        # 拉取代码到本地
        checkout_code(git_url, tag)

        # 获取 .podspec 文件路径
        module = component.module
        podspec_file = f'{module}/{module}.podspec'

        # 获取直接依赖
        direct_dependencies = get_direct_dependencies(podspec_file)
        all_dependencies.append((module, level))

        for dependency in direct_dependencies:
            # 如果依赖不在已获取的依赖列表中，则递归获取依赖的依赖
            if dependency not in [dep[0] for dep in all_dependencies]:
                find_module = getmodule_with_name(dependency)
                if find_module is not None:
                    get_all_dependencies(find_module, all_dependencies, level + 1)
    ...
    for a in invert_list:
        ...
        if not (a.branch and len(a.branch) > 0) and not (a.tag and len(a.tag) > 0):
            a.branch = f_branch
        if a.branch and len(a.branch) > 0:
            get_all_dependencies(a, all_dependencies)
        index += 1
        os.chdir(c_path)
    bar.finish()
...
class yhgit:
    ...
    def dependency(self, include_list=[]):
        c_path = os.getcwd()
        fa_path = ''
        ...
        conver_deplist = load_yaml(dependenceList)
        logging.info("转换成模型")
        # def fetch_dependency(conver_deplist, include_list, c_path, tag_path, f_branch):
        if len(include_list) == 0:
            include_list = conver_deplist;
        branch_res = fetch_dependency(conver_deplist, include_list, c_path, 'master')
        print()
        # json_str = json.dumps([person.__dict__ for person in branch_res])
        logging.info("dependency成功的模块\n")
        # 按层次结构显示依赖关系
        for dependency, level in branch_res:
            print(f"{'  ' * level}{dependency}")
        # print(json_str)
