...
'''
	options:
		actual:
			fleet HA-HTTPS-HTTP --to-port 53012 --workers 4
			
			[gunicorn start]  gunicorn -w 4 flask_module:app --bind 127.0.0.1:53012 --pid gunicorn.process_ID --daemon
			[gunicorn stop]   fleet flask-gunicorn-stop
			[gunicorn status] ps -ax | grep python
		
		
		
		dev:
			gunicorn -w 4 flask_module:app --bind 127.0.0.1:53012 --reload
		
			
		
	priorities:
		fleet flask-gunicorn 
'''
