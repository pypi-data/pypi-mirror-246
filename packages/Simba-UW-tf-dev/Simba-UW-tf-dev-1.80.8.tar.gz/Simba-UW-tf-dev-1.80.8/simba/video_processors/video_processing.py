...
def crop_multiple_videos(directory_path: Union[str, os.PathLike],
    ...
    for file_cnt, file_path in enumerate(video_paths):
        ...
        if gpu:
            ...
            command = f'ffmpeg -i "{file_path}" -vf "crop={roi_selector.width}:{roi_selector.height}:{roi_selector.top_left[0]}:{roi_selector.top_left[1]}" -c:v libx264 -crf 21 -c:a copy "{save_path}"'
        subprocess.call(command, shell=True)
