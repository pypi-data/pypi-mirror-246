...
if __name__ == '__main__':
    ...
    if len(pcr['Primary Template']) == 3:
        include = [conc1,conc2,conc3]
        
    if len(pcr['Primary Template']) == 4:
        include = [conc1,conc2,conc3,conc4]
        
    if len(pcr['Primary Template']) == 5:
        include = [conc1,conc2,conc3,conc4,conc5]
    ...
    def main():
        ...
        f.write('############################################################################################## \r\n')
        f.write('The Following reagents go in the cold tuberack (deck slot 1) \r\n')
    ...
    os.system("notepad.exe reagent2_setup.txt")
    ...
    os.mkdir(str(2)+'_'+date+time+'_EPPCR')
    ...
    # os.chdir(paths.loc[0].at['opentrons_repo']+'/Error_prone_PCR_Golden_Gate/'+date+time+'_PCR/'+str(2)+'_'+date+time+'_EPPCR/')
    ...
    #shutil.move(paths.loc[0].at['opentrons_repo']+'/Golden_Gate/digests.csv',paths.loc[0].at['opentrons_repo']+'/Golden_Gate/'+date+time+'_GoldenGate/')str(2)+'_'+date+time+'_EPPCR/
    ...
    shutil.move(paths.loc[0].at['opentrons_repo']+'/Error_prone_PCR_Golden_Gate/oligo.csv',paths.loc[0].at['opentrons_repo']+'/Error_prone_PCR_Golden_Gate/'+date+time+'_PCR/'+str(2)+'_'+date+time+'_EPPCR/')
    ...
    shutil.move(paths.loc[0].at['opentrons_repo']+'/Error_prone_PCR_Golden_Gate/'+date+time+'_PCR/reagent2_setup.txt',paths.loc[0].at['opentrons_repo']+'/Error_prone_PCR_Golden_Gate/'+date+time+'_PCR/'+str(2)+'_'+date+time+'_EPPCR/')
    os.rename(paths.loc[0].at['opentrons_repo']+'/Error_prone_PCR_Golden_Gate/'+date+time+'_PCR/'+str(2)+'_'+date+time+'_EPPCR/reagent2_setup.txt', paths.loc[0].at['opentrons_repo']+'/Error_prone_PCR_Golden_Gate/'+date+time+'_PCR/'+str(2)+'_'+date+time+'_EPPCR/reagent_setup.txt')
    ...
    def set_variables():
        ...
        Time = Time_entry.get()
        # ErrorRate = float(ErrorRate_entry.get())
        Taq_buffer = float(Taq_buffer_entry.get())
        BSA = float(BSA_entry.get())
    ...
    label_Q5 = tk.Label(text='Taq to add - uL',font=('Helvatical bold',14))
    ...
    # label_ErrorRate = tk.Label(text='Error Rate',font=('Helvatical bold',14))
    # label_Time.place(relx=0,rely=0.400)

    label_Taq_buffer = tk.Label(text='5X Taq Buffer',font=('Helvatical bold',14))
    label_Taq_buffer.place(relx=0,rely=0.425)

    label_BSA = tk.Label(text='BSA',font=('Helvatical bold',14))
    label_BSA.place(relx=0,rely=0.45)
    ...
    # ErrorRate_entry = tk.Entry()
    # ErrorRate_entry.insert(END, '1.0')
    # ErrorRate_entry.place(relx=0.2,rely=0.4,width=55)

    Taq_buffer_entry = tk.Entry()
    Taq_buffer_entry.insert(END, '20')
    Taq_buffer_entry.place(relx=0.2,rely=0.425,width=55)

    BSA_entry = tk.Entry()
    BSA_entry.insert(END, '5')
    BSA_entry.place(relx=0.2,rely=0.45,width=55)
    ...
    row = [[stkprm,stkvol,dilprm,primerconc,pcrvol,templatengs,Q5,DPNI,DPwater,cutsmart,Date,ngdesired,Combinatorial_pcr_params,Time,Taq_buffer,BSA]]
    variables = pd.DataFrame(test,columns=['stkprm','stkvol','dilprm','primerconc','pcrvol','templatengs','Q5','DPNI','DPwater','cutsmart','Date','ngdesired','Combinatorial_pcr_params','Time','Taq_buffer','BSA'],index=range(len(temppwls)))
    variables.iloc[0]= [stkprm,stkvol,dilprm,primerconc,pcrvol,templatengs,Q5,DPNI,DPwater,cutsmart,Date,ngdesired,Combinatorial_pcr_params,Time,Taq_buffer,BSA]
...
#################################################################################
    ...
    shutil.move(paths.loc[0].at['opentrons_repo']+'/Error_prone_PCR_Golden_Gate/section.csv',paths.loc[0].at['opentrons_repo']+'/Error_prone_PCR_Golden_Gate/'+date+time+'_PCR/'+str(2)+'_'+date+time+'_EPPCR/')
    ##############################################################################################################

    #Error Rate selector

    ErrorRate = pandas.DataFrame()
    ...
    show = Label(ws, text = "Choose your Error Rate", font = ("Times", 14), padx = 10, pady = 10)
    show.pack() 
    lb = Listbox(ws, selectmode = "multiple")
    lb.pack(padx = 10, pady = 10, expand = YES, fill = "both") 

    x =["E% = 0.2","E% = 0.3","E% = 0.4","E% = 0.5","E% = 0.6","E% = 0.8","E% = 1.0","E% = 1.5"]
    ...
    ErrorRate.to_csv(paths.loc[0].at['opentrons_repo']+'/Error_prone_PCR_Golden_Gate/ErrorRate.csv')
    shutil.move(paths.loc[0].at['opentrons_repo']+'/Error_prone_PCR_Golden_Gate/ErrorRate.csv',paths.loc[0].at['opentrons_repo']+'/Error_prone_PCR_Golden_Gate/'+date+time+'_PCR/'+str(2)+'_'+date+time+'_EPPCR/')
    ...
    gradient.to_csv(paths.loc[0].at['opentrons_repo']+'/Error_prone_PCR_Golden_Gate/'+date+time+'_PCR/'+str(2)+'_'+date+time+'_EPPCR/gradient.csv')
    ...
    pcr.to_csv(paths.loc[0].at['opentrons_repo']+'/Error_prone_PCR_Golden_Gate/'+date+time+'_PCR/'+str(2)+'_'+date+time+'_EPPCR/pcr.csv')
    #shutil.move(paths.loc[0].at['opentrons_repo']+'/Error_prone_PCR_Golden_Gate/pcr.csv',paths.loc[0].at['opentrons_repo']+'/Error_prone_PCR_Golden_Gate/'+date+time+'_PCR/'+str(2)+'_'+date+time+'_EPPCR/')
    ...
    # shutil.copy2(paths.loc[0].at['opentrons_repo']+'/'+date+time+'_PCR/Error_prone_PCR_Golden_Gate/'+date+time+'_EPPCR/Input.csv', paths.loc[0].at['opentrons_repo']+'/Error_prone_PCR_Golden_Gate/')
    ...
    #Input_values = pandas.read_csv('Input.csv') 
    Input_values = pandas.read_csv(paths.loc[0].at['opentrons_repo']+'/Error_prone_PCR_Golden_Gate/'+date+time+'_PCR/'+str(2)+'_'+date+time+'_EPPCR/Input.csv') 
    Input_values
    # Date = str(int(Input_values.loc[0].at['Date']))
    # Date
    ...
    #os.chdir("C:/Users/jonbr/Documents/GitHub/opentrons/Golden_Gate/Part1_PCR_Mason/")
    pcr = pandas.read_csv(paths.loc[0].at['opentrons_repo']+'/Error_prone_PCR_Golden_Gate/'+date+time+'_PCR/'+str(2)+'_'+date+time+'_EPPCR/pcr.csv')
    pcr.columns = pcr.columns.str.replace("'","")
    pcr
    ...
    df
    df.to_csv(paths.loc[0].at['opentrons_repo']+'/Error_prone_PCR_Golden_Gate/'+date+time+'_PCR/'+str(2)+'_'+date+time+'_EPPCR/templates.csv')
    ...
    pcr_plustemplates.to_csv(paths.loc[0].at['opentrons_repo']+'/Error_prone_PCR_Golden_Gate/'+date+time+'_PCR/'+str(2)+'_'+date+time+'_EPPCR/pcr.csv')
    ...
    # plasmid['Volume of Plasmid'] = (1/(plasmid['Concentration'])) * 1500 * 1
