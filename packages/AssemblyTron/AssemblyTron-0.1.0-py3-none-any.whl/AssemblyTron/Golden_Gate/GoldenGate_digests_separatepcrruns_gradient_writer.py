        ...
        if x in section['parts'].values:
            ...
            for i, row in GG_dfs.iterrows():
                ...
                if Input_values.loc[0].at['paqCI'] == 2:
                    ...
                    #water
                        "    left_pipette.pick_up_tip() \r\n"
                        "    left_pipette.aspirate("+str(15 - round(locals()[x]['final amount to add'].sum(),2) - 1 -1  - 1 - 1.65)+", watertuberack['A1']) \r\n" #accounts for activator
                        "    left_pipette.dispense("+str(15 - round(locals()[x]['final amount to add'].sum(),2) - 1 -1- 1 - 1.65)+", pcrplate['"+str(locals()[x].loc[0].at['location_of_assembly'])+"']) \r\n"
                        "    left_pipette.blow_out() \r\n"
                        "    left_pipette.drop_tip() \r\n"
                    )
