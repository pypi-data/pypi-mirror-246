...
from typing import Awaitable, Callable, List, Tuple, Optional, Union
...
from golem.resources import Allocation, Demand, Proposal, NoMatchingAccount, AllocationException
...
class RefreshingDemandManager(BackgroundLoopMixin, DemandManager):
    def __init__(
        ...
        self._initial_proposals: asyncio.Queue[Union[Proposal, AllocationException]] = asyncio.Queue()

        self._demands: List[Tuple[Demand, asyncio.Task]] = []
        self._allocation_exception: Optional[AllocationException] = None
    ...
    async def get_initial_proposal(self) -> Proposal:
        if self._allocation_exception is not None:
            raise self._allocation_exception

        item = await self._initial_proposals.get()

        if isinstance(item, AllocationException):
            raise item
    ...
    async def _create_and_subscribe_demand(self):
        ...
        except NoMatchingAccount as e:
            logger.info('Exception caught in get_allocation, forwarding to initial_proposals queue')
            self._allocation_exception = e
            self._initial_proposals.put_nowait(e)
            raise
        else:
            self._allocation_exception = None
