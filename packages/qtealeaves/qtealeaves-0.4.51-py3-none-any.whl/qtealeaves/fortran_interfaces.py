...
def write_nml(namelist_name, namelist_dict, namelist_file):
    ...
    **Parameters**
    ...
    if not isinstance(namelist_dict, OrderedDict):
        raise Exception("Please pass ordered dict.")
    ...
    for key, value in namelist_dict.items():
        if not isinstance(key, str):
            raise Exception("Key is no string!")
    ...
    **Examples**
    ...
    for elem in generators:
        ...
        if np.sum(np.abs(tmp)) > 1e-14:
            raise Exception("Generator not diagonal.")

    # Run check that there are only integer elements
    for elem in generators:
        if np.sum(np.abs(np.imag(np.diag(elem)))) > 1e-14:
            raise Exception("Diagonal elements cannot be complex.")

        tmp = elem - np.array(np.diag(np.real(np.diag(elem))), dtype=int)
        if np.sum(np.abs(tmp)) > 1e-14:
            raise Exception("Diagonal elements must be integers in generator.")

    # Support only matrices for now (additional links can be added)
    if len(tensor.shape) != 2:
        raise Exception("Please pass matrix to write AbelianSymTensor.")
    ...
    if (add_links is None) and (np.sum(np.abs(qnum_remainder)) > 0):
        raise Exception("We recommend adding a third link. Check model etc.")
    ...
    for elem in gen_type:
        if elem.startswith("U"):
            ...
            raise Exception("Unknown generator type %s." % (elem))
    ...
    if add_links is None:
        ...
        raise Exception("Unknown add_links value.")
...
def argsort_irreps(irreps):
    ...
    if len(set(list(irreps_val))) != dim0:
        raise Exception("Double entry in irreps.")
...
def read_tensor(file, cmplx=True, order="F", lptn=False):
    """
    ...
    lptn: Boolean, optional
        If lptn = True, the function is adjusted to correctly read the LPTN tensor
        form
    """
    if order not in ["F", "C"]:
