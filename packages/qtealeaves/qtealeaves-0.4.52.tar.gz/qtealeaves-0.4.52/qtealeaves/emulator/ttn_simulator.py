...
class TTNLayer(list):
    ...
    def __init__(
        ...
        #########################################################
        self.is_measured = [True, True, True, True, False, False, False, False, False]
...
class TTN(_AbstractTN):
    ...
    def clear_cache(self, num_qubits_keep=0):
        """
        ...
        **Details**
        ...
        for key in keys_to_delete:
            if self._cachelimit_bytes_get_children_prob is not None:
                size_bytes = sys.getsizeof(self._cache_get_children_prob[key])
                self._cachesize_bytes_get_children_prob -= size_bytes
    ...
    def _clear_cache_based_on_cache_size(self, all_probs):
        ...
        **Arguments**
        ...
        if (
            ...
            < self._cachelimit_bytes_get_children_prob
        ):
            # Nothing to do, cache size is still respected
            return all_probs
        ...
        for ii in range(max_key_length - 1, -1, -1):
            self.clear_cache(num_qubits_keep=ii)
            all_probs[ii] = {}
    ...
    def optimize_tree(self, num_sweeps):
        """
        ...
        for _ in range(num_sweeps):
            ...
            for ldx in range(self.num_layers - 1, -1, -1):
                ...
                # Cycle over tensors
                for tdx in iterator:
                    self.optimize_single_tensor([ldx, tdx], verbose=False)
