...
class NetValueLoader(BaseDataLoader):
    ...
    def filter(self, nvdf: pd.DataFrame, reg_ids=None,start_date=None, end_date=None,**kwargs):
        if nvdf.empty:
            return nvdf
    ...
    def load_ppw(self, reg_ids=None, ppw_ids=None, start_date=None, end_date=None,code_mapper:pd.Series=None):
        ...
        if reg_ids:
            if isinstance(reg_ids, str):
                reg_ids = [reg_ids]
            reg_ids = set(reg_ids)
            fund_ids = api.get_fund(reg_ids=reg_ids).index
            if ppw_ids:
                if isinstance(ppw_ids, str):
                    ppw_ids = [ppw_ids]
                ppw_ids = set(ppw_ids)
                fund_ids = set(fund_ids.append(ppw_ids))
            ppwnv = ppwnvformatter(
                api.get_netvalue(fund_ids, start_date, end_date), code_mapper=code_mapper,df=True
            )
        else:
            ppwnv = ppwnvformatter(
                api.get_netvalue(fund_id=ppw_ids, start_date=start_date, end_date=end_date),
    ...
    def load(
        self, reg_ids=None, ppw_ids=None, start_date=None, end_date=None, code_mapper=None,df=True
    ) -> Union[List[NetValueData], pd.DataFrame]:
        return super().load(
            reg_ids=reg_ids, ppw_ids=ppw_ids,start_date=start_date, end_date=end_date, code_mapper = code_mapper,df=df
        )
