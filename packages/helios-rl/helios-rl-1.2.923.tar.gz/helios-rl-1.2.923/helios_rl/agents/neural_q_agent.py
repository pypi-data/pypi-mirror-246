    ...
    def choose_action(self, state_enc: Tensor, legal_actions: List[str]) -> str:
        ...
        if rng < self.epsilon:
            ...
            with torch.no_grad():
                ...
                print("\n --- ")
                if len(max(list_value_legal_moves, key=lambda e: e[1])) > 1:
                    action = random.choice(max(list_value_legal_moves, key=lambda e: e[1]))[0].item()
                else:
                    action = max(list_value_legal_moves, key=lambda e: e[1]).item()
                print(action)
        return action
