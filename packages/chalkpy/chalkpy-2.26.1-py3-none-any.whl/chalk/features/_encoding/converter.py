...
from chalk.features._encoding.utils import if_not_instance_raise
from chalk.parsed.proto import data_pb2 as pb
...
class FeatureConverter(Generic[_TPrim, _TRich]):
    ...
    def from_pyarrow_to_protobuf(values: Union[pa.Array, pa.ChunkedArray]) -> list[pb.ScalarValue]:
        ...
        if_not_instance_raise(values, pa.Array)
    ...
    def _from_pb_to_pa(pb_value: pb.ScalarValue) -> pa.scalar:
        ...
        if pb_value.HasField("chalk_float16_value"):
            return pa.scalar(np.float16(pb_value.chalk_float16_value), pa.float16())
    ...
    def _from_pa_to_pb(value: pa.Scalar) -> pb.ScalarValue:
        ...
        if pa.types.is_float16(value.type):
            return pb.ScalarValue(chalk_float16_value=value.as_py())
        ...
        if pa.types.is_date32(value.type):
            ...
            if_not_instance_raise(date_value, date)
        ...
        if pa.types.is_time32(value.type):
            ...
            if_not_instance_raise(time_val, time)
            ms_since_midnight = (
        ...
        if isinstance(value.type, pa.TimestampType):
            ...
            if_not_instance_raise(dt_val, datetime)
            maybe_tz_kwarg = {}
            if dt_val.tzinfo:
        ...
        if pa.types.is_dictionary(value.type):
            # Not tested in `test_encoding.py`
            raise NotImplementedError
        if isinstance(value, pa.StructScalar):
            if_not_instance_raise(value.type, pa.StructType)
            fields = []
            field_values = []
    ...
    def convert_proto_dtype_to_pa_dtype(dtype: pb.ArrowType) -> pa.DataType:
        ...
        elif dtype.HasField("CHALK_DECIMAL_128"):
            return pa.decimal128(
                dtype.DECIMAL.precision, dtype.DECIMAL.scale  # pyright: ignore[reportGeneralTypeIssues]
            )
        elif dtype.HasField("CHALK_DECIMAL_256"):
    ...
    def convert_pa_dtype_to_proto_dtype(dtype: pa.DataType) -> pb.ArrowType:
        ...
        if dtype == pa.null():
            ...
            return pb.ArrowType(TIME32=pb.TimeUnit.Second)
        elif dtype == pa.time32("ms"):
            return pb.ArrowType(TIME32=pb.TimeUnit.Millisecond)
        elif dtype == pa.time64("us"):
            return pb.ArrowType(TIME64=pb.TimeUnit.Microsecond)
        elif dtype == pa.time64("ns"):
            return pb.ArrowType(TIME64=pb.TimeUnit.Nanosecond)
            ...
            return pb.ArrowType(CHALK_DECIMAL_128=pb.Decimal(precision=dtype.precision, scale=dtype.scale))
        elif isinstance(dtype, pa.Decimal256Type):
            return pb.ArrowType(CHALK_DECIMAL_256=pb.Decimal(precision=dtype.precision, scale=dtype.scale))
    ...
    def _serialize_pa_decimal_to_pb(value: Union[pa.Decimal128Scalar, pa.Decimal256Scalar]) -> pb.ScalarValue:
        ...
        if isinstance(value, pa.Decimal256Scalar):
            return pb.ScalarValue(decimal256_value=pb.Decimal256(value=bytes_val, p=precision, s=scale))

        return pb.ScalarValue(decimal128_value=pb.Decimal128(value=bytes_val, p=precision, s=scale))
