...
class jsondb_in_memory(collections.UserDict, dict):
    ...
    def __init__(self, *args, name=None, create=None, buffer_size=10_000, serializer=pickle, shared_lock=None, full_dump_size=None,
        ...
        # Parameters that could be read from remote if we are connecting to an existing jsondb_in_memory
        self.recurse = recurse

        # In recurse mode, we must ensure a recurse register
        if self.recurse:
            # Must be either the name of an jsondb_in_memory as a string or an jsondb_in_memory instance
            if recurse_register is not None:
                if type(recurse_register) == str:
                    self.recurse_register = jsondb_in_memory(name=recurse_register)
                elif type(recurse_register) == jsondb_in_memory:
            ...
            # If no register was defined, we should create one
                ...
                self.recurse_register = jsondb_in_memory(name=f'{self.name}_register',
    ...
    def raise_already_closed(self, *args, **kwargs):
        raise Exceptions.AlreadyClosed('jsondb_in_memory already closed, you can only access the `jsondb_in_memory.data` buffer!')
    ...
    def unlink_recursed(self):
        ...
        if not self.recurse or (type(self.recurse_register) != jsondb_in_memory):
            raise Exception("Cannot unlink recursed for non-recurse jsondb_in_memory")
    ...
    def unlink_by_name(name, ignore_errors=False):
        ...
        try:
            ...
            memory = jsondb_in_memory.get_memory(create=False, name=name)
            memory.unlink()
            memory.close()
            return True
        except Exceptions.CannotAttachSharedMemory as e:
