...
def deploy_table(**context):
    ...
    warehouse_engine.execute(f"GRANT SELECT ON ALL TABLES IN SCHEMA {target_shema} TO GROUP data_dev")
    warehouse_engine.execute(f"GRANT SELECT ON ALL TABLES IN SCHEMA {source_shema} TO GROUP data_dev")                  
    sleep(2)     
