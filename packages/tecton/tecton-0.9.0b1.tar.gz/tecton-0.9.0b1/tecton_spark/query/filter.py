...
def _filter_to_feature_data_time_limits(
    ...
    if end_timestamp_key != start_timestamp_key:
        # valid_to will be null for the last row of temporal fvs, we will use
        # "end_date" as the 'valid_to' value in this case.
        end_timestamp = functions.lit(feature_data_time_limits.end).cast(TimestampType())
        feature_df = feature_df.withColumn(
            end_timestamp_key, functions.coalesce(feature_df[end_timestamp_key], end_timestamp)
        )
