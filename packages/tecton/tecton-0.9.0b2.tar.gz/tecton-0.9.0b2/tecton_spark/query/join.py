...
class ExplodeTimestampByTimeWindowsSparkNode(SparkExecNode):
    ...
    def _to_dataframe(self, spark: pyspark.sql.SparkSession) -> pyspark.sql.DataFrame:
        ...
        # if the anchor time is less than FST, add FST as an anchor time so we can start `valid_from` from FST
        if feature_start_time is not None:
            feature_start_time_ns = convert_timestamp_for_version(
                feature_start_time, self.fdw.get_feature_store_format_version
            )
            df_exploded = df_exploded.withColumn(
                anchor_time(),
                F.when(F.col(anchor_time()) < feature_start_time_ns, F.lit(feature_start_time_ns)).otherwise(
                    F.col(anchor_time())
                ),
            )
