...
class BaseWebsocket(ABC):
    ...
    def on_close(self, wsapp, close_status_code, close_msg):
        logger.info(
            f"{self.websocket_type} connection closed. Status code: {close_status_code}, Message: {close_msg}"
        )
        self.connected = False
        if self.intentionally_closed:
            logger.info(f"{self.websocket_type} intentionally closed.")
            return
            ...
            logger.info(f"{self.websocket_type} connection closed for unknown reason.")
