...
def delta_hedged_strangle(
    ...
    augmenting_multiple: int | float = 2.5,
    ...
    delta_threshold = delta_threshold_pct * initial_qty_shares

    logger.info(
        f"---- {underlying.name} {strategy_tag} ----\n"
        f"{underlying.name} {strategy_tag} "
        f"max qty: {max_qty_shares}, "
        f"initial qty: {initial_qty_shares} "
    ...
    while current_time().time() < time(*scan_end_time):
        ...
        if target_strangle is None or minimum_unevenness > augmenting_multiple * 0.8:
            message = (
