...
class HTTPClient(DownloadFromBase):
    ...
    def __init__(
        ...
        self._semaphore = asyncio.Semaphore(int(HTTPCLIENT_MAX_SEMAPHORE))  # Adjust the number as neededd
    ...
    async def process_response(self, response, url: str) -> tuple:
        ...
        try:
            ...
            if hasattr(self, 'no_errors'):
                for key, _ in self.no_errors.items():
                    if int(key) == http_err.status:
                        return (response, http_err.status)
            else:
                # Handle HTTP errors here
                error = http_err
                # Log the error or perform other error handling
                self._logger.error(
                    f"Error: {http_err}, status: {http_err.status}"
                )
                raise
        if response.status < 399:
            ...
            if 'X-Error' in response.headers:
                ...
                err = f"{response}"
            self._logger.error(
                f"HTTP Error {response.status}: {err}"
            )
            print('ERROR Headers ')
