    ...
    def generate_deletes(splits):
        ...
        for l, r in splits:
            if ' ' in l:
                ...
                if len(firstWordInPharse) > 1:
                    result.add(firstWordInPharse[1:0] + ' ' + secondWordInPharse)
                    
                if len(secondWordInPharse) > 1:
                    result.add(firstWordInPharse + ' ' + secondWordInPharse[1:0])
            if ' ' in r:
                arr = r.split()
                if len(arr) == 1:
                    continue
