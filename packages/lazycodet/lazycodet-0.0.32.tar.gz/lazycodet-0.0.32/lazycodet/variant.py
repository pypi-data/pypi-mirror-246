...
class Variant:
        ...
        for idx, (l, r) in enumerate(Variant.split(word)):
            ...
            if isPhrase:
                if Variant.isFirstWordOfPhrase(wordArray, l):
                    ...
                    position = Variant.getPositionOfSecondWordInPhrase(wordArray, l)
            else:
                lenWord = len(word)
                position = len(l) + 1
    ...
    def editAndCreate(word, debug = False):
        ...
        if len(word) + 1 <= Variant.MAX_CHAR:
            ...
            if len(word) + 2 <= Variant.MAX_CHAR:
                inserts_2 = set([l + c1 + r + c2 for l, r in splits for c1 in Variant.letters for c2 in Variant.letters])
                inserts.update(inserts_2)
