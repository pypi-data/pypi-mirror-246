...
class Variant:
    ...
    def replace(word):
        ...
        arr = [l + c + r[1:] for l, r in Variant.split(word) if r for c in Variant.letter_v_normal]
            ...
            for key, values in char_dict.items():
                if word[0] == key:
                    for v in values:
                        newWord = v + word[1:]
                        arr += [newWord]
                        arr += [l + c + r[1:] for l, r in Variant.split(newWord) if r for c in Variant.letter_v_normal]
