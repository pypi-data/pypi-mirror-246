...
class SQLModelCleaner:
    def __init__(self):

        parser = argparse.ArgumentParser(
            description='Sorts lines and expands combined words in a DBT staging SQL file. ')

        parser.add_argument('--file-path', type=str, default=None, help='Path to the SQL file to clean.')

        parser.add_argument('--folder-path', type=str, default=None,
                            help='Path to the folder containing SQL files to clean.')

        parser.add_argument('--remove-airbyte', type=bool, default=True,
                            help='Whether to remove Airbyte specific lines. Default is True.')

        parser.add_argument('--overwrite', type=bool, default=False, help='Overwrite existing SQL files. '
                                                                          'Default is false.')

        parser.add_argument('--suffix', type=str, default=None,
                            help='Suffix to add to the output file name. Default is none. File will be overwritten if'
                                 ' overwrite option is enabled.')

        parser.add_argument('--output-sub-folder', type=str, default='cleaned',
                            help='Name of subfolder to place the cleaned files in for copy to the main folder.')

        parser.add_argument('--approve', type=bool, default=False,
                            help='Whether to approve the changes. Default is False.')
    ...
    def main(self) -> None:
        ...
        if self._args.file_path is not None:
            files = [self._args.file_path]
        elif self._args.folder_path is not None:
            files = [os.path.join(self._args.folder_path, f) for f in os.listdir(self._args.folder_path) if
                     f.endswith('.sql')]
        else:
            raise Exception('Either file_path or folder_path must be specified.')
        ...
        for file_path in files:
            ...
            # Write the cleaned and sorted lines back to the file
            suffix = self._args.suffix if self._args.suffix is not None else ''
            self._write_sql_file(all_lines, directory, filename, suffix, self._args.output_sub_folder)
    ...
    @staticmethod
    def _write_sql_file(lines: List[str], directory: str, filename: str, suffix: str,
