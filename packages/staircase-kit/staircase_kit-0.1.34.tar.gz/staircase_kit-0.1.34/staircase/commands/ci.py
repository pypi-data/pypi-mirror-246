...
@click.option(
    "--type",
    help="Type of bundle can be either service, data or frontend",
    default="service",
    type=click.Choice(["service", "data", "frontend"]),
)
@click.option(
    "--ci-env",
    "-ci",
    ...
    *,
    ...
    if "clone" in actions:
        ...
        bundle_url = await clone(
            ci_env, repo=repo, branch=branch, github_token=user_config.github_token, verbose=verbose
        )
        if verbose:
    ...
    if "build" in actions:
        ...
        bundle_url = await build(ci_env=ci_env, bundle_url=bundle_url, type=type, env_variables=env_variables, verbose=verbose)
        if verbose:
    ...
    if "deploy" in actions:
        ...
        await deploy(ci_env=ci_env, url=bundle_url, env_token=env_token, verbose=verbose)
        if verbose:
