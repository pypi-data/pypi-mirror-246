...
def _override_tenant_specific_config_if_present(config, tenant_id, logger):
    ...
    dynamic_tenant_config_path = os.path.join(
        ailens_home_value, "config", "dynamic_tenant_specific_config.yaml"
    )
    dynaconf_tenant_settings = Dynaconf(settings_files=[tenant_config_path])
    dynaconf_dynamic_tenant_settings = Dynaconf(
        settings_files=[dynamic_tenant_config_path]
    )

    # reload fresh config for tenant id from source file
    tenant_config = dynaconf_tenant_settings.get(tenant_id, fresh=True)
    dynamic_tenant_config = dynaconf_dynamic_tenant_settings.get(
        tenant_id, fresh=True
    )
