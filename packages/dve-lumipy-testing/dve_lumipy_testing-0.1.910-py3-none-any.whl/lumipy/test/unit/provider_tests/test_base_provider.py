...
class TestBaseProvider(unittest.TestCase):
    ...
    def test_base_provider_get_data_empty(self):
        ...
        with ProviderManager(p, port=7466, _skip_checks=True, run_type='python_only', domain='test') as m:
            ...
            self.assertSequenceEqual([3, 8], df.shape)
        ...
        with ProviderManager(p, port=7466, _skip_checks=True, run_type='python_only', domain='test') as m:
            ...
            ctx = {
                'param_specs': {
                    ...
                    'SetIsOrdered': {'name': 'SetIsOrdered', 'data_type': 'Boolean', 'value': True},
                    'SetIsOffset': {'name': 'SetIsOffset', 'data_type': 'Boolean', 'value': True},
...
CEEFCDEAAD,-6.0,-0.4598301026013887,2022-09-28,2022-05-31 22:26:52.758119,False,data,
DFBBACBDDE,1.0,0.8922475162688099,2022-02-26,2022-02-28 18:21:39.558486,True,data,
EBFBADBBEA,5.0,-1.3397222765423975,2022-02-13,2022-02-09 20:30:25.632523,False,data,
,,,,,,progress,Test progress 3
BBDBEBABFA,-4.0,1.0129628256599017,2022-01-19,2022-10-20 17:06:32.686157,True,data,
,,,,,,sys_info,Test sys info 4
BFDEFCCDEC,9.0,-0.4398481338342341,2022-08-13,2022-11-23 07:30:35.918276,False,data,
            '''

            exp_df = pd.read_csv(io.StringIO(exp_csv))
            exp_df.columns = [str(c) for c in exp_df.columns]

            self.assertSequenceEqual([12, 8], obs_df.shape)
            compare = (exp_df.fillna(0) == obs_df.fillna(0))
    ...
    def test_base_provider_get_data_error_on_bad_cols(self):
        ...
        with ProviderManager(p, port=7466, _skip_checks=True, run_type='python_only', domain='test') as m:
            ...
            self.assertSequenceEqual([4, 8], obs_df.shape)
