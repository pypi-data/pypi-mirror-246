...
# Static methods
    ...
    def FindShellInBox(model, xmin, xmax, ymin, ymax, zmin, zmax, flag=Oasys.gRPC.defaultArg, excl=Oasys.gRPC.defaultArg, vis_only=Oasys.gRPC.defaultArg):
        ...
        excl : integer
            Optional flag ( 0) Apply inclusive selection with local box extension = "spot_thickness" (default 10)
    ...
    def MakeConsistentNormalsFlagged(model, flag, shell_label=Oasys.gRPC.defaultArg):
        ...
        flag : Flag
            flag bit
        shell_label : integer
            The label of the seed shell. If omitted, or None, the first flagged shell is used as the seed shell
    ...
    def Pick(prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg, button_text=Oasys.gRPC.defaultArg):
        ...
        limit : Model or Flag
            If the argument is a Model then only shells from that model can be picked.
    ...
    def Select(flag, prompt, limit=Oasys.gRPC.defaultArg, modal=Oasys.gRPC.defaultArg):
        ...
        prompt : string
            Text to display as a prompt to the user
        limit : Model or Flag
            If the argument is a Model then only shells from that model can be selected.
            If the argument is a Flag then only shells that
    ...
    def SketchFlagged(model, flag, redraw=Oasys.gRPC.defaultArg):
        ...
        redraw : boolean
            If model should be redrawn or not after the shells are sketched.
    ...
    def UnsketchAll(model, redraw=Oasys.gRPC.defaultArg):
        ...
        model : Model
            Model that all shells will be unblanked in
        redraw : boolean
            If model should be redrawn or not after the shells are unsketched.
...
# Instance methods
    ...
    def ElemCut(self, database_cross_section_label):
        ...
        -------
        list
            A list containing the x1,y1,z1,x2,y2,z2 coordinates of the cut line, or NULL if it does not cut. Note this function does not check that the shell is in the cross section definition (part set)
        """
    ...
    def GetAttachedShells(self, tolerance=Oasys.gRPC.defaultArg, recursive=Oasys.gRPC.defaultArg):
        ...
        ----------
        tolerance : float
            This tolerance can be used to limit the selection to shells whose
            normal vector is within this tolerance (in degrees) of the original shell. If omitted the tolerance is
            180 degrees
        recursive : boolean
            If recursive is false then only the shells actually attached to the shell will be
    ...
    def Sketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            If model should be redrawn or not after the shell is sketched.
    ...
    def Unsketch(self, redraw=Oasys.gRPC.defaultArg):
        ...
        ----------
        redraw : boolean
            If model should be redrawn or not after the shell is unsketched.
