...
class FunctionsDistribution():
    ...
    def __init__() -> None:      
        """
        Initializes the class with the necessary parameters for distribution calculation.
        """
    ...
    def adjust_delivery(self, df):
        ...
        df["TransferLotSize_sum"] = df.groupby("Item")["TransferLotSize"].transform("sum")
        list_items = df["Item"].unique()

        for i in list_items:
            indices = df[df["Item"]==i].index.tolist()
            df_filter = df.loc[indices].copy()
            ...
            df.loc[indices, "TransferLotSize"] = df_filter.loc[indices, "TransferLotSize"]
        
        df = df[df["TransferLotSize"] > 0]
