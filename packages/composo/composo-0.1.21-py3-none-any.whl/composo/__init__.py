...
if os.environ.get(_D,_C)=='local':print("Using DEBUG logging as you're running locally");logger.setLevel(logging.DEBUG)
			...
			if isinstance(event,PollResponse):
				...
				if event.payload is not _A:
					...
					for(i,case)in enumerate(event.payload.cases):
						case_result=_A
						try:ret=run_experiment(case.vars);ret_type=type(ret);enum_type=native_type_to_enum[ret_type];case_result=CaseResult(case_id=case.case_id,value=ret,value_type=enum_type,error=_A);case_results.append(case_result)
						...
						event_egress.report_run_results(RunResult(run_id=event.payload.run_id,results=case_results),run_id=event.payload.run_id)
