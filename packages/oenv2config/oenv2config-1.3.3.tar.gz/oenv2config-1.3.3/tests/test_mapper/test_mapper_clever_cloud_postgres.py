...
from odoo_env_config.mappers import clevercloud_postgresql


class TestCleverCloudMapperPostgres(unittest.TestCase):
    def _test_priority(self, key, to_map, expected, nb_key=6):
        result = clevercloud_postgresql(api.Env(to_map))
        self.assertEqual(nb_key, len(result.keys()))
        self.assertEqual(
            expected, result[key], "Value of key [%s] is not '%s' result : '%s'" % (key, expected, result[key])
        )

    def test_no_value(self):
        result = clevercloud_postgresql(api.Env({}))
    ...
    def test_postgres_addon_direct(self):
        ...
        self.assertEqual(10, len(result.keys()))
        self.assertIsNone(result["DB_NAME"], "POSTGRESQL_ADDON_DIRECT_DB n'est pas prise en compte par le mapper")
        self.assertEqual(db_host, result["DB_HOST"])
        self.assertEqual(db_port, result["DB_PORT"])
        self.assertIsNone(result["DB_USER"], "POSTGRESQL_ADDON_DIRECT_USER n'est pas prise en compte par le mapper")
        self.assertIsNone(
            result["DB_PASSWORD"], "POSTGRESQL_ADDON_DIRECT_PASSWORD n'est pas prise en compte par le mapper"
        )
    ...
    def test_postgres_priority_global(self):
        ...
        self.assertEqual(10, len(result.keys()))
        self.assertEqual(db_name, result["DB_NAME"], "DB_NAME est prioritaire sur POSTGRESQL_ADDON_DB")
        self.assertEqual(db_host, result["DB_HOST"], "DB_HOST est prioritaire sur POSTGRESQL_ADDON_HOST")
        self.assertEqual(db_port, result["DB_PORT"], "DB_PORT est prioritaire sur POSTGRESQL_ADDON_PORT")
        self.assertEqual(db_user, result["DB_USER"], "DB_USER est prioritaire sur POSTGRESQL_ADDON_USER")
        self.assertEqual(
            db_password, result["DB_PASSWORD"], "DB_PASSWORD est prioritaire sur POSTGRESQL_ADDON_PASSWORD"
        )
