...
class VisionTransformer(nn.Module):
    ...
    def extract_feature(self, x, ada_token=None):
        x = self.prepare_tokens(x, ada_token)
        feat = []
        for i, blk in enumerate(self.blocks):
            x = blk(x)
...
class vitsmall_with_learngene(nn.Module):
    """ Vision Transformer """
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, inherited_layers_0 = [], inherited_layers_2 = [], patch_embed=[], cls_token=[], pos_embed=[], pos_drop=[], embed_dim=384, depth=12,
