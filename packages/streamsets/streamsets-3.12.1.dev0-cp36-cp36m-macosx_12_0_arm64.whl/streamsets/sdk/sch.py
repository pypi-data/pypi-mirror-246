...
class ControlHub:
    ...
    def validate_pipeline(self, pipeline):
        ...
        Args:
            pipeline (:py:obj:`streamsets.sdk.sch_models.pipeline`): A pipeline instance to be validated.
        ...
        engine_instance, engine_pipeline_instance = self._add_pipeline_to_executor_if_not_exists(pipeline)
        # Because the call to _add_pipeline_to_executor_if_not_exists requires us to use the underlying Engine's API
        # client, it returns the sdc_models definition of a pipeline - thus we reference `id` instead of `pipeline_id`.
        validate_command = engine_instance.api_client.validate_pipeline(pipeline_id=engine_pipeline_instance.id
                                                                        ).wait_for_validate()
    ...
    def _add_pipeline_to_executor_if_not_exists(self, pipeline, reset_origin=False, parameters=None):
        ...
        try:
            ...
            raise ValueError("The authoring engine for this pipeline does not exist in your organization. "
                             "Please verify that the pipeline's authoring engine is correct and that the engine is "
                             "online and registered with your organization.")
