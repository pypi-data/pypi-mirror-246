...
def main():
    ...
    logger.debug(args)
    if args.gui:
        command = "python {} -l '{}'".format(os.path.join(os.path.dirname(__file__), "gui.py"), args.log)
    else:
        if args.train:
            command = "python {} -d {} -m {} -j {} -a {}".format(os.path.join(os.path.dirname(__file__), "train.py"), args.dataset, args.model, args.json, args.timestamp)
        elif args.optimize:
            command = "python {} -d {} -m {} -j {} -a {} -s {} -u {}".format(os.path.join(os.path.dirname(__file__),  "optimize.py"), args.dataset, args.model, args.json, args.timestamp, args.study, args.database)
        else:
