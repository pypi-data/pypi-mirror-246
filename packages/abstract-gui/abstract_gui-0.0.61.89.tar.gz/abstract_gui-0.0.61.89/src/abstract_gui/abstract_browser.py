...
class AbstractBrowser:
    ...
    def initiate_browser_window(self,window_name="Browser_Window",title="Abstract Browser",extra_buttons=[],section=None):
        self.window_mgr = AbstractWindowManager()
        self.window_name = self.window_mgr.add_window(window_name=window_name,title=title,layout=self.get_scan_browser_layout(section=section,extra_buttons=extra_buttons),**expandable())
        self.window = self.window_mgr.get_window_method(self.window_name)
        self.window_mgr.while_window(window_name=self.window_name, event_handlers=self.handle_event)
    ...
    def get_scan_browser_layout(section=None, extra_buttons=[]):
        ...
        browser_buttons_layout = [
            sg.Button('All Scan Mode', key=text_to_key(text='mode',section=section)),sg.Checkbox('All', default=True, key=text_to_key(text='scan mode all',section=section), enable_events=True),sg.Checkbox('Files', key=text_to_key(text='scan mode files',section=section), enable_events=True),sg.Checkbox('Folders', key=text_to_key(text='scan mode folders',section=section), enable_events=True)
        ]
        listboxes_layout=[]
    ...
    def get_opposite_scans(self,key):
        bool_key = self.values[self.event_key_js[key]]
        each_type = self.modes
        each_type.remove(bool_key)
        if bool_key:
            self.window[self.event_key_js[each_type][0]].update(False)
    ...
    def while_static(self,event_key_js,values,window):
        ...
        if self.event_key_js['found'] == '-MODE-':
            ...
            if self.event_key_js['found'][:len('-SCAN_MODE')]=='-SCAN_MODE':
                get_opposite_scans(self,self.event_key_js['found'])
