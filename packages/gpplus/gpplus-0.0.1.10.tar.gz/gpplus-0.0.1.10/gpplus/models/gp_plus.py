...
class GP_Plus(GPR):
    ...
    def forward(self,x:torch.Tensor) -> MultivariateNormal:
        ...
        Sigma_sum=torch.zeros(x.size(0),x.size(0), dtype=torch.float64).to(**self.tkwargs)
        mean_x_sum=torch.zeros(x.size(0), dtype=torch.float64).to(**self.tkwargs)
        ...
        #################### Multiple baises (General Case) ####################################  
            def multi_mean(x,x_forward_raw):
                ...
                return mean_x 
            mean_x = multi_mean(x_new,x_forward_raw).to(**self.tkwargs)
