...
# %% auto 0
__all__ = ['ascidx', 'getcidx', 'getaidx', 'getiloc', 'getloc', 'getcats', 'getbiloc', 'wherecat', 'cateqidx', 'wherecats',
           'catseqidx', 'getciloc', 'geticat', 'idxiloc', 'lociloc', 'getsub']
...
from chck import isint, isiter, isints, isslc, isnone, iscatidx, isboolidx, isidxlike, isdf
...
from .util import DataFrameLike, DataFrameLikeQ

# %% ../nbs/20_idxs.ipynb 28
def ascidx(idx: IndexLike) -> pd.Categorical:
    if hasattr(idx, 'astype'): return idx.astype(CATEGORY)
    if hasattr(idx, 'categories'): return idx
...
def getcidx(dl: DataFrameLike = None, cidx: CatIndexQ = None, *args: DataFrameLike) -> CatIndexQ:
    ...
    '''
    return ascidx(cidx if iscatidx(cidx) else getdfidx(dl, *args))
...
def getcats(dl: DataFrameLike = None, cidx: CatIndexQ = None, *args: DataFrameLike) -> IndexQ:
    ...
    '''
    return getattr(getcidx(dl, cidx, *args), CATEGORIES, pd.Index([]))
