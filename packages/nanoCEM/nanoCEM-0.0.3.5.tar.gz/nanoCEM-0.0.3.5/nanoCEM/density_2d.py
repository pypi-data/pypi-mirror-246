...
new_df['Group'] = new_df['P value(-log10)'].apply(lambda x: 'Significant' if x>=2 else 'Insignificant')
plot = p9.ggplot(new_df, p9.aes(x='Position', y='P value(-log10)',fill='Group'))\
    + p9.theme_bw() \
    + p9.geom_col()\
    + p9.scale_fill_manual(values={'Significant': "#BB9A8B", 'Insignificant': "#D7D7DD"}) \
