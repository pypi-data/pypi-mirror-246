...
from funboost.concurrent_pool import BoundedThreadPoolExecutor
...
class BaseJsonAbleModel:
    ...
    def update_from_model(self, modelx: BaseJsonAbleModel):
...
class FunctionResultStatusPersistanceConfig(BaseJsonAbleModel):
    def __init__(self,is_save_status,is_save_result,expire_seconds: int = 7 * 24 * 3600,is_use_bulk_insert=False):
        """
        :param is_save_status: 是否保存函数的运行状态信息
        :param is_save_result: 是否保存函数的运行结果
        :param expire_seconds: mongo中的函数运行状态保存多久时间,自动过期
        ...
        if self.expire_seconds > 10 * 24 * 3600:
            flogger.warning(f'你设置的过期时间为 {self.expire_seconds} ,设置的时间过长。 ')
...
class BoosterParams(BaseJsonAbleModel):
    ...
    def __init__(self,queue_name:str,
                 ...
                 specify_concurrent_pool: typing.Any = None,
                 specify_async_loop: typing.Any = None,
    ...
    auto_generate_info: dict = {} , # 自动生成的信息,不需要用户主动传参.
        ...
        """
        :param queue_name: 队列名字,必传项,每个函数要使用不同的队列名字.
        ...
        specify_concurrent_pool: typing.Any = None  # 使用指定的线程池/携程池，可以多个消费者共使用一个线程池,节约线程.不为None时候。threads_num失效
        specify_async_loop: typing.Any = None  # 指定的async的loop循环，设置并发模式为async才能起作用。 有些包例如aiohttp,请求和httpclient的实例化不能处在两个不同的loop中,可以传过来.
...
class BoosterParamsComplete(BoosterParams):
    ...
    specify_concurrent_pool:typing.Any = BoundedThreadPoolExecutor(20)
